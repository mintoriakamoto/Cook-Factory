---
name: ferrox:quick
description: "A small self contained task, with atomic commits and state tracking but no planning ceremony"
...
---
<objective>
Execute small, ad-hoc tasks with Ferrox guarantees (atomic commits, STATE.md tracking).

Quick mode is the same system with a shorter path:
- Spawns ferrox-planner (quick mode) + ferrox-executor(s)
...
<execution_context>
@~/.claude/ferrox-core/workflows/quick.md
</execution_context>
...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  ferrox-tools query frontmatter.get .planning/quick/{dir}/SUMMARY.md status
  ```
...
─────────────────────────────────────
Resume with: /ferrox:quick resume {slug}
```
...
- Status fields read via `ferrox-tools query frontmatter.get` — never eval'd or shell-expanded
</security_notes>
