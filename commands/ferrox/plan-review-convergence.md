...
<objective>
Cross-AI plan convergence loop — an outer revision gate around ferrox-review and ferrox-planner.
...
**Flow:** Skill("ferrox-plan-phase") → Agent→Skill("ferrox-review") → check unresolved HIGH + actionable non-HIGH → Skill("ferrox-plan-phase --reviews") → Agent→Skill("ferrox-review") → ... → Converge or escalate

Replaces ferrox-plan-phase's internal ferrox-plan-checker with external AI reviewers (codex, gemini, etc.). Plan-phase runs **inline** (bare Skill at depth 0) so it can spawn ferrox-planner/ferrox-plan-checker at depth 1. Review runs inside an isolated Agent (ferrox-review is a Bash leaf — no sub-agents needed). Orchestrator only does loop control.

**Orchestrator role:** Parse arguments, validate phase, run plan-phase inline (Skill at depth 0), spawn an Agent for ferrox-review, check unresolved HIGH and actionable non-HIGH counts, stall detection, escalation gate.
</objective>

<execution_context>
@$HOME/.claude/ferrox-core/workflows/plan-review-convergence.md
