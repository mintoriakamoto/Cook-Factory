...
<objective>
Check for Ferrox updates, install if available, and display what changed.
...
<execution_context>
@~/.claude/ferrox-core/workflows/update.md
</execution_context>

<flags>
- **--sync**: Sync managed Ferrox skills across runtime roots so multi-runtime users stay aligned after an update. Runs the sync-skills workflow (--from, --to, --dry-run, --apply flags supported).
- **--reapply**: Reapply local modifications after a Ferrox update. Uses three-way comparison (pristine baseline, user-modified backup, newly installed version) to merge user customizations back. Runs the reapply-patches workflow.
...
@~/.claude/ferrox-core/workflows/sync-skills.md
@~/.claude/ferrox-core/workflows/reapply-patches.md
</execution_context_extended>
