...
> **Note:** Planning methodology is in `agents/ferrox-planner.md`.
...
**Wave is pre-computed:** Wave numbers are assigned during `/ferrox:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/ferrox-core/references/tdd.md` for TDD plan structure.
...
See `~/.claude/ferrox-core/templates/user-setup.md` for full schema and examples
...
See `~/.claude/ferrox-core/workflows/verify-phase.md` for verification logic.
