...
/**
 * Deterministic verifier for the /ferrox-reapply-patches Step 5 "Hunk Verification
 ...
 *     --patches-dir <path>        \  # ferrox-local-patches/
 *     --config-dir <path>         \  # ~/.claude (or runtime equivalent)
 *     [--pristine-dir <path>]        # ferrox-pristine/; if absent, falls back to
...
const FERROX_HOOK_VERSION_LINE_RE = /^(?:\/\/|#)\s*ferrox-hook-version:\s*\S+\s*$/i;
...
function normalizeUpstreamOwnedLine(line) {
  ...
  if (FERROX_HOOK_VERSION_LINE_RE.test(trimmed)) {
    const prefix = trimmed.startsWith('#') ? '#' : '//';
    return `${prefix} ferrox-hook-version: __FERROX_VERSION_TOKEN__`;
  }
  return line;
}
...
const REASON = Object.freeze({
  ...
  // Bug #3657: the on-disk ferrox-pristine/ file's SHA-256 does not match the
  // hash recorded in backup-meta.json.pristine_hashes.  This means the
  // pristine snapshot was refreshed to a newer Ferrox version after the backup
  ...
  // ferrox-pristine/ file is absent from disk.  This happens on post-#604-rename
  ...
  if (pristineDir) {
    ...
    try {
      ...
      if (stat.isFile()) {
        ...
        // means the installer refreshed ferrox-pristine/ to a newer Ferrox version
        ...
        if (recordedHash) {
          if (sha256(candidate) === recordedHash) {
            ...
            // Hash mismatch: the on-disk ferrox-pristine/ was refreshed to a newer
            // Ferrox version after the backup was captured.  Using it as the diff
    ...
    // ferrox-pristine/ file (the only candidate was discarded because it was from
