 ...
 *
 * The Ferrox runtime CLI (ferrox-tools.cjs) require()s ~150 compiled modules from
 ...
 *
 * This module heals that: before ferrox-tools.cjs require()s ./lib, if the compiled
 * output is absent it invokes tsc once — lock-guarded so the many parallel
 * ferrox-tools shell-outs a workflow performs do not race the build — then lets the
...
// The first module ferrox-tools.cjs require()s, and a gitignored build artifact
 ...
 * <root>/ferrox-core/bin/ensure-runtime-build.cjs, so the root is two levels up —
 * true for both the dev repo layout and the marketplace plugin-cache checkout
 * (…/<version>/ferrox-core/bin/ensure-runtime-build.cjs).
 */
...
class RuntimeBuildError extends Error {
  constructor(message) {
    ...
    this.code = 'FERROX_RUNTIME_BUILD_FAILED';
  }
}
...
function ensureRuntimeBuild(opts = {}) {
  ...
  if (!fs.existsSync(tsconfig)) {
    throw new RuntimeBuildError(
      'Ferrox runtime library is not built and cannot be auto-built: ' +
        `${tsconfig} not found. Run \`npm run build:lib\` in the ferrox-core package.`,
    );
  }
  ...
  if (!tscScript) {
    throw new RuntimeBuildError(
      `Ferrox runtime library is not built (missing ${path.join(libDir, SENTINEL)}) ` +
        'and TypeScript is unavailable to build it. Run ' +
        `\`npm install && npm run build:lib\` in the ferrox-core package (${packageRoot}).`,
    );
  }
  ...
  if (!haveLock) {
    ...
    if (!haveLock && !isBuilt(libDir)) {
      throw new RuntimeBuildError(
        `Ferrox runtime build did not complete within ${waitTimeoutMs}ms and the ` +
  ...
  try {
    log('ferrox: runtime library not built — compiling once (tsc -p tsconfig.build.json)…');
    ...
    if (!res || res.status !== 0) {
      ...
      throw new RuntimeBuildError(
        `Ferrox runtime build failed (tsc exit ${res ? res.status : 'unknown'}). ` +
  ...
  if (!isBuilt(libDir)) {
    throw new RuntimeBuildError(
      `Ferrox runtime build ran but ${path.join(libDir, SENTINEL)} is still missing. ` +
