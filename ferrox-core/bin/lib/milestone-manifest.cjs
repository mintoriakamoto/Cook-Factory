...
const LIFECYCLES = ['draft', 'active', 'complete'];
const KINDS = ['milestone', 'benchmark'];
 ...
 */
function parseMilestoneArtifact(text, filename) {
    ...
    if (m[1] !== milestone) {
        return err(file, 'E_MILESTONE_FILENAME_MISMATCH', `filename declares v${m[1]} but frontmatter declares ${milestone}`);
    }
    const name = fm['name'];
    ...
    if (typeof lifecycle !== 'string' || !LIFECYCLES.includes(lifecycle)) {
 ...
 */
function collectMilestones(entries) {
    const errors = [];
    ...
    for (const v of versions) {
        const parts = byVersion.get(v)
            .slice()
            .sort((a, b) => (a.part !== b.part ? a.part - b.part : a.file.localeCompare(b.file)));
        const head = parts[0];
        for (const p of parts.slice(1)) {
            for (const key of ['name', 'lifecycle', 'artifact_kind']) {
                if (p[key] !== head[key]) {
                    errors.push(err(p.file, 'E_MILESTONE_GROUP_CONFLICT', `${key} disagrees within milestone ${v}: ${String(head[key])} vs ${String(p[key])}`));
                }
            }
            if (p.shipped.join(',') !== head.shipped.join(',')) {
                errors.push(err(p.file, 'E_MILESTONE_GROUP_CONFLICT', `shipped disagrees within milestone ${v}`));
            }
        }
        groups.push({
            milestone: v,
            ...
            parts,
            progress: parts.reduce((acc, p) => ({ done: acc.done + p.progress.done, total: acc.total + p.progress.total }), { done: 0, total: 0 }),
        });
    }
    const active = groups.filter((g) => g.lifecycle === 'active');
    if (active.length > 1) {
        errors.push(err(active.map((g) => g.milestone).join(', '), 'E_MILESTONE_MULTIPLE_ACTIVE', `exactly one milestone may be lifecycle: active, found ${active.length}`));
    }
...
function renderMilestonesIndex(groups) {
    ...
    for (const g of groups) {
        ...
        lines.push(`| v${g.milestone} | ${g.name} | ${life} | ${g.artifact_kind} | ${shipped} | ${progress} | ${files} |`);
    }
    lines.push('');
    lines.push('## Honest gaps');
    lines.push('');
    const unshipped = groups.filter((g) => g.shipped.length === 0).map((g) => `v${g.milestone}`);
    lines.push(unshipped.length === 0
        ? '- Every milestone recorded here has a published release.'
        : `- Never published to npm: ${unshipped.join(', ')}.`);
    lines.push('- Per-milestone start dates are not recorded. There is no hermetic local source for them');
