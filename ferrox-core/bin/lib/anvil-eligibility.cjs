...
const gateSelect = require("./gate-select.cjs");
/**
...
function evaluateAnvilEligibility(opts) {
    ...
    const reasons = [];
    if (o.depth !== 'fast')
        reasons.push('not-fast-depth');
    if (o.deliverableKind !== 'python-single-file')
        reasons.push('not-python-single-file');
    if (o.gatePresent !== true)
        reasons.push('no-gate');
    if (o.anvilAvailable !== true)
        reasons.push('anvil-unavailable');
    if (o.enabled !== true)
...
function evaluateGateFirstEligibility(opts) {
    ...
    if (o.gatePresent !== true)
        reasons.push('no-gate');
    if (gateSelect.selectGate(o.domain).route !== 'gate-first')
        reasons.push('domain-not-gateable');
    if (o.executorAvailable !== true)
        reasons.push('executor-unavailable');
    if (o.enabled !== true)
        reasons.push('disabled');
...
module.exports = { evaluateAnvilEligibility, evaluateGateFirstEligibility };
