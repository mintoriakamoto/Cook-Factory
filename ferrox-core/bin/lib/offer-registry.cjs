...
exports.recordOfferAccepted = recordOfferAccepted;
 ...
 */
function recordOfferAccepted(projectRoot, commandStem) {
    const accepted = [];
    try {
        if (typeof commandStem !== 'string' || commandStem === '')
            return accepted;
        ...
        for (const offer of exports.OFFERS) {
            if (offer.command !== commandStem)
                continue;
            if (memory.outcomes[offer.id] !== 'shown')
                continue;
