...
const { resolveConfigKey } = activationMod;
 ...
 * interpreter.
 */
function probeInterpreter(opts) {
    ...
    if (node_path_1.default.isAbsolute(name)) {
        try {
            return node_fs_1.default.existsSync(name)
    ...
    for (const dir of pathEnv.split(node_path_1.default.delimiter)) {
        ...
        try {
            if (node_fs_1.default.existsSync(candidate))
                return { found: true, name, path: candidate };
        }
        catch {
...
function defaultEngineDir() {
    return node_path_1.default.join(__dirname, '..', 'vendor', 'ratchet', 'bin');
}
/**
 ...
 */
function formatFleetDoctorLine(readiness) {
    if (!readiness || !readiness.active)
        return null;
