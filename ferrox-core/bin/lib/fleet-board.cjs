...
function _toTicket(value) {
    if (typeof value !== 'number' || !Number.isInteger(value) || value < 0)
        return null;
    return value;
}
/** The holder identity an event carried, or null when it carried none. */
function _toHolder(value) {
    if (value === null || typeof value !== 'object')
        return null;
    const rec = value;
    if (rec.pid === undefined && rec.pid_start === undefined)
        return null;
...
function _attemptKey(nodeId, attemptId) {
...
function projectBoard(events) {
    ...
    for (const raw of list) {
        ...
        switch (kind) {
            ...
            case 'lease_reclaimed': {
                ...
                if (current !== undefined && epoch < current.lease_epoch)
                    break;
                leases[nodeKey] = {
                    node_id: nodeKey,
            ...
            case 'lease_released': {
                ...
                if (releaseEpoch !== current.lease_epoch)
                    break;
    ...
    const attemptsOut = Object.create(null);
 ...
 */
function isReclaimable(lease, nowMs, liveness) {
    if (lease === null || lease === undefined)
        return false;
    if (lease.state !== LEASE_STATES.HELD)
        return false;
    if (isLeaseExpired(lease, nowMs))
        return true;
    if (liveness !== null && typeof liveness === 'object' && liveness.alive === false)
        return true;
    return false;
}
/**
 ...
 */
function nextLeaseEpoch(board, nodeId) {
    if (board === null || board === undefined)
        return 1;
    const leases = board.leases;
    if (leases === null || typeof leases !== 'object')
        return 1;
 ...
 */
function _requireId(value, label) {
    if (typeof value !== 'string' || value === '') {
        throw _codedError(FLEET_BOARD_ERROR_CODES.E_FLEET_BAD_IDENTIFIER, `fleet-board: ${label} must be a non-empty string, received ${typeof value} `
...
function _requireNowMs(value) {
    if (typeof value !== 'number' || !Number.isFinite(value)) {
        throw _codedError(FLEET_BOARD_ERROR_CODES.E_FLEET_BAD_IDENTIFIER, `fleet-board: nowMs must be a finite number, received ${JSON.stringify(value)}. `
...
function _requireTtlMs(value) {
    if (typeof value !== 'number' || !Number.isFinite(value) || value <= 0) {
        throw _codedError(FLEET_BOARD_ERROR_CODES.E_FLEET_BAD_IDENTIFIER, `fleet-board: ttlMs must be a positive finite number, received ${JSON.stringify(value)}. `
 ...
 */
function _readPidStart(pid) {
    try {
        const out = (0, node_child_process_1.spawnSync)('ps', ['-o', 'lstart=', '-p', String(pid)], {
            encoding: 'utf8',
            timeout: 10_000,
        });
 ...
 * claimant.
 */
function probeLiveness(holder, deps = {}) {
    ...
    if (exists === false)
        return { alive: false, reason: 'pid_absent' };
    if (exists !== true)
        return { alive: true, reason: 'pid_presence_unknowable' };
    ...
    if (actual === null)
        return { alive: true, reason: 'start_stamp_unreadable' };
    if (actual.trim() !== recorded.trim())
        return { alive: false, reason: 'start_stamp_mismatch' };
 ...
 */
function _readProjectionTolerant(projectionPath) {
    try {
 ...
 */
function _runLeaseTransaction(logPath, projectionPath, decide) {
    ...
    return atomicState.withFileLock(projectionPath, (handle) => {
        ...
        runlog.appendFleetEvent(event, { path: logPath });
...
function claimNode(opts) {
    ...
    return _runLeaseTransaction(opts.logPath, opts.projectionPath, (current) => {
        const lease = current.leases[nodeId];
        if (lease !== undefined && lease.state === LEASE_STATES.HELD) {
            throw _codedError(FLEET_BOARD_ERROR_CODES.E_FLEET_NODE_HELD, `fleet-board: node ${nodeId} is held by ${JSON.stringify(lease.worker_id)} at epoch `
        ...
        return {
            ...
            result: (next) => next.leases[nodeId],
        };
    });
}
/**
 ...
 */
function _requireHolder(current, nodeId, workerId, leaseEpoch) {
    const lease = current.leases[nodeId];
    if (lease === undefined || lease.state !== LEASE_STATES.HELD) {
        throw _codedError(FLEET_BOARD_ERROR_CODES.E_FLEET_NOT_HOLDER, `fleet-board: node ${nodeId} carries no held lease, so ${workerId} cannot be its holder`);
    }
    if (lease.lease_epoch !== leaseEpoch) {
        throw _codedError(FLEET_BOARD_ERROR_CODES.E_FLEET_STALE_EPOCH, `fleet-board: node ${nodeId} is at lease epoch ${lease.lease_epoch} and the caller presented `
            + `${JSON.stringify(leaseEpoch)}. The lease was reclaimed, so this worker no longer owns it.`);
    }
    if (lease.worker_id !== workerId) {
        throw _codedError(FLEET_BOARD_ERROR_CODES.E_FLEET_NOT_HOLDER, `fleet-board: node ${nodeId} at epoch ${lease.lease_epoch} is held by `
...
function reclaimLease(opts) {
    ...
    return _runLeaseTransaction(opts.logPath, opts.projectionPath, (current) => {
        const lease = current.leases[nodeId];
        if (lease === undefined || lease.state !== LEASE_STATES.HELD) {
            throw _codedError(FLEET_BOARD_ERROR_CODES.E_FLEET_NOT_RECLAIMABLE, `fleet-board: node ${nodeId} carries no held lease, so there is nothing to reclaim. `
        ...
        if (!isReclaimable(lease, nowMs, verdict)) {
            throw _codedError(FLEET_BOARD_ERROR_CODES.E_FLEET_NOT_RECLAIMABLE, `fleet-board: node ${nodeId} is held by ${JSON.stringify(lease.worker_id)} at epoch `
...
function reconcileBoard(opts) {
    ...
    return atomicState.withFileLock(projectionPath, (handle) => {
        ...
        if (agreed)
            return { agreed: true, rebuilt: false, board: truth };
