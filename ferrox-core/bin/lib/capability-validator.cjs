...
// Permissive *shape* check for a semver range (engines.ferrox / compatVersions
 ...
 *                    without one).
 *   - engines        optional object; engines.ferrox optional semver-range string.
 *   - compatVersions optional object mapping a capability version (semver) to a
 *                    ferrox version range.
...
function validateVersionEnvelope(cap) {
  ...
  if (cap.engines !== undefined) {
    if (typeof cap.engines !== 'object' || cap.engines === null || Array.isArray(cap.engines)) {
      errors.push('engines must be an object (e.g. { "ferrox": ">=1.6.0" })');
    } else if (cap.engines.ferrox !== undefined && !isPlausibleRange(cap.engines.ferrox)) {
      errors.push('engines.ferrox must be a semver range string; got: ' + JSON.stringify(cap.engines.ferrox));
    }
  }
  ...
  if (cap.compatVersions !== undefined) {
    if (typeof cap.compatVersions !== 'object' || cap.compatVersions === null || Array.isArray(cap.compatVersions)) {
      errors.push('compatVersions must be an object mapping capability versions to ferrox version ranges');
    } else {
...
function validateStep(step, prefix, declaredSkills, declaredAgents) {
  ...
  if (typeof step.ref !== 'object' || step.ref === null) {
    ...
    } else if (hasSkill && typeof step.ref.skill === 'string' && step.ref.skill.startsWith('ferrox-')) {
      // Double-prefix guard: ref.skill is an unprefixed stem (e.g. "ui-review").
      // Workflow dispatch prepends "ferrox-" at runtime → "ferrox-ui-review".
      // A stem that already starts with "ferrox-" would produce "ferrox-ferrox-..." at dispatch.
      errors.push(
        prefix + '.ref.skill "' + step.ref.skill + '" must not start with "ferrox-" ' +
        '(it is an unprefixed stem; the workflow prepends "ferrox-" at dispatch — ' +
        'starting with "ferrox-" would produce "ferrox-' + step.ref.skill + '")',
      );
