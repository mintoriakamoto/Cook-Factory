 ...
 * package across every Ferrox-managed runtime config directory.
 *
 * Issue: #607
 *
 ...
 */
const OLD_PACKAGE_SIGNAL = 'ferrox-core' + '-cc';

/**
 * Subtrees within a configDir that Ferrox actively scans for old-package content.
 * Deliberately excludes 'ferrox-core' — the current package's own infra and
 ...
 * IS scanned.
 */
const FERROX_MANAGED_SUBTREES = ['hooks', 'commands'];

/**
 * Substring that identifies a skill file as referencing the pre-rename Ferrox
 * runtime config subdirectory. Assembled from parts to avoid self-flagging.
 *
 * Old installs wrote skill bodies that embed the path to the Ferrox runtime
 * directory — e.g. `@$HOME/.codex/get-shit-done/workflows/plan.md`. After // ferrox-allow-legacy-name
 * the rename to `ferrox-core/` (#604), those embedded paths are stale and the
 * skill file must be removed so the runtime does not pick up the wrong copy.
 *
 * Issue: #1453
 */
const LEGACY_SKILL_PATH_SIGNAL = 'get-shit-done'; // ferrox-allow-legacy-name

/**
 * Prefix that identifies a skill directory as Ferrox-managed.
 * Only `ferrox-*` subdirectories under the `skills/` subtree are scanned; user
 * skill directories with other prefixes are never touched.
 */
const FERROX_SKILL_DIR_PREFIX = 'ferrox-';
 ...
 * embeds an @-import path to the Ferrox runtime config tree.
 */
 ...
 *
 * WHY: The current ferrox-core package ships ZERO references to the old
 ...
 * installer to delete the freshly-installed ferrox-core/CHANGELOG.md,
 ...
 * (`get-shit-done` as a path component inside an @-import or similar reference). // ferrox-allow-legacy-name
 * Skips unreadable files (returns false on any error).
 *
 * @param {string} absPath
 * @param {object} fsMod
...
function fileContainsLegacySkillPathSignal(absPath, fsMod) {
  try {
    const content = fsMod.readFileSync(absPath, 'utf8');
    return content.includes('/' + LEGACY_SKILL_PATH_SIGNAL + '/'); // ferrox-allow-legacy-name
  } catch {
    return false;
  }
}
 ...
 *     a path reference to the pre-rename `get-shit-done/` runtime directory // ferrox-allow-legacy-name
 *     (skills/ subtree, ferrox-* directories only). Issue #1453.
...
function planLegacyCleanup(configDirs, opts = {}) {
  ...
  for (const configDir of configDirs) {
    for (const subtree of FERROX_MANAGED_SUBTREES) {
    ...
    // #1453: Scan skills/ferrox-* directories for stale get-shit-done path references. // ferrox-allow-legacy-name
    //
    // Background: older Ferrox installs wrote SKILL.md files that embedded a path to
    // the Ferrox runtime config directory, e.g.:
    //   @$HOME/.codex/get-shit-done/workflows/docs-update.md // ferrox-allow-legacy-name
    //
    // After the rename to ferrox-core/ (#604), the correct path is:
    //   @$HOME/.codex/ferrox-core/workflows/docs-update.md
    //
    // When Codex upgrades to ferrox-core 1.5.0 it writes fresh skill files to
    ...
    //
    // This scan removes Ferrox-managed skill files (under ferrox-* subdirs) that still
    ...
    // user-owned non-ferrox-* skill dirs are never touched.
    ...
    if (skillDirEntries) {
      for (const entry of skillDirEntries) {
        // Only process ferrox-* subdirectories (Ferrox-managed skill dirs).
        if (!entry.isDirectory()) continue;
        if (!entry.name.startsWith(FERROX_SKILL_DIR_PREFIX)) continue;
        ...
        for (const absPath of files) {
          ...
          if (SKILL_MD_EXTENSIONS.has(ext) && fileContainsLegacySkillPathSignal(absPath, fsMod)) {
            addCandidate(absPath, 'stale-get-shit-done-path'); // ferrox-allow-legacy-name
          }
        }
      }
    }
  ...
  const legacyCachePath = path.join(homeDir, '.cache', 'ferrox', 'ferrox-update-check.json');
  try {
