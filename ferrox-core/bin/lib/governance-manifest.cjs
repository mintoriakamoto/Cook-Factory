...
function err(file, code, message, extra) {
    ...
    if (extra !== undefined) {
        ...
        if (extra.text !== undefined)
            e.text = extra.text;
        if (extra.section !== undefined)
            e.section = extra.section;
    }
    return e;
}
// ----------------------------------------------------------------------------
 ...
 */
function parseScopeDeclaration(text, filename, activeVersion) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (m !== null) {
            ...
            if (typeof activeVersion === 'string' && activeVersion !== '' && activeVersion !== version) {
                return err(file, 'E_GOV_SCOPE_MISMATCH', `scope declaration names milestone v${version} but the active milestone is v${activeVersion}`, { line: i + 1, text: lines[i] });
            }
            return { version, name, line: i + 1 };
        }
        if (hinted === -1 && SCOPE_HINT.test(lines[i]))
            hinted = i;
    }
    if (hinted !== -1) {
        return err(file, 'E_GOV_SCOPE_MALFORMED', 'scope declaration is incomplete. Expected one bolded sentence of the form: '
            + '**Scope: milestone v<version> (<Name>).**', { line: hinted + 1, text: lines[hinted] });
    }
    return err(file, 'E_GOV_SCOPE_MISSING', `no scope declaration in the first ${SCOPE_SCAN_LINES} lines. Add one bolded sentence: `
 ...
 */
function staleVersionsIn(line, activeVersion) {
    MILESTONE_VERSION_REF.lastIndex = 0;
    const stale = [];
    let m;
    while ((m = MILESTONE_VERSION_REF.exec(line)) !== null) {
        if (m[1] !== activeVersion && !stale.includes(m[1]))
            stale.push(m[1]);
    }
    return stale;
}
 ...
 * check useless.
 */
function detectStaleClaims(text, filename, activeVersion) {
    const errors = [];
    const file = typeof filename === 'string' && filename !== '' ? filename : '<unknown>';
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/^\s*#/.test(line))
            continue;
        if (!CURRENT_STATE_CLAIM_PATTERNS.some((p) => p.test(line)))
            continue;
        const stale = staleVersionsIn(line, activeVersion);
        if (stale.length === 0)
            continue;
        errors.push(err(file, 'E_GOV_STALE_BODY', `current-state claim names milestone v${stale.join(', v')} but the active milestone is v${activeVersion}`, { line: i + 1, text: line }));
    }
    return errors;
}
/**
 ...
 */
function detectHeadingClaims(text, filename) {
    const errors = [];
    const file = typeof filename === 'string' && filename !== '' ? filename : '<unknown>';
    if (typeof text !== 'string' || text === '')
        return errors;
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (headingText === '')
            continue;
        if (!CURRENT_STATE_CLAIM_PATTERNS.some((p) => p.test(headingText)))
            continue;
        errors.push(err(file, 'E_GOV_HEADING_CLAIM', `heading "${headingText}" opens a current-state section, and a prose governance file `
            + 'may not claim current state. Delete the section and read the state from the file '
            + 'that derives it.', { line: i + 1, text: line }));
    }
    return errors;
}
// ----------------------------------------------------------------------------
 ...
 */
function resolveShippedIn(pointers, groups, filename) {
    const errors = [];
    ...
    if (Array.isArray(groups)) {
        for (const g of groups) {
            if (g !== null && typeof g === 'object' && typeof g.milestone === 'string')
                known.add(g.milestone);
        }
    }
    for (const raw of pointers) {
        ...
        if (value.toLowerCase() === SHIPPED_IN_PENDING)
            continue;
        if (known.has(value.replace(/^v/i, '')))
            continue;
        errors.push(err(file, 'E_GOV_SHIPPED_IN_UNRESOLVED', `requirement ${id} claims shipped_in ${value === '' ? '<empty>' : value}, which resolves to no milestone. `
 ...
 */
function activeVersionOf(groups) {
    if (!Array.isArray(groups))
        return null;
    const active = milestoneManifest.activeMilestone(groups);
    if (active === null)
        return null;
    const version = active.milestone;
...
function canonicalName(name, allowed) {
...
function positionFieldOk(name, value) {
    const canonical = canonicalName(name, Object.keys(STATE_POSITION_VALUE_SHAPES));
    if (canonical === null)
        return false;
    const shape = STATE_POSITION_VALUE_SHAPES[canonical];
 ...
 */
function readFrontmatterKeys(fmLines) {
    const top = [];
    const nested = [];
    let parent = '';
    for (let i = 0; i < fmLines.length; i++) {
        ...
        if (nestedMatch !== null)
            nested.push({ parent, key: nestedMatch[1], offset: i });
    }
    return { top, nested };
}
/**
 ...
 */
function checkStateStructure(text, filename) {
    const errors = [];
        ...
        if (close === -1) {
            ...
            for (const entry of top) {
                if (entry.key === 'milestone')
                    declaredVersion = entry.value.replace(/^["']|["']$/g, '').replace(/^v/i, '');
                if (!STATE_ALLOWED_FM_KEYS.includes(entry.key)) {
                    errors.push(err(file, 'E_GOV_STATE_FM_KEY', `frontmatter key "${entry.key}" is outside the closed set written by buildStateFrontmatter`, {
                        line: entry.offset + 2,
            ...
            for (const entry of nested) {
                if (entry.parent !== 'progress')
                    continue;
                if (!STATE_ALLOWED_PROGRESS_KEYS.includes(entry.key)) {
                    errors.push(err(file, 'E_GOV_STATE_FM_KEY', `progress counter "${entry.key}" is outside the closed set`, {
                        line: entry.offset + 2,
                        text: fmLines[entry.offset],
                    }));
                }
            ...
            for (const entry of top) {
                if (!STATE_NARRATIVE_FM_KEYS.includes(entry.key))
                    continue;
                const value = entry.value.replace(/^["']|["']$/g, '');
                if (value.length > STATE_NARRATIVE_FM_MAX) {
                    errors.push(err(file, 'E_GOV_STATE_FM_PROSE', `frontmatter key "${entry.key}" holds ${value.length} characters, over the ${STATE_NARRATIVE_FM_MAX} character cap`, {
                        line: entry.offset + 2,
                    }));
                }
                if (declaredVersion !== null) {
                    ...
                    if (stale.length > 0) {
                        errors.push(err(file, 'E_GOV_STATE_FM_PROSE', `frontmatter key "${entry.key}" names milestone v${stale.join(', v')} but the file declares v${declaredVersion}`, {
                            line: entry.offset + 2,
                            text: value,
                        }));
                    }
    ...
    for (let i = bodyStart; i < lines.length; i++) {
        ...
        if (heading !== null) {
            ...
            if (level >= 3) {
                errors.push(err(file, 'E_GOV_STATE_SUBSECTION', `heading of level ${level} is not admitted anywhere in the state file`, { line: lineNo, text: line }));
                section = FORBIDDEN;
                continue;
            }
            const canonical = canonicalName(heading[2], STATE_ALLOWED_SECTIONS);
            if (canonical === null) {
                errors.push(err(file, 'E_GOV_STATE_SECTION', `section "${heading[2]}" is outside the closed set: ${STATE_ALLOWED_SECTIONS.join(', ')}`, { line: lineNo, text: line }));
                section = FORBIDDEN;
                continue;
            }
            section = canonical;
        ...
        if (section === 'Current Position') {
            ...
            if (TABLE_ROW.test(line)) {
                ...
                if (TABLE_DELIMITER_ROW.test(next))
                    continue;
                const cells = splitTwoCellRow(line);
                if (cells !== null && positionFieldOk(cells[0], cells[1]))
                    continue;
            ...
            if (bold !== null && positionFieldOk(bold[1], bold[2].trim()))
                continue;
            const plain = FIELD_PLAIN.exec(line);
            if (plain !== null && positionFieldOk(plain[1], plain[2].trim()))
                continue;
        ...
        const shapes = STATE_SECTION_BODY_SHAPES[section];
        if (shapes !== undefined && shapes.some((s) => s.test(line)))
            continue;
