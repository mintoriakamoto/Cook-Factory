 ...
 */
const HASHED_KINDS = ['spec', 'gate', 'hidden', 'structure'];
...
const ALWAYS_BLOCKING = ['spec', 'gate', 'structure'];
...
function err(code, message, task) {
...
function readStructure(file) {
    ...
    if (!Array.isArray(o.files))
        return null;
    const strings = (v) => (Array.isArray(v) ? v : []).filter((x) => typeof x === 'string').slice().sort();
...
function scanSingleTasks(root, base) {
    const tasks = [];
...
function scanMultiTasks(root, base) {
    const tasks = [];
 ...
 */
function findGaps(tasks, added) {
    const gaps = [];
    for (const t of tasks) {
        for (const kind of ARTIFACT_KINDS) {
            if (kind === 'structure' && t.kind !== 'multi')
                continue;
            const value = kind === 'structure' ? t.structure : t[kind];
            if (value !== null)
                continue;
            const severity = ALWAYS_BLOCKING.includes(kind) || added.has(t.id)
                ? 'blocking'
                : 'coverage';
...
function summarise(tasks) {
    const count = (pick) => tasks.filter((t) => pick(t) !== null).length;
    return {
        tasks: tasks.length,
 ...
 * (T-22-01-04).
 */
function hashCorpus(root, base, tasks) {
    const lines = [];
    for (const t of tasks) {
...
function indexCorpus(options) {
    ...
    const added = new Set((Array.isArray(o.addedTasks) ? o.addedTasks : []).filter((x) => typeof x === 'string'));
    if (root === '' || !isDir(root)) {
        return {
            ok: false,
            index: null,
            errors: [
                err(E_ROOT_MISSING, `the corpus root ${JSON.stringify(root)} is not a directory, so there is nothing to `
    ...
    for (const m of multis) {
        if (!singleIds.has(m.id))
            continue;
        errors.push(err(E_DUPLICATE_TASK, `the task id ${JSON.stringify(m.id)} appears under both ${DIR_SPECS}/ and `
            + `${DIR_MULTI}/. 2 tasks with 1 id cannot be scored apart, so this is an error `
            + 'rather than a merge. Rename one of them.', m.id));
    }
    if (errors.length > 0)
 ...
 */
function parseGateScore(text, axis) {
    const wanted = axis === 'hidden' ? 'hidden' : 'gate';
    ...
    if (last === null) {
        ...
        return {
            ...
            axis: wanted,
            code: E_NO_SUMMARY,
            message: `no \`${wanted}: N/M\` summary line was printed, so this invocation reported no score at `
    ...
    if (last.total === 0) {
        return {
            ...
            axis: wanted,
            code: E_ZERO_TOTAL,
            message: `the summary line reported \`${wanted}: ${last.passed}/0\`. A score out of 0 is not a `
    ...
    if (last.passed > last.total) {
        return {
            ...
            message: `the summary line reported \`${wanted}: ${last.passed}/${last.total}\`, and more checks `
...
function describe(label, s) {
    ...
    if (s.ok === false)
        return `${label} refused: ${s.message}`;
...
function discriminate(input) {
    ...
    const stdout = (o.stdout === null || typeof o.stdout !== 'object' ? {} : o.stdout);
    const presentRaw = (o.present === null || typeof o.present !== 'object' ? {} : o.present);
    const pairOf = (name) => {
        const p = (stdout[name] === null || typeof stdout[name] !== 'object' ? {} : stdout[name]);
        return {
    ...
    if (!present.reference)
        absent.push('the reference fixture is absent');
    if (!present.mutant)
        absent.push('the mutant fixture is absent');
    if (!present.shallow)
        absent.push('the shallow fixture is absent');
    if (!present.hidden)
        absent.push('the hidden gate is absent, so shallow is undefined for this task');
    if (absent.length > 0)
        return record('incomplete', absent);
    ...
    const refusals = [
        ...
        .filter(([, s]) => s === null || s.ok === false)
        .map(([label, s]) => describe(label, s));
    if (refusals.length > 0)
        return record('incomplete', refusals);
    ...
    const shortfalls = [];
    if (!isFull(reference.gate))
        shortfalls.push(describe('the reference on the visible axis', reference.gate) + ', and a reference must score full');
    if (!isFull(reference.hidden))
        shortfalls.push(describe('the reference on the hidden axis', reference.hidden) + ', and a reference must score full');
    if (!isFull(shallow.gate)) {
        shortfalls.push(describe('the shallow fixture on the visible axis', shallow.gate)
            + ', so it is a second mutant rather than a hardness proof');
    }
    if (shortfalls.length > 0)
        return record('incomplete', shortfalls);
 ...
 * twice.
 */
function detectSaturation(input) {
    const o = (input === null || typeof input !== 'object' ? {} : input);
    const axis = o.axis === 'hidden' ? 'hidden' : 'gate';
    const pcts = (Array.isArray(o.pcts) ? o.pcts : []).filter((x) => typeof x === 'number' && Number.isFinite(x));
    const n = pcts.length;
    ...
    if (n < 2) {
        return {
            ...
            note: `${n} observation${n === 1 ? '' : 's'} on the ${axis} axis has no distribution, so `
    ...
    if (full === n) {
        return {
            ...
            distinct_pct: distinct,
            verdict: 'saturated',
            note: `all ${n} observations scored full on the ${axis} axis. This axis is SATURATED and NON `
    ...
    if (distinct === 1) {
        return {
            ...
            distinct_pct: distinct,
            verdict: 'saturated',
            note: `all ${n} observations carry the identical percentage ${pcts[0]} on the ${axis} axis. `
    ...
    return {
        ...
        distinct_pct: distinct,
        verdict: 'discriminating',
        note: `${full} of ${n} observations scored full on the ${axis} axis across ${distinct} distinct `
