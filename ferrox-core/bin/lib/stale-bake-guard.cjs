 ...
 * `.planning/config.json` or `~/.ferrox/defaults.json` has NO effect until the
 * user re-runs `ferrox install <runtime>` (or `ferrox update`). The failure is
 ...
 * (runtime, cwd) within a process so a single `ferrox-tools init *` invocation
...
/** Per-runtime `ferrox install` flag, for the remediation hint in the warning. */
...
function formatStaleBakeWarning({ runtime, configPath, configMtimeMs, agentMtimeMs }) {
  ...
  return [
    `ferrox: model config in ${configPath} changed since agents were last baked (${configDate}).`,
    `     Static-frontmatter runtime '${runtime}' ignores the new model_overrides`,
    `     until you re-run:  ferrox install ${installFlag}`,
    `     (or 'ferrox update')`,
  ].join('\n');
}
 ...
 * up-to-8-levels-up walk in readFerroxEffectiveModelOverrides (bin/install.js)
 * and includes the global ~/.ferrox/defaults.json. Returns
 ...
 * dir (otherwise the real ~/.ferrox/defaults.json on the CI runner leaks in and
...
function findNewestConfigMtime(cwd, { fsStatSync = fs.statSync, homedir = os.homedir } = {}) {
  ...
  candidates.push(path.join(homedir(), '.ferrox', 'defaults.json'));
...
/**
 * Find the oldest mtime across installed ferrox-* agent files for the runtime.
 * Returns `{ mtimeMs, dir }` or `null` if the agent dir is absent or holds no
 * ferrox-* files (e.g. not yet installed, or uninstalled).
 */
 ...
 *   so repeated `resolveModelInternal` calls under one `ferrox-tools init *` do
