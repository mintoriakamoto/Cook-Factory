...
const FOREIGN_STAMP = Object.freeze({ pid: null, nonce: null, token: 0 });
 ...
 */
function _readLockStamp(lockPath) {
    try {
        const raw = node_fs_1.default.readFileSync(lockPath, 'utf8').trim();
        if (raw === '')
 ...
 */
function _detachLockFile(lockPath, confirmed) {
    ...
    for (let attempt = 1; attempt <= RENAME_MAX_ATTEMPTS; attempt++) {
        try {
            node_fs_1.default.renameSync(lockPath, detached);
            try {
                node_fs_1.default.rmSync(detached, { force: true });
            }
            ...
            if (attempt < RENAME_MAX_ATTEMPTS && RENAME_RETRY_ERRNOS.has(code)) {
                ...
                let again;
                try {
                    again = node_fs_1.default.statSync(lockPath);
                }
                catch {
                    return; // vanished under us
                }
                if (again.ino !== confirmed.ino || again.mtimeMs !== confirmed.mtimeMs)
                    return;
                continue;
            }
...
function _resolveMaxHoldMs(opts, staleMs) {
...
function acquireLock(lockPath, opts = {}) {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        ...
        try {
            ...
            try {
                ...
                if (Date.now() - st.mtimeMs > staleMs) {
                    ...
                    const confirm = node_fs_1.default.statSync(lockPath);
                    if (confirm.ino === st.ino && confirm.mtimeMs === st.mtimeMs) {
                        _detachLockFile(lockPath, confirm);
                        if (doomed.token > reapedToken)
                            reapedToken = doomed.token;
            ...
            if (attempt >= retries) {
                throw _codedError(ATOMIC_STATE_ERROR_CODES.E_LOCK_UNAVAILABLE, `atomic-state: could not acquire lock ${lockPath} after ${retries} retries`);
            }
        ...
        const ino = node_fs_1.default.fstatSync(fd).ino;
        const stamp = { pid: process.pid, nonce, token };
        try {
            node_fs_1.default.writeSync(fd, JSON.stringify(stamp) + '\n');
        }
        catch { /* stamp best-effort */ }
        return { fd, lockPath, ino, nonce, token, acquiredAtMs: Date.now(), maxHoldMs };
    }
    throw _codedError(ATOMIC_STATE_ERROR_CODES.E_LOCK_UNAVAILABLE, `atomic-state: could not acquire lock ${lockPath}`);
}
/**
 ...
 */
function verifyFence(handle, opts = {}) {
    const maxHoldMs = opts.maxHoldMs ?? handle.maxHoldMs;
    const heldMs = Date.now() - handle.acquiredAtMs;
    if (heldMs >= maxHoldMs) {
        throw _codedError(ATOMIC_STATE_ERROR_CODES.E_LOCK_HOLD_EXPIRED, `atomic-state: lock ${handle.lockPath} held ${heldMs} ms, past its ${maxHoldMs} ms hold budget`);
    }
    let st;
    try {
        st = node_fs_1.default.statSync(handle.lockPath);
    }
    catch {
        throw _codedError(ATOMIC_STATE_ERROR_CODES.E_FENCE_LOST, `atomic-state: lock ${handle.lockPath} is gone; this holder no longer owns it`);
    }
    if (st.ino !== handle.ino) {
        throw _codedError(ATOMIC_STATE_ERROR_CODES.E_FENCE_LOST, `atomic-state: lock ${handle.lockPath} was reaped and recreated; this holder no longer owns it`);
    }
    const stamp = _readLockStamp(handle.lockPath);
    if (stamp.nonce !== handle.nonce || stamp.token !== handle.token) {
        throw _codedError(ATOMIC_STATE_ERROR_CODES.E_FENCE_LOST, `atomic-state: lock ${handle.lockPath} carries another holder's stamp`);
    }
}
/**
...
function releaseLock(handle) {
    try {
        node_fs_1.default.closeSync(handle.fd);
    }
    catch { /* already closed */ }
    try {
        const st = node_fs_1.default.statSync(handle.lockPath);
        if (st.ino !== handle.ino)
            return;
        if (_readLockStamp(handle.lockPath).nonce !== handle.nonce)
            return;
        node_fs_1.default.rmSync(handle.lockPath, { force: true });
    }
 ...
 * descriptor.
 */
function _writeAndSyncFile(tmpPath, data) {
 ...
 */
function _syncParentDirectory(targetPath) {
    ...
    if (process.platform === 'win32')
        return;
    try {
        const dfd = node_fs_1.default.openSync(node_path_1.default.dirname(targetPath), 'r');
        try {
            node_fs_1.default.fsyncSync(dfd);
        }
        finally {
            try {
                node_fs_1.default.closeSync(dfd);
            }
            catch { /* already closed */ }
        }
    }
        ...
        const code = err.code ?? '';
        if (DIRECTORY_FSYNC_TOLERATED_ERRNOS.has(code))
            return;
 ...
 */
function atomicWriteFileSync(targetPath, data, fence) {
    node_fs_1.default.mkdirSync(node_path_1.default.dirname(targetPath), { recursive: true });
