...
function findCanonBlocks(lines) {
    const blocks = [];
    let open = null;
...
// ---------- lore validation ----------
function validateLore(doc, errors, facts) {
    if ('sources' in doc) {
    ...
    const claimLabel = (label, owner) => {
        const key = label.trim().toLowerCase();
        if (key === '')
            return;
        const owners = labelOwners.get(key) ?? new Set();
        owners.add(owner);
    ...
    for (let i = 0; i < entities.length; i++) {
        ...
        if (isValidDate(birth) && isValidDate(death) && comparableDate(death) < comparableDate(birth)) {
        ...
        if (e.facts !== undefined && e.facts !== null) {
            if (!Array.isArray(e.facts)) {
                ...
                for (let j = 0; j < e.facts.length; j++) {
                    ...
                    if (!keyOk)
                        pushError(errors, CODES.E_BAD_FACT, `${fat}.key`, 'fact key must be a non-empty string');
                    if (!isScalar(f.value))
                        pushError(errors, CODES.E_BAD_FACT, `${fat}.value`, 'fact value must be a scalar');
    ...
    for (const p of parsed) {
        for (const a of p.aliases) {
            ...
            if (owners !== undefined && [...owners].some((o) => o !== p.idx)) {
                pushError(errors, CODES.E_ALIAS_COLLISION, `entities[${p.idx}].aliases`, `alias ${JSON.stringify(a)} collides with another entity's name or alias`);
            }
        }
    }
...
// ---------- sources validation ----------
function validateSources(doc, errors) {
    ...
    for (let i = 0; i < sources.length; i++) {
        ...
        if (s.url !== undefined && s.url !== null) {
            ...
            if (!urlOk)
                pushError(errors, CODES.E_BAD_URL, `${at}.url`, 'url must be syntactically a URL');
        }
 ...
 */
function parseCanonFacts(markdown) {
    const errors = [];
    const facts = [];
    const fail = () => ({ ok: false, store: null, facts, errors });
    ...
    try {
        ...
        pushError(errors, CODES.E_YAML_PARSE, '', `YAML parse failed: ${String(e.message)}`);
        return fail();
    }
    if (!isRecord(raw)) {
    ...
    if (store === 'lore')
        validateLore(raw, errors, facts);
    else
        validateSources(raw, errors);
    return { ok: errors.length === 0, store, facts, errors };
}
// ---------- ingestFacts ----------
function isFactShaped(v) {
    return (isRecord(v) &&
 ...
 */
function ingestFacts(existingFacts, observations) {
    const facts = Array.isArray(existingFacts)
    ...
    for (const obs of obsList) {
        ...
        if (declared.value === String(obs.value))
            continue; // silent confirmation
        findings.push({
            kind: 'CONTRADICTION',
...
function normalizeDatesDeep(v) {
    if (v instanceof Date)
        return v.toISOString().slice(0, 10);
    if (Array.isArray(v))
        return v.map(normalizeDatesDeep);
    if (isRecord(v)) {
        const out = {};
        for (const [k, val] of Object.entries(v))
            out[k] = normalizeDatesDeep(val);
        return out;
    }
    return v;
}
 ...
 */
function serializeCanonFacts(markdown, facts) {
    ...
    if (!isRecord(raw))
        return markdown;
    const doc = normalizeDatesDeep(raw);
    if (doc.store === 'lore') {
        ...
        for (const f of factList) {
            if (!isFactShaped(f))
                continue;
            let entity = entities.find((e) => isRecord(e) && e.id === f.entity_id);
            if (entity === undefined) {
            ...
            const existing = entityFacts.find((x) => isRecord(x) && x.key === f.key);
...
module.exports = { parseCanonFacts, ingestFacts, serializeCanonFacts, CODES };
