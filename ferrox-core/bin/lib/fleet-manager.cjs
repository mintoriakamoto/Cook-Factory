...
function managerPass(input) {
    ...
    const heldByLiveLease = new Set();
    let liveLeaseCount = 0;
    for (const raw of leases) {
        if (raw === null || typeof raw !== 'object')
            continue;
        const lease = raw;
        if (!_isLive(lease, nowMs))
            continue;
        liveLeaseCount += 1;
        if (typeof lease.node_id === 'string')
            heldByLiveLease.add(lease.node_id);
    }
    const candidates = [];
    const refused = [];
    for (const raw of nodes) {
        ...
        if (completed.has(id))
            continue;
        if (heldByLiveLease.has(id))
            continue;
        let ready = true;
        for (const dep of _asArray(node.depends_on)) {
            if (typeof dep !== 'string' || !completed.has(dep)) {
                ready = false;
                break;
            }
        }
    ...
    for (const candidate of candidates) {
        if (dispatch.length >= freeCapacity)
            break;
