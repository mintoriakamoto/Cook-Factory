...
// eslint-disable-next-line @typescript-eslint/no-require-imports
const antiloopLog = require("./antiloop-log.cjs");
// eslint-disable-next-line @typescript-eslint/no-require-imports
const antiloopGate = require("./antiloop-gate.cjs");
// eslint-disable-next-line @typescript-eslint/no-require-imports
...
function acceptedFlagList(verb) {
 ...
 */
function parseStrict(rest, verb, error) {
    const spec = ACCEPTED_FLAGS[verb];
    const values = {};
    const booleans = {};
    for (let i = 0; i < rest.length; i++) {
        const token = rest[i];
        if (!token.startsWith('--')) {
            error(`antiloop.${verb}: unexpected argument "${token}". Every input is a named flag. `
        ...
        error(`antiloop.${verb}: unrecognised flag ${token}. Accepted: ${acceptedFlagList(verb)}. `
...
function missingRequired(parsed, verb, required, error) {
    for (const name of required) {
        const v = parsed.values[name];
        if (v === undefined || v.trim() === '') {
            error(`antiloop.${verb}: --${name} is required and was not supplied. `
 ...
 */
function resolveLogPath(values, cwd) {
    const explicit = values.log;
    if (typeof explicit === 'string' && explicit !== '') {
        return node_path_1.default.isAbsolute(explicit) ? explicit : node_path_1.default.join(cwd, explicit);
    }
 ...
 */
function resolveNowIso(values) {
    const raw = values['now-ts'];
    const ms = Number(raw);
    if (raw !== undefined && Number.isFinite(ms))
 ...
 * drift D8 forbids.
 */
let manifestSecurityCache;
function manifestSecurityCategories() {
    if (manifestSecurityCache !== undefined)
        return manifestSecurityCache;
    try {
        const manifestPath = node_path_1.default.join(__dirname, ...MANIFEST_RELATIVE);
        const parsed = JSON.parse(node_fs_1.default.readFileSync(manifestPath, 'utf8'));
        const strength = parsed && typeof parsed === 'object' ? parsed.strength : undefined;
        const list = strength && typeof strength === 'object'
            ? strength.security_categories
            : undefined;
        manifestSecurityCache = Array.isArray(list) ? list : [];
    }
    catch {
...
function resolveSecurityCategories(cwd) {
 ...
 * at all.
 */
function handleDeclareBudget(rest, cwd, raw, error) {
    ...
    if (state !== undefined && state.budget_event_index !== -1) {
        error(`antiloop.declare-budget: a review budget is already declared for this pair at `
    ...
    writeJson({
        ...
        max_lineages: maxLineages,
        log: logPath,
        message: `the review budget for this pair is (${maxRounds} rounds, ${maxLineages} lineages). `
 ...
 */
function handleOpenRound(rest, cwd, raw, error) {
    const parsed = parseStrict(rest, 'open-round', error);
    if (parsed === null)
        return;
    if (missingRequired(parsed, 'open-round', ['artifact', 'question', 'gate', 'lineage'], error))
        return;
    const artifact = parsed.values.artifact;
    const question = parsed.values.question;
    const logPath = resolveLogPath(parsed.values, cwd);
    ...
    if (decision.decision !== 'open-ok') {
        ...
        error(`antiloop.open-round: ${decision.code} (${decision.decision}). `
 ...
 */
function handleFileFinding(rest, cwd, raw, error) {
    ...
    if (blockingRequested && !verdict.blocking) {
        ...
        error(`antiloop.file-finding: ${verdict.code} (${verdict.reason}). ${verdict.message} `
 ...
 */
function handleStatus(rest, cwd, raw, error) {
    const parsed = parseStrict(rest, 'status', error);
    if (parsed === null)
        return;
    if (missingRequired(parsed, 'status', ['artifact', 'question'], error))
        return;
    const artifact = parsed.values.artifact;
    const question = parsed.values.question;
    const logPath = resolveLogPath(parsed.values, cwd);
    ...
    for (const f of findings) {
        ...
        if (verdict.blocking)
            blocking.push({ finding: f, verdict });
        else
            nonBlocking.push(f);
    }
...
function routeAntiloopCommand({ args, cwd, raw, error }) {
    const rest = args.slice(2);
    routeCjsCommandFamily({
        args,
        subcommands: command_aliases_cjs_1.ANTILOOP_SUBCOMMANDS,
        unsupported: {},
        error,
        unknownMessage: (_s, available) => `Unknown antiloop subcommand. Available: ${available.join(', ')}`,
        handlers: {
...
module.exports = { routeAntiloopCommand };
