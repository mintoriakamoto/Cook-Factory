...
function evaluateCrossAuditCadence(opts) {
    ...
    if (event === 'milestone-complete')
        return { crossAudit: true, reason: 'milestone-boundary' };
    if (event === 'standalone-complete')
        return { crossAudit: true, reason: 'standalone-chunk' };
    if (event === 'phase-complete') {
        if (o.crossesRiskBoundary === true)
            return { crossAudit: true, reason: 'risk-boundary-crossed' };
        if (o.atIntegrationBoundary === true)
            return { crossAudit: true, reason: 'integration-boundary' };
        ...
        const since = typeof o.phasesSinceLastAudit === 'number' && Number.isFinite(o.phasesSinceLastAudit)
            ...
            : 0;
        if (since >= max)
            return { crossAudit: true, reason: 'max-phases-without-audit' };
...
module.exports = { evaluateCrossAuditCadence };
