'use strict';
/**
 * Profile-pipeline command router — CLI dispatcher for ferrox-tools profiling commands.
 *
 * ADR-857 phase 6 / ADR-959: profile-pipeline capability command cutover.
 * Extracted from hardcoded case arms in ferrox-tools.cjs (lines 1324-1410).
 ...
 * dispatchCapabilityCommand (ferrox-tools.cjs:366-371) explicitly errors if a
...
function routeExtractMessages({ args, cwd, raw, error, _pipeline }) {
  ...
  if (!projectArg || projectArg.startsWith('--')) {
    error('Usage: ferrox-tools extract-messages <project> [--session <id>] [--limit N] [--path <dir>]\nRun scan-sessions first to see available projects.', ERROR_REASON.USAGE);
    return;
  }
