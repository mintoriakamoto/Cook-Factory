...
function parseSpine(markdown) {
    const errors = [];
    const inner = (0, markdown_sectionizer_cjs_1.extractFencedBlock)(markdown, SPINE_INFO);
    if (inner === null) {
        errors.push({
 ...
 */
function renderManuscript(input) {
    ...
    for (const f of input.chapterFiles)
    ...
    if (orphans.length > 0) {
        throw new ManuscriptError(CODES.E_ORPHAN_CHAPTER, `orphan chapter file(s) with no spine row: ${orphans.join(', ')}. ` +
    ...
    if (missing.length > 0) {
        throw new ManuscriptError(CODES.E_MISSING_CHAPTER_FILE, `spine row(s) with no chapter file: ${missing.map((c) => c.id).join(', ')}. ` +
    ...
    for (const c of spine.chapters) {
        ...
        if (file === undefined)
            continue; // unreachable: missing-file check above is a hard error
        const fm = frontmatter_cjs_1.default.extractFrontmatter(file.markdown);
        const declaredId = typeof fm.chapter_id === 'string' ? fm.chapter_id : null;
        if (declaredId !== null && declaredId !== c.id) {
            throw new ManuscriptError(CODES.E_CHAPTER_ID_MISMATCH, `${file.filename} declares chapter_id ${declaredId} but the spine row says ${c.id}; ` +
        ...
        const target = typeof rawTarget === 'number' && Number.isFinite(rawTarget)
            ...
            : typeof rawTarget === 'string' && /^\d+$/.test(rawTarget)
                ? Number(rawTarget)
                : null;
        const body = frontmatter_cjs_1.default.stripFrontmatter(file.markdown).trim();
 ...
 */
function serializeManuscript(ir) {
    const parts = [];
 ...
 */
function assembleProject(projectDir) {
    const spinePath = node_path_1.default.join(projectDir, 'book', 'SPINE.md');
    if (!node_fs_1.default.existsSync(spinePath)) {
        throw new ManuscriptError(CODES.E_NO_SPINE, `no spine manifest at ${spinePath}; book/SPINE.md is the ordering truth`);
    }
    const spineMarkdown = node_fs_1.default.readFileSync(spinePath, 'utf8');
    const chaptersDir = node_path_1.default.join(projectDir, 'book', 'chapters');
    const chapterFiles = (node_fs_1.default.existsSync(chaptersDir) ? node_fs_1.default.readdirSync(chaptersDir) : [])
        .filter((name) => name.endsWith('.md'))
        .sort()
        .map((name) => ({ filename: name, markdown: node_fs_1.default.readFileSync(node_path_1.default.join(chaptersDir, name), 'utf8') }));
    const ir = renderManuscript({ spineMarkdown, chapterFiles });
    const outDir = node_path_1.default.join(projectDir, 'build');
    node_fs_1.default.mkdirSync(outDir, { recursive: true });
    const outPath = node_path_1.default.join(outDir, 'manuscript.md');
    node_fs_1.default.writeFileSync(outPath, serializeManuscript(ir));
    return { ir, outPath };
}
/** Format the compile report for the CLI (pure, returns printable lines). */
function formatCompileReport(report, outLabel) {
    const lines = [];
...
if (require.main === module) {
    try {
        const projectDir = node_path_1.default.resolve(process.argv[2] !== undefined ? process.argv[2] : process.cwd());
        const { ir, outPath } = assembleProject(projectDir);
        for (const line of formatCompileReport(ir.report, node_path_1.default.relative(projectDir, outPath) || outPath)) {
            console.log(line);
        }
    }
    catch (err) {
