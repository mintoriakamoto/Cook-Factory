...
// eslint-disable-next-line @typescript-eslint/no-require-imports
const antiloop = require("./antiloop-log.cjs");
// ─── the vocabulary ──────────────────────────────────────────────────────────
 ...
 * copy.
 */
const PROOF_EVENT_KINDS = runlog.FLEET_EVENT_KINDS;
const _KIND_SET = new Set(PROOF_EVENT_KINDS);
/**
...
const ARMS = Object.freeze(['serial', 'fleet']);
/** The 3 provenances. A `replayed` arm can never return POSITIVE. See D7. */
const PROVENANCES = Object.freeze(['measured', 'replayed', 'unavailable']);
/**
 ...
 */
const WORKER_OUTCOMES = Object.freeze(['completed', 'failed', 'abnormal']);
const GATE_VERDICTS = Object.freeze(['green', 'red', 'unknown']);
 ...
 */
const DECIDED_CLASSIFICATIONS = Object.freeze([
...
// ─── small helpers ───────────────────────────────────────────────────────────
function _err(code, message, index) {
...
function validateRunRecord(record) {
    ...
    if (!Array.isArray(record)) {
        errors.push(_err(PROOF_CODES.E_PR_NOT_ARRAY, 'the run record must be an array of parsed events; reading the JSONL file is the caller\'s job', null));
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (!_isPlainObject(raw)) {
            errors.push(_err(PROOF_CODES.E_PR_EVENT_NOT_OBJECT, `event ${i} is ${raw === null ? 'null' : typeof raw}, not an object`, i));
            skip.add(i);
            continue;
        }
        const event = raw;
        ...
        if (typeof kind !== 'string' || kind === '') {
            errors.push(_err(PROOF_CODES.E_PR_KIND_MISSING, `event ${i} carries no kind, so nothing can be folded from it`, i));
            skip.add(i);
            continue;
        }
        if (!_KIND_SET.has(kind)) {
            errors.push(_err(PROOF_CODES.E_PR_KIND_UNKNOWN, `event ${i} carries kind ${JSON.stringify(kind)}, which is outside the `
                ...
                + 'this reader must not fold.', i));
            skip.add(i);
            continue;
        }
        if (!_isValidTs(event.ts)) {
            errors.push(_err(PROOF_CODES.E_PR_TS_INVALID, `event ${i} of kind ${kind} carries ts ${JSON.stringify(event.ts)}, which is not a `
                + 'finite non negative number. Time arrives as a number in the record and this '
                + 'module never reads a clock, so an unusable ts is unrecoverable here.', i));
            skip.add(i);
            continue;
        }
        if (PROOF_ATTEMPT_KINDS.has(kind) && !_isPresentId(event.attempt_id)) {
            errors.push(_err(PROOF_CODES.E_PR_ATTEMPT_MISSING, `event ${i} of kind ${kind} is attempt scoped and carries no attempt_id. `
                + 'The attempt bearing set is derived from the shipped required field map, so the '
                + 'park kinds, which carry a node and no attempt on purpose, are outside it.', i));
            skip.add(i);
            continue;
        }
    ...
    for (let i = 0; i < events.length; i++) {
        if (skip.has(i))
            continue;
        usable.push({ index: i, event: events[i] });
    }
    // ── pass 2: record level ───────────────────────────────────────────────────
...
function _checkClosedEnum(event, kind, index) {
    let field;
    let allowed;
    ...
    if (typeof value === 'string' && allowed.includes(value))
        return null;
    return _err(PROOF_CODES.E_PR_OUTCOME_UNKNOWN, `event ${index} of kind ${kind} carries ${field} ${JSON.stringify(value)}, outside the `
        + `closed vocabulary ${allowed.join(', ')}. The emitter fixes this enum, so a value `
        + 'outside it is a record this reader cannot classify.', index);
}
function _findFirst(usable, kind) {
...
function _checkRunBoundary(usable, errors) {
    if (_findFirst(usable, 'run_started') === null) {
        errors.push(_err(PROOF_CODES.E_PR_RUN_NOT_STARTED, 'the record carries no run_started event, so it has no origin to measure any '
            + 'elapsed time against', null));
    }
    if (_findFirst(usable, 'run_closed') === null) {
        errors.push(_err(PROOF_CODES.E_PR_RUN_NOT_CLOSED, 'the record carries no run_closed event, so it may be a truncated log rather than '
            + 'a finished run, and a figure folded from a truncated log understates itself', null));
    }
}
function _checkRunIds(usable, errors) {
    const seen = new Set();
    for (const row of usable) {
        const runId = row.event.run_id;
        if (_isPresentId(runId))
            seen.add(_idString(runId));
    }
    if (seen.size > 1) {
        const ids = [...seen].sort();
        errors.push(_err(PROOF_CODES.E_PR_RUN_ID_MIXED, `the record mixes ${seen.size} distinct run_id values (${ids.join(', ')}). `
            + 'Two runs folded as 1 produce a width and a wall clock that belong to neither.', null));
    }
}
function _checkRunStartedFields(usable, errors) {
    ...
    if (typeof arm !== 'string' || !ARMS.includes(arm)) {
        errors.push(_err(PROOF_CODES.E_PR_ARM_UNKNOWN, `run_started carries arm ${JSON.stringify(arm)}, which is not 1 of ${ARMS.join(', ')}. `
            + 'A record that does not name its arm cannot be placed on either side of the comparison.', index));
    }
    const provenance = event.provenance;
    if (typeof provenance !== 'string' || !PROVENANCES.includes(provenance)) {
        errors.push(_err(PROOF_CODES.E_PR_PROVENANCE_UNKNOWN, `run_started carries provenance ${JSON.stringify(provenance)}, which is not 1 of `
            + `${PROVENANCES.join(', ')}. Provenance is what stops a replayed arm being reported `
            + 'as proof of speed, so an unnameable provenance is a disabled guard.', index));
    }
    const corpusHash = event.corpus_hash;
    if (typeof corpusHash !== 'string' || corpusHash === '') {
        errors.push(_err(PROOF_CODES.E_PR_CORPUS_HASH_MISSING, 'run_started carries no corpus_hash. Without it 2 arms cannot be shown to have run '
            + 'the same work, and a comparison drawn over different corpora is not a comparison.', index));
    }
}
function _checkMonotonic(usable, errors) {
    const start = _findFirst(usable, 'run_started');
    if (start === null)
        return;
    const originTs = start.event.ts;
    for (const row of usable) {
        if (row.index === start.index)
            continue;
        const ts = row.event.ts;
        if (ts < originTs) {
            errors.push(_err(PROOF_CODES.E_PR_TS_NON_MONOTONIC, `event ${row.index} of kind ${String(row.event.kind)} carries ts ${ts}, which precedes `
                + `the run_started ts ${originTs}. An event before the run began yields a negative `
                + 'elapsed time, and a negative duration folded into a mean silently shortens it.', row.index));
        }
    }
}
function _checkWorkerPairs(usable, errors) {
    const started = new Map();
    const ended = new Set();
    for (const row of usable) {
        if (row.event.kind !== 'worker_started')
            continue;
        const key = _key(row.event.worker_id, row.event.node_id, row.event.attempt_id);
        if (!started.has(key))
            started.set(key, row.index);
    }
    for (const row of usable) {
        if (row.event.kind !== 'worker_ended')
            continue;
        const key = _key(row.event.worker_id, row.event.node_id, row.event.attempt_id);
        if (started.has(key)) {
            ended.add(key);
        ...
        errors.push(_err(PROOF_CODES.E_PR_WORKER_END_UNMATCHED, `event ${row.index} is a worker_ended with no matching worker_started for worker `
            + `${_idString(row.event.worker_id)} on attempt ${_idString(row.event.attempt_id)}`, row.index));
    }
    for (const [key, index] of started) {
        if (ended.has(key))
            continue;
        errors.push(_err(PROOF_CODES.E_PR_WORKER_UNTERMINATED, `event ${index} is a worker_started with no matching worker_ended. An unterminated `
            + 'interval makes demonstrated width UNKNOWABLE rather than approximate: closing it '
            + 'at run end is the single easiest way to manufacture a positive result.', index));
    }
}
function _checkGatePairs(usable, errors) {
    const started = new Set();
    for (const row of usable) {
        if (row.event.kind !== 'gate_started')
            continue;
        started.add(_key(row.event.attempt_id, _gateOf(row.event)));
    }
    for (const row of usable) {
        if (row.event.kind !== 'gate_ended')
            continue;
        const key = _key(row.event.attempt_id, _gateOf(row.event));
        if (started.has(key))
            continue;
        errors.push(_err(PROOF_CODES.E_PR_GATE_END_UNMATCHED, `event ${row.index} is a gate_ended with no matching gate_started on attempt `
            + `${_idString(row.event.attempt_id)} and gate ${_gateOf(row.event)}. Matching is by `
            + 'that pair rather than by order of appearance, because concurrent gates interleave.', row.index));
    }
}
function _checkQueuePairs(usable, errors) {
    const entered = new Set();
    for (const row of usable) {
        if (row.event.kind !== 'queue_entered')
            continue;
        entered.add(_key(row.event.attempt_id, row.event.ticket));
    }
    for (const row of usable) {
        if (row.event.kind !== 'queue_acquired')
            continue;
        const key = _key(row.event.attempt_id, row.event.ticket);
        if (entered.has(key))
            continue;
        errors.push(_err(PROOF_CODES.E_PR_QUEUE_ACQUIRED_UNMATCHED, `event ${row.index} is a queue_acquired with no matching queue_entered on attempt `
            + `${_idString(row.event.attempt_id)} and ticket ${_idString(row.event.ticket)}, so the `
            + 'queue wait for that attempt cannot be separated from its gate cost', row.index));
    }
}
function _known(value, unit) {
    return { value, unit, state: METRIC_STATES.KNOWN, reason: '' };
}
function _unknown(unit, reason) {
    return { value: null, unit, state: METRIC_STATES.UNKNOWN, reason };
}
function _undefinedMetric(unit, reason) {
...
function _foldable(record) {
    ...
    for (const raw of record) {
        ...
        if (typeof kind !== 'string' || !_KIND_SET.has(kind))
            continue;
        if (!_isValidTs(raw.ts))
            continue;
        out.push(raw);
    }
    return out;
}
function _ofKind(events, kind) {
 ...
 */
const LAND_RESULTS_LANDED = Object.freeze(['completed', 'landed']);
/**
...
function _partitionLands(events) {
    ...
    for (const e of _ofKind(events, 'land_completed')) {
        ...
        if (LAND_RESULTS_LANDED.includes(family)) {
            landed.add(key);
            const ts = e.ts;
            if (lastLandedTs === null || ts > lastLandedTs)
                lastLandedTs = ts;
        }
...
function _unrecognisedPhrase(unrecognised) {
 ...
 * comparison.
 */
function foldWallClockToLand(record) {
    ...
    if (lands.lastLandedTs === null) {
        return _undefinedMetric('ms', `the run landed nothing, so it has no time to land. ${lands.notLanded.size} land_completed `
            + 'event' + (lands.notLanded.size === 1 ? '' : 's') + ' record a land that did not land');
    }
    return _known(lands.lastLandedTs - started.ts, 'ms');
}
 ...
 */
function foldDemonstratedWidth(record) {
    const events = _foldable(record);
    ...
    for (const e of _ofKind(events, 'worker_ended')) {
        const key = _key(e.worker_id, e.node_id, e.attempt_id);
        if (starts.has(key) && !ends.has(key))
            ends.set(key, e.ts);
    }
    if (starts.size === 0) {
        return _undefinedMetric('workers', 'the record opened 0 worker intervals, and a maximum over an empty set is not a width');
    }
    const open = starts.size - ends.size;
    if (open > 0) {
        return _unknown('workers', `${open} unterminated worker interval${open === 1 ? '' : 's'}: a worker that ended without `
    ...
    for (const [key, ts] of starts) {
        ...
        marks.push({ ts: ends.get(key), delta: -1 });
    }
 ...
 */
function foldLandGateCost(record) {
    const events = _foldable(record);
    const none = [];
    const starts = new Map();
    ...
    for (const e of _ofKind(events, 'gate_ended')) {
        ...
        durations.push({ concurrency: start.concurrency, ms: e.ts - start.ts });
    }
    if (unmatched > 0) {
        return {
    ...
    const missingConcurrency = durations.filter((d) => !(typeof d.concurrency === 'number' && Number.isFinite(d.concurrency) && d.concurrency >= 1)).length;
    ...
    for (const d of durations) {
        ...
        if (list === undefined)
            byConcurrency.set(c, [d.ms]);
        else
            list.push(d.ms);
    }
    const buckets = [...byConcurrency.entries()]
        .sort((a, b) => a[0] - b[0])
 ...
 */
function foldLandGateContention(record) {
    const events = _foldable(record);
    const none = [];
    const refuse = (reason) => ({
    ...
    let unterminated = 0;
    for (const key of entered.keys())
        if (!completed.has(key))
            unterminated += 1;
    if (unterminated > 0) {
        return refuse(`${unterminated} unterminated queue interval${unterminated === 1 ? '' : 's'}: an increment that `
    ...
    for (const key of completed.keys())
        if (!entered.has(key))
            orphaned += 1;
    if (orphaned > 0) {
        return refuse(`${orphaned} land_completed event${orphaned === 1 ? '' : 's'} with no matching queue_entered on `
            + 'the same node and attempt, so the record cannot say how deep the queue was when they landed');
    }
    const intervals = [];
    for (const [key, start] of entered)
        intervals.push({ start, end: completed.get(key) });
    ...
    for (const e of _ofKind(events, 'gate_ended')) {
        ...
        placed.push({ depth: depthAt(start.ts), ms: e.ts - start.ts });
    }
    if (unmatched > 0) {
        return refuse(`${unmatched} gate_ended event${unmatched === 1 ? ' has' : 's have'} no matching gate_started on `
            + 'the same attempt and gate, so an unmatched gate cost cannot be placed on the curve');
    }
    if (placed.length === 0) {
        return refuse(`a contention depth of ${peak} was derived from ${intervals.length} queue interval`
    ...
    for (const row of placed) {
        ...
        if (list === undefined)
            byDepth.set(row.depth, [row.ms]);
        else
            list.push(row.ms);
    }
    const buckets = [...byDepth.entries()]
        .sort((a, b) => a[0] - b[0])
        .map(([depth, list]) => {
        const sorted = [...list].sort((a, b) => a - b);
 ...
 */
function foldFalseGreenRate(record) {
    const events = _foldable(record);
    const lands = _partitionLands(events);
    ...
    for (const e of _ofKind(events, 'post_land_truth')) {
        const key = _key(e.node_id, e.attempt_id);
        if (!truths.has(key))
            truths.set(key, String(e.classification));
    }
    ...
    if (decided === 0) {
        ...
        return _undefinedMetric('ratio', `${detail}. A rate of 0 and a rate nobody measured are different claims, so no number `
            + 'is reported here');
    }
    if (landed.size === 0) {
        return _unknown('ratio', `the record carries ${decided} decided post_land_truth classifications and 0 landed `
            + 'increments, so the denominator does not exist');
    }
    if (decided < landed.size) {
        const shortfall = landed.size - decided;
        return _unknown('ratio', `a shortfall of ${shortfall}: ${decided} DECIDED post_land_truth classifications `
 ...
 */
function foldRoundsPerArtifact(logPath) {
    ...
    for (const entry of entries) {
        if (entry === null || typeof entry !== 'object')
            continue;
        if (entry.kind !== 'round-opened')
            continue;
        const key = _key(entry.artifact, entry.question);
        perKey.set(key, (perKey.get(key) ?? 0) + 1);
    }
    if (perKey.size === 0) {
        return _undefinedMetric('rounds', 'the anti loop log holds 0 round-opened events, so there is no artifact to average over');
    }
    let total = 0;
    let max = 0;
 ...
 */
function foldCostPerLandedIncrement(record) {
    const events = _foldable(record);
    const spends = events.filter((e) => typeof e.usd === 'number' && Number.isFinite(e.usd));
    if (spends.length === 0) {
        return _undefinedMetric('usd', 'no event in the record carries a usd figure. The shipped run record vocabulary has no '
    ...
    const landed = lands.landed;
    if (landed.size === 0) {
        return _unknown('usd', `the record carries ${spends.length} events with spend and 0 landed increments, so a cost `
...
function _startedField(events, field, fallback) {
    const started = events.find((e) => e.kind === 'run_started');
    if (started === undefined)
        return fallback;
 ...
 * for a metric of 0.
 */
function assembleFoldDocument(input) {
...
function _metricOf(doc, key) {
    if (!_isPlainObject(doc))
        return null;
    const metrics = doc.metrics;
    if (!_isPlainObject(metrics))
        return null;
    const metric = metrics[key];
    if (!_isPlainObject(metric))
        return null;
    return metric;
}
/** The metric's number when it is KNOWN, and null in every other case. */
function _knownValue(doc, key) {
    const metric = _metricOf(doc, key);
    if (metric === null || metric.state !== METRIC_STATES.KNOWN)
        return null;
...
function _stateOf(doc, key) {
    const metric = _metricOf(doc, key);
    return metric === null ? '' : String(metric.state);
}
function _corpusOf(doc) {
    if (!_isPlainObject(doc))
        return null;
    const hash = doc.corpus_hash;
    return typeof hash === 'string' && hash !== '' ? hash : null;
}
function _provenanceOf(doc) {
    if (!_isPlainObject(doc))
        return '';
 ...
 */
function _statePhrase(states) {
    if (states.length === 0)
        return 'absent: the arm carries 0 records';
...
function _pooledSpread(docs, key) {
    const values = [];
    for (const doc of docs) {
        const value = _knownValue(doc, key);
        if (value === null)
    ...
    if (values.length < MIN_SERIAL_RUNS)
        return null;
 ...
 * one.
 */
function _pooledFalseGreen(docs) {
    let falseGreens = 0;
    let landed = 0;
    for (const doc of docs) {
...
function compareArms(input) {
    ...
    if (serial.length === 0) {
        refusals.push(`${PROOF_REFUSALS.NO_BASELINE}: the serial arm is empty. A fleet number with no serial `
            + 'number on the same tasks is not a measurement, it is a reading.');
    }
    if (fleet.length === 0) {
        refusals.push(`${PROOF_REFUSALS.NO_FLEET_ARM}: the fleet arm is empty. A serial number with no fleet number `
    ...
    if (serial.length < MIN_SERIAL_RUNS || sigmaMs === null) {
        refusals.push(`${PROOF_REFUSALS.SIGMA_UNDEFINED}: sigma is the observed serial spread and needs at least `
    ...
    if (hashes.size > 1) {
        refusals.push(`${PROOF_REFUSALS.CORPUS_MISMATCH}: the arms carry ${hashes.size} distinct corpus hashes `
            + `(${[...hashes].sort().join(', ')}). A comparison drawn over different work is not a `
            + 'comparison.');
    }
    const widthStates = _statesOf(fleet, 'demonstrated_width');
    if (fleetWidth === null) {
        refusals.push(`${PROOF_REFUSALS.WIDTH_UNKNOWN}: the fleet arm's demonstrated width is `
    ...
    if (falseGreenStates.some((s) => s !== METRIC_STATES.KNOWN) || fleetFalseGreenPooled === null) {
        ...
        const weightless = fleet.filter((d) => _knownValue(d, 'false_green_rate') !== null && _landedOf(d) === null).length;
        refusals.push(`${PROOF_REFUSALS.FALSE_GREEN_UNDEFINED}: at least 1 arm carries a false green rate that is `
    ...
    if (wallStates.some((s) => s !== METRIC_STATES.KNOWN) || fleetWall === null) {
        refusals.push(`${PROOF_REFUSALS.WALL_CLOCK_UNKNOWN}: at least 1 arm carries a wall clock to land that is `
    ...
    if (refusals.length > 0) {
        reasons.push(`${VERDICTS.INSUFFICIENT}: ${refusals.length} refusal${refusals.length === 1 ? '' : 's'} `
    ...
    const F = fleetWall;
    const sigma = sigmaMs;
    const fleetFalseGreen = fleetFalseGreenPooled.rate;
    const width = fleetWidth;
    const serialRoundsValues = [];
    for (const doc of serial) {
        const rounds = _knownValue(doc, 'rounds_per_artifact');
        if (rounds !== null)
            serialRoundsValues.push(rounds);
    }
    ...
    const negatives = [];
    if ((F - S) > sigma) {
        negatives.push(`${VERDICTS.NEGATIVE}: F ${_num(F)} minus S ${_num(S)} is ${_num(F - S)}, which exceeds `
    ...
    if (fleetFalseGreen > FALSE_GREEN_THRESHOLD) {
        const counts = fleetFalseGreenPooled;
        negatives.push(`${VERDICTS.NEGATIVE}: the fleet false green rate ${_num(fleetFalseGreen)} is above the `
    ...
    if (roundsComparable && fleetRounds < serialRounds) {
        negatives.push(`${VERDICTS.NEGATIVE}: fleet rounds per artifact ${_num(fleetRounds)} fell below `
    ...
    const blockers = [];
    if (!((S - F) > sigma)) {
        blockers.push(`the speed gain S ${_num(S)} minus F ${_num(F)} is ${_num(S - F)}, at or inside sigma `
            + `${_num(sigma)}`);
    }
    if (width < MIN_DEMONSTRATED_WIDTH) {
        blockers.push(`the demonstrated width ${_num(width)} is below ${MIN_DEMONSTRATED_WIDTH}, so this run is a `
    ...
    if (roundsComparable && fleetRounds < serialRounds) {
        blockers.push(`fleet rounds per artifact ${_num(fleetRounds)} is below the serial arm's `
            + `${_num(serialRounds)}`);
    }
    const provenances = [...serial, ...fleet].map(_provenanceOf);
    if (provenances.some((p) => p !== MEASURED_PROVENANCE)) {
        blockers.push(`at least 1 arm carries provenance ${provenances.filter((p) => p !== MEASURED_PROVENANCE).join(', ')} `
    ...
    if (blockers.length === 0) {
        ...
        const roundsClause = roundsComparable
            ...
            : 'rounds per artifact UNKNOWN on at least 1 arm and therefore EXCLUDED from this gate, '
                ...
                + 'number';
        reasons.push(`${VERDICTS.POSITIVE}: S ${_num(S)} minus F ${_num(F)} is ${_num(S - F)}, which exceeds sigma `
    ...
    // ── the remainder ──────────────────────────────────────────────────────────
    reasons.push(`${VERDICTS.MARGINAL}: the absolute difference of S ${_num(S)} and F ${_num(F)} is `
    ...
    for (const blocker of blockers)
        reasons.push(`${VERDICTS.MARGINAL}: ${blocker}.`);
