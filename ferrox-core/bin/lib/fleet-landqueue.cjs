...
// eslint-disable-next-line @typescript-eslint/no-require-imports
const boardLib = require("./fleet-board.cjs");
// eslint-disable-next-line @typescript-eslint/no-require-imports
 ...
 */
function _assertVocabulary() {
    const kinds = runlog.FLEET_EVENT_KINDS;
    if (!kinds.includes(QUEUE_ENTERED)) {
        throw new Error(`fleet-landqueue: ${QUEUE_ENTERED} is not in the run log vocabulary. `
 ...
 */
function nextTicket(events) {
    const list = Array.isArray(events) ? events : [];
    let count = 0;
    for (const raw of list) {
        if (raw === null || typeof raw !== 'object')
            continue;
        if (raw.kind === QUEUE_ENTERED)
            count += 1;
    }
    return count;
}
...
function queueHead(queue) {
    ...
    for (const row of tickets) {
        ...
        if (row.completed_at !== null && row.completed_at !== undefined)
            continue;
        if (typeof row.ticket !== 'number' || !Number.isFinite(row.ticket))
            continue;
        if (head === null || row.ticket < head)
            head = row.ticket;
    }
    return head;
}
 ...
 */
function mayAcquireToken(queue, ticket, nowMs, liveness) {
    ...
    const holderTicket = (holder !== null && typeof holder === 'object'
        && typeof holder.ticket === 'number')
        ? holder.ticket
        : null;
    if (holder !== null && !boardLib.isReclaimable(holder, nowMs, liveness)) {
        return {
            allowed: false,
...
function _nowOf(opts) {
    ...
    if (typeof opts.nowMs === 'number' && Number.isFinite(opts.nowMs))
        return opts.nowMs;
    throw new Error('fleet-landqueue: opts.nowMs or opts.clock is REQUIRED. This module never reads the '
 ...
 */
function _requireRepoKey(repoKey) {
    if (typeof repoKey !== 'string' || repoKey === '') {
        throw _codedError(LANDQUEUE_ERROR_CODES.E_FLEET_BAD_REPO_KEY, `fleet-landqueue: repoKey must be a non-empty string, received ${typeof repoKey}. `
 ...
 */
function landTokenPath(opts) {
    const key = _requireRepoKey(opts.repoKey);
    const dir = (typeof opts.tokenDir === 'string' && opts.tokenDir !== '')
        ? opts.tokenDir
        : node_path_1.default.dirname(String(opts.logPath ?? '.'));
    const slug = key.replace(/[^A-Za-z0-9._-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 48) || 'repo';
    const digest = (0, node_crypto_1.createHash)('sha256').update(key, 'utf8').digest('hex').slice(0, 16);
    return node_path_1.default.join(dir, `land-token-${slug}-${digest}.json`);
}
/**
 ...
 */
function _writeTokenSnapshot(tokenPath, handle, repoKey, events, nowMs) {
...
function _append(opts, entry) {
    runlog.appendFleetEvent(entry, { path: opts.logPath });
    return entry;
}
/**
 ...
 */
function enterLandQueue(opts) {
    const repoKey = _requireRepoKey(opts.repoKey);
    const tokenPath = landTokenPath(opts);
    return atomicState.withFileLock(tokenPath, (handle) => {
 ...
 */
function _completeTicket(opts, tokenPath, repoKey, nodeId, attemptId, result, nowMs, events, handle) {
    const entry = _append(opts, {
        ts: nowMs,
        kind: 'land_completed',
        run_id: opts.runId,
...
function acquireLandToken(opts) {
    ...
    return atomicState.withFileLock(tokenPath, (handle) => {
        ...
        if (holder !== null
            ...
            events = _completeTicket(opts, tokenPath, repoKey, holder.node_id, holder.attempt_id, RESULT_RECLAIMED, nowMs, events, handle);
        ...
        if (opts.holder !== null && opts.holder !== undefined)
...
function releaseLandToken(opts) {
    const repoKey = _requireRepoKey(opts.repoKey);
    const tokenPath = landTokenPath(opts);
    ...
    return atomicState.withFileLock(tokenPath, (handle) => {
        const nowMs = _nowOf(opts);
        const events = _readEvents(opts);
        _completeTicket(opts, tokenPath, repoKey, opts.nodeId, opts.attemptId, result, nowMs, events, handle);
...
function _resultOf(value) {
    if (value !== null && typeof value === 'object' && typeof value.result === 'string') {
        return value.result;
    }
 ...
 */
function withLandToken(opts, fn) {
    ...
    try {
        for (;;) {
            ...
            if (elapsed >= waitTimeoutMs || polls >= maxPolls) {
                throw _codedError(LANDQUEUE_ERROR_CODES.E_FLEET_TOKEN_WAIT_TIMEOUT, `fleet-landqueue: ticket ${ticket} waited ${elapsed}ms across ${polls} attempts for the `
 ...
 */
function landEngineEntrypoint(ctx) {
    ...
    if (explicit !== '') {
        const resolvedRepo = node_path_1.default.resolve(repoRoot);
        const resolvedEngine = node_path_1.default.resolve(explicit);
        if (resolvedEngine === resolvedRepo || resolvedEngine.startsWith(resolvedRepo + node_path_1.default.sep)) {
            throw _codedError(LAND_ERROR_CODES.E_FLEET_ENGINE_ROOT_INSIDE_REPO, `fleet-landqueue: the engine root ${JSON.stringify(resolvedEngine)} is inside the repository `
    ...
    const command = node_path_1.default.join(engineRoot, ...RATCHET_ENTRYPOINT);
    if (!node_fs_1.default.existsSync(command)) {
        throw _codedError(LAND_ERROR_CODES.E_FLEET_NO_ENGINE_ROOT, `fleet-landqueue: no land engine at ${JSON.stringify(command)}. The engine root and the `
...
function _git(repoRoot, args, extraEnv) {
    const proc = (0, node_child_process_1.spawnSync)('git', ['-C', repoRoot, ...args], {
        encoding: 'utf8',
...
function _localRefusal(code, detail, fields) {
    return {
        code: 1,
        verdict: 'red',
        result: `refused:${code}`,
...
function _resolveRef(repoRoot, ref) {
 ...
 */
function landLocal(ctx) {
    const repoRoot = (typeof ctx.repoRoot === 'string' && ctx.repoRoot !== '') ? ctx.repoRoot : '';
    if (repoRoot === '') {
        return _localRefusal(LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_NO_REPO_ROOT, 'the local land strategy was given no repoRoot, and the trunk it merges into is that repository');
    }
    const inside = _git(repoRoot, ['rev-parse', '--is-inside-work-tree']);
    if (inside.status !== 0 || inside.stdout.trim() !== 'true') {
        return _localRefusal(LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_NOT_A_REPO, `${repoRoot} is not inside a git work tree: ${inside.stderr.trim() || inside.stdout.trim()}`);
    }
    ...
    if (mainline === '') {
        return _localRefusal(LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_NO_MAINLINE, `no mainline was named and ${repoRoot} has a detached HEAD, so there is no trunk to land into`);
    }
    const before = _resolveRef(repoRoot, `refs/heads/${mainline}`);
    if (before === '') {
        return _localRefusal(LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_NO_MAINLINE, `refs/heads/${mainline} does not resolve to a commit in ${repoRoot}`, { mainline });
    }
    ...
    if (branch === '') {
        return _localRefusal(LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_NO_BRANCH, 'no worker branch was named and none could be read off the worktree', { mainline, before });
    }
    const tip = _resolveRef(repoRoot, `refs/heads/${branch}`);
    if (tip === '') {
        return _localRefusal(LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_NO_BRANCH, `refs/heads/${branch} does not resolve to a commit in ${repoRoot}`, { branch, mainline, before });
    }
    ...
    if (mainlineCheckedOut) {
        ...
        if (dirty.status !== 0 || dirty.stdout.trim() !== '') {
            return _localRefusal(LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_DIRTY_TRUNK, `${repoRoot} has uncommitted work on ${mainline}: ${dirty.stdout.trim() || dirty.stderr.trim()}`, { branch, mainline, before });
        }
    }
    ...
    if (merged.status === 1) {
        return _localRefusal(LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_CONFLICT, `merging ${branch} into ${mainline} conflicts:\n${merged.stdout.trim()}`, { branch, mainline, before });
    }
    if (merged.status !== 0) {
        const text = `${merged.stderr}\n${merged.stdout}`;
        const unsupported = /unknown option|usage: git merge-tree|not a git command/i.test(text);
        return _localRefusal(unsupported
            ? LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_UNSUPPORTED_GIT
            : LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_MERGE_FAILED, `git merge-tree exited ${String(merged.status)}: ${text.trim()}`, { branch, mainline, before });
    }
    const tree = merged.stdout.split('\n')[0].trim();
    if (!/^[0-9a-f]{40,64}$/.test(tree)) {
        return _localRefusal(LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_MERGE_FAILED, `git merge-tree reported no tree object: ${merged.stdout.trim()}`, { branch, mainline, before });
    }
    ...
    if (committed.status !== 0) {
        return _localRefusal(LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_MERGE_FAILED, `git commit-tree exited ${String(committed.status)}: ${committed.stderr.trim()}`, { branch, mainline, before });
    }
    const next = committed.stdout.trim();
    if (!/^[0-9a-f]{40,64}$/.test(next)) {
        return _localRefusal(LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_MERGE_FAILED, `git commit-tree reported no commit object: ${committed.stdout.trim()}`, { branch, mainline, before });
    }
    ...
    if (published.status !== 0) {
        return _localRefusal(LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_PUBLISH_FAILED, `the trunk refused to move to ${next}: ${published.stderr.trim() || published.stdout.trim()}`, { branch, mainline, before, after: _resolveRef(repoRoot, `refs/heads/${mainline}`) || null });
    }
    ...
    if (after !== next) {
        return _localRefusal(LAND_ERROR_CODES.E_FLEET_LAND_LOCAL_PUBLISH_FAILED, `${mainline} is at ${after || 'nothing'} after the publish, not at the merge ${next}`, { branch, mainline, before, after: after || null });
    }
    return {
        code: 0,
        verdict: 'green',
 ...
 */
function defaultLandCommand(ctx) {
    ...
    if (strategy === LAND_STRATEGY.LOCAL)
        return landLocal(ctx);
    if (strategy !== LAND_STRATEGY.ENGINE) {
        throw _codedError(LAND_ERROR_CODES.E_FLEET_UNKNOWN_LAND_STRATEGY, `fleet-landqueue: ${JSON.stringify(strategy)} is not a land strategy. The 2 are `
    ...
    const proc = (0, node_child_process_1.spawnSync)(spec.command, spec.args, { encoding: 'utf8', env: landSpawnEnv(ctx) });
    if (proc.error !== undefined && proc.error !== null)
        throw proc.error;
    const code = typeof proc.status === 'number' ? proc.status : null;
    if (code === 0)
        return { code, verdict: 'green', result: 'landed' };
    if (code === null)
        return { code, verdict: VERDICT_UNKNOWN, result: 'unknown:no-exit-status' };
...
function _normaliseOutcome(raw) {
...
function runLand(opts) {
    ...
    return withLandToken(opts, (ctx) => {
        ...
        let outcome = { code: null, verdict: VERDICT_UNKNOWN, result: VERDICT_UNKNOWN };
        try {
