...
/**
 * Deterministic latest-version check for /ferrox-update (#2992).
 *
 * The /ferrox-update workflow's check_latest_version step was previously
 * prescribed in LLM-driven prose ("run `npm view ferrox-core
 * version`"). The executing model could shortcut the prescription and
 * invent npm queries against wrong-shaped names (`@ferrox-core/cli`,
 * `get-shit-done-cli`, `ferrox`), all of which 404 or — worse — return an
 ...
 * `npm view ferrox-core@next version` resolves the RC channel (#660).
 */
...
function checkLatestVersion(opts = {}) {
  ...
  // Bounded at 15s so a hung registry doesn't block /ferrox-update (#2993 CR).
  const defaultSpawn = () => {
