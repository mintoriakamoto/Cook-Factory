7b. **Pre-wave dependency check (waves 2+ only):**
    Before wave N+1, run `ferrox-tools.cjs query verify.key-links {phase_dir}/{plan}-PLAN.md` for each upcoming plan.
...
7c. **Between-wave manifest reset and worktree base refresh (waves 2+ only — #1369):**
   ...
   **REQUIRED before each wave transition when `USE_WORKTREES != "false"` and `RUNTIME = "claude"`.**
   ...
   if [ "$RUNTIME" = "claude" ] && [ "$USE_WORKTREES" != "false" ]; then
     ferrox_run query worktree.set-baseref 2>/dev/null || true
     ...
     _BETWEEN_DEGRADE=$(ferrox_run query worktree.base-check --pick shouldDegrade 2>/dev/null || echo "false")
