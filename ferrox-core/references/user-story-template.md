...
> Used by `mvp-phase` workflow and `ferrox-planner` agent when `MVP_MODE=true`. Defines the canonical "As a / I want to / So that" format and the rules for converting it into the `**Goal:**` line in ROADMAP.md.
...
The `ferrox-planner` agent (with MVP_MODE=true) emits the user story as the first content under the phase header in `PLAN.md`:
