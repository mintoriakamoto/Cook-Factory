...
```bash
PHASE_INFO=$(ferrox-tools query find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(ferrox-tools query roadmap.get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(ferrox-tools query find-phase "${PHASE}" --raw)
```
