...
Display:
```
Ferrox > BLOCKED: {N} blockers must be resolved before {operation} can proceed.
```
...
Proceed silently or display `Ferrox > No conflicts detected.` Either is acceptable; workflows choose based on verbosity context.
