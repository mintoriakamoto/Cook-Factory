...
> see ferroxfactory/ferrox-core#1600 / #1601). This is the durable counterpart to the
...
2. **Query your configured skills.** Use **your own agent type** — the `name:` value in
   your frontmatter (e.g. an agent whose frontmatter says `name: ferrox-executor` queries
   `ferrox-executor`). The query is read-only and idempotent — it exits 0 with an empty
   block when nothing is configured for your type:

   ```bash
   _AGENT_SKILLS=$(ferrox_run query agent-skills <YOUR-FRONTMATTER-NAME> 2>/dev/null || true)
   ```

   The runtime `ferrox_run` resolver is the standard one from
