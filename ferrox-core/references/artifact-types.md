# Ferrox Artifact Types

This reference documents all artifact types in the Ferrox planning taxonomy. Each type has a defined
...
- **Location**: `~/.claude/ferrox-core/USER-PROFILE.md`
...
### Spike README.md / MANIFEST.md (per-spike, via /ferrox:spike)
- **Shape**: YAML frontmatter (spike, name, validates, verdict, related, tags) + run instructions + results
- **Lifecycle**: Created by `/ferrox:spike` → Verified → Wrapped up by `/ferrox-spike-wrap-up`
- **Location**: `.planning/spikes/NNN-name/README.md`, `.planning/spikes/MANIFEST.md`
- **Consumed by**: `/ferrox-spike-wrap-up` for curation; `pause-work` for spike context handoff
...
- **Lifecycle**: Created by `/ferrox:sketch` → Evaluated → Wrapped up by `/ferrox-sketch-wrap-up`
- **Location**: `.planning/sketches/NNN-name/README.md`, `.planning/sketches/NNN-name/index.html`, `.planning/sketches/MANIFEST.md`
- **Consumed by**: `/ferrox-sketch-wrap-up` for curation; `pause-work` for sketch context handoff

### WRAP-UP-SUMMARY.md (per wrap-up session)
- **Shape**: Curation results, included/excluded items, feature/design area groupings
- **Lifecycle**: Created by `/ferrox-spike-wrap-up` or `/ferrox-sketch-wrap-up`
