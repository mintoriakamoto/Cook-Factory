...
The `--ws <name>` flag scopes Ferrox operations to a specific workstream, enabling
parallel milestone work by multiple Claude Code instances on the same codebase.

## Resolution Priority

1. `--ws <name>` flag (explicit, highest priority)
2. `FERROX_WORKSTREAM` environment variable (per-instance)
...
Ferrox now prefers a session-scoped pointer keyed by runtime/session identity
(`FERROX_SESSION_KEY`, `CODEX_THREAD_ID`, `CLAUDE_CODE_SSE_PORT`, terminal session IDs,
...
When Ferrox resolves the session-scoped pointer in step 3 above, it uses this order:

1. Explicit runtime/session env vars such as `FERROX_SESSION_KEY`, `CODEX_THREAD_ID`,
...
If none of those produce a stable identity, Ferrox does not keep probing. It falls
...
non-interactive, Ferrox intentionally skips shelling out to `tty` because that path
...
- If that was the last pointer for the repo, Ferrox also removes the now-empty
  per-project temp directory
- If sibling session pointers still exist, the temp directory is left in place
- When a pointer refers to a workstream directory that no longer exists, Ferrox
...
Ferrox does not currently run a background garbage collector for historical temp
...
All workflow routing commands include `${FERROX_WS}` which:
...
```bash
# All ferrox-tools query commands accept --ws
ferrox-tools query state.json --ws feature-a
ferrox-tools query find-phase 3 --ws feature-b

# Session-local switching without --ws on every command
FERROX_SESSION_KEY=my-terminal-a ferrox-tools query workstream.set feature-a
FERROX_SESSION_KEY=my-terminal-a ferrox-tools query state.json
FERROX_SESSION_KEY=my-terminal-b ferrox-tools query workstream.set feature-b
FERROX_SESSION_KEY=my-terminal-b ferrox-tools query state.json

# Workstream CRUD
ferrox-tools query workstream.create <name>
ferrox-tools query workstream.list
ferrox-tools query workstream.status <name>
ferrox-tools query workstream.complete <name>
```
