...
Smart discuss is the autonomous-optimized variant of `ferrox-discuss-phase`. It proposes grey area answers in batch tables — the user accepts or overrides per area — then writes an identical CONTEXT.md to what discuss-phase produces.

**Inputs:** `PHASE_NUM` from execute_phase. Run init to get phase paths:

```bash
PHASE_STATE=$(ferrox-tools query init.phase-op ${PHASE_NUM})
```
...
```bash
DETAIL=$(ferrox-tools query roadmap.get-phase ${PHASE_NUM})
```
...
```bash
ferrox-tools query commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
