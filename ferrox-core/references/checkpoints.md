...
The gate spans two layers, and both must honor it. `ferrox-executor` refuses to auto-approve a `gate="blocking-human"` checkpoint and escalates it via `checkpoint_return_format` precisely so a human sees it; `execute-phase`'s `checkpoint_handling` step then decides what the user is actually shown. An orchestrator that dispatches on checkpoint *type* alone would auto-approve the very checkpoint the executor just refused to auto-approve, nullifying that refusal one layer up and letting an unattended `--auto` / `--chain` run install a package no human ever vetted.

**Human checkpoints have an SLA (anti-loop).** An open human checkpoint must not stall the Build
Line forever. The halting verb `human-sla.check` times an open checkpoint against
`halting.human_sla_seconds` and, on breach, returns `breached` and parks the increment so the line
can pull the next queued increment forward (HALT-03). Consult the verb for the cap — never hard-code
the SLA. This is documented protocol the workflow follows, not a running scheduler; see
