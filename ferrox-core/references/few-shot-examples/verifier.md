...
**Input:** Must-have artifact: `ferrox-core/workflows/context-bridge.md: >50 lines`

**Output:**

> **L1 (Existence):** EXISTS -- `ferrox-core/workflows/context-bridge.md` found, 187 lines.
...
> **L3 (Wired):** WIRED -- Referenced by `ferrox-core/workflows/execute-phase.md` at step 3. Called by `agents/ferrox-executor.md` via @-reference include. Arguments verified: receives planning directory path matching the expected parameter.
> **Status: PASSED**
...
> **Method:** Ran `grep -n "scope" ferrox-core/bin/validate-commit.js` to locate scope validation logic.
...
> **Ran:** `echo "feat(: test" | node ferrox-core/bin/validate-commit.js` -- exit code 0 (accepted).
...
> **Method:** Ran `grep -n "feedback-loop\|extractFeedback" ferrox-core/bin/check-subagent-output.js`.
...
> **Cross-check:** `grep -rn "require.*feedback-loop" ferrox-core/bin/` returns zero results across all scripts.
...
> **Method:** Ran `grep "architecture_graph\|architecture_guard" ferrox-core/bin/lib/config-schema.json`.
> **Evidence:** Zero matches in schema. However, `grep "architecture" ferrox-core/bin/lib/config.cjs` shows both flags in CONFIG_DEFAULTS (lines 426, 428).
...
> **Method:** Ran `grep -rn "\/\[a-zA-Z\]\/" ferrox-core/bin/` to find inline MSYS regex.
> **Evidence:** 2 matches in `commands/ferrox/misc.md` (lines 26, 487). These were NOT in the original plan scope -- Phase 148 extracted code into misc.md AFTER Phase 149's plan was written.
...
**Input:** Must-have artifact: `ferrox-core/bin/lib/audit-checks/index.cjs: >100 lines`

**Output:**

> **L1 (Existence):** EXISTS -- File found at `ferrox-core/bin/lib/audit-checks/index.cjs`, 142 lines.
> **Status: PASSED**
