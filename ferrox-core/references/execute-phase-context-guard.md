0. **Context exhaustion guard — `context_guard` (BEFORE spawning, #1452):**
   ...
   | POOR (70%+) | Emit: "🛑 Context pressure POOR — risk of context exhaustion. Run `/ferrox:pause-work` to checkpoint before this wave, then resume in a fresh session." Continue (user decides). | Invoke `/ferrox:pause-work` immediately and halt. Do NOT spawn wave agents. | Skip |
