...
SPECLESS_CFG=$(ferrox_run query config-get workflow.specless_probe_fallback 2>/dev/null || echo "true")
...
_ferrox_lib() { for _d in "$_FERROX_RT/ferrox-core/bin/lib" "$_FERROX_RT/bin/lib" "$_FERROX_RT/.claude/bin/lib" "$HOME/.claude/ferrox-core/bin/lib" "$HOME/.claude/bin/lib"; do [ -f "$_d/$1" ] && { echo "$_d/$1"; return; }; done; }
SPEC_SECTION_JS=$(_ferrox_lib spec-section.cjs)
if [ -z "$SPEC_SECTION_JS" ] && [ -f "$_FERROX_RT/tsconfig.build.json" ] && [ -f "$_FERROX_RT/src/spec-section.cts" ]; then
  npm --prefix "$_FERROX_RT" run build:lib 2>/dev/null || true; SPEC_SECTION_JS=$(_ferrox_lib spec-section.cjs)
fi
[ -n "$SPEC_SECTION_JS" ] || { echo "ERROR: spec-section.cjs not found - reinstall Ferrox or run build:lib." >&2; exit 1; }
...
```bash
# Resolve the compiled edge-probe.cjs against the Ferrox install dir via RUNTIME_DIR (#448) — NOT the
# consuming project's git root — falling back to git toplevel / $HOME/.claude (spec-phase.md:198 idiom).
_FERROX_RT="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
EDGE_PROBE_JS=$(for _c in \
  "$_FERROX_RT/ferrox-core/bin/lib/edge-probe.cjs" "$_FERROX_RT/bin/lib/edge-probe.cjs" \
  "$_FERROX_RT/.claude/bin/lib/edge-probe.cjs" "$HOME/.claude/ferrox-core/bin/lib/edge-probe.cjs" \
  "$HOME/.claude/bin/lib/edge-probe.cjs"; do [ -f "$_c" ] && { echo "$_c"; break; }; done)
# Build ONLY inside a verified Ferrox source checkout; --prefix pins npm so we never trigger the
...
if [ -z "$EDGE_PROBE_JS" ]; then
  ...
  [ -n "$EDGE_PROBE_JS" ] || { echo "ERROR: edge-probe.cjs not found — reinstall Ferrox or run \`npm run build:lib\`." >&2; exit 1; }
fi
...
assumption. Pass `$COVERAGE` (+ the gate's `$SPECLESS_FALLBACK_DISABLED` note) into the ferrox-planner
...
There is NO compiled prohibition engine and NO `node` invocation (ADR-550 D7b) — the ferrox-planner runs
this in-prompt. Full two-stage protocol, canon-referral rule, and status×verification schema live in
`~/.claude/ferrox-core/references/prohibition-probe.md` (do not inline it). Summary:
