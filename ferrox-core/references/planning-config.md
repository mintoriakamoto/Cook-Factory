...
"git": {
  ...
  "base_branch": null,
  "phase_branch_template": "ferrox/phase-{phase}-{slug}",
  "milestone_branch_template": "ferrox/{milestone}-{slug}",
...
| `workflow.test_command` | `null` | Custom shell command run as the regression/test gate by verify-phase, execute-phase, audit-fix, and post-merge-gate. When unset, Ferrox auto-detects (Makefile / package.json / Cargo.toml / go.mod / pyproject.toml). Example: `npm test`. |
...
| `manager.flags.discuss` | `""` | Flags passed to `/ferrox:discuss-phase` when dispatched from manager (e.g. `"--auto --analyze"`) |
...
**Using `ferrox-tools query` (preferred):**

```bash
# Commit with automatic commit_docs + gitignore checks:
ferrox-tools query commit "docs: update state" --files .planning/STATE.md

# Load config via state load (returns JSON):
INIT=$(ferrox-tools query state.load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
# commit_docs is available in the JSON output

# Or use init commands which include commit_docs:
INIT=$(ferrox-tools query init.execute-phase "1")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
...
**Note:** Most Ferrox operations use direct file reads or explicit paths, which work regardless of gitignore status.
...
- Standard Ferrox behavior

**When `git.branching_strategy: "phase"`:**
- `execute-phase` creates/switches to a branch before execution
- Branch name from `phase_branch_template` (e.g., `ferrox/phase-03-authentication`)
...
- Branch name from `milestone_branch_template` (e.g., `ferrox/v1.0-mvp`)
...
| `review.models.<cli>` | string\|null | `null` | Any model ID string | Per-CLI model override for /ferrox:review (e.g., `review.models.gemini`). Falls back to CLI default when null. |
...
| `workflow.ai_integration_phase` | boolean | `true` | `true`, `false` | Run /ferrox:ai-integration-phase before planning AI system phases |
...
| `workflow.test_command` | string\|null | `null` | Any shell command | Regression/test gate command run by verify-phase, execute-phase, audit-fix, and post-merge-gate. Unset → Ferrox auto-detects (Makefile / package.json / Cargo.toml / go.mod / pyproject.toml). |
...
| `workflow.context_guard_mode` | string | `"warn"` | `"auto"`, `"warn"`, `"off"` | Context exhaustion guard mode for `execute-phase`. Before each wave, the orchestrator self-assesses context pressure using degradation signals from `context-budget.md`. `"warn"` (default): emit a warning and recommend `/ferrox:pause-work` when POOR tier is detected. `"auto"`: automatically invoke `/ferrox:pause-work` before the next wave when POOR tier is detected. `"off"`: disable the guard. The guard is heuristic — no programmatic context-% API exists. |
...
| `workflow.code_review_command` | string\|null | `null` | Any shell command | External code-review command integrated into `/ferrox:ship`. The diff is piped to the command via stdin; the command must output JSON with a `verdict` field (`"APPROVED"` or `"REVISE"`). Non-zero exit or `"REVISE"` verdict blocks the ship workflow. When unset, the built-in review flow runs. Example: `my-review-tool --review`. |
...
Set via `ship.*` namespace in config.json. These fields affect `/ferrox:ship` PRD-style pull request body composition only.
...
| `git.phase_branch_template` | string | `"ferrox/phase-{phase}-{slug}"` | Template with `{phase}`, `{slug}` | Branch naming template for `phase` strategy |
| `git.milestone_branch_template` | string | `"ferrox/{milestone}-{slug}"` | Template with `{milestone}`, `{slug}` | Branch naming template for `milestone` strategy |
...
| `features.global_learnings` | boolean | `false` | `true`, `false` | Enable injection of global learnings from `~/.ferrox/knowledge/` into agent prompts |
...
Set via `intel.*` namespace (e.g., `"intel": { "enabled": true }`). Controls the queryable codebase intelligence system consumed by `/ferrox:map-codebase --query`.

| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `intel.enabled` | boolean | `false` | `true`, `false` | Enable queryable codebase intelligence system. When `true`, `/ferrox:map-codebase --query` builds and queries a JSON index in `.planning/intel/`. |
...
| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `manager.flags.discuss` | string | `""` | Any CLI flags string | Flags passed to `/ferrox:discuss-phase` from manager (e.g., `"--auto --analyze"`) |
...
5. **Search API keys and flags** -- `brave_search`, `firecrawl`, and `exa_search` are auto-set to `true` during project creation if the corresponding API key is detected (environment variable or `~/.ferrox/<name>_api_key` file). Setting them to `true` without the API key has no effect.
...
8. **`sub_repos` auto-sync** -- On every config load, Ferrox scans for child directories with `.git` and updates the `sub_repos` array if the filesystem has changed. Legacy `multiRepo: true` is automatically migrated to a detected `sub_repos` array.

9. **`workflow.use_worktrees` and branch divergence** -- When `use_worktrees` is `true` (default), executor worktrees are forked from `origin/HEAD` by the Claude Code harness. If your current branch has commits that `origin/HEAD` does not (for example an unmerged milestone or feature branch), Ferrox automatically degrades to sequential execution for that run and prints a one-line `⚠ Worktree base mismatch` warning. To restore parallel execution permanently, set `worktree.baseRef:"head"` in `.claude/settings.local.json` (run `node ferrox-tools.cjs worktree set-baseref`). This makes the harness fork worktrees from the live HEAD instead of `origin/HEAD`. Both fresh installs and upgrades of Ferrox Factory set this automatically (no-clobber) when `use_worktrees` is enabled; you can also run the command manually at any time. Setting `workflow.use_worktrees: false` is the alternative if worktrees are not needed at all.
