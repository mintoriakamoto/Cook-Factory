...
Generic reference for consuming the `--raw` JSON output of `ferrox_run loop render-hooks <point>`
...
- `ref.skill` present → dispatch via the Skill tool with skill id `ferrox-<ref.skill>`.
