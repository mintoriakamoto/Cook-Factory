...
| POOR | 70%+ | Emergency mode. Checkpoint progress immediately. No new reads unless critical. | `warn`: emit warning + recommend `/ferrox:pause-work`. `auto`: invoke pause-work before next wave. `off`: proceed anyway. |
...
Every enabled MCP server injects its tool schema into **every turn**, regardless of whether you call any of its tools. Heavyweight servers can cost 20k+ tokens per turn each — often dwarfing whatever Ferrox itself can save through `model_profile` tuning. This is a Claude Code harness concern, not a Ferrox concern: Ferrox does **not** manage MCP enablement. The toggle lives in `.claude/settings.json` under `enabledMcpjsonServers` and `disabledMcpjsonServers`.
...
Before starting a long phase (especially `/ferrox:execute-phase`, `/ferrox:plan-phase`, or anything that fans out across many subagents), run this audit:
