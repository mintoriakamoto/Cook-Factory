...
10. **NEVER** use non-Ferrox agent types (`general-purpose`, `Explore`, `Plan`, `Bash`, `feature-dev`, etc.) -- ALWAYS use `subagent_type: "ferrox-{agent}"` (e.g., `ferrox-phase-researcher`, `ferrox-executor`, `ferrox-planner`). Ferrox agents have project-aware prompts, audit logging, and workflow context. Generic agents bypass all of this.
...
15. **No direct Write/Edit to STATE.md or ROADMAP.md for mutations.** Always use `ferrox-tools query` for registered state/roadmap handlers (e.g. `state.update`, `state.advance-plan`, `roadmap.update-plan-progress`), or legacy `node …/ferrox-tools.cjs` for CLI-only commands. Direct Write tool usage bypasses safe update logic and is unsafe in multi-session environments. Exception: first-time creation of STATE.md from template is allowed.
...
## Ferrox-Specific Rules

24. **Do not** check for `mode === 'auto'` or `mode === 'autonomous'` -- Ferrox uses `yolo` config flag. Check `yolo: true` for autonomous mode, absence or `false` for interactive mode.
25. **Prefer `ferrox-tools query`** for orchestration when a handler exists; when shelling out to the legacy CLI, use **`ferrox-tools.cjs`** (not `ferrox-tools.js` or any other filename) — Ferrox ships the programmatic API as CommonJS for Node.js CLI compatibility.
26. **Plan files MUST follow `{padded_phase}-{NN}-PLAN.md` pattern** (e.g., `01-01-PLAN.md`). Never use `PLAN-01.md`, `plan-01.md`, or any other variation -- ferrox-tools detection depends on this exact pattern.
