...
**Test:** Could a different Claude instance execute without asking clarifying questions? If not, add specificity. See @~/.claude/ferrox-core/references/planner-antipatterns.md for vague-vs-specific comparison table.
...
Run `/clear` first for a fresh context window, then execute: `/ferrox:execute-phase {phase}`
```
...
Execute: `/ferrox:execute-phase {phase} --gaps-only`
```
