...
|---|---|---|
| `references/planner-mvp-mode.md` | **Rules.** Vertical-slice planning rules, slice ordering, Walking Skeleton constraints. | `ferrox-planner` agent when `MVP_MODE=true` |
| `references/skeleton-template.md` | **Template.** Shape of `SKELETON.md` for new-project Phase 1 under `--mvp`. | `ferrox-planner` agent when the Walking Skeleton gate fires |
| `references/user-story-template.md` | **Template.** Format and slot definitions for `As a / I want to / So that`. | `ferrox-mvp-phase` workflow during interactive prompting; `ferrox-planner` when emitting the `## Phase Goal` header |
| `references/spidr-splitting.md` | **Splitting discipline.** Five-axis decomposition (Spike, Paths, Interfaces, Data, Rules) for stories too large for one phase. | `ferrox-mvp-phase` workflow when the user story exceeds size threshold |
| `references/execute-mvp-tdd.md` | **Gate.** MVP+TDD runtime gate semantics: when it fires, what it checks, halt-and-report protocol, end-of-phase blocking escalation, Behavior-Adding Task definition. | `ferrox-executor` agent when `MVP_MODE=true && TDD_MODE=true` |
| `references/verify-mvp-mode.md` | **UAT framing.** Three-section UAT structure (user-flow → technical → coverage), anti-patterns, `User Flow Coverage` section in VERIFICATION.md. | `ferrox-verifier` agent when the phase under verification has `mode: mvp` |
...
- **`**Mode:** mvp` parser** — `ferrox-core/bin/lib/roadmap.cjs` (`searchPhaseInContent` + `cmdRoadmapAnalyze`). Workflows compare against the parser output, never re-parse.
- **User Story regex** — `/^As a .+, I want to .+, so that .+\.$/` — applied at runtime by `ferrox-verifier` (the user-story-format guard) and `ferrox-mvp-phase` (interactive validation).
- **Behavior-Adding Task predicate** — `references/execute-mvp-tdd.md` (the canonical three-check definition). Applied at runtime by `ferrox-executor`.
...
- `planner-mvp-mode.test.cjs` — ferrox-planner agent MVP section
- `mvp-phase-command.test.cjs`, `mvp-phase-integration.test.cjs`, `mvp-phase-spidr.test.cjs` — `/ferrox:mvp-phase`
