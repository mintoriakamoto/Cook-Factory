...
Completion markers and handoff schemas for all Ferrox agents. Workflows use these markers to detect agent completion and route accordingly.
...
|-------|------|--------------------|
| ferrox-planner | Plan creation | `## PLANNING COMPLETE` |
| ferrox-executor | Plan execution | `## PLAN COMPLETE`, `## CHECKPOINT REACHED` |
| ferrox-phase-researcher | Phase-scoped research | `## RESEARCH COMPLETE`, `## RESEARCH BLOCKED` |
| ferrox-project-researcher | Project-wide research | `## RESEARCH COMPLETE`, `## RESEARCH BLOCKED` |
| ferrox-plan-checker | Plan validation | `## VERIFICATION PASSED`, `## ISSUES FOUND` |
| ferrox-research-synthesizer | Multi-research synthesis | `## SYNTHESIS COMPLETE`, `## SYNTHESIS BLOCKED` |
...
| ferrox-integration-checker | Cross-phase integration check | `## Integration Check Complete` (title case) |
| ferrox-nyquist-auditor | Sampling audit | `## PARTIAL`, `## ESCALATE` (non-standard) |
| ferrox-security-auditor | Security audit | `## OPEN_THREATS`, `## ESCALATE` (non-standard) |
| ferrox-codebase-mapper | Codebase analysis | No marker (writes docs directly) |
| ferrox-assumptions-analyzer | Assumption extraction | No marker (returns `## Assumptions` sections) |
| ferrox-doc-verifier | Doc validation | No marker (writes JSON to `.planning/tmp/`) |
| ferrox-doc-writer | Doc generation | No marker (writes docs directly) |
| ferrox-advisor-researcher | Advisory research | No marker (utility agent) |
| ferrox-user-profiler | User profiling | No marker (returns JSON in analysis tags) |
| ferrox-intel-updater | Codebase intelligence analysis | `## INTEL UPDATE COMPLETE`, `## INTEL UPDATE FAILED` |

## Marker Rules

1. **ALL-CAPS markers** (e.g., `## PLANNING COMPLETE`) are the standard convention
2. **Title-case markers** (e.g., `## Verification Complete`) exist in ferrox-verifier and ferrox-integration-checker -- these are intentional as-is, not bugs
...
> **NOTE:** `## PLAN COMPLETE` is the ferrox-executor's completion marker but execute-phase.md does not regex-match it. Instead, it detects executor completion via spot-checks (SUMMARY.md existence, git commit state). This is intentional behavior, not a mismatch.
