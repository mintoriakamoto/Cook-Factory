...
> **Cap is config-driven, not hard-coded.** The "max 3 iterations" bound below, the stall
> detection, and the escalation are all governed by `gate.cap-check` reading the
> `halting.gates.<gate>` config (`max_passes` / `wall_clock_seconds` → the gate's `cap_outcome`).
> The verb is the single source of truth for the cap — the `3` shown here is only the built-in
> default. See [halting.md](halting.md) for the config block, the verb, and the run-log that
...
|----------|---------------|---------------|-------|
| plan-phase | ferrox-planner | ferrox-plan-checker | Revision prompt via planner-revision.md |
| execute-phase | ferrox-executor | ferrox-verifier | Post-execution verification |
| discuss-phase | orchestrator | ferrox-plan-checker | Inline revision by orchestrator |
