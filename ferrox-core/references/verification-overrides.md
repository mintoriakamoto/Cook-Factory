...
- Multiple must-haves are failing for the same phase — the "revisit the plan instead of overriding in bulk" threshold is the same config-driven cap surfaced by the halting layer: resolve it with `gate.cap-check` against `halting.gates.<gate>.max_passes` rather than restating a hard-coded number here (the 2-3 shown is only the built-in default). When the cap fires, revisit the plan instead of overriding in bulk. See [halting.md](halting.md).
...
During `/ferrox:audit-milestone`, overrides are surfaced in the audit report:
