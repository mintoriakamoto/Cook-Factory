...
- ferrox-doc-writer — Writes and updates project documentation files
- ferrox-doc-verifier — Verifies factual claims in docs against the live codebase
</available_agent_types>
...
INIT=$(ferrox_run query docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(ferrox_run query agent-skills ferrox-doc-writer)
```
...
- `existing_docs` — array of `{path, has_ferrox_marker}` objects for existing Markdown files
...
- These will be passed to ferrox-doc-verifier in the verify_docs step for accuracy checking
- If inaccuracies are found, they will be dispatched to ferrox-doc-writer in `fix` mode for surgical corrections
...
4. For each gap the user selects:
   ...
   - The ferrox-doc-writer will receive a `doc_assignment` with `type: "custom"` and a description of what to document, using the project's source files as content discovery targets
...
3. If no docs in the queue have `has_ferrox_marker: false` in the `existing_docs` array: skip to detect_runtime_capabilities.

**For each queued doc where `has_ferrox_marker` is false (hand-written doc detected):**

Present the following choice using `AskUserQuestion` if available, or inline prompt otherwise:

```
{filename} appears to be hand-written (no Ferrox marker found).
...
How should this file be handled?
  ...
  [3] regenerate  -- Overwrite with a fresh Ferrox-generated doc.
```
...
Spawn 3 parallel ferrox-doc-writer agents for Wave 1 docs: README, ARCHITECTURE, CONFIGURATION (each runs in a subagent — no output until they return, ~1–5 min; expected, not a freeze).
...
```
Agent(
  subagent_type="ferrox-doc-writer",
...
Spawn a `ferrox-doc-writer` agent with `run_in_background=true`:
...
**Fallback when Task tool is unavailable:** Generate per-package READMEs sequentially inline after the `sequential_generation` step. For each package directory with a `package.json`, construct the equivalent `doc_assignment` block and generate the README following ferrox-doc-writer instructions.
...
Read `agents/ferrox-doc-writer.md` instructions once before beginning. Follow the create_mode or update_mode instructions from that agent for each doc, using the same doc_assignment fields as the parallel path.
...
1. **README** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase (Read, Grep, Glob, Bash) following ferrox-doc-writer create_mode / update_mode instructions
...
2. **ARCHITECTURE** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase following ferrox-doc-writer instructions
...
- Follow ferrox-doc-writer instructions for per_package scope
...
1. Print: `◆ Spawning doc verifier for {doc_path}... (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze)`
   Spawn the `ferrox-doc-verifier` agent (or invoke sequentially if Task tool is unavailable) with a `<verify_assignment>` block:
   ```xml
   <verify_assignment>
...
1. Print: `◆ Spawning doc verifier for {doc_path}... (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze)`
   Spawn the `ferrox-doc-verifier` agent with the same `<verify_assignment>` block as above.
...
Non-canonical docs with failures ARE eligible for the fix_loop. When a non-canonical doc has `claims_failed > 0`, dispatch it to ferrox-doc-writer in `fix` mode with the failures array — the writer's fix mode does surgical corrections on specific lines regardless of doc type (no template needed). The writer MUST NOT restructure, rephrase, or reformat any content beyond the failing claims.
...
1. For each doc with `claims_failed > 0` in the latest verification_results:
   a. Read the current file content from disk. Record the pre-fix line count:
      ...
      ```
   b. Spawn `ferrox-doc-writer` agent (or invoke sequentially) with a fix assignment:
      ```xml
      <doc_assignment>
...
Invoke the ferrox-doc-verifier agent in read-only mode for each file in `existing_docs` from the init JSON:

1. For each doc in `existing_docs`:
   a. Spawn `ferrox-doc-verifier` (or invoke sequentially if Task tool is unavailable) with:
      ```xml
      <verify_assignment>
      doc_path: {doc.path}
      project_root: {project_root from init JSON}
...
Display note:
```
To fix failures automatically: /ferrox:docs-update (runs generation + fix loop)
To regenerate all docs from scratch: /ferrox:docs-update --force
```
...
3. Re-run /ferrox:docs-update to regenerate clean docs
```
...
```bash
ferrox_run query commit "docs: generate project documentation" \
...
```
Run `/ferrox:docs-update --verify-only` to fact-check generated docs against the codebase.
```
...
- [ ] Generated docs contain zero Ferrox methodology content
...
- [ ] Hand-written docs (no Ferrox marker) prompted for preserve/supplement/regenerate before dispatch (unless --force)
...
- [ ] --verify-only invokes ferrox-doc-verifier for full fact-checking (not just VERIFY marker count)
</success_criteria>
