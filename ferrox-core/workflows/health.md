...
ferrox_run query validate.context \
...
```bash
ferrox_run query validate.health $REPAIR_FLAG $BACKFILL_FLAG
```
...
- [E001] config.json: JSON parse error at line 5
  Fix: Run /ferrox:health --repair to reset to defaults

- [E002] PROJECT.md not found
  Fix: Run /ferrox:new-project to create
```
...
```
---
N issues can be auto-repaired. Run: /ferrox:health --repair
```
</step>
...
```
Would you like to run /ferrox:health --repair to fix N issues automatically?
```
...
```bash
ferrox_run query validate.health
```
...
| W019 | warning | Unrecognized .planning/ root file — not a canonical Ferrox artifact | No |
