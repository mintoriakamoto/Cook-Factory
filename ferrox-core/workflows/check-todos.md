...
- `/ferrox:capture --list` → show all
- `/ferrox:capture --list api` → filter to area:api only
</step>
...
- `/ferrox:capture --list [area]` to filter by area
- `q` to exit
```
...
ferrox_run query commit "docs: start work on todo - [title]" --files .planning/todos/completed/[filename] .planning/STATE.md
```
