...
4. If nothing found: error "No milestone found. Run /ferrox:new-project or /ferrox:new-milestone first."
...
ferrox_run query init.progress
```
...
```bash
ferrox_run query commit "docs(v${VERSION}): generate milestone summary for onboarding" --files \
...
If the user is done:
- Suggest next steps: `/ferrox:new-milestone`, `/ferrox:progress`, or sharing the summary with the team
