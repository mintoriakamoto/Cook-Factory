...
post-execute codebase-drift gate in `/ferrox:execute-phase` or by a user running
`/ferrox:map-codebase --paths apps/accounting,packages/ui`), the workflow
operates in **incremental-remap mode**:

- Pass `--paths <p1>,<p2>,...` through to each spawned `ferrox-codebase-mapper`
...
INIT=$(ferrox_run query init.map-codebase)
...
Spawn 4 parallel ferrox-codebase-mapper agents.

Use Agent tool with `subagent_type="ferrox-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `ferrox-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.
...
Use the same document templates as the `ferrox-codebase-mapper` agent. Include actual file paths formatted with backticks.
...
```bash
ferrox_run query commit "docs: map existing codebase" --files .planning/codebase/*.md
```
...
**Also available:**
- Re-run mapping: `/ferrox:map-codebase`
...
- If Agent tool available: 4 parallel ferrox-codebase-mapper agents spawned with run_in_background=true
- If Agent tool NOT available: 4 sequential mapping passes performed inline (never using browser_subagent)
- All 7 codebase documents exist
- No empty documents (each should have >20 lines)
- Clear completion summary with line counts
- User offered clear next steps in Ferrox style
</success_criteria>
