...
```bash
AGENT_SKILLS_DEBUGGER=$(ferrox_run query agent-skills ferrox-debugger)
...
Before spawning, materialize the guard into WORKTREE_GUARD: read `ferrox-core/references/worktree-branch-check.md`, substitute `{EXPECTED_BASE}` with `$EXPECTED_BASE`, and use the resulting `<worktree_branch_check>` block (the runnable guard) as WORKTREE_GUARD below.
...
```bash
ferrox_run query commit "docs({phase_num}): add root causes from diagnosis" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
