...
Use /ferrox:quick for anything that needs multi-step planning or research.
</purpose>
...
```
This looks like it needs planning. Use /ferrox:quick instead:
  /ferrox:quick "{task description}"
```
...
that matches the existing table's schema via the schema-backed `ferrox-tools
...
if grep -q "Quick Tasks Completed" .planning/STATE.md 2>/dev/null; then
  ferrox_run quick-tasks-append --task "$TASK" || echo "⚠ fast.md log_to_state: could not append Quick Tasks row (see message above); continuing."
fi
```
</step>
...
- If the task takes more than 3 file edits, STOP and redirect to /ferrox:quick
- If you're unsure how to implement it, STOP and redirect to /ferrox:quick
</guardrails>
