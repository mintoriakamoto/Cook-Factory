...
AUDIT=$(ferrox_run query audit-uat --raw)
```
...
1. **Close stale items:** `/ferrox:verify-work {phase}` — mark stale tests as resolved
2. **Run active tests:** Human UAT test plan below
3. **When prerequisites met:** Retest blocked items with `/ferrox:verify-work {phase}`
```
</step>
