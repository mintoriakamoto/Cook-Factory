...
Run /ferrox:new-project to initialize.
```
Exit.
</step>
...
```bash
PHASE_DATA=$(ferrox_run query roadmap get-phase "${target}")
```
...
Available phases can be seen with /ferrox:progress.
```
...
```bash
ALL_PHASES=$(ferrox_run query roadmap analyze)
```
...
- `/ferrox:progress` — view updated roadmap
- `/ferrox:plan-phase {target}` — re-plan this phase (if needed)
- `/ferrox:discuss-phase {target}` — discuss implementation approach
