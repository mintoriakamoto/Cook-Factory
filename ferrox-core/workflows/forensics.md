...
Post-mortem investigation for failed or stuck Ferrox workflows. Analyzes git history,
...
3. {Recovery command if applicable — e.g., `/ferrox:resume-work`, `/ferrox:execute-phase N`}

---

*Report generated by `/ferrox:forensics`. All paths redacted for portability.*
```
...
BUG_LABEL=$(gh label list --repo ferroxfactory/ferrox-core --search "bug" --json name -q '.[0].name' 2>/dev/null)
LABEL_FLAG=""
if [ -n "$BUG_LABEL" ]; then
...
gh issue create \
  --repo ferroxfactory/ferrox-core \
