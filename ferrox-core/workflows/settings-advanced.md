<purpose>
Interactive configuration of Ferrox power-user knobs — plan bounce, node repair, subagent timeouts,
...
This is a companion to `/ferrox:settings` — the common-case prompt there covers model profile,
...
All subsequent reads and writes go through `$FERROX_CONFIG_PATH`. Never hardcode
...
- `git.phase_branch_template` (default: `ferrox/phase-{phase}-{slug}`)
- `git.milestone_branch_template` (default: `ferrox/{milestone}-{slug}`)
...
AskUserQuestion([
  ...
  {
    question: "Phase branch template? (current: <value or ferrox/phase-{phase}-{slug}>)",
  ...
  {
    question: "Milestone branch template? (current: <value or ferrox/{milestone}-{slug}>)",
...
This section lets the user inspect and override the built-in model IDs Ferrox resolves for each
...
`ferrox-tools.cjs query config-set runtime <value>` before proceeding to Step C.
...
```bash
ferrox_run query config-set model_profile_overrides.<runtime>.<tier> "<model-id>"
```

For each tier where the user chose "Clear override", remove the key by setting it to null:
```bash
ferrox_run query config-set model_profile_overrides.<runtime>.<tier> null
```
...
Merge the new settings into the existing config at `$FERROX_CONFIG_PATH`. This merge is the
core correctness invariant: **preserve every unrelated key** — do not clobber siblings.

Apply each selected value via `ferrox-tools.cjs query config-set <key> <value>` so the central
...
ferrox_run query config-set workflow.plan_bounce_passes 5
ferrox_run query config-set workflow.subagent_timeout 300000
ferrox_run query config-set git.base_branch main
ferrox_run query config-set context_window 1000000
# Runtime model tier examples:
ferrox_run query config-set runtime gemini
ferrox_run query config-set model_profile_overrides.gemini.opus gemini-3-ultra
ferrox_run query config-set model_profile_overrides.gemini.haiku null
```
...
route each write through `ferrox-tools.cjs query config-set` so sibling preservation is handled by
the central setter.
</step>
...
defines which AI models Ferrox uses at each cost tier (low / medium / high), independently
of the `runtime` and `model_profile` selections above. Two paths are offered:

- **Known provider:** choose a provider and a budget level; Ferrox materializes the canonical
...
```bash
cat "$FERROX_CONFIG_PATH" | python3 -c "import sys,json; c=json.load(sys.stdin); mp=c.get('model_policy',{}); print(json.dumps(mp,indent=2))" 2>/dev/null || echo "{}"
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Known provider", description: "Choose a provider (Claude / OpenAI / Gemini / Qwen) and a budget level — Ferrox writes the canonical tier mapping automatically." },
...
ferrox_run query config-set model_policy.provider "<provider>"   # e.g., anthropic / anthropic-fable / openai / google / qwen
ferrox_run query config-set model_policy.budget   "<budget>"    # high / medium / low
ferrox_run query config-set model_policy.high     "<high-id>"
ferrox_run query config-set model_policy.medium   "<medium-id>"
ferrox_run query config-set model_policy.low      "<low-id>"

# Generic-provider path — write only tiers the user changed ("Keep current" skipped):
ferrox_run query config-set model_policy.provider "custom"
ferrox_run query config-set model_policy.budget   null
# Per-tier writes for each non-"Keep current" answer:
ferrox_run query config-set model_policy.high     "<high-id>"   # omit if user chose "Keep current"
ferrox_run query config-set model_policy.medium   "<medium-id>" # omit if user chose "Keep current"
ferrox_run query config-set model_policy.low      "<low-id>"    # omit if user chose "Keep current"
```
...
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► ADVANCED SETTINGS UPDATED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
These settings apply to future /ferrox:plan-phase, /ferrox:execute-phase, /ferrox:discuss-phase,
and /ferrox:ship runs.

For common-case toggles (model profile, research/plan_check/verifier, branching strategy,
UI/AI phase gates), use /ferrox:settings.
```
</step>
...
<success_criteria>
- [ ] Current config read from resolved `$FERROX_CONFIG_PATH`
...
- [ ] Writes routed through `ferrox-tools.cjs query config-set` so unrelated keys are preserved
