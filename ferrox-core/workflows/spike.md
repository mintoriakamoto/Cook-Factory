...
Saves artifacts to `.planning/spikes/`. Companion to `/ferrox:spike --wrap-up`.
...
```bash
ferrox_run query commit "docs(spike-NNN): [VERDICT] — [key finding]" --files .planning/spikes/NNN-descriptive-name/ .planning/spikes/MANIFEST.md
```
...
```bash
ferrox_run query commit "docs(spikes): update conventions" --files .planning/spikes/CONVENTIONS.md
```
</step>
...
**Also available:**
- `/ferrox:spike` — spike more ideas (or run with no argument for frontier mode)
- `/ferrox:plan-phase` — start planning the real implementation
- `/ferrox:explore` — continue exploring the idea
