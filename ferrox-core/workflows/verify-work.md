...
-->
<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /ferrox:plan-phase --gaps.
...
<template>
@~/.claude/ferrox-core/templates/UAT.md
</template>
...
FERROX_WS=""
echo "$ARGUMENTS" | grep -qE -- '--ws[[:space:]]+[^[:space:]]+' && FERROX_WS=$(echo "$ARGUMENTS" | grep -oE -- '--ws[[:space:]]+[^[:space:]]+')
PHASE_ARG=$(echo "$ARGUMENTS" | sed -E 's/--ws[[:space:]]+[^[:space:]]+//g' | xargs)

INIT=$(ferrox_run query init.verify-work "${PHASE_ARG}" ${FERROX_WS})
...
MVP_MODE=$(ferrox_run query phase.mvp-mode "${phase_number}" ${FERROX_WS} --pick active)
```
</step>
...
```bash
VERIFY_PRE_HOOKS_JSON=$(ferrox_run loop render-hooks verify:pre --raw)
...
`ferrox_run check ${hook.check.query} "${PHASE_DIR}" --raw`; a `predicate` gate
runs `ferrox_run check predicate --predicate '<hook.check.predicate as JSON>' --phase-dir "${PHASE_DIR}" --raw`):

```bash
GATE_RESULT=$(ferrox_run check "${hook_check_query}" "${PHASE_DIR}" --raw)
...
- **Step 1 — command failure:** if the `ferrox_run check ...` invocation itself
...
Provide a phase number to start testing (e.g., /ferrox:verify-work 4)
```
...
```bash
PLAN_HOOKS_JSON=$(ferrox_run loop render-hooks plan:pre --raw)
...
**MVP-mode UAT framing.** When `MVP_MODE=true`, follow the rules in `@~/.claude/ferrox-core/references/verify-mvp-mode.md`. Briefly:
...
```bash
PHASE_GOAL=$(ferrox_run query roadmap.get-phase "${phase_number}" ${FERROX_WS} --pick goal)
USER_STORY_VALID=$(ferrox_run query user-story.validate --story "$PHASE_GOAL" --pick valid)
if [ "$USER_STORY_VALID" != "true" ]; then
  echo "Phase ${phase_number} has '**Mode:** mvp' in ROADMAP.md but the **Goal:** is not in user-story format."
  echo "Run /ferrox mvp-phase ${phase_number} to set a user-story goal before verifying."
  exit 1
fi
```
...
```bash
COVERAGE=$(ferrox_run query uat.classify-coverage --summary "$SUMMARY_FILE")
```
...
```bash
CHECKPOINT=$(ferrox_run query uat.render-checkpoint --file "$uat_path" --raw)
...
When verify-work resumes after `/ferrox:execute-phase --gaps-only`, the UAT `## Gaps` entries still read `status: failed` even though their fix plans have executed. Without reconciliation verify-work re-diagnoses them as fresh blockers and spawns new gap plans — losing the verification state. This step closes the loop.
...
**First run `reconcile_gaps`** (above) so gaps already fixed by `/ferrox:execute-phase --gaps-only` are marked `resolved` before testing resumes (#1921).
...
Commit the UAT file:
```bash
ferrox_run query commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
```
Skill(skill="ferrox-${ref.skill}", args="{phase}")
```
...
```
⚠ Security enforcement enabled — /ferrox:secure-phase {phase} did not produce SECURITY.md.
...
- `/ferrox:secure-phase {phase}` — security review (required before advancing)
- `/ferrox:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
...
VERIFICATION_STATUS=$(ferrox_run query verification.status "$PHASE_DIR" 2>/dev/null)
...
if [ "$VERIFICATION_STATUS_VALUE" = "human_needed" ]; then
  ferrox_run query frontmatter.set "$VERIFICATION_FILE" --field status --value passed
fi
```
...
- `/ferrox:verify-work {phase}` — re-run verification against the latest summaries
```

Otherwise, check the shared UAT-plus-verification completion predicate before transition:

```bash
PHASE_COMPLETE=$(ferrox_run phase uat-passed "{phase}" --require-verification)
...
- `/ferrox:execute-phase {phase}` — regenerate execution verification
- `/ferrox:verify-work {phase}` — resume UAT if blockers remain
```
...
Read and follow `~/.claude/ferrox-core/workflows/transition.md`.
...
`audit-open` is CJS-only until registered on `ferrox-tools.cjs query`:

```bash
ferrox_run query audit-open --json
```
...
If user declines: stop. User resolves items and re-runs `/ferrox:verify-work`.
...
- Follow @~/.claude/ferrox-core/workflows/diagnose-issues.md
...
<downstream_consumer>
Output consumed by /ferrox:execute-phase
...
---
```
This lets `/ferrox:verify-work` reconcile resolved gaps on resume (a gap whose plan has a matching `*-SUMMARY.md` is marked `status: resolved`, not re-diagnosed as a fresh blocker).
</downstream_consumer>
""",
...
Spawn ferrox-planner with revision context:
...
3. Abandon (exit, user runs /ferrox:plan-phase manually)
...
`/clear` then `/ferrox:execute-phase {phase} --gaps-only`
...
- [ ] If issues: ferrox-planner creates fix plans (gap_closure mode)
- [ ] If issues: ferrox-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/ferrox:execute-phase --gaps-only` when complete
</success_criteria>
