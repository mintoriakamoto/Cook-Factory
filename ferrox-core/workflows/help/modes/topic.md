...
| `map`, `map-codebase` | The `/ferrox:map-codebase` block under `### Project Initialization` |
| `discuss`, `discuss-phase` | The `/ferrox:discuss-phase` block under `### Phase Planning` |
...
| `fast` | The `/ferrox:fast` block under `### Quick Mode` |
...
| `ship`, `pr` | `### Ship Work` plus the `/ferrox:pr-branch` block |
| `review`, `peer-review` | The `/ferrox:review` block under `### Ship Work` |
| `audit`, `auditing`, `audit-milestone` | `### Milestone Auditing` |
| `config`, `settings`, `configuration` | `### Configuration` |
| `cleanup` | The `/ferrox:cleanup` block under `### Utility Commands` |
| `update` | The `/ferrox:update` block under `### Utility Commands` |
...
3. If no match: emit a one-line error followed by a comma-separated list of the canonical topic names from the leftmost column (one per row, deduplicated). Suggest `/ferrox:help --full` for the complete reference. Stop.
...
5. Read `workflows/help/modes/full.md`. Strip `<reference>` / `</reference>` wrapper tags — never emit them. Apply the extraction rule for the matched table cell, modulated by scope:
   ...
   - *Compact scope:* emit the heading, then the first `` **`/ferrox:...`** `` bold line within the section (the signature) and the single non-blank line immediately after it (the one-line summary). If the section has no `` **`/ferrox:...`** `` bold line, emit the heading and the first paragraph.
   ...
   5c. **Sub-block** (cell says `the /ferrox:X block under ### Heading` or `the /ferrox:X ... blocks under ### Heading`): within the named heading's section, start at each `` **`/ferrox:X ...`** `` bold line.
   - *Full scope:* stop immediately before the next `` **`/ferrox:...`** `` bold line or the next heading, whichever comes first.
...
6. After the section content, emit a single closing line:
   ...
   ```text
   More: /ferrox:help --full · /ferrox:help <topic> · /ferrox:help --brief <topic>
   ```
