...
- ferrox-executor — executes a specific, scoped code change
</available_agent_types>
...
INIT=$(ferrox_run query audit-uat 2>/dev/null || echo "{}")
...
AUDIT_TEST_CMD=$(ferrox_run query normalize-test-command "$AUDIT_TEST_CMD" --cwd . 2>/dev/null || echo "$AUDIT_TEST_CMD")
