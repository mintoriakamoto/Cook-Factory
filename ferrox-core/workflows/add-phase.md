...
INIT=$(ferrox_run query init.phase-op "0")
...
<step name="add_phase">
**Delegate the phase addition to `ferrox-tools.cjs query phase.add`:**

```bash
RESULT=$(ferrox_run query phase.add "${description}")
```
...
<success_criteria>
- [ ] `ferrox-tools.cjs query phase.add` executed successfully
