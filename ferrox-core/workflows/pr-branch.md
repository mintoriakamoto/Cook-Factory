...
don't see Ferrox transient artifacts (PLAN.md, SUMMARY.md, CONTEXT.md, RESEARCH.md, etc.)
...
TARGET=${1:-$(ferrox_run query git.base-branch)}
```
...
```bash
SUB_REPOS_JSON=$(ferrox_run query config-get planning.sub_repos 2>/dev/null)
...
RESULT=$(ferrox_run query pr-subrepo "$COMMIT_MSG" \
  --repo "$REPO_REL" \
  --branch "$SUB_BRANCH")
SUBREPO_EXIT=$?
```
...
Or use /ferrox:ship to create the PR automatically.
```
</step>
