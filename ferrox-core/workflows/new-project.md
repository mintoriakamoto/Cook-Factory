...
- ferrox-project-researcher — Researches project-level technical decisions
- ferrox-research-synthesizer — Synthesizes findings from parallel research agents
- ferrox-roadmapper — Creates phased execution roadmaps
</available_agent_types>
...
- File reference: `/ferrox:new-project --auto @prd.md`
...
Usage:
  /ferrox:new-project --auto @your-idea.md
  /ferrox:new-project --auto [paste or write your idea here]
...
INIT=$(ferrox_run query init.new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(ferrox_run query agent-skills ferrox-project-researcher)
AGENT_SKILLS_SYNTHESIZER=$(ferrox_run query agent-skills ferrox-research-synthesizer)
AGENT_SKILLS_ROADMAPPER=$(ferrox_run query agent-skills ferrox-roadmapper)
```
...
```text
⚠ Ferrox agents not installed. The following agents are missing from your agents directory:
...
Subagent spawns (ferrox-project-researcher, ferrox-research-synthesizer, ferrox-roadmapper) will fail
with "agent type not found" if `required_agents_installed` is false. Run the installer with --global to make agents available:

  npx ferrox-factory@latest --global
...
Set the instruction file variable via the shared runtime-name policy adapter (`ferrox-tools query project-instruction-file`, backed by `getProjectInstructionFile` in `runtime-name-policy.cjs` — the single source of truth shared with `profile-output.cjs`):
```bash
INSTRUCTION_FILE=$(ferrox_run query project-instruction-file --runtime "$RUNTIME")
```

All subsequent references to the project instruction file use `$INSTRUCTION_FILE`.

**If `project_exists` is true:** Error — project already initialized. Use `/ferrox:progress`.
...
- options:
  - "Map codebase first" — Run /ferrox:map-codebase to understand existing architecture (Recommended)
  - "Skip mapping" — Proceed with project initialization

**If "Map codebase first":**

```
Run `/ferrox:map-codebase` first, then return to `/ferrox:new-project`
```
...
// /ferrox:settings split (#3784, #1516).
AskUserQuestion([
  {
    header: "AI Models",
...
Ask which optional PRD-style sections `/ferrox:ship` should append to generated PR bodies. These map to `ship.pr_body_sections`; selected sections are written with `"enabled": true`, unselected seeded sections are written with `"enabled": false` so the project can enable them later without editing `ship.md`.
...
AskUserQuestion([
  {
    header: "PR Body",
    question: "Which optional PRD-style sections should /ferrox:ship include in PR bodies?",
    multiSelect: true,
    options: [
...
```bash
mkdir -p .planning
ferrox_run query commit "chore: add project config" --files .planning/config.json
```
...
{if HAS_SPIKES && !SPIKE_SKILL}  ◆ Raw spikes in .planning/spikes/ — consider `/ferrox:spike --wrap-up` to package findings
{if HAS_SKETCHES && !SKETCH_SKILL}  ◆ Raw sketches in .planning/sketches/ — consider `/ferrox:sketch --wrap-up` to package findings
...
```bash
mkdir -p .planning
ferrox_run query commit "docs: initialize project" --files .planning/PROJECT.md
```
...
**Check for global defaults** at `~/.ferrox/defaults.json`. If the file exists, read and display its contents before asking:

```bash
DEFAULTS_RAW=$(cat ~/.ferrox/defaults.json 2>/dev/null)
```
...
```text
Your saved defaults (~/.ferrox/defaults.json):
  • Mode: [value]
  • Granularity: [value]
...
**If "Configure fresh" or `~/.ferrox/defaults.json` doesn't exist:** proceed with the questions below.
...
**PR body onboarding:** Ask which optional PRD-style sections `/ferrox:ship` should append to generated PR bodies. Use the same `ship.pr_body_sections` mapping as Step 2a: selected sections get `enabled: true`, seeded-but-unselected sections get `enabled: false`, and selecting none writes an empty list. Prefer lean/agile PRD sections that make user value, acceptance criteria, Definition of Done, and stakeholder traceability explicit.
...
```bash
mkdir -p .planning
ferrox_run query config-new-project '{"mode":"[yolo|interactive]","granularity":"[selected]","parallelization":true|false,"commit_docs":true|false,"model_profile":"quality|balanced|budget|adaptive|inherit","workflow":{"research":true|false,"plan_check":true|false,"verifier":true|false,"nyquist_validation":[false if granularity=coarse, true otherwise]},"plan_review":{"source_grounding":true|false},"ship":{"pr_body_sections":[{"heading":"User Stories & Acceptance Criteria","enabled":true|false,"source":"REQUIREMENTS.md ## User Stories || REQUIREMENTS.md ## Acceptance Criteria","fallback":"- Acceptance criteria are covered by the linked requirements and verification evidence."},{"heading":"Risks & Dependencies","enabled":true|false,"source":"PLAN.md ## Risks || PLAN.md ## Dependencies","fallback":"- No known high-risk rollout dependencies."},{"heading":"Success Metrics & Release Criteria","enabled":true|false,"source":"REQUIREMENTS.md ## Definition of Done || VERIFICATION.md ## Release Criteria","fallback":"- Release when automated verification and required manual checks pass."},{"heading":"Stakeholder Review & Approval","enabled":true|false,"template":"- Product owner approval pending for {phase_name}."}]}}'
```

**Note:** Run `/ferrox:settings` anytime to update model profile, workflow agents, branching strategy, and other preferences.
...
- question: "I detected separate git repos in this workspace. Which directories contain code that Ferrox should commit to?"
- multiSelect: true
...
Use template: ~/.claude/ferrox-core/templates/research-project/STACK.md
</output>
", subagent_type="ferrox-project-researcher", model="{researcher_model}", description="Stack research")
...
Use template: ~/.claude/ferrox-core/templates/research-project/FEATURES.md
</output>
", subagent_type="ferrox-project-researcher", model="{researcher_model}", description="Features research")
...
Use template: ~/.claude/ferrox-core/templates/research-project/ARCHITECTURE.md
</output>
", subagent_type="ferrox-project-researcher", model="{researcher_model}", description="Architecture research")
...
Use template: ~/.claude/ferrox-core/templates/research-project/PITFALLS.md
</output>
", subagent_type="ferrox-project-researcher", model="{researcher_model}", description="Pitfalls research")
```
...
Use template: ~/.claude/ferrox-core/templates/research-project/SUMMARY.md
Commit after writing.
</output>
", subagent_type="ferrox-research-synthesizer", model="{synthesizer_model}", description="Synthesize research")
```

> **ORCHESTRATOR RULE — CODEX RUNTIME**: After calling Agent() above, stop working on this task immediately. Do not read more files, edit code, or run tests related to this task while the subagent is active. Wait for the subagent to return its result. This prevents duplicate work, conflicting edits, and wasted context. Only resume when the subagent result is available.

**Synthesizer output self-heal (#222) — verify SUMMARY.md materialized:** The synthesizer's canonical output is `.planning/research/SUMMARY.md` on disk; its brief structured return (`## SYNTHESIS COMPLETE` plus a few `###` confirmation lines) is NOT the file content. A known LLM false-refusal (issue #222) sometimes makes the agent return the full SUMMARY.md document inline — fabricating a write restriction (e.g. "the runtime is blocking file writes") — instead of writing the file. Prompt hardening alone does not fully eliminate it, so the orchestrator MUST absorb the failure deterministically before spawning `ferrox-roadmapper`:

1. Verify `.planning/research/SUMMARY.md` exists AND is substantive — non-empty, and free of any leftover `<!-- ferrox:write-continue -->` continuation sentinel (which marks a truncated/incomplete write). You may validate with `ferrox-tools verify-summary .planning/research/SUMMARY.md` — it exits 0 regardless, so check its JSON `passed` field (`"passed": false` means missing or invalid), not the process exit code. If it passes, continue normally.
2. If it is MISSING or invalid AND the synthesizer's return message contains the FULL SUMMARY.md document — recognizable by the template's top-level markers `# Project Research Summary`, `## Key Findings`, `## Implications for Roadmap`, and `## Sources`, not merely the brief `## SYNTHESIS COMPLETE` confirmation — the false-refusal fired: write that returned document to `.planning/research/SUMMARY.md` with the Write tool, then commit ALL research artifacts the synthesizer owns (it commits on behalf of the four researchers) with `ferrox-tools query commit "docs: complete project research" --files .planning/research/` unless they are already committed. Log `⚠ #222 self-heal: synthesizer returned SUMMARY.md inline without writing it; orchestrator persisted the file.`
3. If it is MISSING or invalid AND the return is only a brief confirmation (no full SUMMARY document to recover), the synthesizer genuinely failed — surface the error and stop; do NOT spawn `ferrox-roadmapper` against a missing or incomplete SUMMARY.md.

This guarantees `ferrox-roadmapper` (which lists SUMMARY.md as required reading) never runs against a missing or truncated SUMMARY.md.

Display research complete banner and key findings:

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► RESEARCH COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```bash
ferrox_run query commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```
...
Spawn ferrox-roadmapper agent with path references:
...
", subagent_type="ferrox-roadmapper", model="{roadmapper_model}", description="Create roadmap")
```
...
- Re-spawn roadmapper with revision context:
  ...
  ", subagent_type="ferrox-roadmapper", model="{roadmapper_model}", description="Revise roadmap")
  ```
...
```bash
ferrox_run query generate-claude-md --output "$INSTRUCTION_FILE"
```

This ensures new projects get the default Ferrox workflow-enforcement guidance and current project context in `$INSTRUCTION_FILE`.

**Commit roadmap (after approval or auto mode):**

```bash
ferrox_run query commit "docs: create roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md "$INSTRUCTION_FILE"
```
...
Exit skill and invoke SlashCommand("/ferrox:discuss-phase 1 --auto")
...
/ferrox:discuss-phase 1 — gather context and clarify approach
...
- [ ] ferrox-roadmapper spawned with context
