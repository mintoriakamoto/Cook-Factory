...
SEEDS=$(ferrox_run list-seeds "$STATUS_FILTER")
...
Plant one with /ferrox:capture --seed "<forward-looking idea>".
```
...
```
- /ferrox:capture --seed --enrich <ID>   enrich a seed with trigger, why, and scope
- /ferrox:capture --list-seeds <status>  filter by status
```
</step>
