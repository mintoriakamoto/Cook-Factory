...
offers mid-conversation research when useful, then routes crystallized outputs to Ferrox artifacts.
</purpose>
...
- ferrox-phase-researcher — Researches specific questions and returns concise findings
</available_agent_types>
...
| Spike | `/ferrox:spike` (invoke) | Feasibility uncertainty surfaced — "will this API work?", "can we do X?" |
| Sketch | `/ferrox:sketch` (invoke) | Design direction unclear — "what should this look like?", "how should this feel?" |
...
- **Phases:** Use existing `/ferrox-add-phase` command via SlashCommand
...
ferrox_run query commit "docs: capture exploration — {topic_slug}" --files {file_list}
```
...
Continue exploring with `/ferrox:explore` or start working with `/ferrox:progress --next`.
```
