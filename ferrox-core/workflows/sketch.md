...
Companion to `/ferrox:sketch --wrap-up`.
...
```bash
ferrox_run query commit "docs(sketch-NNN): [winning direction] — [key visual insight]" --files .planning/sketches/NNN-descriptive-name/ .planning/sketches/MANIFEST.md
```
...
**Also available:**
- `/ferrox:sketch` — sketch more (or run with no argument for frontier mode)
- `/ferrox:plan-phase` — start building the real UI
- `/ferrox:spike` — spike technical feasibility of a design pattern
