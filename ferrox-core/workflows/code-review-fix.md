<purpose>
Auto-fix issues from REVIEW.md. Validates phase, checks config gate, verifies REVIEW.md exists and has fixable issues, spawns ferrox-code-fixer agent, handles --auto iteration loop (capped at 3), commits REVIEW-FIX.md once at the end, and presents results.
</purpose>
...
- ferrox-code-fixer: Applies fixes to code review findings
- ferrox-code-reviewer: Reviews source files for bugs and issues
</available_agent_types>
...
AGENT_SKILLS_FIXER=$(ferrox_run query agent-skills ferrox-code-fixer)
...
# (ferrox-code-reviewer → "verification", ferrox-code-fixer → "execution"); thread them below.
REVIEWER_MODEL=$(ferrox_run query resolve-model ferrox-code-reviewer --raw)
FIXER_MODEL=$(ferrox_run query resolve-model ferrox-code-fixer --raw)
```
...
if [ ! -f "${REVIEW_PATH}" ]; then
  echo "Error: No REVIEW.md found for Phase ${PHASE_ARG}. Run /ferrox:code-review ${PHASE_ARG} first."
  exit 1
fi
```
...
Spawn the ferrox-code-fixer agent with config (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze):
...
```text
Agent(subagent_type="ferrox-code-fixer", model="{FIXER_MODEL}", prompt="
<files_to_read>
${REVIEW_PATH}
</files_to_read>
...
You can retry with /ferrox:code-review ${PHASE_ARG} --fix.
```
...
if [ "$AUTO_MODE" = "true" ]; then
  ...
  - ${f}"
    ...
    # Spawn ferrox-code-reviewer agent to re-review (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze)
...
if [ ! -f "${FIX_REPORT_PATH}" ]; then
  ...
  echo ""
  echo "Retry: /ferrox:code-review ${PHASE_ARG} --fix"
  echo ""
  echo "═══════════════════════════════════════════════════════════════"
  exit 1
fi
...
if [ "$FIX_STATUS" = "all_fixed" ]; then
  ...
  echo ""
  echo "Next step:"
  echo "  /ferrox:verify-work  — Verify phase completion"
  echo ""
fi
```
...
if [ "$FIX_STATUS" = "partial" ] || [ "$FIX_STATUS" = "none_fixed" ]; then
  ...
  echo "  /ferrox:code-review ${PHASE_NUMBER}           — Re-review code"
  echo "  /ferrox:verify-work                           — Verify phase completion"
  echo ""
fi
```
