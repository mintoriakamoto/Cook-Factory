<purpose>
Review source files changed during a phase for bugs, security issues, and code quality problems. Computes file scope (--files override > SUMMARY.md > git diff fallback), checks config gate, spawns ferrox-code-reviewer agent, commits REVIEW.md, and presents results to user. When --fix is passed, delegates to code-review-fix.md after review to auto-apply findings via ferrox-code-fixer.
</purpose>
...
- ferrox-code-reviewer: Reviews source files for bugs and quality issues
- ferrox-code-fixer: Applies fixes to code review findings (used via dispatch_fix → code-review-fix.md when --fix is passed)
</available_agent_types>
...
AGENT_SKILLS_REVIEWER=$(ferrox_run query agent-skills ferrox-code-reviewer)
# #2072: resolve the routed model so model_overrides / models.verification are honored
# (the resolver maps ferrox-code-reviewer → phaseType "verification"); thread it below.
REVIEWER_MODEL=$(ferrox_run query resolve-model ferrox-code-reviewer --raw)
```
...
```
Error: Phase ${PHASE_ARG} not found. Run /ferrox:progress to see available phases.
```
...
FLAGS_JSON=$(node -e "
  const { parseCodeReviewFlags } = require('./ferrox-core/bin/lib/code-review-flags.cjs');
...
2. Config value: `ferrox-tools.cjs query config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(ferrox_run query config-get workflow.code_review_depth 2>/dev/null || echo "")
...
if [ ${#REVIEW_FILES[@]} -eq 0 ]; then
  ...
  if [ -n "$PHASE_COMMITS" ]; then
    ...
    echo "Use --files flag to specify files explicitly: /ferrox:code-review ${PHASE_ARG} --files=file1,file2,..."
  fi
fi
```
...
```bash
FALLOW_ENABLED=$(ferrox_run query config-get code_quality.fallow.enabled 2>/dev/null || echo "false")
FALLOW_SCOPE=$(ferrox_run query config-get code_quality.fallow.scope 2>/dev/null || echo "phase")
FALLOW_PROFILE=$(ferrox_run query config-get code_quality.fallow.profile 2>/dev/null || echo "standard")
FALLOW_MCP=$(ferrox_run query config-get code_quality.fallow.mcp 2>/dev/null || echo "false")
...
const { resolveFallowBinary } = require('./ferrox-core/bin/lib/fallow-runner.cjs');
...
if [ -n "$FALLOW_JSON_PATH" ] && [ -f "$FALLOW_JSON_PATH" ]; then
  ...
  FALLOW_SRC="$FALLOW_JSON_PATH" FALLOW_OUT="$FALLOW_NORMALIZED_PATH" node -e "
    const fs = require('fs');
    const { normalizeFallowReportFile } = require('./ferrox-core/bin/lib/fallow-runner.cjs');
...
Spawn the ferrox-code-reviewer agent:

Print: `◆ Spawning code reviewer... (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze)`

```
Agent(subagent_type="ferrox-code-reviewer", model="{REVIEWER_MODEL}", prompt="
<files_to_read>
${FILES_TO_READ}
</files_to_read>
...
No REVIEW.md created. You can retry with /ferrox:code-review ${PHASE_ARG} or check agent logs.
```
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  echo "No REVIEW.md to commit. Please retry with /ferrox:code-review ${PHASE_ARG}"
fi
```
</step>
...
if [ "$FIX_FLAG" = "true" ]; then
  ...
  # ferrox-code-fixer agent dispatch, --auto iteration loop, REVIEW-FIX.md commit,
  # and result presentation. Do not duplicate that logic here.
  Workflow(workflow="ferrox-core/workflows/code-review-fix.md", args="${FIX_ARGS}")
...
Next steps:
  /ferrox:code-review ${PHASE_NUMBER} --fix  — Auto-fix issues
