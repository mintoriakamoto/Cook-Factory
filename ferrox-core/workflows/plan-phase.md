...
-->
<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates ferrox-phase-researcher, ferrox-planner, and ferrox-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
@~/.claude/ferrox-core/references/revision-loop.md
...
- ferrox-pattern-mapper — Analyzes codebase for existing patterns, produces PATTERNS.md
...
ferrox-phase-researcher, ferrox-planner, and ferrox-plan-checker as separate Agent() calls.
...
Agent() call for ferrox-phase-researcher, ferrox-planner, and ferrox-plan-checker. Only
...
INIT=$(ferrox_run query init.plan-phase "$PHASE" $GRAN_PARAM)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(ferrox_run query agent-skills ferrox-phase-researcher)
AGENT_SKILLS_PLANNER=$(ferrox_run query agent-skills ferrox-planner)
AGENT_SKILLS_CHECKER=$(ferrox_run query agent-skills ferrox-plan-checker)
CONTEXT_WINDOW=$(ferrox_run query config-get context_window 2>/dev/null || echo "200000")
MVP_MODE_CFG=$(ferrox_run query config-get workflow.mvp_mode 2>/dev/null || echo "false")
```
...
if [[ "$ARGUMENTS" =~ (^|[[:space:]])--tdd([[:space:]]|$) ]]; then
  ferrox_run query config-set workflow.tdd_mode true 2>/dev/null || true
fi
```
...
if [ "$MVP_MODE" = "true" ] && [ "$padded_phase" = "01" ]; then
  PRIOR_SUMMARIES=$(ferrox_run query phases.list --pick summaries_total 2>/dev/null || echo "0")
  if [ "$PRIOR_SUMMARIES" = "0" ]; then WALKING_SKELETON=true; fi
fi
```

When `WALKING_SKELETON=true`:
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `~/.claude/ferrox-core/references/skeleton-template.md` — the planner reads it when producing SKELETON.md (lazy; not loaded on non-skeleton runs).
...
```bash
CHUNKED_CFG=$(ferrox_run query config-get workflow.plan_chunked 2>/dev/null || echo "false")
CHUNKED_MODE=false
...
/ferrox:review --phase {N}

Then re-run /ferrox:plan-phase {N} --reviews
```
Exit workflow.
...
```bash
MVP_MODE=$(ferrox_run query phase.mvp-mode "${PHASE}" $MVP_FLAG_ARG --pick active)
```
...
1. Display banner: `Ferrox ► ADR Ingest Express Path` with `{INGEST_PATH}` and `{INGEST_FORMAT}`.
2. Parse each resolved ADR through `ferrox-core/bin/lib/adr-parser.cjs` (`--input`, `--format`) and collect normalized records.
3. Status gate: reject `superseded`/`rejected`/`deprecated`; warn on `proposed`; missing status defaults to `accepted`.
4. Empty-decisions fallback: if all parsed ADRs have zero `decisions[]`, emit `ADR ingest produced no locked decisions; fall back to discuss-phase for this phase.` and exit with `/ferrox:discuss-phase {N}` guidance.
...
6. Commit with `ferrox-tools.cjs query commit "docs(${padded_phase}): generate context from ADR ingest" --files "${phase_dir}/${padded_phase}-CONTEXT.md"` and set `context_content`; continue to step 5.
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — AskUserQuestion
  ...
  ```
  Run this command first, then re-run /ferrox:plan-phase {X} ${FERROX_WS}:

  /ferrox:discuss-phase {X} ${FERROX_WS}
  ```
...
If `AI_SPEC_FILE` is non-empty, pass `AI_SPEC_PATH` and `FRAMEWORK_LINE` to the planner in step 8 so it can reference the AI design contract. If it is empty, the active `ai-integration` capability hook in step 5.6 handles any AI-system nudge or `/ferrox:ai-integration-phase` dispatch.
...
2. **`--research`** (force-refresh): re-spawn researcher unconditionally — fall through to "Spawn ferrox-phase-researcher" below.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► RESEARCHING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning researcher... (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze)
```

### Spawn ferrox-phase-researcher

```bash
if ferrox_run query teams-status --active >/dev/null 2>&1; then
  echo "⚠️  CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS detected. Ferrox's multi-agent orchestration is not validated under claude-code agent-teams and may stall (a subagent's completion can fail to route to the orchestrator). Recommend disabling agent-teams for Ferrox workflows. See https://github.com/ferroxfactory/ferrox-core/issues/1355" >&2
fi
```

```bash
PHASE_DESC=$(ferrox_run query roadmap.get-phase "${PHASE}" --pick section)
if [ -z "${PLAN_PRE_HOOKS_JSON:-}" ]; then
  PLAN_PRE_HOOKS_JSON=$(ferrox_run loop render-hooks plan:pre --raw)
fi
```
...
**If `RESEARCH_ONLY=true`:** the user invoked `/ferrox:plan-phase --research-phase <N>` for research-only mode. Do **not** continue to Section 5.5+ (validation strategy, planner, plan-checker, verification, gaps, bounce, post-planning-gaps). Print the research-complete summary and exit cleanly:
...
Re-run /ferrox:plan-phase ${PHASE} to plan the phase using this research,
or /ferrox:plan-phase ${PHASE} --research to refresh research and plan.
```
...
**If found:**
1. Read template: `~/.claude/ferrox-core/templates/VALIDATION.md`
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► SECURITY THREAT MODEL REQUIRED (ASVS L{SECURITY_ASVS})
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```bash
PLAN_PRE_HOOKS_JSON=${PLAN_PRE_HOOKS_JSON:-$(ferrox_run loop render-hooks plan:pre --raw)}
...
- If `ref.skill` is set, dispatch with `Skill(skill="ferrox-${ref.skill}", args="${PHASE} --auto ${FERROX_WS}")` when pipeline mode allows auto-chaining. Prepend `ferrox-` to `ref.skill` — `ui-phase` → `ferrox-ui-phase`.
...
- In pipeline / `--auto` mode, invoke the hook's `ref.skill` via `Skill(skill="ferrox-${ref.skill}", args="${PHASE} --auto ${FERROX_WS}")`.
- In manual mode, display the existing non-blocking `/ferrox:ai-integration-phase {N}` recommendation and let the user continue planning without AI-SPEC or stop to run the capability workflow first.
...
```bash
GATE=$(ferrox_run check ui-plan-gate "${PHASE}" --raw)
```
...
```bash
AUTO_CHAIN=$(ferrox_run query check auto-mode --pick auto_chain_active 2>/dev/null || echo "false")
```
...
```
Skill(skill="ferrox-${ref.skill}", args="${PHASE} --auto ${FERROX_WS}")
```
...
`/ferrox:ui-phase {N} ${FERROX_WS}` — generate UI design contract before planning
───────────────────────────────────────────────
Also available:
- `/ferrox:plan-phase {N} --skip-ui ${FERROX_WS}` — plan without UI-SPEC (not recommended for frontend phases)
```
...
```bash
DRIFT=$(ferrox_run verify codebase-drift 2>/dev/null || echo '{"skipped":true}')
```
...
verbatim (it ends with a `/ferrox:map-codebase` pointer) and continue — planning proceeds whether or not the
...
```bash
_ferrox_field() { node -e "const o=JSON.parse(process.argv[1]); const v=o[process.argv[2]]; process.stdout.write(v==null?'':String(v))" "$1" "$2"; }
STATE_PATH=$(_ferrox_field "$INIT" state_path)
ROADMAP_PATH=$(_ferrox_field "$INIT" roadmap_path)
REQUIREMENTS_PATH=$(_ferrox_field "$INIT" requirements_path)
RESEARCH_PATH=$(_ferrox_field "$INIT" research_path)
VERIFICATION_PATH=$(_ferrox_field "$INIT" verification_path)
UAT_PATH=$(_ferrox_field "$INIT" uat_path)
CONTEXT_PATH=$(_ferrox_field "$INIT" context_path)
REVIEWS_PATH=$(_ferrox_field "$INIT" reviews_path)
PATTERNS_PATH=$(_ferrox_field "$INIT" patterns_path)
...
PHASE_DIR_FOR_SPEC=$(_ferrox_field "$INIT" phase_dir)
...
1. Re-run: `/ferrox:plan-phase {PHASE} --research ${FERROX_WS}`
2. Disable Nyquist with the exact command:
   `ferrox-tools.cjs query config-set workflow.nyquist_validation false`
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 7.8 (or Step 8 if pattern mapper is disabled) only if user selects 2 or 3.

## 7.8. Spawn ferrox-pattern-mapper Agent (Optional)

Pattern mapper activation is owned by the `pattern-mapper` capability's `plan:pre` step hook. Read `PLAN_PRE_HOOKS_JSON` and skip if no active step hook has `capId == "pattern-mapper"` and `ref.agent == "ferrox-pattern-mapper"`. Also skip if no CONTEXT.md and no RESEARCH.md exist for this phase (nothing to extract file lists from).
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► PATTERN MAPPING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
after `$SPEC_FILE` (Step 7), before the ferrox-planner spawn (Step 8).

**Read and run** the gate + edge probe in `~/.claude/ferrox-core/references/specless-probe-fallback.md`
...
**Frame-and-Recall — prior decisions (Phase 7 memory seam).** Before planning, the Frame stage
CONSULTS prior decisions/patterns for the phase subject via `ferrox-tools query memory.recall`
(behind the mempalace-recall seam — the native `.planning/graphs/` bi-temporal fact store is its
KG backend). This returns the valid-now prior decisions bounded by `memory.recall_limit`, so the
planner does not re-litigate a settled choice. This is orchestration protocol (the workflow invokes
the verb at the stage), not a tool-layer gate — an unreachable/empty store is a graceful no-op.


## 8. Spawn ferrox-planner Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
**If Mode is reviews:** REVIEWS.md is feedback input, not a hidden execution contract. /ferrox:execute-phase primarily consumes PLAN.md plus the normal phase context, so every current actionable review finding must become visible in the relevant PLAN.md before planning can pass.
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `~/.claude/ferrox-core/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
**WALKING_SKELETON:** ${WALKING_SKELETON} (when true, the first deliverable must be a Walking Skeleton — Read the template at `~/.claude/ferrox-core/references/skeleton-template.md` and produce SKELETON.md alongside PLAN.md.)
**Granularity:** {granularity}

${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Read `~/.claude/ferrox-core/references/planner-mvp-mode.md` now and follow its vertical-slice planning rules. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
...
else-branch below, per §A/§B/§C of `~/.claude/ferrox-core/references/specless-probe-fallback.md`
...
`/ferrox:plan-phase {N} --chunked` resumes from the last successfully committed plan.

**Intended for new or in-progress chunked runs.** To recover plans already written by a prior
*non-chunked* run, use step 6's "Add more plans" or proceed directly to `/ferrox:execute-phase`
...
5. **Commit per-plan:**
   ```bash
   ferrox_run query commit "docs(${PADDED_PHASE}): plan ${plan_id} (chunked)" --files "${PHASE_DIR}/${plan_id}-PLAN.md"
   ```
...
3. **Stop** — exit; user can re-run `/ferrox:plan-phase {N}` to resume
...
**If "Split":** Use `/ferrox:phase --insert` to create the sub-phases, then replan each.
...
**If "Split":** Use `/ferrox:phase --insert` for overflow items, then replan.
**If "Defer":** Record in CONTEXT.md `## Deferred Ideas` with developer's confirmation. Proceed to step 10.

## 10. Spawn ferrox-plan-checker Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► VERIFYING PLANS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Agent(
  ...
  subagent_type="ferrox-plan-checker",
  model="{checker_model}",
...
**Stall detection:** If `issue_count >= prev_issue_count`:
  ...
  **If `stall_reentry_count >= 2`:**
    ...
    Suggest: "Consider resolving these issues manually or running `/ferrox:debug` to investigate root causes."
...
**Read pass count:**
```bash
BOUNCE_PASSES=$(ferrox_run query config-get workflow.plan_bounce_passes 2>/dev/null || echo "2")
BOUNCE_SCRIPT=$(ferrox_run query config-get workflow.plan_bounce_script 2>/dev/null | jq -r '.' 2>/dev/null || true)
```

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► BOUNCING PLANS (External Refinement)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
5. **Re-run plan checker on bounced plans:** Spawn ferrox-plan-checker (same as step 10) on all modified plans. If a bounced plan fails the checker, restore original from its pre-bounce.md backup:
```
...
```bash
ferrox_run query commit "refactor(${padded_phase}): bounce plans through external refinement" --files "${PHASE_DIR}/*-PLAN.md"
```
...
```bash
GATE_CFG=$(ferrox_run query config-get workflow.context_coverage_gate 2>/dev/null || echo "true")
if [ "$GATE_CFG" != "false" ]; then
  GATE_RESULT=$(ferrox_run query check.decision-coverage-plan "${PHASE_DIR}" "${CONTEXT_PATH}")
...
```bash
ferrox_run query state.planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
ferrox_run query roadmap.annotate-dependencies "${PHASE_NUMBER}"
```
...
```bash
ferrox_run query commit "docs(${PADDED_PHASE}): create phase plan" --files "${PHASE_DIR}"/*-PLAN.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
PLAN_POST_HOOKS_JSON=$(ferrox_run loop render-hooks plan:post --raw)
PHASE_REQ_IDS=$(ferrox_run query init.plan-phase "$PHASE" --pick phase_req_ids 2>/dev/null || echo TBD)
```
...
```bash
# named-query gate:
GATE_RESULT=$(ferrox_run check ${hook.check.query} "${PHASE_DIR}" "${PHASE_REQ_IDS}" --raw)
CHECK_EXIT=$?
```
...
```bash
GATE_RESULT=$(ferrox_run check predicate --predicate '<hook.check.predicate as JSON>' --phase-dir "${PHASE_DIR}" --phase-req-ids "${PHASE_REQ_IDS}" --raw)
CHECK_EXIT=$?
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► AUTO-ADVANCING TO EXECUTE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
Skill(skill="ferrox-execute-phase", args="${PHASE} --auto --no-transition ${FERROX_WS}")
```
...
- **PHASE COMPLETE** → Display final summary:
  ...
  Next: /ferrox:discuss-phase ${NEXT_PHASE} --auto ${FERROX_WS}
  ```
- **GAPS FOUND / VERIFICATION FAILED** → Display result, stop chain:
  ...
  /ferrox:execute-phase ${PHASE} ${FERROX_WS}
  ```
...
/ferrox:execute-phase {X} ${FERROX_WS}

───────────────────────────────────────────────────────────────

**Also available:**
- cat .planning/phases/{phase-dir}/*-PLAN.md — review plans
- /ferrox:plan-phase {X} --research — re-research first
- /ferrox:review --phase {X} --all — peer review plans with external AIs
- /ferrox:plan-phase {X} --reviews — replan incorporating review feedback

───────────────────────────────────────────────────────────────
</offer_next>

<windows_troubleshooting>
Read `ferrox-core/workflows/plan-phase/steps/windows-troubleshooting.md` if plan-phase freezes on Windows during agent spawning (stdio deadlocks with MCP servers, anthropics/claude-code#28126) — it covers force-kill, orphaned-node cleanup, stale task-dir cleanup, reducing the MCP server count, and the `--skip-research` fallback.
</windows_troubleshooting>
...
- [ ] ferrox-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] ferrox-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] ferrox-plan-checker spawned with CONTEXT.md
