...
> `$HOME/.claude/ferrox-core/USER-PROFILE.md` exists). Skip the Read
...
ADVISOR_MODEL=$(ferrox_run query resolve-model ferrox-advisor-researcher --raw)
```
...
```bash
PROFILE_CONTENT=$(cat "$HOME/.claude/ferrox-core/USER-PROFILE.md" 2>/dev/null || true)
```
