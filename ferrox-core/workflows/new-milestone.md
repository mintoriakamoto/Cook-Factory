...
- Check for MILESTONE-CONTEXT.md (from /ferrox-discuss-milestone)
...
section to the new-milestone template. Nothing else in the body is touched.
Decisions and blockers are unaffected because since phase 14.1 D3d they do not
live in STATE.md at all: they live in the milestone artifacts, and the new
milestone's own artifact owns them from here.
...
ferrox_run query state.milestone-switch --milestone "v[X.Y]" --name "[Name]"
```
...
```bash
ferrox_run query phases.clear --confirm
```
...
```bash
ferrox_run query commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(ferrox_run query init.new-milestone)
...
- Tell the user to complete/archive the previous milestone first, then rerun `/ferrox:new-milestone --reset-phase-numbers ${FERROX_WS}`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/ferrox:plan-phase` behavior. To change the default, use `/ferrox:settings`.
...
Spawn 4 parallel ferrox-project-researcher agents. Each uses this template with dimension-specific fields:
...
Use template: ~/.claude/ferrox-core/templates/research-project/{FILE}
</output>
", subagent_type="ferrox-project-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
**Commit requirements:**
```bash
ferrox_run query commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
ferrox_run query commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
```bash
ferrox_run query commit "docs: tag [count] pending todos with resolves_phase after milestone v[X.Y] roadmap" --files .planning/todos/pending/*.md
```
...
`/ferrox:discuss-phase [N] ${FERROX_WS}` — gather context and clarify approach

Also: `/ferrox:plan-phase [N] ${FERROX_WS}` — skip discussion, plan directly
```
...
- [ ] ferrox-roadmapper spawned with phase numbering context
...
- [ ] User knows next step: `/ferrox:discuss-phase [N] ${FERROX_WS}`
