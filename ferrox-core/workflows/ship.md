...
- ferrox-mempalace-curator — Ship-time MemPalace curation (diary, KG mirror, cross-project tunnels, wing-scoped prune); dispatched at ship:post when the mempalace capability is enabled.
</available_agent_types>
...
```bash
CONFIG=$(ferrox_run query state.load)
```
...
```bash
BASE_BRANCH=$(ferrox_run query git.base-branch)
```
</step>
...
6. **Security ship gate (capability-driven).**
   ...
   ```bash
   SHIP_PRE_HOOKS_JSON=$(ferrox_run loop render-hooks ship:pre --raw)
   ...
   - **`SECURITY_FILE` is empty** → block with `SECURITY_SHIP_GATE_NO_REVIEW`:
     ...
     Run /ferrox:secure-phase {phase} and resolve findings before shipping.
     ```
   - **`SECURITY_FILE` exists** → read its frontmatter `threats_open`. The gate passes **only** when `threats_open` is exactly `0`. For any other value — `threats_open` > 0, or a missing / non-numeric / unparsable field — **fail closed and block** with `SECURITY_SHIP_GATE_OPEN_THREATS` (the predicate is strict equality to `0`; never ship on an ambiguous value):
     ...
     Resolve open threats (or re-run /ferrox:secure-phase {phase}) before shipping.
     ```

   If no active security `ship:pre` gate hook is present (security enforcement off), skip this check silently.

7. **Strength merge gate (FF-B10 — consulted here, enforced at the tool layer).**

   Before creating/merging the PR, CONSULT `strength.merge-gate` for the aggregate landed
   verdict (receipts valid + coverage landed + ownership ok + hot seam serialized + burndown ok
   + no open security + no open CRITICAL/HIGH). See [strength.md](../references/strength.md) for
   the evidence contract and the eight strength verbs.

   ```bash
   ferrox_run query strength.merge-gate \
     --increment "${PHASE_NUMBER}" --requirements "${REQ_IDS}" \
     --declared "${DECLARED}" --actual "${ACTUAL}" --files "${FILES}" \
     --opened "${OPENED}" --resolved "${RESOLVED}" \
     --mutation-test "${MUT_TEST}" --mutation-flipped "${MUT_FLIPPED}" --raw
   ```

   Only a `pass` decision may ship; a `block` names the failing `reasons[]` to resolve first.

   This consult is not the only line of defence: the `ferrox-merge-gate-guard.js` **PreToolUse
   hook INDEPENDENTLY ENFORCES** `strength.merge-gate` at the tool layer — the actual
   `git merge` / `gh pr merge` / release-push tool call is blocked (`exit 2`) unless the gate
   returns `pass`, so the strength gate cannot be skipped even if this consult is forgotten.
   **FF-B10 is closed by that hook.**

   **C3 — close-on-merge-gate-PASS (TRIAGE-01).** If this increment is linked to a
   GitHub issue (`github_issue: NN` in the plan frontmatter, stamped by C2), the
   issue closes as a CONSEQUENCE of the gate passing — never a manual step, and
   never on a `block`. **The two inputs MUST be captured explicitly** — the hook is
   inert if they are unset:

   ```bash
   # 1. Capture the gate verdict from the consult above (add --pick to that call, or re-derive):
   GATE_DECISION=$(ferrox_run query strength.merge-gate \
     --increment "${PHASE_NUMBER}" --requirements "${REQ_IDS}" \
     --declared "${DECLARED}" --actual "${ACTUAL}" --files "${FILES}" \
     --opened "${OPENED}" --resolved "${RESOLVED}" \
     --mutation-test "${MUT_TEST}" --mutation-flipped "${MUT_FLIPPED}" --raw --pick decision)
   # 2. Read the increment's linked issue from the plan frontmatter (empty if none):
   GH_ISSUE=$(ferrox_run frontmatter get "${PLAN_FILE}" --pick github_issue 2>/dev/null || echo "")

   # 3. Fail-closed lifecycle: emits close actions ONLY when GATE_DECISION is exactly 'pass'.
   GATE_DECISION="$GATE_DECISION" GH_ISSUE="$GH_ISSUE" PHASE_NUMBER="$PHASE_NUMBER" node -e '
   const lc = require("./ferrox-core/bin/lib/issue-lifecycle.cjs");
   const r = lc.onMergeGate({ githubIssue: Number(process.env.GH_ISSUE) || null,
     increment: process.env.PHASE_NUMBER, gateDecision: process.env.GATE_DECISION });
   for (const a of r.actions) {
     if (a.action === "close")   console.log(`gh issue close ${a.issue} --reason completed`);
     if (a.action === "comment") console.log(`gh issue comment ${a.issue} --body ${JSON.stringify(a.value)}`);
     if (a.action === "unlabel") console.log(`gh issue edit ${a.issue} --remove-label ${a.value}`);
   }'
   ```

   `onMergeGate` is fail-closed: an unset or non-`pass` `GATE_DECISION` emits NO close
   action, so a blocked merge holds its issue open. Run the emitted `gh` lines.
</step>
...
```bash
CUSTOM_PR_SECTIONS=$(ferrox_run query config-get ship.pr_body_sections --default '[]' 2>/dev/null || echo '[]')
```
...
Use these sections for lean/agile PRD material that should travel with the PR without making the core `/ferrox:ship` body configurable:
...
PR_BODY_FILE=$(mktemp "${TMPDIR:-/tmp}/ferrox-pr-body-XXXXXX") && mv "$PR_BODY_FILE" "${PR_BODY_FILE}.md" && PR_BODY_FILE="${PR_BODY_FILE}.md" || exit 1
...
```bash
REVIEW_CMD=$(ferrox_run query config-get workflow.code_review_command 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
```
...
2. **Load phase context from STATE.md:**
   ```bash
   STATE_STATUS=$(ferrox_run query state.load 2>/dev/null | head -20)
   ```
...
3. **Build review prompt and pipe to command via stdin:**
   ...
   REVIEW_OUTPUT=$(echo "${REVIEW_PROMPT}" | timeout 120 ${REVIEW_CMD} 2>/tmp/ferrox-review-stderr.log)
   REVIEW_EXIT=$?
   ```
...
4. **Handle timeout (120s) and failure:**
   ...
   if [ $REVIEW_EXIT -ne 0 ]; then
     REVIEW_STDERR=$(cat /tmp/ferrox-review-stderr.log 2>/dev/null)
...
```bash
ferrox_run query state.update "Last Activity" "$(date +%Y-%m-%d)"
ferrox_run query state.update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```
...
```bash
ferrox_run query commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER} [ci skip]" --files .planning/STATE.md
...
```bash
SHIP_POST_HOOKS_JSON=$(ferrox_run loop render-hooks ship:post --raw)
```
...
- If `ref.agent` is set, first show the spawn banner, then dispatch the agent named by `ref.agent` (use the exact `ref.agent` value as the subagent type — e.g. `ferrox-mempalace-curator` — never `general-purpose`):
...
- If `ref.skill` is set, dispatch with `Skill(skill="ferrox-${ref.skill}", args="${PHASE_NUMBER} --auto ${FERROX_WS}")` (prepend `ferrox-` to `ref.skill`).
...
- /ferrox:complete-milestone (if last phase in milestone)
- /ferrox:progress (to see what's next)
...
- /ferrox:complete-milestone — if all phases in milestone are done
- /ferrox:progress — see overall project state
- /ferrox:execute-phase {next} — continue to next phase
</offer_next>
