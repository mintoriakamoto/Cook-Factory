...
- **--from**: Import external plan → conflict detection → write PLAN.md → validate via ferrox-plan-checker
...
- If `--prd` is present: display message that `--prd` is not yet implemented and exit:
  ```
  Ferrox > --prd mode is planned for a future release. Use --from to import plan files.
  ```
- If neither flag is found: display usage and exit:

```
Usage: /ferrox:import --from <path>

  --from <path>   Import an external plan file into Ferrox format
```
...
2. Read `.planning/PROJECT.md` — extract project constraints, tech stack, scope boundaries.
   **If PROJECT.md does not exist:** skip constraint checks that rely on it and display:
   ```
   Ferrox > Note: No PROJECT.md found. Conflict checks against project constraints will be skipped.
   ```
...
- **Ferrox PLAN.md format**: Has YAML frontmatter with `phase:`, `plan:`, `type:` fields
...
Convert the imported content to Ferrox PLAN.md format.
...
If the imported plan references PBR plan naming (e.g., `PLAN-01.md`, `plan-01.md`), rename all references to Ferrox `{NN}-{MM}-PLAN.md` convention during conversion.

Apply Ferrox naming convention for the output filename:
...
INIT=$(ferrox_run query init.phase-op "{NN}")
...
Delegate validation to ferrox-plan-checker:

Print: "Delegating to ferrox-plan-checker (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze)"

```
Agent({
  subagent_type: "ferrox-plan-checker",
  prompt: "Validate: .planning/phases/{phase}/{plan}-PLAN.md — check frontmatter completeness, task structure, and Ferrox conventions. Report any issues."
})
```
...
```bash
ferrox_run query commit "docs({phase}): import plan from {basename FILEPATH}" --files .planning/phases/{phase}/{plan}-PLAN.md .planning/ROADMAP.md
```

Display completion:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► IMPORT COMPLETE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
- Use `pbr:plan-checker` or `pbr:planner` — use `ferrox-plan-checker` and `ferrox-planner`
- Write `.planning/.active-skill` — this is a PBR pattern with no Ferrox equivalent
