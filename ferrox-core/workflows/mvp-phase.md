<purpose>
Guide the user through MVP-mode planning for a phase. Prompts for an "As a / I want to / So that" user story, runs SPIDR splitting check on the story, writes the result to ROADMAP.md, and delegates to `/ferrox plan-phase` (which auto-detects MVP via the roadmap mode field shipped in PRD Phase 1).
</purpose>

<required_reading>
@~/.claude/ferrox-core/references/user-story-template.md
@~/.claude/ferrox-core/references/spidr-splitting.md
@~/.claude/ferrox-core/references/planner-mvp-mode.md
</required_reading>
...
Usage: /ferrox mvp-phase <phase-number>
Example: /ferrox mvp-phase 1
Example: /ferrox mvp-phase 2.1
```
Exit.

Normalize per `@~/.claude/ferrox-core/references/phase-argument-parsing.md` (zero-pad integer phases to two digits).
...
ANALYZE=$(ferrox_run query roadmap.analyze)
...
If `PHASE_FOUND` is `false`: error and exit. Suggest `/ferrox add-phase` or `/ferrox insert-phase` to create the phase first.
...
To proceed anyway: /ferrox mvp-phase ${PHASE} --force
```
...
Run three sequential `AskUserQuestion` calls. Each is free-text. After all three, assemble into the canonical sentence per `@~/.claude/ferrox-core/references/user-story-template.md`:
...
```bash
USER_STORY_RESULT=$(ferrox_run query user-story.validate --story "$USER_STORY")
...
Run the SPIDR rules from `@~/.claude/ferrox-core/references/spidr-splitting.md`. Briefly:
...
- **Accept**: `USER_STORY` becomes the first split's story (`${HAPPY_STORY}` in the example). Surface the remaining splits as a list of `/ferrox add-phase` invocations the user can run after this command completes — do NOT auto-create the new phases (preserve user control over numbering).
...
```bash
NEW_MODE=$(ferrox_run query roadmap.get-phase "${PHASE}" --pick mode)
NEW_GOAL=$(ferrox_run query roadmap.get-phase "${PHASE}" --pick goal)
```
...
## 7. Delegate to /ferrox plan-phase

Invoke `/ferrox plan-phase ${PHASE}` (no flags). Phase 1's MVP_MODE resolution chain (CLI flag → roadmap mode → config → false) will detect the new `**Mode:** mvp` line and run plan-phase in vertical-slice mode automatically.
...
>
> - `/ferrox add-phase` — for the next slice: «${SPLIT_2_STORY}»
> - `/ferrox add-phase` — for the next slice: «${SPLIT_3_STORY}»
>
> Each will be added to the end of the current milestone. You can then run
> `/ferrox mvp-phase <new-phase-number>` on each to plan them as MVP slices."
