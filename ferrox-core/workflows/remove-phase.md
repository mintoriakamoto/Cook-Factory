...
INIT=$(ferrox_run query init.phase-op "${target}")
...
To abandon current work, use /ferrox:pause-work instead.
```
...
**Delegate the entire removal operation to `ferrox-tools.cjs query phase.remove`:**

```bash
RESULT=$(ferrox_run query phase.remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), the CLI will error. Use `--force` only if the user confirms:

```bash
RESULT=$(ferrox_run query phase.remove "${target}" --force)
```
...
```bash
ferrox_run query commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
...
Would you like to:
- `/ferrox:progress` — see updated roadmap status
...
- Don't manually renumber — use `ferrox-tools.cjs query phase.remove` which handles all renumbering
...
- [ ] `ferrox-tools.cjs query phase.remove` executed successfully
