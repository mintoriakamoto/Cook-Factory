# sync-skills — Cross-Runtime Ferrox Skill Sync

**Command:** `/ferrox-sync-skills`

Sync managed `ferrox-*` skill directories from one canonical runtime's skills root to one or more destination runtime skills roots. Keeps multi-runtime installs aligned after a `ferrox-update` on one runtime.
...
```bash
INSTALL_JS="$(dirname "$0")/../ferrox-core/bin/install.js"
# If running from a global install, resolve relative to the Ferrox package
INSTALL_JS_GLOBAL="$HOME/.claude/ferrox-core/bin/install.js"
...
error: source skills root not found: <path>
       Is Ferrox installed globally for the '<runtime>' runtime?
       Run: node ~/.claude/ferrox-core/bin/install.js --global --<runtime>
```
Then exit.
...
```bash
# List ferrox-* subdirectories in source
SRC_SKILLS=$(ls -1 "$SRC_SKILLS_ROOT" 2>/dev/null | grep '^ferrox-')

# List ferrox-* subdirectories in destination (may not exist yet)
DST_SKILLS=$(ls -1 "$DEST_ROOT" 2>/dev/null | grep '^ferrox-')

# Diff:
# CREATE  — in SRC but not in DST
# UPDATE  — in both; content differs (compare recursively via checksums)
# REMOVE  — in DST but not in SRC (stale Ferrox skill no longer in source)
# SKIP    — in both; content identical (already up to date)
```

**Non-Ferrox preservation:** Only `ferrox-*` entries are ever created, updated, or removed. Entries in the destination that do not start with `ferrox-` are never touched.
...
CREATE: ferrox-help
UPDATE: ferrox-update
REMOVE: ferrox-old-command
SKIP:   ferrox-plan-phase (up to date)
(N changes)
...
1. **Only `ferrox-*` directories** are created, updated, or removed. Any directory not starting with `ferrox-` in a destination root is untouched.
2. **Dry-run is the default.** `--apply` must be passed explicitly to write anything.
3. **Source root must exist.** Never create the source root; it must have been created by a prior `ferrox-update` or installer run.
...
- Sync copies files verbatim and does not apply runtime-specific content transformations. Use the Ferrox installer directly for runtimes that require format conversion.
