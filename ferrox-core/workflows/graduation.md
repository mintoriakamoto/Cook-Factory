...
GRADUATION_ENABLED=$(ferrox_run query config-get features.graduation 2>/dev/null || echo "true")
GRADUATION_WINDOW=$(ferrox_run query config-get features.graduation_window 2>/dev/null || echo "5")
GRADUATION_THRESHOLD=$(ferrox_run query config-get features.graduation_threshold 2>/dev/null || echo "3")
```
