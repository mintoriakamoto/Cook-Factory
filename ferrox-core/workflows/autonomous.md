...
When `PLAN_STRATEGY=converge`, the planning step MUST invoke the plan-review convergence workflow instead of `ferrox-plan-phase`. `--cross-ai` is an alias for `--converge`. Forward `CONVERGENCE_ARGS` exactly as parsed so reviewer flags and `--max-cycles N` retain the same meaning as they have on `/ferrox:plan-review-convergence`.
...
INIT=$(ferrox_run query init.milestone-op)
...
if [ "$PLAN_STRATEGY" = "converge" ]; then
  ...
  if [ "$CONVERGENCE_ENABLED" != "true" ]; then
    printf '%s\n' \
      'ferrox-autonomous --converge is disabled (workflow.plan_review_convergence=false).' \
      '' \
...
**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/ferrox:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/ferrox:new-milestone` first."
...
**Agent skills (delegated agents self-load):** This workflow delegates plan/execute/review via flat `Skill()` invocations rather than resolving `agent_skills` itself. Each consumer agent (`ferrox-planner`, `ferrox-executor`, `ferrox-plan-checker`, `ferrox-verifier`, …) self-loads its configured `.planning/config.json` `agent_skills` in its own mandatory init step per `@~/.claude/ferrox-core/references/agent-skills-bootstrap.md`. This is the durable path that works on every runtime — including Cursor, where `Skill()`-delegated workflow bash init does not reliably execute. No per-delegation injection is needed here. See ferroxfactory/ferrox-core#1866.
...
```bash
INIT_MANAGER=$(ferrox_run query init.manager)
...
- `verification_deferred_human` -> `/ferrox:verify-work <phase>`
- `verification_deferred_gaps` -> `/ferrox:plan-phase <phase> --gaps`
...
```bash
DETAIL=$(ferrox_run query roadmap.get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_STATE=$(ferrox_run query init.phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(ferrox_run query config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
ferrox_run query commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="ferrox-discuss-phase", args="${PHASE_NUM}")
```
...
HOOKS_JSON=$(ferrox_run loop render-hooks plan:pre --raw)
```
...
```bash
GATE=$(ferrox_run check ui-plan-gate "${PHASE_NUM}" --raw)
```
...
```
Skill(skill="ferrox-${ref.skill}", args="${PHASE_NUM}")
```

(Prepend `ferrox-` to `ref.skill` — so `ui-phase` → `ferrox-ui-phase`. Bare `${PHASE_NUM}` args — autonomous style, same pattern as the verify:post dispatch.) Entries where `kind == "gate"` are silently ignored — autonomous is always pipeline mode, there is no blocking gate here.
...
```bash
FLATTEN=$(ferrox_run query dispatch-should-flatten --raw 2>/dev/null || echo "true")
```
...
- **If `FLATTEN` is `false`:** Dispatch plan as a background agent to keep the main context lean. While plan runs, the workflow can immediately start discussing the next phase (see step 4).
  ...
  Agent(
    ...
    prompt="Run plan convergence for phase ${PHASE_NUM}: Skill(skill=\"ferrox-plan-review-convergence\", args=\"${PHASE_NUM} ${CONVERGENCE_ARGS}\")"
  )
  ```
  ...
  Agent(
    ...
    run_in_background=true,
    prompt="Run plan-phase for phase ${PHASE_NUM}: Skill(skill=\"ferrox-plan-phase\", args=\"${PHASE_NUM}\")"
  )
  ```
...
- **Otherwise (`FLATTEN` is `true` — run inline):** Run plan **inline** (do NOT background) so the plan-checker runs. The next phase's discuss does not overlap planning here — correctness over overlap.
  ...
  ```
  Skill(skill="ferrox-plan-review-convergence", args="${PHASE_NUM} ${CONVERGENCE_ARGS}")
  ```

  - Otherwise (local planning):

  ```
  Skill(skill="ferrox-plan-phase", args="${PHASE_NUM}")
  ```
...
Agent(
  ...
  run_in_background=true,
  prompt="Run execute-phase for phase ${PHASE_NUM}: Skill(skill=\"ferrox-execute-phase\", args=\"${PHASE_NUM} --no-transition\")"
)
```
...
```
Skill(skill="ferrox-execute-phase", args="${PHASE_NUM} --no-transition")
```
...
```
Skill(skill="ferrox-code-review", args="${PHASE_NUM} --fix --auto")
```
...
```bash
VERIFY_STATUS=$(ferrox_run query verification.status "${PHASE_DIR}" 2>/dev/null | jq -r '.status//empty')
```
...
Display `Phase ${PHASE_NUM} ✅ ${PHASE_NAME} — Verification passed`, run `@~/.claude/ferrox-core/workflows/transition.md`, then Proceed to iterate step.
...
On "All good — continue": set VERIFICATION frontmatter `status: passed`, display `Phase ${PHASE_NUM} ✅ Human validation passed`, run `@~/.claude/ferrox-core/workflows/transition.md`, then iterate.
...
| Phase | State | Resume |
|-------|-------|--------|
| ${PHASE_NUM} | verification_deferred_human | /ferrox:verify-work ${PHASE_NUM} |
```

Append/update this STATE.md section, display `Phase ${PHASE_NUM} ⏭ verification_deferred_human — resume with /ferrox:verify-work ${PHASE_NUM}`, then handle_blocker: "Human verification deferred for phase ${PHASE_NUM}."
...
```
Skill(skill="ferrox-plan-phase", args="${PHASE_NUM} --gaps")
```
...
On "Continue anyway": record `verification_deferred_gaps` using the table below, display `Phase ${PHASE_NUM} ⏭ verification_deferred_gaps — resume with /ferrox:plan-phase ${PHASE_NUM} --gaps`, then handle_blocker: "Verification gaps deferred for phase ${PHASE_NUM}."
...
| Phase | State | Resume |
|-------|-------|--------|
| ${PHASE_NUM} | verification_deferred_gaps | /ferrox:plan-phase ${PHASE_NUM} --gaps |
```

Append/update this STATE.md section, display `Phase ${PHASE_NUM} ⏭ verification_deferred_gaps — resume with /ferrox:plan-phase ${PHASE_NUM} --gaps`, then handle_blocker: "Verification gaps deferred for phase ${PHASE_NUM}."
...
```bash
UI_SPEC_FILE=$(ls "${PHASE_DIR}"/*-UI-SPEC.md 2>/dev/null | head -1)
HOOKS_JSON=$(ferrox_run loop render-hooks verify:post --raw)
```
...
(i.e. prepend `ferrox-` to `ref.skill` — so `ui-review` → `ferrox-ui-review`.)
...
> Full instructions are in `ferrox-core/references/autonomous-smart-discuss.md`. Read that file now and follow it exactly.

Smart discuss is an autonomous-optimized variant of `ferrox-discuss-phase`. It proposes grey area answers in batch tables — the user accepts or overrides per area — and writes an identical CONTEXT.md to what discuss-phase produces.

**Inputs:** `PHASE_NUM` from execute_phase.

Read and execute: `$HOME/.claude/ferrox-core/references/autonomous-smart-discuss.md`
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► AUTONOMOUS ▸ --to ${TO_PHASE} REACHED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Completed through phase ${TO_PHASE} as requested.
 Remaining phases were not executed.

 Resume with: /ferrox:autonomous --from ${next_incomplete_phase}
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► AUTONOMOUS ▸ PHASE ${ONLY_PHASE} COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Phase ${ONLY_PHASE}: ${PHASE_NAME} — Done
 Mode: Single phase (--only)

 Lifecycle skipped — run /ferrox:autonomous without --only
...
```
Skill(skill="ferrox-audit-milestone")
```
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /ferrox:audit-milestone to review, then /ferrox:complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /ferrox:audit-milestone to review details."

**5b. Complete Milestone**

```
Skill(skill="ferrox-complete-milestone", args="${milestone_version}")
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /ferrox:autonomous ${ONLY_PHASE ? "--only " + ONLY_PHASE : "--from " + next_phase}${TO_PHASE ? " --to " + TO_PHASE : ""}
```
...
- [ ] Audit invoked via Skill(skill="ferrox-audit-milestone")
...
- [ ] `--interactive` runs discuss inline via ferrox-discuss-phase (asks questions, waits for user)
...
- [ ] `--converge` routes planning through `ferrox-plan-review-convergence`
...
- [ ] Default autonomous planning remains `ferrox-plan-phase` when convergence is not requested
</success_criteria>
