<purpose>
Generate an AI design contract (AI-SPEC.md) for phases that involve building AI systems. Orchestrates ferrox-framework-selector → ferrox-ai-researcher → ferrox-domain-researcher → ferrox-eval-planner with a validation gate. Inserts between discuss-phase and plan-phase in the Ferrox lifecycle.
...
```bash
SELECTOR_MODEL=$(ferrox_run query resolve-model ferrox-framework-selector 2>/dev/null | jq -r '.model' 2>/dev/null || true)
RESEARCHER_MODEL=$(ferrox_run query resolve-model ferrox-ai-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
DOMAIN_MODEL=$(ferrox_run query resolve-model ferrox-domain-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
PLANNER_MODEL=$(ferrox_run query resolve-model ferrox-eval-planner 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Check config:
```bash
AI_PHASE_ENABLED=$(ferrox_run query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```

**If `AI_PHASE_ENABLED` is `false`:**
```
AI phase is disabled in config. Enable via /ferrox:settings.
```
Exit workflow.
...
Recommended: run /ferrox:discuss-phase {N} first to capture framework preferences.
...
## 5. Spawn ferrox-framework-selector

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► AI DESIGN CONTRACT — PHASE {N}: {name}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Step 1/4 — Framework Selection...
```

Spawn `ferrox-framework-selector` with:
```markdown
Read ~/.claude/agents/ferrox-framework-selector.md for instructions.
...
**If selector fails or returns empty:** Exit with error — "Framework selection failed. Re-run /ferrox:ai-integration-phase {N} or answer the framework question in /ferrox:discuss-phase {N} first."

## 6. Initialize AI-SPEC.md

Copy template:
```bash
cp "$HOME/.claude/ferrox-core/templates/AI-SPEC.md" "${PHASE_DIR}/${PADDED_PHASE}-AI-SPEC.md"
```
...
Spawn `ferrox-ai-researcher` with:
```markdown
Read ~/.claude/agents/ferrox-ai-researcher.md for instructions.
...
## 8. Spawn ferrox-domain-researcher
...
Spawn `ferrox-domain-researcher` with:
```markdown
Read ~/.claude/agents/ferrox-domain-researcher.md for instructions.
...
Spawn `ferrox-eval-planner` with:
```markdown
Read ~/.claude/agents/ferrox-eval-planner.md for instructions.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► AI-SPEC COMPLETE — PHASE {N}: {name}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Next step:
  /ferrox:plan-phase {N}   — planner will consume AI-SPEC.md
```
