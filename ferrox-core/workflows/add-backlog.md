...
Invoked by `/ferrox:capture --backlog` (`commands/ferrox/capture.md`).
...
NEXT=$(ferrox_run query phase.next-decimal 999 --raw)
```
...
Plans:
- [ ] TBD (promote with /ferrox:review-backlog when ready)
```
...
PROJECT_CODE=$(ferrox_run query config-get project_code --raw 2>/dev/null || echo "")
...
```bash
ferrox_run query commit "docs: add backlog item ${NEXT} — ${ARGUMENTS}" --files .planning/ROADMAP.md "${PHASE_DIR}/.gitkeep"
```
...
Use /ferrox:discuss-phase {NEXT} to explore it further.
Use /ferrox:review-backlog to promote items to active milestone.
```
...
- Phase directories are created immediately so /ferrox:discuss-phase and /ferrox:plan-phase work on them
- No `Depends on:` field — backlog items are unsequenced by definition
- Sparse numbering is fine (999.1, 999.3) — always uses next-decimal
- Promote backlog items to the active milestone with /ferrox:review-backlog
</notes>
