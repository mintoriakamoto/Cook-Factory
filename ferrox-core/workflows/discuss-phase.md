...
<required_reading>
@~/.claude/ferrox-core/references/domain-probes.md
@~/.claude/ferrox-core/references/gate-prompts.md
@~/.claude/ferrox-core/references/universal-anti-patterns.md
</required_reading>
...
1. **ferrox-phase-researcher** — Reads CONTEXT.md to know WHAT to research
2. **ferrox-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/ferrox:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
INIT=$(ferrox_run query init.phase-op "${PHASE}"); [[ "$INIT" == @file:* ]] && INIT=$(cat "${INIT#@file:}")
AGENT_SKILLS_ADVISOR=$(ferrox_run query agent-skills ferrox-advisor-researcher)
```
...
Use /ferrox:progress ${FERROX_WS} to see available phases.
```
Exit workflow.
...
if [ -f "$HOME/.claude/ferrox-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Check if a SPEC.md (from `/ferrox:spec-phase`) exists for this phase. SPEC.md locks requirements before implementation decisions.
...
**Note:** SPEC.md files named `AI-SPEC.md` (from `/ferrox:ai-integration-phase`) are excluded — different purpose.
</step>
...
```bash
TODO_MATCHES=$(ferrox_run query todo.match-phase "${PHASE_NUMBER}")
```
...
Read `@~/.claude/ferrox-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
```bash
DISCUSS_PRE_HOOKS_JSON=$(ferrox_run loop render-hooks discuss:pre --raw)
```
Apply each entry in `activeHooks` per @~/.claude/ferrox-core/references/loop-hook-dispatch.md. Empty list → continue to `analyze_phase`.
</step>
...
```bash
DISCUSS_POST_HOOKS_JSON=$(ferrox_run loop render-hooks discuss:post --raw)
```
Apply each entry in `activeHooks` per @~/.claude/ferrox-core/references/loop-hook-dispatch.md. Empty list → continue to `confirm_creation`.
</step>
...
`/ferrox:plan-phase ${PHASE} ${FERROX_WS}`

---

**Also available:** `--chain` for auto plan+execute after; `/ferrox:plan-phase ${PHASE} --skip-research ${FERROX_WS}` to plan without research; `/ferrox:ui-phase ${PHASE} ${FERROX_WS}` for UI design contracts; review/edit CONTEXT.md before continuing.
```
</step>
...
```bash
ferrox_run query commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
