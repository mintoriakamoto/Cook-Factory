...
`.planning/spikes/WRAP-UP-SUMMARY.md`. Companion to `/ferrox:spike`.
</purpose>
...
Run `/ferrox:spike` first to create experiments.
```
Exit.
...
```bash
ferrox_run query commit "docs(spike-wrap-up): package [N] spike findings into project skill" --files .planning/spikes/WRAP-UP-SUMMARY.md .planning/spikes/CONVENTIONS.md
```
</step>
...
`/ferrox:spike` (run with no argument — its frontier mode analyzes the spike landscape and proposes integration and frontier spikes)

───────────────────────────────────────────────────────────────

**Also available:**
- `/ferrox:plan-phase` — start planning the real implementation
- `/ferrox:spike [idea]` — spike a specific new idea
...
- [ ] Summary presented
- [ ] Next-step options presented (including frontier spike exploration via `/ferrox:spike`)
</success_criteria>
