<purpose>
Capture an idea, task, or issue that surfaces during a Ferrox session as a structured todo for later work. Enables "thought → capture → continue" flow without losing context.
</purpose>
...
INIT=$(ferrox_run query init.todos)
...
- `/ferrox-add-todo Add auth token refresh` → title = "Add auth token refresh"
...
```bash
slug=$(ferrox_run query generate-slug "$title" --raw)
```
...
```bash
ferrox_run query commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
...
2. Add another todo
3. View all todos (/ferrox:capture --list)
```
</step>
