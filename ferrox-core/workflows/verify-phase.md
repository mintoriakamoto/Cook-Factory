...
<required_reading>
@~/.claude/ferrox-core/references/verification-patterns.md
@~/.claude/ferrox-core/templates/verification-report.md
</required_reading>
...
```bash
ferrox_run query roadmap.analyze
```
...
Use `ferrox-tools.cjs query` verify handlers (or legacy ferrox-tools) to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(ferrox_run query frontmatter.get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(ferrox_run query roadmap.get-phase "${phase_number}" --raw)
```
...
Use `ferrox-tools.cjs query verify.artifacts` (or legacy ferrox-tools) for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(ferrox_run query verify.artifacts "$plan")
...
Use `ferrox-tools.cjs query verify.key-links` (or legacy ferrox-tools) for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(ferrox_run query verify.key-links "$plan")
...
if [ "$GATE_CFG" != "false" ]; then
  ...
  DECISION_RESULT=$(ferrox_run query check.decision-coverage-verify "${PHASE_DIR}" "${CONTEXT_PATH}")
fi
```
...
TEST_CMD=$(ferrox_run query config-get workflow.test_command --default "" 2>/dev/null || true)
if [ -z "$TEST_CMD" ]; then
...
TEST_CMD=$(ferrox_run query normalize-test-command "$TEST_CMD" --cwd . 2>/dev/null || echo "$TEST_CMD")
TEST_EXIT=0
...
See ~/.claude/ferrox-core/templates/verification-report.md for complete template.
</step>
