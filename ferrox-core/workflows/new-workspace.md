...
INIT=$(ferrox_run query init.new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /ferrox:workspace --new --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /ferrox:workspace --new --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd "$TARGET_PATH"
  /ferrox:new-project    # Initialize Ferrox in the workspace
```
...
**Offer to initialize Ferrox (if not `--auto`):**

Use AskUserQuestion:
- header: "Initialize Ferrox"
- question: "Would you like to initialize a Ferrox project in the new workspace?"
- options:
  - "Yes — run /ferrox:new-project" → tell user to `cd "$TARGET_PATH"` first, then run `/ferrox:new-project`
