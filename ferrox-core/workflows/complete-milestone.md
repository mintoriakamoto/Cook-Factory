...
[R] Resolve — stop and fix items, then re-run /ferrox:complete-milestone
...
2. **Run verification first** — `/ferrox:verify-work {phase}` or `/ferrox:execute-phase {phase}`
...
2. **Run audit first** — `/ferrox:audit-milestone` to assess gap severity
3. **Abort** — return to development
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  ferrox_run query summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `ferrox-tools.cjs query milestone.complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to `ferrox-tools.cjs query milestone.complete`:**

```bash
ARCHIVE=$(ferrox_run query milestone.complete "v[X.Y]" --name "[Milestone Name]")
```
...
**Phase archival (default-on):** `milestone complete` archives phase directories to `milestones/v[X.Y]-phases/` by default (#1871), so the next `/ferrox:new-milestone` never inherits un-archived dirs. No manual `mkdir`/`mv` or `--archive-phases` flag is needed.
...
```bash
ferrox_run query milestone complete v[X.Y] --no-archive-phases
```
...
```bash
ferrox_run query commit "chore: archive v[X.Y] milestone files" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md .planning/ROADMAP.md
```
...
**If doesn't exist:** Create from template at `~/.claude/ferrox-core/templates/retrospective.md`.
...
**Commit:**
```bash
ferrox_run query commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(ferrox_run query init.execute-phase "1")
...
<config-check>
Read `git.create_tag` via `ferrox-tools.cjs query config-get git.create_tag 2>/dev/null || echo "true"`.
...
- [ ] User knows next step (/ferrox:new-milestone)
