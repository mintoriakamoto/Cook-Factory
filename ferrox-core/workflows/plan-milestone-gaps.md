...
```
No audit gaps found. Run `/ferrox:audit-milestone` first.
```
...
HIGHEST=$(ferrox_run query phases.list --pick directories[-1])
```
...
```bash
ferrox_run query commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/ferrox:execute-phase {N}` — if plans already exist
...
`/ferrox:audit-milestone` — re-audit to verify gaps closed
`/ferrox:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/ferrox:plan-phase` next
</success_criteria>
