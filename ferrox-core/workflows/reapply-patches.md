...
Invoked by `/ferrox:update --reapply` (`commands/ferrox/update.md`).

After a Ferrox update wipes and reinstalls files, this workflow merges user's previously saved local modifications back into the new version. Uses three-way comparison (pristine baseline, user-modified backup, newly installed version) to reliably distinguish user customizations from version drift.

**Critical invariant:** Every file in `ferrox-local-patches/` was backed up because the installer's hash comparison detected it was modified. The workflow must NEVER conclude "no custom content" for any backed-up file — that is a logical contradiction. When in doubt, classify as CONFLICT requiring user review, not SKIP.
...
if [ -n "$KILO_CONFIG_DIR" ]; then
  candidate="$(expand_home "$KILO_CONFIG_DIR")/ferrox-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -n "$KILO_CONFIG" ]; then
  candidate="$(dirname "$(expand_home "$KILO_CONFIG")")/ferrox-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/kilo/ferrox-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$OPENCODE_CONFIG_DIR")/ferrox-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG" ]; then
  candidate="$(dirname "$(expand_home "$OPENCODE_CONFIG")")/ferrox-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/opencode/ferrox-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$GEMINI_CONFIG_DIR" ]; then
  candidate="$(expand_home "$GEMINI_CONFIG_DIR")/ferrox-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CODEX_HOME" ]; then
  candidate="$(expand_home "$CODEX_HOME")/ferrox-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CLAUDE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$CLAUDE_CONFIG_DIR")/ferrox-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/ferrox-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/kilo/ferrox-local-patches"
  elif [ -d "$HOME/.config/opencode/ferrox-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/opencode/ferrox-local-patches"
  elif [ -d "$HOME/.opencode/ferrox-local-patches" ]; then
    PATCHES_DIR="$HOME/.opencode/ferrox-local-patches"
  elif [ -d "$HOME/.gemini/ferrox-local-patches" ]; then
    PATCHES_DIR="$HOME/.gemini/ferrox-local-patches"
  elif [ -d "$HOME/.codex/ferrox-local-patches" ]; then
    PATCHES_DIR="$HOME/.codex/ferrox-local-patches"
  else
    PATCHES_DIR="$HOME/.claude/ferrox-local-patches"
  fi
fi
# Local install fallback — check all runtime directories
if [ ! -d "$PATCHES_DIR" ]; then
  for dir in .config/kilo .kilo .config/opencode .opencode .gemini .codex .claude; do
    if [ -d "./$dir/ferrox-local-patches" ]; then
      PATCHES_DIR="./$dir/ferrox-local-patches"
      break
    fi
  done
fi
...
Local patches are automatically saved when you run /ferrox:update
after modifying any Ferrox workflow, command, or agent files.
```
Exit.

## Step 2: Determine baseline for three-way comparison

The quality of the merge depends on having a **pristine baseline** — the original unmodified version of each file from the pre-update Ferrox release. This enables three-way comparison:
- **Pristine baseline** (original Ferrox file before any user edits)
- **User's version** (backed up in `ferrox-local-patches/`)
...
**Why this matters:** `git log --diff-filter=A` returns the commit that *first added* the file, which is the wrong baseline on repos that have been through multiple Ferrox update cycles. The `pristine_hashes` field in `backup-meta.json` records the SHA-256 of the file as it existed in the pre-update Ferrox release — matching against it finds the correct baseline regardless of how many updates have occurred.

### Option B: Pristine snapshot directory
Check if a `ferrox-pristine/` directory exists alongside `ferrox-local-patches/`:
```bash
PRISTINE_DIR="$CONFIG_DIR/ferrox-pristine"
```
...
1. **Read the backed-up version** (user's modified copy from `ferrox-local-patches/`)
2. **Read the newly installed version** (current file after update)
3. **If available, read the pristine baseline** (from git history or `ferrox-pristine/`)
...
b. Identify ALL differences, then classify each as:
   - **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `$HOME/.claude/`), variable additions (`${FERROX_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
...
git -C "$CONFIG_DIR" log --oneline --no-merges -- "{file_path}" | grep -v "ferrox:update\|ferrox-update\|Ferrox update\|ferrox-install"
```
...
Run the verifier as a child process (the ferrox-tools binary directory is not required — the script ships under `ferrox-core/bin/` in the source repo and is installed to `${FERROX_HOME}/ferrox-core/bin/`):
...
VERIFY_OUTPUT="$(node "${FERROX_HOME}/ferrox-core/bin/verify-reapply-patches.cjs" "${VERIFY_ARGS[@]}")"
VERIFY_STATUS=$?
```
...
ferrox-pristine/ snapshot on disk does not match the hash recorded in backup-meta.json
(pristine drift — the snapshot was refreshed to a newer Ferrox version after the backup
...
Resolve before re-running:
  ...
  (c) If the upstream changes are acceptable, update the backup-meta.json
      ...
      re-run /ferrox:update --reapply to re-verify with the refreshed baseline.

Then re-run /ferrox:update --reapply to re-verify.
```
...
/ferrox:update --reapply to retry with full verification.
```
...
Ask user:
- "Keep patch backups for reference?" → preserve `ferrox-local-patches/`
- "Clean up patch backups?" → remove `ferrox-local-patches/` directory
...
- [ ] Three-way merge used when pristine baseline available (git history or ferrox-pristine/)
