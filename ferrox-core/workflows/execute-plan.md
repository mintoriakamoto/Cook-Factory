...
@~/.claude/ferrox-core/references/git-integration.md
</required_reading>
...
INIT=$(ferrox_run query init.execute-phase "${PHASE}")
...
# config settings can be fetched via ferrox-tools.cjs query config-get if needed
```
...
INLINE_THRESHOLD=$(ferrox_run query config-get workflow.inline_plan_threshold 2>/dev/null || echo "2")
...
2. Per segment:
   - Subagent route: spawn ferrox-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
ferrox_run query phases.list --type summaries --raw
...
Apply deviation rules from the ferrox-executor agent definition (single source of truth):
...
See `~/.claude/ferrox-core/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/ferrox-core/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(ferrox_run query config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/ferrox-core/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/ferrox-core/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/ferrox-core/templates/summary.md`.
...
Update STATE.md using ferrox-tools.cjs query (or legacy ferrox-tools) state mutations:
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  ferrox_run query state.advance-plan

  # Recalculate progress bar from disk state
  ferrox_run query state.update-progress

  # Record execution metrics
  ferrox_run query state.record-metric \
...
ferrox_run query state.add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
ferrox_run query state.add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using ferrox-tools.cjs query (or legacy ferrox-tools):
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  ferrox_run query roadmap.update-plan-progress "${PHASE}"
fi
```
...
```bash
ferrox_run query requirements.mark-complete ${REQ_IDS}
```
...
if [ "$IS_WORKTREE" = "true" ]; then
  ferrox_run query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/REQUIREMENTS.md
else
  ferrox_run query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
fi
```
</step>
...
```bash
ferrox_run query commit "" --files .planning/codebase/*.md --amend
```
</step>
