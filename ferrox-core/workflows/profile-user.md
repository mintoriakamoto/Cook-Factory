...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing `ferrox-tools.cjs query` handlers (with legacy `ferrox-tools.cjs` parity where needed) and the ferrox-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/ferrox-core/references/ui-brand.md (display patterns)
- @$HOME/.claude/agents/ferrox-user-profiler.md (profiler agent definition)
- @$HOME/.claude/ferrox-core/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.claude/ferrox-core/USER-PROFILE.md"
...
```bash
cp "$HOME/.claude/ferrox-core/USER-PROFILE.md" "$HOME/.claude/USER-PROFILE.backup.md"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox > PROFILE YOUR CODING STYLE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
✓ Stores profile at $HOME/.claude/ferrox-core/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /ferrox:profile-user when ready." and exit
...
SCAN_RESULT=$(ferrox_run query scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(ferrox_run query profile-sample --json 2>/dev/null)
```
...
**Spawn ferrox-user-profiler agent using Task tool:**

Use the Task tool to spawn the `ferrox-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/ferrox-core/references/user-profiling.md`
...
Reference: @$HOME/.claude/ferrox-core/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(ferrox_run query profile-questionnaire --json 2>/dev/null)
```
...
ANSWERS_PATH=$(mktemp "${TMPDIR:-/tmp}/ferrox-profile-answers-XXXXXX") && mv "$ANSWERS_PATH" "${ANSWERS_PATH}.json" && ANSWERS_PATH="${ANSWERS_PATH}.json" || exit 1
```
...
```bash
ANALYSIS_RESULT=$(ferrox_run query profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
# BSD/macOS mktemp only randomizes XXXXXX when it is the final path component, so make a
# suffixless temp then append the extension — portable across BSD + GNU (#1520).
ANALYSIS_PATH=$(mktemp "${TMPDIR:-/tmp}/ferrox-profile-analysis-XXXXXX") && mv "$ANALYSIS_PATH" "${ANALYSIS_PATH}.json" && ANALYSIS_PATH="${ANALYSIS_PATH}.json" || exit 1
```
...
```bash
ferrox_run query write-profile --input "$ANALYSIS_PATH" --json
```

Display: "✓ Profile written to $HOME/.claude/ferrox-core/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/ferrox-dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/ferrox-core/USER-PROFILE.md" and jump to step 10.
...
**For /ferrox-dev-preferences (if selected):**

```bash
ferrox_run query generate-dev-preferences --analysis "$ANALYSIS_PATH" --json
```

Display: "✓ Generated /ferrox-dev-preferences at $HOME/.claude/skills/ferrox-dev-preferences/SKILL.md"

**For CLAUDE.md profile section (if selected):**

```bash
ferrox_run query generate-claude-profile --analysis "$ANALYSIS_PATH" --json
```
...
```bash
ferrox_run query generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any `ferrox-tools.cjs query` or ferrox-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox > PROFILE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your profile:    $HOME/.claude/ferrox-core/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /ferrox-dev-preferences   $HOME/.claude/skills/ferrox-dev-preferences/SKILL.md
...
- [ ] Artifacts generated sequentially via ferrox-tools.cjs query (or ferrox-tools.cjs) subcommands
