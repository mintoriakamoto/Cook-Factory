...
**Learn — native memory capture (Phase 7 memory seam).** Independently of any optional MCP tool, the Learn stage CONSULTS `ferrox-tools query memory.capture` (behind the mempalace-capture seam — the native `.planning/graphs/` bi-temporal fact store) to persist each extracted decision/surprise after ship: `memory.capture --subject <phase-subject> --predicate decided --object <decision> --valid-from <ms> --recorded-at <ms>`, adding `--contradicts` when a new decision supersedes a prior one (which invalidates — never deletes — the old fact). Like `capture_thought`, this is optional and a graceful no-op when the store/verb is unavailable; it must not block or fail the Learn stage. This is orchestration protocol (the workflow invokes the verb at the stage), not a tool-layer gate.
</step>
