...
@~/.claude/ferrox-core/references/ferrox-run-resolver.md
...
Run the standard `ferrox_run` resolver from the reference above, then run the projection from the runtime root:

```bash
# If --fast was parsed from $ARGUMENTS:
INIT=$(ferrox_run --cwd "$_FERROX_RUNTIME_ROOT" init onboard --fast --raw)
# Otherwise:
INIT=$(ferrox_run --cwd "$_FERROX_RUNTIME_ROOT" init onboard --raw)
...
- `ONBOARDING_ROOT={git_worktree_root || _FERROX_RUNTIME_ROOT}`.
...
```bash
ferrox_run --cwd "$ONBOARDING_ROOT" query commit "docs: create onboarding summary" --files .planning/onboarding/SUMMARY.md
```
