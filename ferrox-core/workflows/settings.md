<purpose>
Interactive configuration of Ferrox workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.ferrox/defaults.json) for future projects.
</purpose>
...
INIT=$(ferrox_run query state.load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
# `state.load` returns STATE frontmatter JSON from the SDK — it does not include `config_path`. Orchestrators may set `FERROX_CONFIG_PATH` from init phase-op JSON; otherwise resolve the same path ferrox-tools uses for flat vs active workstream (#2282).
...
Store `$FERROX_CONFIG_PATH` — all subsequent reads and writes use this path, not a hardcoded `.planning/config.json`, so active-workstream installs target the correct file (#2282).
</step>

<step name="read_current">
```bash
cat "$FERROX_CONFIG_PATH"
```
...
- `workflow.pattern_mapper` — run ferrox-pattern-mapper between research and planning (default: true if absent)
- `workflow.ui_phase` — generate UI-SPEC.md design contracts for frontend phases (default: true if absent)
- `workflow.ui_safety_gate` — prompt to run /ferrox:ui-phase before planning frontend phases (default: true if absent)
...
- `workflow.code_review` — enable /ferrox:code-review and /ferrox:code-review --fix commands (default: true if absent)
- `workflow.code_review_depth` — default depth for /ferrox:code-review: `quick`, `standard`, or `deep` (default: `"standard"` if absent; only relevant when `code_review` is on)
- `workflow.ui_review` — run visual quality audit (/ferrox:ui-review) in autonomous mode (default: true if absent)
- `commit_docs` — whether `.planning/` files are committed to git (default: true if absent)
- `intel.enabled` — enable queryable codebase intelligence (/ferrox:map-codebase --query) (default: false if absent)
- `graphify.enabled` — enable project knowledge graph (/ferrox:graphify) (default: false if absent)
...
- `model_policy.provider` — provider slug for model policy (default: `null`; known values: anthropic, openai, google, qwen; set via /ferrox:config --advanced)
- `model_policy.budget` — budget level for model policy (default: `null`; known values: high, medium, low; set via /ferrox:config --advanced)
- `model_policy.high` — model ID for high-cost tier (default: `null`; set via /ferrox:config --advanced)
- `model_policy.medium` — model ID for medium-cost tier (default: `null`; set via /ferrox:config --advanced)
- `model_policy.low` — model ID for low-cost tier (default: `null`; set via /ferrox:config --advanced)
</step>
...
AskUserQuestion([
  ...
  {
    question: "Enable Code Review? (/ferrox:code-review and /ferrox:code-review --fix commands)",
    header: "Code Review",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Enable /ferrox:code-review commands for reviewing source files changed during a phase." },
  ...
  {
    question: "Code Review Depth? (default depth for /ferrox:code-review — override per-run with --depth=)",
    header: "Review Depth",
    multiSelect: false,
    options: [
  ...
  {
    question: "Enable UI Review? (visual quality audit via /ferrox:ui-review in autonomous mode)",
    header: "UI Review",
    multiSelect: false,
    options: [
  ...
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "ferrox-pattern-mapper runs between research and plan steps. Surfaces conventions so new code follows house style." },
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /ferrox:ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /ferrox:ui-phase first when frontend indicators detected." },
  ...
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Run /ferrox:ai-integration-phase before planning AI system phases. Surfaces the right framework, researches its docs, and designs the evaluation strategy." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (ferrox/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (ferrox/{version}-{name})" }
    ]
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in /ferrox:autonomous — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  },
  {
  ...
  {
    question: "Enable Intel? (queryable codebase intelligence via /ferrox:map-codebase --query — builds a JSON index in .planning/intel/)",
    header: "Intel",
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Skip intel indexing. Use when codebase is small or intel queries are not needed." },
      { label: "Yes", description: "Enable /ferrox:map-codebase --query commands. Builds and queries a JSON index of the codebase." }
    ]
  },
  {
    question: "Enable Graphify? (project knowledge graph via /ferrox:graphify — builds a graph in .planning/graphs/)",
    header: "Graphify",
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Skip knowledge graph. Use when dependency graphs are not needed." },
      { label: "Yes", description: "Enable /ferrox:graphify commands. Builds and queries a project knowledge graph." }
    ]
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Manual /ferrox:graphify build only. Conservative default — opt in if you want fresh context on every /ferrox:quick or /ferrox:plan-phase." },
...
```json
{
  ...
  "model_policy": {
    // Read-only in this flow — written only by /ferrox:config --advanced (Section 8).
...
- **Capability hook-gate keys** (owned by a capability in the registry — see `registry.configSchema`): write via the capability writer:
  ```bash
  ferrox_run capability set <owner> --gate <key>=<value> [--config-dir "$RUNTIME_CONFIG_DIR"]
  ```
...
Write updated config to `$FERROX_CONFIG_PATH` (the workstream-aware path resolved in `ensure_and_load_config`). Never hardcode `.planning/config.json` — workstream installs route to `.planning/workstreams/<slug>/config.json`.
</step>
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.ferrox/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.ferrox/defaults.json`:

```bash
mkdir -p ~/.ferrox
```

Write `~/.ferrox/defaults.json` with:
```json
{
  "mode": <current>,
  "granularity": <current>,
...
These settings apply to future /ferrox:plan-phase and /ferrox:execute-phase runs.

Quick commands:
- /ferrox:config --integrations — configure API keys (Brave/Firecrawl/Exa), review.models CLI routing, and agent_skills injection
- /ferrox:config --profile <profile> — switch model profile
- /ferrox:plan-phase --research — force research
- /ferrox:plan-phase --skip-research — skip research
- /ferrox:plan-phase --skip-verify — skip plan check
- /ferrox:config --advanced — power-user tuning (plan bounce, timeouts, branch templates, cross-AI, context window, model policy)
```
</step>
...
- [ ] User offered to save as global defaults (~/.ferrox/defaults.json)
