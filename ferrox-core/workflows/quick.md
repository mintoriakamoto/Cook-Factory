<purpose>
Execute small, ad-hoc tasks with Ferrox guarantees (atomic commits, STATE.md tracking). Quick mode spawns ferrox-planner (quick mode) + ferrox-executor(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
- ferrox-code-reviewer — Reviews source files for bugs, security issues, and code quality
</available_agent_types>
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► QUICK TASK (DISCUSS + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► QUICK TASK (DISCUSS + RESEARCH)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► QUICK TASK (RESEARCH + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
INIT=$(ferrox_run query init.quick "$DESCRIPTION")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_PLANNER=$(ferrox_run query agent-skills ferrox-planner)
AGENT_SKILLS_EXECUTOR=$(ferrox_run query agent-skills ferrox-executor)
AGENT_SKILLS_CHECKER=$(ferrox_run query agent-skills ferrox-plan-checker)
AGENT_SKILLS_VERIFIER=$(ferrox_run query agent-skills ferrox-verifier)
```
...
if [ "$USE_WORKTREES" != "false" ]; then
  ferrox_run query worktree.reap-orphans 2>/dev/null || true
fi
```
...
**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/ferrox:new-project` first.
...
",
  subagent_type="ferrox-phase-researcher",
  model="{planner_model}",
...
if [ "${USE_WORKTREES}" != "false" ]; then
  ...
  COMMIT_DOCS=$(ferrox_run query config-get commit_docs 2>/dev/null || echo "true")
...
if [ "$RUNTIME" = "claude" ] && [ "${USE_WORKTREES:-true}" != "false" ]; then
  _QUICK_SHOULD_DEGRADE=$(ferrox_run query worktree.base-check --pick shouldDegrade 2>/dev/null || true)
  if [ "$_QUICK_SHOULD_DEGRADE" = "true" ]; then
    _QUICK_DEGRADE_MSG=$(ferrox_run query worktree.base-check --pick message 2>/dev/null || true)
...
if [ "${USE_WORKTREES:-true}" != "false" ]; then
  ...
  QUICK_WORKTREE_MANIFEST=$(mktemp "${TMPDIR:-/tmp}/ferrox-quick-worktree-XXXXXX") && mv "$QUICK_WORKTREE_MANIFEST" "${QUICK_WORKTREE_MANIFEST}.json" && QUICK_WORKTREE_MANIFEST="${QUICK_WORKTREE_MANIFEST}.json" || exit 1
  printf '{"worktrees":[]}\n' > "$QUICK_WORKTREE_MANIFEST"
  export QUICK_WORKTREE_MANIFEST
fi
```

Spawn ferrox-executor with plan reference:
...
1. **Worktree cleanup:** If the executor ran with `isolation="worktree"`, merge the worktree branch back and clean up:
   ...
   ferrox_run query worktree.cleanup-wave --manifest "$QUICK_WORKTREE_MANIFEST" || exit 1
   ```
...
Agent(
  ...
  Depth: quick",
  subagent_type="ferrox-code-reviewer",
...
For a schema-safe append outside this workflow (e.g. from fast.md), `ferrox-tools quick-tasks-append --task <text>` performs the equivalent write via the shared, schema-backed `appendQuickTaskRow` helper (#2133, ADR-2143 §3/§7).
...
Stage and commit quick task artifacts. This step MUST always run — even if the executor already committed some files (e.g. when running without worktree isolation). The `ferrox-tools.cjs query commit` command (or legacy `ferrox-tools.cjs` commit) handles already-committed files gracefully.
...
ferrox_run query commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
...
Ferrox > QUICK TASK COMPLETE (VALIDATED)
...
Ready for next task: /ferrox:quick ${FERROX_WS}
```
