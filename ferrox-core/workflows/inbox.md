...
3. **Parse flags:**
   ...
   - `--to-backlog` → set TO_BACKLOG=true
...
<step name="to_backlog">
Only execute if `--to-backlog` (TO_BACKLOG=true) was set.

Route APPROVED, gated issues into the Ferrox backlog (TRIAGE-01). The issue-first
gate is enforced by the pure core — an issue without its approval label is REFUSED,
never appended. For each classified issue that scored COMPLETE/MOSTLY COMPLETE,
parse `{number, title, type, labels, acceptanceCriteria}` from the review, then:

```bash
# type ∈ feature|enhancement|bug|chore ; labels = comma-joined issue labels
node -e '
const core = require("./ferrox-core/bin/lib/inbox-to-backlog.cjs");
const issue = JSON.parse(process.argv[1]);
const r = core.evaluateToBacklog(issue);
if (r.decision === "append") { console.log("APPEND\t" + r.row); }
else { console.error("REFUSED (" + r.requiredLabel + "): #" + issue.number); }
' "$ISSUE_JSON"
```

For each `APPEND` line, append the returned row to the `## Open` table in
`.planning/BACKLOG.md` (the orchestrator is the sole writer of the backlog —
COORD-05 — so this write happens here, never inside a worktree). Report a summary:
N appended, M refused (with the missing approval label for each).

**C2 — link field + label on start (TRIAGE-01).** The backlog row's `id` (`gh-NN`)
and `source` (`gh#NN`) carry the link; when the increment is planned, stamp
`github_issue: NN` in the plan frontmatter so C3 can close it on merge-gate pass.
As the increment starts, label the issue `in-progress` and clear its waiting
labels via the lifecycle core. **Set both env vars explicitly** (the hook noops if
`GH_ISSUE` is unset) — `NN` is the issue number from the `gh#NN` source, `INCREMENT`
is the backlog id (`gh-NN`):

```bash
GH_ISSUE="${NN}" INCREMENT="gh-${NN}" node -e '
const lc = require("./ferrox-core/bin/lib/issue-lifecycle.cjs");
const r = lc.onIncrementStart({ githubIssue: Number(process.env.GH_ISSUE) || null, increment: process.env.INCREMENT });
for (const a of r.actions) {
  if (a.action === "label")   console.log(`gh issue edit ${a.issue} --add-label ${a.value}`);
  if (a.action === "unlabel") console.log(`gh issue edit ${a.issue} --remove-label ${a.value}`);
}'
```

Run the emitted `gh` lines. (Note: `frontmatter --pick github_issue` on the plan
supplies `NN` once the increment is planned; at backlog-entry time use the `gh#NN`
source directly.)
</step>
...
```bash
gh issue close {number} --comment "Closed by Ferrox inbox triage: this issue is missing required fields per the issue template. Missing: {list}. Please reopen with a complete submission. See CONTRIBUTING.md for requirements."
```

For PRs with gate violations:
```bash
gh pr close {number} --comment "Closed by Ferrox inbox triage: this PR does not meet the issue-first requirement. {specific violation}. See CONTRIBUTING.md for the correct process."
```
...
- /ferrox:review — Run cross-AI peer review on a specific phase plan
- /ferrox:ship — Create a PR from completed work
- /ferrox:progress — See overall project state
- /ferrox:inbox --label — Re-run with auto-labeling enabled
</offer_next>
