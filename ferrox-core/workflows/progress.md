...
INIT=$(ferrox_run query init.progress)
...
Run /ferrox:new-project to start a new project.
```

Exit.

If missing STATE.md: suggest `/ferrox:new-project`.

**If ROADMAP.md missing but PROJECT.md exists:**

This means a milestone was completed and archived. Go to **Route F** (between milestones).

If missing both ROADMAP.md and PROJECT.md: suggest `/ferrox:new-project`.
</step>

<step name="load">
**Use structured extraction from `ferrox-tools.cjs query` (or legacy ferrox-tools.cjs):**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(ferrox-tools.cjs query roadmap.analyze)`
- `STATE=$(ferrox-tools.cjs query state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  ferrox_run query summary-extract <path> --fields one_liner
  ```
...
**Generate progress bar from `ferrox-tools.cjs query progress` / `progress.json`, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(ferrox_run query progress.bar --raw)
```
...
## Pending Todos
- [count] pending — /ferrox:capture --list to review

## Active Debug Sessions
- [count] active — /ferrox:debug to continue
...
```bash
MVP_MODE=$(ferrox_run query phase.mvp-mode "${PHASE_NUMBER}" --pick active)
```
...
**If `INCOMPLETE_PHASE` is non-empty:** emit a one-line resume notice in the routing output and route to `/ferrox:execute-phase ${INCOMPLETE_PHASE}` instead of running Step 1's current-phase routing. The progress report (already displayed by the `report` step above) gives the user full project status before this routing decision is shown.
...
`/ferrox:execute-phase ${INCOMPLETE_PHASE} ${FERROX_WS}`
...
```bash
DEBT=$(ferrox_run query audit-uat --raw 2>/dev/null)
```
...
Review: `/ferrox:audit-uat ${FERROX_WS}` — full cross-phase audit
Resume testing: `/ferrox:verify-work {phase} ${FERROX_WS}` — retest specific phase
```
...
```bash
PHASE_SECTION=$(ferrox_run query roadmap.get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
`/ferrox:plan-phase {phase-number} ${FERROX_WS}`
...
`/ferrox:discuss-phase {phase}` — gather context and clarify approach

---

**Also available:**
- `/ferrox:ui-phase {phase}` — generate UI design contract (recommended for frontend phases)
- `/ferrox:plan-phase {phase}` — skip discussion, plan directly
- `/ferrox:discuss-phase {phase}` — include assumptions check before planning
...
`/ferrox:discuss-phase {phase} ${FERROX_WS}` — gather context and clarify approach

---

**Also available:**
- `/ferrox:plan-phase {phase} ${FERROX_WS}` — skip discussion, plan directly
- `/ferrox:discuss-phase {phase} ${FERROX_WS}` — include assumptions check before planning
...
`/ferrox:plan-phase {phase} --gaps ${FERROX_WS}`

---

**Also available:**
- `/ferrox:execute-phase {phase} ${FERROX_WS}` — execute phase plans
- `/ferrox:verify-work {phase} ${FERROX_WS}` — run more UAT testing
...
`/ferrox:verify-work {phase} ${FERROX_WS}` — resume testing from where you left off

---

**Also available:**
- `/ferrox:audit-uat ${FERROX_WS}` — full cross-phase UAT audit
- `/ferrox:execute-phase {phase} ${FERROX_WS}` — execute phase plans
...
```
`/ferrox:execute-phase {phase} ${FERROX_WS}` — re-run execution verification
```
...
```
`/ferrox:execute-phase {phase} ${FERROX_WS}` — regenerate verification
```
...
```
`/ferrox:verify-work {phase} ${FERROX_WS}` — re-run verification against the latest summaries
```
...
`/ferrox:verify-work {phase} ${FERROX_WS}` — resume human verification
...
```bash
NEXT_PHASE_SECTION=$(ferrox_run query roadmap.get-phase "$((Z+1))" 2>/dev/null)
...
`/ferrox:discuss-phase {Z+1}` — gather context and clarify approach

---

**Also available:**
- `/ferrox:ui-phase {Z+1}` — generate UI design contract (recommended for frontend phases)
- `/ferrox:plan-phase {Z+1}` — skip discussion, plan directly
- `/ferrox:verify-work {Z}` — user acceptance test before continuing
...
`/ferrox:discuss-phase {Z+1} ${FERROX_WS}` — gather context and clarify approach

---

**Also available:**
- `/ferrox:plan-phase {Z+1} ${FERROX_WS}` — skip discussion, plan directly
- `/ferrox:verify-work {Z} ${FERROX_WS}` — user acceptance test before continuing
...
`/ferrox:complete-milestone ${FERROX_WS}`

---

**Also available:**
- `/ferrox:verify-work ${FERROX_WS}` — user acceptance test before completing milestone
...
`/ferrox:new-milestone ${FERROX_WS}`
...
- Phase complete but next phase not planned → offer `/ferrox:plan-phase [next] ${FERROX_WS}`
- All work complete → offer milestone completion
- Blockers present → highlight before offering to continue
- Handoff file exists → mention it, offer `/ferrox:resume-work ${FERROX_WS}`
</step>
...
- Check 2 (orphaned handoff): `Read the handoff file(s) and resume from where work was paused: /ferrox:resume-work ${FERROX_WS}`
...
- Check 1 (STATE inconsistency): `Run /ferrox:verify-work ${PHASE} ${FERROX_WS} to reconcile state`
</step>
...
- [ ] Smart routing: /ferrox:execute-phase if plans exist, /ferrox:plan-phase if not
- [ ] User confirms before any action
- [ ] Seamless handoff to appropriate ferrox command
      </success_criteria>
