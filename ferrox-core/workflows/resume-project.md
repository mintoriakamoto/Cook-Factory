...
<required_reading>
@~/.claude/ferrox-core/references/continuation-format.md
</required_reading>
...
INIT=$(ferrox_run query init.resume)
...
**If `planning_exists` is false:** This is a new project - route to /ferrox:new-project
</step>
...
- This is the primary resumption source — structured data from `/ferrox:pause-work`
...
📋 [N] pending todos — /ferrox:capture --list to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/ferrox:execute-phase {phase} ${FERROX_WS})
   OR
1. Discuss Phase 3 context (/ferrox:discuss-phase 3 ${FERROX_WS}) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/ferrox:plan-phase 3 ${FERROX_WS}) [if CONTEXT.md exists or discuss option declined]
...
- **Plan phase** → Show direct next command:
  ...
  **Also available:**
  - `/ferrox:discuss-phase [N] ${FERROX_WS}` — gather context first
  - `/ferrox:plan-phase --research-phase [N] ${FERROX_WS}` — investigate unknowns
