...
INIT=$(ferrox_run init phase-op "${PHASE}")
...
Use /ferrox:progress to see available phases.
```
Exit.
...
If "Skip": Exit with message: "Existing SPEC.md unchanged. Run /ferrox:discuss-phase [X] to continue."
...
ones). Reference: @~/.claude/ferrox-core/references/edge-probe.md.

**Runtime coverage compute — resolve and invoke edge-probe.cjs:**

```bash
# Resolve the compiled edge-probe.cjs against the Ferrox install dir via RUNTIME_DIR (#448)
...
EDGE_PROBE_JS=$(for _c in \
  "$_FERROX_RT/ferrox-core/bin/lib/edge-probe.cjs" \
  "$_FERROX_RT/bin/lib/edge-probe.cjs" \
  "$_FERROX_RT/.claude/bin/lib/edge-probe.cjs" \
  "$HOME/.claude/ferrox-core/bin/lib/edge-probe.cjs" \
  "$HOME/.claude/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)

# Graceful degradation — never silent skip (RR-04). Build ONLY when $_FERROX_RT is a verified
# Ferrox source checkout (has tsconfig.build.json + src/edge-probe.cts), and pin npm to it with
...
# prepublishOnly, so this build path only matters in a Ferrox dev checkout (review High).
if [ -z "$EDGE_PROBE_JS" ]; then
  if [ -f "$_FERROX_RT/tsconfig.build.json" ] && [ -f "$_FERROX_RT/src/edge-probe.cts" ]; then
  ...
  if [ -z "$EDGE_PROBE_JS" ]; then
    echo "ERROR: edge-probe.cjs not found — reinstall Ferrox or run \`npm run build:lib\` in your Ferrox checkout." >&2
    exit 1
  fi
fi
...
requirement list). Reference: @~/.claude/ferrox-core/references/prohibition-probe.md — the
...
3. **Canon-referral (ADR-550 D6, PROB-13).** A kept candidate that is canon security/compliance
   ...
   /ferrox:secure-phase + eslint; not minted here"*) and DROP it. Minting canon items duplicates
   /ferrox:secure-phase and drowns the bespoke signal.
   ...
   - **Keep it** → write a NEGATIVE acceptance criterion (a must-NOT line) into Acceptance
     ...
     - **Capture the wired-check descriptor on `test`-tier (#1278, SOFT).** When a prohibition is
       ...
       - `check_violation_fixture` (#1279) — path to a KNOWN-BAD subject the wired check is run
         ...
         read its subject from the `FERROX_PROHIB_SUBJECT` env var so the prover can inject this fixture.
       - `check_clean_fixture` (#1346; **REQUIRED for `node-test` as of #1906**) — path to a
         ...
         `FERROX_PROHIB_SUBJECT` merely being set. For a `node-test` this is **mandatory**: omit it and
         ...
         linted file, no `FERROX_PROHIB_SUBJECT` indirection.)
...
Use the SPEC.md template from @~/.claude/ferrox-core/templates/spec.md.
...
- User directed to /ferrox:discuss-phase as next step
