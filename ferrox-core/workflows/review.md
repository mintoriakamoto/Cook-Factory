...
OLLAMA_HOST=$(ferrox_run query config-get review.ollama_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LM_STUDIO_HOST=$(ferrox_run query config-get review.lm_studio_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LLAMA_CPP_HOST=$(ferrox_run query config-get review.llama_cpp_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
- Value must be a non-empty array of slug strings (configured via `ferrox config-set review.default_reviewers '["gemini","codex"]'`)
...
are in `ferrox-core/references/reviewer-instances.md` — load it lazily only when instances are
...
Write to a temp file: `/tmp/ferrox-review-prompt-{phase}.md`
...
cp "$INSTRUCTIONS_BLOCK_FILE" "/tmp/ferrox-review-${PHASE}-instructions.md"
cp "$ROADMAP_SECTION_FILE" "/tmp/ferrox-review-${PHASE}-roadmap.md"

# Plan files: copy each PLAN.md to a predictable numbered path
PLAN_INDEX=0
for PLAN_FILE in "${PHASE_DIR}"/*-PLAN.md; do
  PADDED_IDX=$(printf '%02d' "$PLAN_INDEX")
  cp "$PLAN_FILE" "/tmp/ferrox-review-${PHASE}-plan-${PADDED_IDX}.md"
  PLAN_INDEX=$((PLAN_INDEX + 1))
done

# Optional section files (only if content was included in the combined prompt)
if [ -f ".planning/PROJECT.md" ]; then
  cp .planning/PROJECT.md "/tmp/ferrox-review-${PHASE}-project.md"
fi
if ls "${PHASE_DIR}/"*"-CONTEXT.md" >/dev/null 2>&1; then
  cat "${PHASE_DIR}/"*"-CONTEXT.md" > "/tmp/ferrox-review-${PHASE}-context.md"
fi
if ls "${PHASE_DIR}/"*"-RESEARCH.md" >/dev/null 2>&1; then
  cat "${PHASE_DIR}/"*"-RESEARCH.md" > "/tmp/ferrox-review-${PHASE}-research.md"
fi
if [ -f ".planning/REQUIREMENTS.md" ]; then
  cp .planning/REQUIREMENTS.md "/tmp/ferrox-review-${PHASE}-requirements.md"
fi
```
...
```bash
# JSON scalars from ferrox-tools.cjs query; use jq -r to strip JSON string quotes (install jq if missing)
GEMINI_MODEL=$(ferrox_run query config-get review.models.gemini 2>/dev/null | jq -r '.' 2>/dev/null || true)
CLAUDE_MODEL=$(ferrox_run query config-get review.models.claude 2>/dev/null | jq -r '.' 2>/dev/null || true)
CODEX_MODEL=$(ferrox_run query config-get review.models.codex 2>/dev/null | jq -r '.' 2>/dev/null || true)
OPENCODE_MODEL=$(ferrox_run query config-get review.models.opencode 2>/dev/null | jq -r '.' 2>/dev/null || true)
...
AGY_MODEL=$(ferrox_run query config-get review.models.agy 2>/dev/null | jq -r '.' 2>/dev/null || true)
...
shell-interpolated). Exact invocation in `ferrox-core/references/reviewer-instances.md`.
...
if [ -n "$GEMINI_MODEL" ] && [ "$GEMINI_MODEL" != "null" ]; then
  cat /tmp/ferrox-review-prompt-{phase}.md | gemini -m "$GEMINI_MODEL" -p - 2>/dev/null > /tmp/ferrox-review-gemini-{phase}.md
else
  cat /tmp/ferrox-review-prompt-{phase}.md | gemini -p - 2>/dev/null > /tmp/ferrox-review-gemini-{phase}.md
fi
```
...
if [ -n "$CLAUDE_MODEL" ] && [ "$CLAUDE_MODEL" != "null" ]; then
  cat /tmp/ferrox-review-prompt-{phase}.md | claude --model "$CLAUDE_MODEL" -p - 2>/dev/null > /tmp/ferrox-review-claude-{phase}.md
else
  cat /tmp/ferrox-review-prompt-{phase}.md | claude -p - 2>/dev/null > /tmp/ferrox-review-claude-{phase}.md
fi
```
...
if [ -n "$CODEX_MODEL" ] && [ "$CODEX_MODEL" != "null" ]; then
  cat /tmp/ferrox-review-prompt-{phase}.md | codex exec --ephemeral $CODEX_BYPASS_FLAG --model "$CODEX_MODEL" --skip-git-repo-check -o /tmp/ferrox-review-codex-{phase}.md - 2>/tmp/ferrox-review-codex-{phase}.err >/dev/null
else
  cat /tmp/ferrox-review-prompt-{phase}.md | codex exec --ephemeral $CODEX_BYPASS_FLAG --skip-git-repo-check -o /tmp/ferrox-review-codex-{phase}.md - 2>/tmp/ferrox-review-codex-{phase}.err >/dev/null
fi
if [ ! -s /tmp/ferrox-review-codex-{phase}.md ]; then
  echo "Codex review failed or returned empty output. stderr:" > /tmp/ferrox-review-codex-{phase}.md
  cat /tmp/ferrox-review-codex-{phase}.err >> /tmp/ferrox-review-codex-{phase}.md
fi
```
...
```bash
coderabbit review --prompt-only 2>/dev/null > /tmp/ferrox-review-coderabbit-{phase}.md
```
...
`ferrox-core/references/reviewer-instances.md`) sidesteps the default `build` agent and
...
cat /tmp/ferrox-review-prompt-{phase}.md | opencode run "$@" --format json - 2>/tmp/ferrox-review-opencode-{phase}.err > /tmp/ferrox-review-opencode-{phase}.json
...
OPENCODE_REVIEW=$(jq -rs '[.[] | select(.type=="text") | .part.text // empty] | join("\n")' /tmp/ferrox-review-opencode-{phase}.json 2>/dev/null)
if [ -n "$OPENCODE_REVIEW" ]; then
  printf '%s\n' "$OPENCODE_REVIEW" > /tmp/ferrox-review-opencode-{phase}.md
else
  # No assistant text (agent emitted no final message, or stdout was not valid JSON events).
  {
    ...
    OPENCODE_DIAG=$(jq -rs '[.[] | select(.type=="step_finish")] | last | "stop reason=\(.part.reason // "?"), output tokens=\(.part.tokens.output // "?")"' /tmp/ferrox-review-opencode-{phase}.json 2>/dev/null)
    [ -n "$OPENCODE_DIAG" ] && echo "Diagnostic: $OPENCODE_DIAG"
    echo "stderr:"
    cat /tmp/ferrox-review-opencode-{phase}.err
  } > /tmp/ferrox-review-opencode-{phase}.md
fi
```

**Qwen Code:**
```bash
cat /tmp/ferrox-review-prompt-{phase}.md | qwen - 2>/dev/null > /tmp/ferrox-review-qwen-{phase}.md
if [ ! -s /tmp/ferrox-review-qwen-{phase}.md ]; then
  echo "Qwen review failed or returned empty output." > /tmp/ferrox-review-qwen-{phase}.md
fi
```
...
# anchor is correct even when /ferrox:review is invoked from a repo subdirectory.
_CURSOR_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
CURSOR_PROMPT_ARG="Read the file at /tmp/ferrox-review-prompt-{phase}.md in full and carry out the review request it contains. The repository under review is at $_CURSOR_ROOT — resolve every relative file path in the review request against that absolute root. Output only the resulting markdown review. Do not edit any files."
cursor-agent -p --mode ask --trust --output-format text "$CURSOR_PROMPT_ARG" 2>/tmp/ferrox-review-cursor-{phase}.err > /tmp/ferrox-review-cursor-{phase}.md
if [ ! -s /tmp/ferrox-review-cursor-{phase}.md ]; then
  echo "Cursor review failed or returned empty output. stderr:" > /tmp/ferrox-review-cursor-{phase}.md
  cat /tmp/ferrox-review-cursor-{phase}.err >> /tmp/ferrox-review-cursor-{phase}.md
fi
```
...
_AGY_PROMPT="Read the file at /tmp/ferrox-review-prompt-{phase}.md in full and carry out the review request it contains. The repository under review is at $_AGY_WS — resolve every relative file path in the review request against that absolute root and verify claims against those files. If you cannot read files under $_AGY_WS, begin your output with the exact line REVIEWED-WITHOUT-REPO-ACCESS before the review. Output only the resulting markdown review. Do not edit any files."
...
if [ -n "$_AGY_KILLER" ]; then
  "$_AGY_KILLER" 600 agy --print-timeout 540s "$@" -p "$_AGY_PROMPT" </dev/null 2>/dev/null > /tmp/ferrox-review-antigravity-{phase}.md
else
  agy --print-timeout 540s "$@" -p "$_AGY_PROMPT" </dev/null 2>/dev/null > /tmp/ferrox-review-antigravity-{phase}.md
fi
_AGY_RC=$?
if [ "$_AGY_RC" -ne 0 ]; then
  : > /tmp/ferrox-review-antigravity-{phase}.md
fi
...
if [ ! -s /tmp/ferrox-review-antigravity-{phase}.md ]; then
  if [ -f "$_AGY_CACHE" ]; then
    ...
    if [ -n "$_AGY_CONV" ] && [ "$_AGY_CONV" != "null" ]; then
      ...
      if [ -f "$_AGY_TX" ]; then
        ...
        _AGY_RESULT=$(tail -n +"$((_AGY_SKIP + 1))" "$_AGY_TX" 2>/dev/null | \
          ...
          2>/dev/null | tail -1)
        [ -n "$_AGY_RESULT" ] && echo "$_AGY_RESULT" > /tmp/ferrox-review-antigravity-{phase}.md
      fi
    fi
  fi
fi
...
if [ -s /tmp/ferrox-review-antigravity-{phase}.md ] && \
   { head -5 /tmp/ferrox-review-antigravity-{phase}.md | grep -q 'REVIEWED-WITHOUT-REPO-ACCESS' || \
     grep -qiE '(workspace|working) (directory|dir).{0,40}antigravity-cli/scratch' /tmp/ferrox-review-antigravity-{phase}.md; }; then
  {
    ...
    echo ""
    cat /tmp/ferrox-review-antigravity-{phase}.md
  } > /tmp/ferrox-review-antigravity-{phase}.md.tmp && \
  mv /tmp/ferrox-review-antigravity-{phase}.md.tmp /tmp/ferrox-review-antigravity-{phase}.md
fi
```
...
prepare_trimmed_prompt_for_reviewer() {
  ...
  PLAN_FILE_ARGS=""
  for p in /tmp/ferrox-review-{phase}-plan-*.md; do
    [ -f "$p" ] && PLAN_FILE_ARGS="$PLAN_FILE_ARGS --plan-file $p"
  done
  PROJECT_ARG=""
  [ -f "/tmp/ferrox-review-{phase}-project.md" ] && PROJECT_ARG="--project-file /tmp/ferrox-review-{phase}-project.md"
  CONTEXT_ARG=""
  [ -f "/tmp/ferrox-review-{phase}-context.md" ] && CONTEXT_ARG="--context-file /tmp/ferrox-review-{phase}-context.md"
  RESEARCH_ARG=""
  [ -f "/tmp/ferrox-review-{phase}-research.md" ] && RESEARCH_ARG="--research-file /tmp/ferrox-review-{phase}-research.md"
  REQUIREMENTS_ARG=""
  [ -f "/tmp/ferrox-review-{phase}-requirements.md" ] && REQUIREMENTS_ARG="--requirements-file /tmp/ferrox-review-{phase}-requirements.md"

  ferrox_run query prompt-budget \
    --budget "$REVIEWER_BUDGET" \
    --instructions-file "/tmp/ferrox-review-{phase}-instructions.md" \
    --roadmap-file "/tmp/ferrox-review-{phase}-roadmap.md" \
...
OLLAMA_REVIEWER_BUDGET=$(ferrox_run query config-get review.max_prompt_tokens_per_reviewer.ollama 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
if [ -z "$OLLAMA_REVIEWER_BUDGET" ] || [ "$OLLAMA_REVIEWER_BUDGET" = "null" ]; then
  OLLAMA_REVIEWER_BUDGET=$(ferrox_run query config-get review.max_prompt_tokens 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
fi

# Apply budget trim for Ollama if a budget is configured
OLLAMA_PROMPT_FILE="/tmp/ferrox-review-prompt-{phase}.md"
OLLAMA_SKIP=0
if [ -n "$OLLAMA_REVIEWER_BUDGET" ] && [ "$OLLAMA_REVIEWER_BUDGET" != "null" ] && [ "$OLLAMA_REVIEWER_BUDGET" != "0" ]; then
  OLLAMA_TRIMMED_PROMPT="/tmp/ferrox-review-prompt-{phase}-ollama.md"
  OLLAMA_TRIM_META="/tmp/ferrox-review-prompt-{phase}-ollama.metadata.json"
...
OLLAMA_HOST=$(ferrox_run query config-get review.ollama_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$OLLAMA_HOST" ] || [ "$OLLAMA_HOST" = "null" ]; then OLLAMA_HOST="http://localhost:11434"; fi
OLLAMA_MODEL=$(ferrox_run query config-get review.models.ollama 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
jq -n --rawfile content "$OLLAMA_PROMPT_FILE" \
  ...
  > /tmp/ferrox-review-ollama-{phase}.md
if [ ! -s /tmp/ferrox-review-ollama-{phase}.md ]; then
  echo "Ollama review failed or returned empty output." > /tmp/ferrox-review-ollama-{phase}.md
fi
fi
```
...
LM_STUDIO_REVIEWER_BUDGET=$(ferrox_run query config-get review.max_prompt_tokens_per_reviewer.lm_studio 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
if [ -z "$LM_STUDIO_REVIEWER_BUDGET" ] || [ "$LM_STUDIO_REVIEWER_BUDGET" = "null" ]; then
  LM_STUDIO_REVIEWER_BUDGET=$(ferrox_run query config-get review.max_prompt_tokens 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
fi

# Apply budget trim for LM Studio if a budget is configured
LM_STUDIO_PROMPT_FILE="/tmp/ferrox-review-prompt-{phase}.md"
LM_STUDIO_SKIP=0
if [ -n "$LM_STUDIO_REVIEWER_BUDGET" ] && [ "$LM_STUDIO_REVIEWER_BUDGET" != "null" ] && [ "$LM_STUDIO_REVIEWER_BUDGET" != "0" ]; then
  LM_STUDIO_TRIMMED_PROMPT="/tmp/ferrox-review-prompt-{phase}-lm_studio.md"
  LM_STUDIO_TRIM_META="/tmp/ferrox-review-prompt-{phase}-lm_studio.metadata.json"
...
LM_STUDIO_HOST=$(ferrox_run query config-get review.lm_studio_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$LM_STUDIO_HOST" ] || [ "$LM_STUDIO_HOST" = "null" ]; then LM_STUDIO_HOST="http://localhost:1234"; fi
LM_STUDIO_MODEL=$(ferrox_run query config-get review.models.lm_studio 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
if [ -n "$LM_STUDIO_CONTENT" ]; then
  echo "$LM_STUDIO_CONTENT" > /tmp/ferrox-review-lm_studio-{phase}.md
else
...
LLAMA_CPP_REVIEWER_BUDGET=$(ferrox_run query config-get review.max_prompt_tokens_per_reviewer.llama_cpp 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
if [ -z "$LLAMA_CPP_REVIEWER_BUDGET" ] || [ "$LLAMA_CPP_REVIEWER_BUDGET" = "null" ]; then
  LLAMA_CPP_REVIEWER_BUDGET=$(ferrox_run query config-get review.max_prompt_tokens 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
fi

# Apply budget trim for llama.cpp if a budget is configured
LLAMA_CPP_PROMPT_FILE="/tmp/ferrox-review-prompt-{phase}.md"
LLAMA_CPP_SKIP=0
if [ -n "$LLAMA_CPP_REVIEWER_BUDGET" ] && [ "$LLAMA_CPP_REVIEWER_BUDGET" != "null" ] && [ "$LLAMA_CPP_REVIEWER_BUDGET" != "0" ]; then
  LLAMA_CPP_TRIMMED_PROMPT="/tmp/ferrox-review-prompt-{phase}-llama_cpp.md"
  LLAMA_CPP_TRIM_META="/tmp/ferrox-review-prompt-{phase}-llama_cpp.metadata.json"
...
LLAMA_CPP_HOST=$(ferrox_run query config-get review.llama_cpp_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$LLAMA_CPP_HOST" ] || [ "$LLAMA_CPP_HOST" = "null" ]; then LLAMA_CPP_HOST="http://localhost:8080"; fi
LLAMA_CPP_MODEL=$(ferrox_run query config-get review.models.llama_cpp 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
if [ -n "$LLAMA_CPP_CONTENT" ]; then
  echo "$LLAMA_CPP_CONTENT" > /tmp/ferrox-review-llama_cpp-{phase}.md
else
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 Ferrox ► CROSS-AI REVIEW — Phase {N}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
`ferrox-core/references/reviewer-instances.md`.
...
Commit:
```bash
ferrox_run query commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
- [ ] User knows how to use feedback (/ferrox:plan-phase --reviews)
</success_criteria>
