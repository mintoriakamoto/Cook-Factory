<purpose>
Lightweight codebase assessment. Spawns a single ferrox-codebase-mapper agent for one focus area,
...
INIT=$(ferrox_run query init.map-codebase 2>/dev/null || echo "{}")
...
Spawn a single `ferrox-codebase-mapper` agent with the selected focus area:
...
Use `/ferrox:map-codebase` for a comprehensive 4-area parallel scan.
```
