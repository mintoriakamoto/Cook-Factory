...
REG_TEST_CMD=$(ferrox_run query config-get workflow.test_command --default "" 2>/dev/null || true)
# v1.13 Wave 0 (B5): non-code domains resolve the regression sniff to a no-op so a
...
if [ -z "$REG_TEST_CMD" ] && [ "$FERROX_NONCODE_DOMAIN" = "1" ]; then
  REG_TEST_CMD="true"
  echo "Non-code domain '$FERROX_DOMAIN': regression test sniff skipped, no code test runner applies to prose deliverables"
elif [ -z "$REG_TEST_CMD" ]; then
...
REG_TEST_CMD=$(ferrox_run query normalize-test-command "$REG_TEST_CMD" --cwd . 2>/dev/null || echo "$REG_TEST_CMD")
