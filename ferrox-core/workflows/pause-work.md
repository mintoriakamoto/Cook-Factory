<purpose>
Create structured `.planning/HANDOFF.json` and `.continue-here.md` handoff files to preserve complete work state across sessions. The JSON provides machine-readable state for `/ferrox:resume-work`; the markdown provides human-readable context.
</purpose>
...
timestamp=$(ferrox_run query current-timestamp full --raw)
```
...
<step name="commit">
```bash
ferrox_run query commit "wip: [context-name] paused at [X]/[Y]" --files [handoff-path] .planning/HANDOFF.json
```
</step>
