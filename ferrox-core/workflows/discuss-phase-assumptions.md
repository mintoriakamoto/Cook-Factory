...
- ferrox-assumptions-analyzer — Analyzes codebase to surface implementation assumptions
</available_agent_types>
...
AGENT_SKILLS_ANALYZER=$(ferrox_run query agent-skills ferrox-assumptions-analyzer)
# #2072: resolve the routed model so model_overrides / models.discuss are honored
# (the resolver maps ferrox-assumptions-analyzer → phaseType "discuss"); thread it below.
ANALYZER_MODEL=$(ferrox_run query resolve-model ferrox-assumptions-analyzer --raw)
```
...
- When spawning the ferrox-assumptions-analyzer, pass the lens list so it can flag which lenses apply
...
Spawn a `ferrox-assumptions-analyzer` agent to deeply analyze the codebase for this phase. This
...
```
Agent(subagent_type="ferrox-assumptions-analyzer", model="{ANALYZER_MODEL}", prompt="""
...
```bash
ferrox_run query commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
`/ferrox:plan-phase ${PHASE}`

---

**Also available:**
- `/ferrox:plan-phase ${PHASE} --skip-research` — plan without research
- `/ferrox:ui-phase ${PHASE}` — generate UI design contract (if frontend work)
...
Launch: `Skill(skill="ferrox-plan-phase", args="${PHASE} --auto")`
