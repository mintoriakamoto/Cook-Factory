<purpose>
Check for Ferrox updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect the installed Ferrox version, scope, runtime, and config dir.
...
- If the path contains `/ferrox-core/workflows/update.md`, strip that suffix and store the remainder as `PREFERRED_CONFIG_DIR`.
...
Then resolve the install context via the deterministic projection (#498). **Do NOT re-derive scope, runtime, or version by hand** — `update-context` owns that cascade in tested code (`ferrox-core/bin/lib/update-context.cjs`), the same way `check-latest-version` owns the package name (#2992):

```bash
# Resolve ferrox-tools.cjs WITHOUT yet knowing FERROX_DIR. The running workflow lives
# at <PREFERRED_CONFIG_DIR>/ferrox-core/workflows/update.md, so its sibling
# bin/ferrox-tools.cjs is the authoritative tool for THIS install. Fall back to a
# global copy, then to ferrox-tools on PATH.
FERROX_TOOLS=""
for cand in \
  "$PREFERRED_CONFIG_DIR/ferrox-core/bin/ferrox-tools.cjs" \
  "$HOME/.claude/ferrox-core/bin/ferrox-tools.cjs"; do
  if [ -n "$cand" ] && [ -f "$cand" ]; then FERROX_TOOLS="$cand"; break; fi
done
# Last resort: the ferrox-tools shim on PATH — resolved to its absolute path and
# invoked via the variable (never a bare `ferrox-tools` command; see #2851).
if [ -z "$FERROX_TOOLS" ] && command -v ferrox-tools >/dev/null 2>&1; then
  FERROX_TOOLS="$(command -v ferrox-tools)"
fi

UC=""
if [ -n "$FERROX_TOOLS" ]; then
  case "$FERROX_TOOLS" in
    *.cjs) UC="$(node "$FERROX_TOOLS" update-context --config-dir "$PREFERRED_CONFIG_DIR" --runtime "$PREFERRED_RUNTIME" --json 2>/dev/null)" ;;
    *)     UC="$("$FERROX_TOOLS" update-context --config-dir "$PREFERRED_CONFIG_DIR" --runtime "$PREFERRED_RUNTIME" --json 2>/dev/null)" ;;
  esac
fi
...
if [ -n "$UC" ]; then
  ...
  FERROX_DIR="$(printf '%s' "$UC" | jq -r '.ferroxDir')"
else
...
- Line 4 = resolved Ferrox config dir (e.g. `/Users/me/.claude`, `/Users/me/.gemini`); empty if scope is `UNKNOWN`. Capture this as `FERROX_DIR` and pass it to subsequent steps so they don't re-derive the runtime path.
...
The `FERROX_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`~/.claude/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just Claude. If `FERROX_DIR` is empty (scope `UNKNOWN`), skip this step and go directly to install.

`LATEST_RESULT` is a JSON document with the documented shape `{ ok: bool, version: string, reason: string, detail?: string }`. Parse via `jq` ONLY when the script actually ran. When `FERROX_DIR` is empty (scope `UNKNOWN`), skip the check entirely and seed the parsed fields with their no-op values so downstream logic does not mistake an unset `LATEST_RESULT` for a failed network check (#2993 CR feedback):

```bash
if [ -z "$FERROX_DIR" ]; then
  ...
  LATEST_RESULT="$(node "$FERROX_DIR/ferrox-core/bin/check-latest-version.cjs" --json --tag "$TAG" 2>/dev/null)"
  LATEST_STATUS=$?
...
To update manually: `npx -y --package=ferrox-core@{TAG} -- ferrox-core --global`
```
...
Running /ferrox:update would install the npm release (A.B.C) and downgrade
...
1. Fetch changelog from GitHub raw URL and save to a temp file, e.g. `/tmp/ferrox-changelog-$$.md`.
...
```bash
CHANGELOG_TMP="/tmp/ferrox-changelog-$$.md"
curl -fsSL "https://raw.githubusercontent.com/ferroxfactory/ferrox-core/main/CHANGELOG.md" -o "$CHANGELOG_TMP" 2>/dev/null \
  || wget -qO "$CHANGELOG_TMP" "https://raw.githubusercontent.com/ferroxfactory/ferrox-core/main/CHANGELOG.md" 2>/dev/null

FERROX_CHANGESET_CLI="$FERROX_DIR/scripts/changeset/cli.cjs"
if [ ! -f "$FERROX_CHANGESET_CLI" ]; then
  CHANGELOG_PREVIEW="(Changelog CLI not found at $FERROX_CHANGESET_CLI — reinstall Ferrox to restore preview. Update will still proceed.)"
else
  EXTRACT_JSON=$(node "$FERROX_CHANGESET_CLI" extract \
  ...
  if [ "$EXTRACT_EXIT" -eq 2 ]; then
    ...
    CHANGELOG_PREVIEW=$(node "$FERROX_CHANGESET_CLI" extract \
      --from "$INSTALLED_VERSION" \
      --to "$LATEST_VERSION" \
...
⚠️  **Note:** The installer performs a clean install of Ferrox folders:
- `commands/ferrox/` will be wiped and replaced
- `ferrox-core/` will be wiped and replaced
- `agents/ferrox-*` files will be replaced
...
- Custom commands not in `commands/ferrox/` ✓
- Custom agents not prefixed with `ferrox-` ✓
- Custom hooks ✓
- Your CLAUDE.md files ✓

If you've modified any Ferrox files directly, they'll be automatically backed up to `ferrox-local-patches/` and can be reapplied with `/ferrox:update --reapply` after the update.
```
...
Ferrox-managed directories. These are files that exist on disk but are NOT listed
in `ferrox-file-manifest.json` — i.e., files the user added themselves that the
...
even when custom files exist (bug #1997). Use `ferrox-tools.cjs query detect-custom-files`
or the bundled `ferrox-tools.cjs detect-custom-files` path — both resolve paths
...
# get_installed_version emits it as FERROX_DIR (LOCAL or GLOBAL install dir, or empty
# when scope is UNKNOWN). Empty RUNTIME_DIR skips the backup below.
RUNTIME_DIR="$FERROX_DIR"
```
...
```bash
CUSTOM_JSON=''
if [ -f "$FERROX_TOOLS" ] && [ -n "$RUNTIME_DIR" ]; then
  CUSTOM_JSON=$(node "$FERROX_TOOLS" detect-custom-files --config-dir "$RUNTIME_DIR" 2>/dev/null)
fi
...
Back up each custom file to `$RUNTIME_DIR/ferrox-user-files-backup/` before the
installer wipes the directories:

```bash
BACKUP_DIR="$RUNTIME_DIR/ferrox-user-files-backup"
mkdir -p "$BACKUP_DIR"
...
```
⚠️  Found N custom file(s) inside Ferrox-managed directories.
    These have been backed up to ferrox-user-files-backup/ before the update.
...
**If LOCAL install:**
```bash
npx -y --package=ferrox-core@"$TAG" -- ferrox-core "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y --package=ferrox-core@"$TAG" -- ferrox-core "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y --package=ferrox-core@"$TAG" -- ferrox-core --claude --global
```
...
for dir in "${CACHE_DIRS[@]}"; do
  if [ -n "$dir" ]; then
    rm -f "$dir/cache/ferrox-update-check"*.json
  fi
done
...
for dir in .claude .config/opencode .opencode .gemini/antigravity-ide .gemini/antigravity-cli .gemini/antigravity .agents .agent .config/kilo .kilo .codex .cursor .codeium/windsurf .augment .trae .qwen .hermes .codebuddy .cline; do
  rm -f "./$dir/cache/ferrox-update-check"*.json
  rm -f "$HOME/$dir/cache/ferrox-update-check"*.json
done

# Clear the shared tool-agnostic cache written by ferrox-check-update.js hook (#2784).
# The hook uses ~/.cache/ferrox/ferrox-update-check.json (legacy) or a per-package name
# like ferrox-update-check-ferrox-core.json; the glob clears all variants so the
# statusline stops showing the stale "⬆ /ferrox:update" indicator after update.
rm -f "$HOME/.cache/ferrox/ferrox-update-check"*.json
```

The SessionStart hook (`ferrox-check-update.js`) writes to the detected runtime's cache directory, so preferred/env-derived paths and default paths must all be cleared to prevent stale update indicators.
</step>
...
```
╔═══════════════════════════════════════════════════════════╗
║  Ferrox Updated: v1.5.10 → v1.5.15                           ║
╚═══════════════════════════════════════════════════════════╝

⚠️  Restart your runtime to pick up the new commands.

[View full changelog](https://github.com/ferroxfactory/ferrox-core/blob/main/CHANGELOG.md)
```
</step>
...
Check for ferrox-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run `/ferrox:update --reapply` to merge your modifications into the new version.
```
