...
Invoked by `/ferrox:debug` (`commands/ferrox/debug.md`).

Systematic debugging using the scientific method with subagent isolation.
Orchestrates symptom gathering, session creation, and delegation to `ferrox-debug-session-manager`.

<available_agent_types>
Valid Ferrox subagent types (use exact names — do not fall back to 'general-purpose'):
- ferrox-debug-session-manager — manages debug checkpoint/continuation loop in isolated context
- ferrox-debugger — investigates bugs using scientific method
</available_agent_types>
...
```bash
debugger_model=$(ferrox_run query resolve-model ferrox-debugger 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Read TDD mode from config:
```bash
TDD_MODE=$(ferrox_run query config-get workflow.tdd_mode 2>/dev/null | jq -r 'if type == "boolean" then tostring else . end' 2>/dev/null || echo "false")
```
...
─────────────────────────────────────────────
Run `/ferrox:debug continue <slug>` to resume a session.
No sessions? `/ferrox:debug <description>` to start.
```

If no files exist or the glob returns nothing: print "No active debug sessions. Run `/ferrox:debug <issue description>` to start one."
...
**Sanitize SLUG first:** strip whitespace, reject unless it matches `^[a-z0-9][a-z0-9-]*$`, enforce max 30 chars, reject any `..`, `/`, or `\`. If invalid, print "No active debug session found with slug: {SLUG}. Check `/ferrox:debug list` for active sessions." and stop.

Check `.planning/debug/{SLUG}.md` exists. If not, print "No active debug session found with slug: {SLUG}. Check `/ferrox:debug list` for active sessions." and stop.
...
""",
  subagent_type="ferrox-debug-session-manager",
...
If summary shows `ABANDONED`: note session saved at `.planning/debug/{slug}.md` for later `/ferrox:debug continue {slug}`.
...
- [ ] ferrox-debug-session-manager spawned with security-hardened session_params
