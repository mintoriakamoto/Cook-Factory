 ...
 * module (ferrox-core/bin/lib/external-job.cjs). The pure module is fully unit-
...
const m = require('../ferrox-core/bin/lib/external-job.cjs');
...
const { resolveConfigKey } = require('../ferrox-core/bin/lib/capability-activation.cjs');
...
function resolveExternalJobSettings({ cwd, env, config, registry } = {}) {
    ...
    if (cfg === undefined) {
        try {
            cfg = require('../ferrox-core/bin/lib/config-loader.cjs').loadConfig(cwd);
        } catch {
            cfg = {}; // resolveConfigKey still falls back to the registry default
        }
    }
    const e = env || {};
    const submitEnv = e.FERROX_SLURM_SUBMIT_TIMEOUT_MS;
    const pollEnv = e.FERROX_SLURM_POLL_TIMEOUT_MS;
    const artifactEnv = e.FERROX_EXTERNAL_JOB_ARTIFACT_DIR;
...
function cmdSubmit(flags) {
  ...
  const resume = flags.resume || ('/ferrox:execute-phase ' + phase);
