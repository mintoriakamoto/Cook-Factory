 ...
 * omit the Ferrox Version field entirely. This module provides the pure logic
...
const VERSION_GATE_MARKER = '<!-- ferrox-version-gate -->';

// Labels that opt an issue out of the version gate.
const EXEMPT_LABELS = ['version-exempt'];

// Heading GitHub renders for the bug template's `label: Ferrox Version` field.
// Issue Forms render input labels as `### <label>`; matches the exact heading
// `### Ferrox Version` (any heading level) — bare `### Version` is NOT matched.
const VERSION_HEADING_RE = /^#{1,6}\s*Ferrox\s+Version\s*$/i;
...
function isBugReport({ labels, body } = {}) {
  ...
  // bug-template's `### Ferrox Version` heading for fully unlabeled submissions
...
/**
 * Extract the value beneath the "Ferrox Version" heading. Returns null when the
...
function renderCloseComment() {
  return [
    VERSION_GATE_MARKER,
    'Closing automatically — this bug report does not include a valid **Ferrox Version**.',
    '',
