...
// ADR-457 build-at-publish: ferrox-core/bin/lib/*.cjs is generated from
...
//   2. Verify every src/*.cts (non-.d.cts) maps to a non-empty ferrox-core/bin/lib/*.cjs.
...
// it works regardless of FERROX_TEST_DIR / temp-dir cwd.
function ensureBuiltArtifacts(overrides = {}) {
  ...
  const outDir = overrides.outDir || join(root, 'ferrox-core', 'bin', 'lib');
  ...
  // Handles subdirectories (e.g. src/installer-migrations/*.cts → ferrox-core/bin/lib/installer-migrations/*.cjs).
  ...
  // #996 placed the tsbuildinfo inside ferrox-core/bin/ (a copied/shipped tree), which
  ...
  const legacyTsBuildInfo = join(root, 'ferrox-core', 'bin', 'tsconfig.build.tsbuildinfo');
...
function main() {
  ...
  const testDir = process.env.FERROX_TEST_DIR
    ? process.env.FERROX_TEST_DIR
  ...
  if (selected.length === 0) {
    ...
    // Allow FERROX_ALLOW_EMPTY_SUITE=1 as an escape hatch (downgrades to a warning).
    if (process.env.FERROX_ALLOW_EMPTY_SUITE === '1') {
      console.error(`run-tests: WARNING: 0 test files selected for suite "${suite || 'all'}" — discovery or suite filter may be broken (FERROX_ALLOW_EMPTY_SUITE=1 suppressed the error)`);
      return 0;
    }
  ...
  // honours FERROX_PROJECT/FERROX_WORKSTREAM. A developer shell inside a Ferrox workstream
  // exports FERROX_WORKSTREAM, which would redirect fixture STATE.md reads away from
  ...
  delete process.env.FERROX_PROJECT;
  delete process.env.FERROX_WORKSTREAM;
  delete process.env.CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS;
  // Sandbox the overlay home so the loader's global scan ($FERROX_HOME/.ferrox/capabilities)
  ...
  // invocation (that churned ~20+ temp dirs per harness run and amplified Docker load).
  {
    ...
    const _gh = process.env.FERROX_HOME;
    if (!_gh || !_basename(_gh).startsWith('ferrox-test-home-')) {
      process.env.FERROX_HOME = mkdtempSync(_join(tmpdir(), 'ferrox-test-home-'));
    }
  }
