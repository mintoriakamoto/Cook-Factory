...
/**
 * gen-capability-registry.cjs — generates ferrox-core/bin/lib/capability-registry.cjs
...
const CONFIG_SCHEMA_PATH = path.join(ROOT, 'ferrox-core', 'bin', 'shared', 'config-schema.manifest.json');

// ─── Loop Host Contract ───────────────────────────────────────────────────────
//
// Generated from workflow markers by scripts/gen-loop-host-contract.cjs (ADR-894 §3).
// Require the committed ferrox-core/bin/lib/loop-host-contract.cjs artifact so the
// registry generator and the loop-host-contract generator share one source of truth.
const { LOOP_HOST_CONTRACT } = require('../ferrox-core/bin/lib/loop-host-contract.cjs');
...
const capValidator = require('../ferrox-core/bin/lib/capability-validator.cjs');
...
// been moved to ferrox-core/bin/lib/capability-validator.cjs per ADR-1244 D2.)

const INSTALL_PROFILES_PATH = path.join(ROOT, 'ferrox-core', 'bin', 'lib', 'install-profiles.cjs');
const CLUSTERS_PATH = path.join(ROOT, 'ferrox-core', 'bin', 'lib', 'clusters.cjs');
...
function runConsistencyGate(capabilityClusters, profileMembership, capMap) {
  ...
  const commandsFerroxDir = path.join(ROOT, 'commands', 'ferrox');
  const skillsManifest = loadSkillsManifest(commandsFerroxDir);
...
function main() {
  ...
  if (flag === '--check') {
    ...
    if (!fs.existsSync(REGISTRY_PATH)) {
      process.stderr.write(
        'ferrox-core/bin/lib/capability-registry.cjs does not exist. Run:\n' +
    ...
    if (normalizeLineEndings(stripGeneratedComment(committed)) !== normalizeLineEndings(stripGeneratedComment(live))) {
      process.stderr.write(
        'ferrox-core/bin/lib/capability-registry.cjs is stale. Run:\n' +
        '  node scripts/gen-capability-registry.cjs --write\n',
      );
      throw new ExitError(1);
    }

    process.stdout.write('ferrox-core/bin/lib/capability-registry.cjs is up to date.\n');
  } else if (flag === '--write') {
    // Fix #3: read the REAL central config keys so collision detection fires and is visible.
    const centralKeys = loadCentralConfigKeys();
    const { capMap, errors } = loadAndValidate(centralKeys);
