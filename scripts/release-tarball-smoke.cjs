 ...
 *     - Assert commands/ferrox/<cmd>.md exists in the installed package
 *     - Parse the .md for a workflow @-import or inline reference
 *     - Assert the referenced workflow .md exists in the installed package
 *   If 'init' is in lifecycleCommands, runs `ferrox-core --local --claude`
 ...
 *   - Scans all installed ferrox-core/workflows/*.md for /ferrox:<known-cmd>
...
function binInvocation(binPath, args = []) {
  ...
  // helpers (findFerroxToolsBin / findInstallerBin) only ever surface a .cmd path
...
/**
 * Locate the lib/node_modules/ferrox-core package root inside
...
/**
 * Locate the installed ferrox-tools binary (symlink in <prefix>/bin/).
 */
function findFerroxToolsBin(installPrefix) {
  for (const c of binCandidates(installPrefix, 'ferrox-tools')) {
...
/**
 * Locate the ferrox-core installer binary (the symlink in <prefix>/bin/).
 */
function findInstallerBin(installPrefix) {
  for (const c of binCandidates(installPrefix, 'ferrox-core')) {
    if (fs.existsSync(c)) return c;
  }
  return null;
}
 ...
 *
 *   1. @-import line:  `@~/.claude/ferrox-core/workflows/<name>.md`
 *   2. Inline mention: any line containing `~/.claude/ferrox-core/workflows/<name>.md`
...
function parseWorkflowRef(mdContent) {
  const WORKFLOW_PREFIX = 'ferrox-core/workflows/';
...
/**
 * Read the list of known Ferrox command names from the installed package.
 * Returns an array of strings like `['init', 'discuss-phase', ...]`.
 */
function readInstalledCmdNames(pkg) {
  const commandsDir = path.join(pkg, 'commands', 'ferrox');
...
/**
 * Scan a single workflow .md file for /ferrox:<cmd> colon-namespace leaks.
 *
 * Uses the word-boundary-safe regex shape from scripts/fix-slash-commands.cjs:
 *   /ferrox-(<cmd1>|<cmd2>|...)(?=[^a-zA-Z0-9_-]|$)/g  — forward
 * We check the colon form: /ferrox:<cmd> leaking in installed workflow bodies.
 *
...
function scanWorkflowColonLeak(filePath, cmdNames) {
  ...
  const pattern = new RegExp(`/ferrox:(${sorted.join('|')})(?=[^a-zA-Z0-9_-]|$)`, 'g');
...
function runSmoke({
  ...
  // --- Locate the installed ferrox-tools binary --------------------------------
  const actualBin = findFerroxToolsBin(installPrefix);

  if (!actualBin) {
    const searched = binCandidates(installPrefix, 'ferrox-tools');
    return {
  ...
  // --- Invoke `ferrox-tools --help` to assert the shipped binary is callable ---
  ...
  if (shouldRunInit && fixtureDir) {
    ...
    if (!installerBin) {
      return {
        ...
        details: {
          ...details,
          reason: 'ferrox-core binary not found in installPrefix',
          installPrefix,
        },
      };
    }
    ...
    // FERROX_TEST_MODE must be cleared — install.js skips its main() block when
    // FERROX_TEST_MODE is set, which would cause the installer to exit 0 silently
    // without actually creating any files.
    const initEnv = { ...process.env };
    delete initEnv.FERROX_TEST_MODE;
    ...
    const expectedDirs = [
      ...
      path.join(fixtureDir, '.claude', 'ferrox-core'),
    ];
  ...
  for (const cmd of commandsToCheck) {
    const cmdFilePath = path.join(pkg, 'commands', 'ferrox', `${cmd}.md`);
    ...
    if (workflowName) {
      // Workflow files live at ferrox-core/workflows/<name> in the package.
      // Some live in subdirectories; try flat first then scan once.
      const flat = path.join(pkg, 'ferrox-core', 'workflows', workflowName);
  ...
  const workflowsDir = path.join(pkg, 'ferrox-core', 'workflows');
...
function cliMain() {
  ...
  const packDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-smoke-pack-'));
  const installPrefix = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-smoke-prefix-'));
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-smoke-fixture-'));
