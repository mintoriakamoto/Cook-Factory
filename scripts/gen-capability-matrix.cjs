 ...
 * registry (ferrox-core/bin/lib/capability-registry.cjs), so the matrix can never
 ...
 * exact `version` (which tracks the Ferrox package version in lockstep and would
 * churn the committed file — and trip the drift guard — on every release). It
 * shows `engines.ferrox` (the stable host-compatibility RANGE) instead, and notes
...
const REGISTRY_PATH = path.join(ROOT, 'ferrox-core', 'bin', 'lib', 'capability-registry.cjs');
...
function fmtEngines(cap) {
  const g = cap && cap.engines && cap.engines.ferrox;
...
function renderTable(caps, role, extByCap) {
  ...
  return [
    '| id | role | tier | engines.ferrox | extension points | hook kinds | source |',
    '|---|---|---|---|---|---|---|',
    ...rows,
  ].join('\n');
}
...
| Column | Description |
|---|---|
| **id** | Canonical capability identifier; unique across first- and third-party capabilities. Reserved prefixes: \`ferrox-\`, \`ferrox-core-\`, \`anthropic-\`. |
| **role** | \`feature\` — extends what the loop does; \`runtime\` — adapts Ferrox to a specific AI runtime/IDE. |
...
> Core package (their \`capability.json\` \`version\` always equals the Ferrox release
...
> signal — \`engines.ferrox\` — is shown instead. A third-party capability's exact
> version is recorded in the per-runtime ledger (\`.ferrox-capabilities.json\`) at
> install time.
...
First-party capabilities are implicitly trusted: they ship as part of the Ferrox
...
Runtime capabilities adapt Ferrox to a specific AI runtime or IDE — emitting
...
given machine is \`ferrox capability list\` (see the
[\`ferrox capability\` command reference](ferrox-capability-command.md)), which reports
...
| Column | Value |
|---|---|
| **id** | As declared in \`capability.json\`. Must not use reserved prefixes (\`ferrox-\`, \`ferrox-core-\`, \`anthropic-\`). |
| **role** | \`feature\` or \`runtime\`, as declared. |
| **tier** | \`core\`, \`standard\`, or \`full\`, as declared. |
| **engines.ferrox** | Range from \`capability.json\`; verified at install and at each load. |
...
Whether Ferrox operates or advertises a central community registry of third-party
...
| \`engines.ferrox\` | Recommended | semver range | Host-version compatibility gate. Enforced at install and load. |
| \`compatVersions\` | No | object: cap-version → ferrox-range | Graceful-downgrade table for sources that enumerate versions (git tags, registry, npm). |
