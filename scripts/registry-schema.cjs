 ...
 * (issue #2182):
 *
 *   - `docs/registries/capabilities.json`  → "Ferrox Community Capability Registry"
 *   - `docs/registries/eos.json`           → "Ferrox EoS Registry" (PR2)
 *
 ...
 *     fields for each entry type, including `enginesFerrox` (ADR-1244 D1
 *     "Versioned capability manifest" — the `engines.ferrox` semver-range gate,
...
// A single `engines.ferrox` range clause: optional comparison operator, optional
...
// would never match.
const FERROX_RANGE_CLAUSE_RE = /^(>=|<=|>|<|=|\^|~)?v?\d+\.\d+\.\d+(-[0-9A-Za-z.-]+)?(\+[0-9A-Za-z.-]+)?$/;

/**
 * Validate the SHAPE of an `engines.ferrox`-style semver range string (ADR-1244
 ...
 * @returns {boolean}
 */
function isValidFerroxRange(range) {
  if (typeof range !== 'string') return false;
  const trimmed = range.trim();
  if (trimmed === '') return false;
  if (trimmed === '*') return true;
  const clauses = trimmed.split(/\s+/);
  return clauses.length > 0 && clauses.every((clause) => clause !== '' && FERROX_RANGE_CLAUSE_RE.test(clause));
}
...
function validateEntries(entries, opts) {
  ...
  entries.forEach((entry, index) => {
    ...
    if (!missing.has('enginesFerrox') && !isValidFerroxRange(entry.enginesFerrox)) {
      addError('enginesFerrox', 'enginesFerrox must be a valid semver range');
    }
    checkMaxLength('enginesFerrox', 100);
...
function renderMarkdown(entries, opts) {
  ...
  lines.push('');
  lines.push(isEos ? '# Ferrox EoS Registry' : '# Ferrox Community Capability Registry');
  lines.push('');
  lines.push(
    "> **Not an endorsement.** Inclusion means only that a maintainer merged a PR linking the author's repository — Ferrox has not reviewed, tested, or verified any listing. See the [registry README](./README.md).",
  );
  lines.push('');
  ...
  lines.push('| Name | What it is | Latest release | Ferrox compat | Discussion |');
  lines.push('|---|---|---|---|---|');
  for (const entry of sorted) {
    // entry.repo/enginesFerrox/discussion are regex-constrained (validateEntries)
    ...
    lines.push(
      `| [${mdInline(entry.name)}](https://github.com/${entry.repo}) | ${mdInline(entry.description)} | ` +
        ...
        `\`${entry.enginesFerrox}\` | [discuss](${entry.discussion}) |`,
    );
  }
  lines.push('');
  ...
  sorted.forEach((entry, i) => {
    ...
    if (isEos) {
      ...
      lines.push(`- **Every interaction with Ferrox:** ${mdInline(summary)}`);
    } else {
      let summary =
    ...
    lines.push(
      isEos
        ? `- **Ferrox compatibility:** \`${entry.enginesFerrox}\`, protocol v${entry.protocolVersion}`
        : `- **Ferrox compatibility:** \`${entry.enginesFerrox}\``,
    );
