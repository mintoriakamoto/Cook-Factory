...
// Ferrox fork: vscode/package.json is likewise no longer a registered manifest.
// VS Code is a reference-only host (capabilities/vscode/capability.json declares
// installSurface: none, hooksSurface: none, configHome.kind: none) with no
// shipped extension source tree; the file was never committed in this fork, so
// registering it left --check reading a nonexistent path (masked baseline RED).
