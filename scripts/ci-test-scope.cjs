...
const RULES = [
  ...
  },
  {
    ...
    // src/*.cts compiles into ferrox-core/bin/lib/*.cjs; a source-only edit must
  ...
  },
  {
    ...
    match: path => path.startsWith('bin/') ||
      path.startsWith('ferrox-core/bin/') ||
    ...
    tests: [
      ...
      // It is a 3 to 6 minute `npm pack` plus `npm install -g` integration test.
      // This fork ships NO workflows, so nothing defers it to one: it is reached
      // by a plain `npm test`, because scripts/run-tests.cjs returns every
      // discovered suite when none is named. The exclusion below stands on its
      // ORIGINAL cost reason only: running it in the scoped/targeted lane as well
  ...
  },
  {
    ...
    // fixtures merge silently (#2266: a hooks/ferrox-statusline.js edit changed
    ...
    // NOTE: intentionally NOT a blanket 'ferrox-core/' prefix, for two reasons:
    // (1) ferrox-core/bin/** is tsc-compiled runtime output — EXCLUDED_PREFIXES-
    //     excluded from both manifests, and already covered by the 'installer and
    //     package layout' rule (path.startsWith('ferrox-core/bin/')) — so matching it
    //     here would be pure noise; and
    // (2) enumerating only the installer-shipped content subtrees preserves the
    //     bug-408 unit-fallback contract: a ferrox-core/ path that is NOT shipped
    //     verbatim (the bug-408 test uses ferrox-core/src/some-util.js) must still
    //     fall back to ['unit'] when no rule matches.
    // Listed: the four ferrox-core content subtrees the installer ships verbatim
    // (contexts, references, templates, workflows) + bin/shared/*.json data files.
    // Verify against Object.keys(golden fixture) grouped by ferrox-core/<subdir>.
    match: path =>
      ['hooks/', 'commands/', 'agents/', 'skills/', 'ferrox-core/workflows/', 'ferrox-core/templates/', 'ferrox-core/references/', 'ferrox-core/contexts/', 'scripts/changeset/', 'scripts/lib/'].some(p => path.startsWith(p)) ||
      (path.startsWith('ferrox-core/bin/shared/') && path.endsWith('.json')) ||
  ...
  },
  {
    ...
    match: path => path.startsWith('ferrox-core/workflows/'),
    tests: [
...
function classify(files) {
  ...
  for (const file of files) {
    ...
    if (
      ['bin/', 'src/', 'ferrox-core/', 'agents/', 'commands/', 'hooks/', 'skills/', 'tests/', 'scripts/', 'eslint-rules/'].some(p => file.startsWith(p)) ||
  ...
  // Heavy integration tests must never run in the scoped/targeted lane: they
  // carry a multi-minute cost that overruns the per-chunk timeout (worst on
  // Windows) when a broad PR bundles them with many other changed test files.
  // Dropping one here does NOT drop it from the gate. This fork ships no
  // workflows, and a plain `npm test` names no suite, so scripts/run-tests.cjs
  // selects every discovered suite and runs each entry below in full.
  // Drop them however they entered (matched rule OR changed-file).
  const SCOPED_LANE_EXCLUDE = new Set([
    // reached by a plain `npm test`; excluded here for its per-chunk cost only
