 ...
 *    For each commands/ferrox/*.md, parse requires: and walk the body for
 *    references to other Ferrox skills (pattern: /ferrox:<stem> or ferrox:<stem>).
 ...
 *
 * Usage:
 *   node scripts/lint-skill-deps.cjs               # scans commands/ferrox/
...
const PROFILES_MODULE = path.join(__dirname, '..', 'ferrox-core', 'bin', 'lib', 'install-profiles.cjs');
...
let commandsDir = path.join(__dirname, '..', 'commands', 'ferrox');
    ...
    for (const ref of referenced) {
      ...
      if (!allStems.has(ref)) {
        violations.push({
          ...
          filePath,
          undeclared: ref,
          message: 'body references unknown skill ferrox:' + ref,
        });
        continue;
      }
      if (!declaredSet.has(ref)) {
        violations.push({
          ...
          filePath,
          undeclared: ref,
          message: 'body references ferrox:' + ref + ' but requires: does not list it',
        });
      }
    }
  }
