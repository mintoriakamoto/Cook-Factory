 ...
 *
 * Idempotent transform: for every ferrox-core/workflows/*.md (and subdirs)
 * AND every agents/*.md, rewrite all bash/sh/shell fenced blocks to:
 *   1. Strip ALL old resolver forms from every bash block (FERROX_TOOLS=,
 *      FERROX_SDK=, the if/elif/else/fi resolver, _FERROX_SHIM_NAME=, and any
 *      previously-inserted ferrox_run preamble).
 *   2. Replace $FERROX_SDK tokens with ferrox_run (idempotent).
 *   3. Insert the canonical preamble at the TOP of ONLY the FIRST bash block
 *      (document order) that contains a ferrox_run call. All other bash blocks
 *      keep their ferrox_run calls with NO preamble. (Define once per file,
...
const WORKFLOWS_DIR = path.join(__dirname, '..', 'ferrox-core', 'workflows');
 ...
 * boilerplate (old and new preamble forms) and replace $FERROX_SDK tokens.
...
function stripAndReplace(lines, preamble) {
  ...
  // Step 2: Replace $FERROX_SDK tokens with ferrox_run
  result = result.map((line) => replaceFerroxSdk(line));
 ...
 *      FERROX_TOOLS="...ferrox-tools.cjs"   (or ...${_FERROX_SHIM_NAME})
 *      if [ -f "$FERROX_TOOLS" ]; then
 *        ...
 *      fi
 *
 * B) One-liner form:
 *      FERROX_TOOLS=...; if [ -f "$FERROX_TOOLS" ]; then FERROX_SDK=...; elif ...; else ...; fi
 *
 * C) _FERROX_SHIM_NAME= line
 *
 * D) Bare FERROX_SDK= line
 *
 * E) The canonical preamble comment line (so we strip old inserted preambles too)
 *
 * F) The ferrox_run() function definitions that are part of the preamble
...
function removeResolverLines(lines, preamble) {
  ...
  while (i < lines.length) {
    ...
    // B) One-liner form: FERROX_TOOLS=... ; if ... FERROX_SDK=... fi
    if (
      /^FERROX_TOOLS=.*ferrox-tools\.cjs.*;\s*if\s+\[/.test(trimmed) ||
      /^FERROX_TOOLS=.*\/\$\{_FERROX_SHIM_NAME\}.*;\s*if\s+\[/.test(trimmed)
    ) {
    ...
    // C) _FERROX_SHIM_NAME= line
    if (/^_FERROX_SHIM_NAME=/.test(trimmed)) {
    ...
    // A) Multi-line form: starts with FERROX_TOOLS=...ferrox-tools.cjs or FERROX_TOOLS=...${_FERROX_SHIM_NAME}
    if (
      /^FERROX_TOOLS=.*ferrox-tools\.cjs"$/.test(trimmed) ||
      /^FERROX_TOOLS=.*\/\$\{_FERROX_SHIM_NAME\}"$/.test(trimmed)
    ) {
      ...
      if (/^if\s+\[\s+-f\s+"\$FERROX_TOOLS"\s*\]/.test(ahead)) {
        // Consume until matching `fi`
        i += 2; // skip FERROX_TOOLS= and `if [...]`
        let depth = 1;
        ...
        // Just skip the FERROX_TOOLS= line
        i++;
        continue;
      }
    }

    // D) Bare FERROX_SDK= line (not inside an if block, defensive)
    if (/^FERROX_SDK=/.test(trimmed) && !/ferrox_run/.test(trimmed)) {
      i++;
      continue;
    }

    // E+F) Strip lines that are part of the canonical preamble (to remove previously-inserted preambles).
    // We match the canonical preamble comment, _FERROX_SHIM_NAME (handled above at C),
    // and ferrox_run() function definition lines.
...
function isSdkComment(line) {
  ...
  return /^\s*#\s*SDK resolution/.test(line) || /^\s*#.*prefer local.*ferrox-tools/.test(line);
}
...
function isCanonicalPreambleComment(line) {
  return /^\s*#\s*Runtime launcher:.*prefer local ferrox-tools\.cjs.*installed ferrox-tools on PATH/.test(line);
}

/**
 * Replace all $FERROX_SDK tokens with ferrox_run in a line.
 * Handles: $FERROX_SDK, ${FERROX_SDK} forms.
 */
function replaceFerroxSdk(line) {
  return line.replace(/\$\{?FERROX_SDK\}?(?=\s|$|;|"|'|\))/g, 'ferrox_run');
}
 ...
 * 2. Find the FIRST block (document order) that has a ferrox_run call.
...
function transformFile(content, preamble) {
  ...
  let changed = false;
  let firstFerroxRunBlockIdx = -1; // index into shellBlockRanges of the first ferrox_run block (after strip)

  // First: strip + replace in all blocks, find first ferrox_run block
  ...
  // Find the first block that has a ferrox_run call (after stripping)
  for (let bi = 0; bi < strippedBlocks.length; bi++) {
    if (strippedBlocks[bi].some((l) => /\bferrox_run\b/.test(l))) {
      firstFerroxRunBlockIdx = bi;
      break;
    }
  }

  // Insert preamble into the first ferrox_run block only — UNLESS this file
  ...
  const finalBlocks = strippedBlocks.map((stripped, bi) => {
    if (!delegates && bi === firstFerroxRunBlockIdx) {
...
/**
 * A file "delegates to the shared resolver" when it pulls the canonical ferrox_run
 * preamble in from ferrox-core/references/ferrox-run-resolver.md via an @-include
...
function delegatesToResolverReference(content) {
  return content.includes('references/ferrox-run-resolver.md');
}
