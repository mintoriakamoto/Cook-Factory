...
/**
 * gen-loop-host-contract.cjs — generates ferrox-core/bin/lib/loop-host-contract.cjs
 * from the <!-- ferrox:loop-host ... --> blocks in the five step workflows.
 *
 * Usage:
...
const WORKFLOWS_DIR = path.join(ROOT, 'ferrox-core', 'workflows');
const CONTRACT_PATH = path.join(ROOT, 'ferrox-core', 'bin', 'lib', 'loop-host-contract.cjs');
...
// the workflow file (e.g. ferrox-planner, ferrox-executor, ferrox-verifier, etc.).
const ROLE_TO_AGENT = {
  researcher: 'ferrox-phase-researcher',
  planner:    'ferrox-planner',
  checker:    'ferrox-plan-checker',
  executor:   'ferrox-executor',
  verifier:   'ferrox-verifier',
};

// ─── Parser ───────────────────────────────────────────────────────────────────

/**
 * Parse a single <!-- ferrox:loop-host ... --> block from file content.
...
function parseLoopHostBlock(content, fileName) {
  ...
  const blockRe = /<!--\s*ferrox:loop-host\s*([\s\S]*?)-->/g;
  const allMatches = Array.from(content.matchAll(blockRe));
  if (allMatches.length === 0) {
    throw new Error(fileName + ': missing <!-- ferrox:loop-host ... --> block');
  }
  if (allMatches.length > 1) {
    throw new Error(
      fileName + ': expected exactly one ferrox:loop-host marker block, found ' + allMatches.length,
    );
  }
  ...
  for (const key of RECOGNIZED_KEYS) {
    if (keyCounts[key] > 1) {
      throw new Error(fileName + ': duplicate key \'' + key + '\' in ferrox:loop-host marker');
    }
  }
  ...
  function parseField(key) {
    ...
    throw new Error(fileName + ': ferrox:loop-host block missing required field "' + key + '"');
  }
  ...
  function parseScalar(key) {
    ...
    for (const line of lines) {
      ...
      if (trimmed === key + ':' || trimmed.startsWith(key + ': ') || trimmed.startsWith(key + ':')) {
        ...
        if (val === '') {
          throw new Error(fileName + ': ferrox:loop-host block field "' + key + '" must be a non-empty string');
        }
        return val;
      }
    }
  ...
  if (points.length === 0) {
    throw new Error(fileName + ': ferrox:loop-host block "points" must have at least one value');
  }
  if (agentRoles.length === 0) {
    throw new Error(fileName + ': ferrox:loop-host block "agent-roles" must have at least one value');
  }
...
function crossCheckRoles(content, agentRoles, fileName) {
  ...
  for (const role of agentRoles) {
    ...
    // FIX 3: Use word-boundary match so "ferrox-plan-checker-v2" does NOT satisfy a required
    // "ferrox-plan-checker". Treat '-' as part of the token: boundary = start/end of string or
...
function buildContract(workflowsDir) {
  ...
  for (const { file, step } of STEP_WORKFLOWS) {
    ...
    if (entry.step !== step) {
      allErrors.push(
        file + ': ferrox:loop-host block declares step "' + entry.step +
        '" but expected "' + step + '"',
      );
    }
  ...
  if (flag === '--check') {
    ...
    if (!fs.existsSync(CONTRACT_PATH)) {
      process.stderr.write(
        'ferrox-core/bin/lib/loop-host-contract.cjs does not exist. Run:\n' +
    ...
    if (normalizeLineEndings(committed) !== normalizeLineEndings(live)) {
      process.stderr.write(
        'ferrox-core/bin/lib/loop-host-contract.cjs is stale. Run:\n' +
        '  node scripts/gen-loop-host-contract.cjs --write\n',
      );
      throw new ExitError(1);
    }

    process.stdout.write('ferrox-core/bin/lib/loop-host-contract.cjs is up to date.\n');
  } else if (flag === '--write') {
    let contract;
    try {
      contract = buildContract();
 ...
 */
const HOST_LOOP_FILES = STEP_WORKFLOWS.map((w) => 'ferrox-core/workflows/' + w.file);
