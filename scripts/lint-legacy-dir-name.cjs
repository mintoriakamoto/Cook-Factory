 ...
 * `get-shit-done` now that the package has been renamed to `ferrox-core` (#604).
 *
 ...
 *   - Lines containing the marker `ferrox-allow-legacy-name` (intentional uses)
...
const ALLOW_MARKER = 'ferrox-allow-legacy-name';
const SELF_PATH = path.resolve(__filename);
// FERROX_LINT_LEGACY_REPO_ROOT is used by tests to redirect the guard to a
// temporary fixture git repo without touching the real working tree.
const REPO_ROOT = process.env.FERROX_LINT_LEGACY_REPO_ROOT
  ? path.resolve(process.env.FERROX_LINT_LEGACY_REPO_ROOT)
...
function main() {
  ...
  process.stderr.write('\n');
  process.stderr.write('Fix: rename to ferrox-core, or add `ferrox-allow-legacy-name` marker on the line if the\n');
