 ...
 * `ferrox-core/bin/lib/markdown-table.cjs`) is the single-source registry naming
 * every canonical Ferrox pipe-table's column-header shape. Each registered
...
const SCHEMA_SOURCE_FILES = {
  RoadmapProgress: path.join('ferrox-core', 'templates', 'roadmap.md'),
  RequirementsTraceability: path.join('ferrox-core', 'templates', 'requirements.md'),
  QuickTasks: path.join('ferrox-core', 'workflows', 'quick.md'),
  Security: path.join('ferrox-core', 'templates', 'SECURITY.md'),
};
...
function scanRepo(root) {
  const seamPath = path.join(root, 'ferrox-core', 'bin', 'lib', 'markdown-table.cjs');
  let seam;
  try {
