'use strict';
/**
 * One-shot script + library: bidirectional Ferrox slash-command namespace normalizer.
 *
 * - Default direction (transformContent): retired /ferrox-<cmd> → /ferrox:<cmd>
 *   (keeps monorepo sources, docs, and workflows in the active colon form).
 * - Reverse direction (transformContentToHyphen): /ferrox:<cmd> / ferrox:<cmd> → ferrox-<cmd>
 *   (used during skill installation for runtimes that register skills under the
 *   canonical hyphen form established in #2808).
 *
 * Both directions only rewrite known commands from `commands/ferrox/*.md` (longest-first
 * matching + word-boundary safety). Non-commands (ferrox-sdk, ferrox-tools, etc.) are
...
const COMMANDS_DIR = path.join(__dirname, '..', 'commands', 'ferrox');
const SEARCH_DIRS = [
  path.join(__dirname, '..', 'ferrox-core', 'bin', 'lib'),
  path.join(__dirname, '..', 'ferrox-core', 'workflows'),
  path.join(__dirname, '..', 'ferrox-core', 'references'),
  path.join(__dirname, '..', 'ferrox-core', 'templates'),
  path.join(__dirname, '..', 'ferrox-core', 'contexts'),
  path.join(__dirname, '..', 'commands', 'ferrox'),
...
function buildPattern(cmdNames) {
  // Empty input would compile `/ferrox-()(?=[^a-zA-Z0-9_-]|$)/g`, which the regex
  // engine still matches at any `/ferrox-` token followed by a non-word boundary
  // (e.g. EOL, whitespace, punctuation) — rewriting it to a stray `/ferrox:`.
  ...
  return new RegExp(`/ferrox-(${sorted.join('|')})(?=[^a-zA-Z0-9_-]|$)`, 'g');
}

/**
 * Pure transform: rewrite retired `/ferrox-<cmd>` to `/ferrox:<cmd>` for the given command names.
 * Returns the rewritten string. Identifiers not in `cmdNames` (e.g. `/ferrox-sdk`,
 * `/ferrox-tools`) are left untouched.
 */
function transformContent(src, cmdNames) {
  const pattern = buildPattern(cmdNames);
  if (!pattern) return src;
  return src.replace(pattern, (_, cmd) => `/ferrox:${cmd}`);
}

/**
 * Build regex for the reverse direction (colon form → hyphen form).
 * Matches both "ferrox:cmd" and "/ferrox:cmd" (the leading / is preserved automatically
...
function buildColonPattern(cmdNames) {
  ...
  return new RegExp(`(?<![a-zA-Z0-9_-])ferrox:(${sorted.join('|')})(?=[^a-zA-Z0-9_-]|$)`, 'g');
}

/**
 * Pure transform (reverse): rewrite `/ferrox:<cmd>` / `ferrox:<cmd>` to hyphen form
 * for known Ferrox commands.
 *
 * Non-command identifiers (e.g. ferrox-sdk, ferrox-tools) are left untouched, matching
...
function readCmdNames() {
  try {
    ...
    // global Claude installs (no local commands/ferrox/ directory). Return [] so
