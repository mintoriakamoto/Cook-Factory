 ...
 *
 * The seam (`get-shit-done/bin/lib/package-identity.cjs`, derived from // ferrox-allow-legacy-name
 * package.json) is the single source of Ferrox's published coordinates. Many
 ...
 * strings.
 *
 * This lint makes those literals *value-checked*: every Ferrox package/repo
 ...
 *
 * Scope: the runtime/code surface (bin/, hooks/, scripts/, get-shit-done/). // ferrox-allow-legacy-name
...
// A Ferrox package coordinate: a scoped npm name whose package part contains
// "get-shit-done" (so @ferroxfactory/ferrox-sdk and unrelated scopes never match). // ferrox-allow-legacy-name
const PACKAGE_RE = /@[A-Za-z0-9._-]+\/[A-Za-z0-9._-]*get-shit-done[A-Za-z0-9._-]*/g; // ferrox-allow-legacy-name
// A Ferrox repo slug, only inside a GitHub URL context so it never overlaps the
// scoped package literal above. The `.git` suffix is trimmed before compare.
const SLUG_RE = /(?:github\.com[/:]|raw\.githubusercontent\.com\/)([A-Za-z0-9._-]+\/[A-Za-z0-9._-]*get-shit-done[A-Za-z0-9._-]*)/g; // ferrox-allow-legacy-name
...
/**
 * Pure: find every Ferrox coordinate literal in `text` that does not match the
...
const SCAN_DIRS = ['bin', 'hooks', 'scripts', 'ferrox-core'];
...
const EXEMPT = new Set([
  path.join('ferrox-core', 'bin', 'lib', 'package-identity.cjs'),
...
function scanRepo(root) {
  const seam = require(path.join(root, 'ferrox-core', 'bin', 'lib', 'package-identity.cjs'));
...
function main() {
  ...
  if (violations.length === 0) {
    process.stdout.write('ok identity-drift: all Ferrox coordinate literals match the seam\n');
    return;
  }
  process.stderr.write('identity-drift: stale Ferrox coordinate literal(s) found.\n');
