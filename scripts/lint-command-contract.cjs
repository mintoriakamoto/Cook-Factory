 ...
 *
 * Enforces the commands/ferrox/*.md contract across all 65 command files:
 *
 *   1. name:        present, non-empty, matches ferrox: or ferrox- prefix
...
const FERROX_ROOT      = path.join(ROOT, 'ferrox-core');
...
function check(filePath) {
  ...
  // 1. name: present + ferrox: / ferrox- prefix
  if (!fm.name || !fm.name.trim()) {
    violations.push('name: field missing or empty');
  } else if (!/^ferrox[:-]/.test(fm.name.trim())) {
    violations.push(`name: must start with "ferrox:" or "ferrox-", got "${fm.name.trim()}"`);
  }
  ...
  for (const { token, normalized, trailingProse } of refs) {
    const absPath = path.join(FERROX_ROOT, normalized);
