...
/**
 * Single source for Ferrox's published-package coordinates (issue #498).
 *
 ...
 * `ferrox-core/bin/lib/package-identity.cjs` bakes those values at build
...
function deriveIdentity(pkg = {}) {
  ...
  const updateCacheFileName = cacheSlug ? `ferrox-update-check-${cacheSlug}.json` : 'ferrox-update-check.json';
...
function main() {
  ...
  const out = path.join(__dirname, '..', 'ferrox-core', 'bin', 'lib', 'package-identity.cjs');
