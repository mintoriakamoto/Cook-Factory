 ...
 * parity test (which the ferrox-test gate runs) has a committed artifact to
 * compare against. The authoritative test gate remains `ferrox-test run`, never
 * a local `node --test`.
 *
...
// SHARED ferrox-core payload file (e.g. model-catalog.json, capability-registry)
...
// This layout is distinct from the global install (commands/ferrox-*.md +
// agents/ferrox-*.md) and has its own parity assertion in the test harness.
