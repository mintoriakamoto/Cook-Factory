...
} = require('../ferrox-core/bin/lib/shell-command-projection.cjs');

// Bidirectional Ferrox slash-command namespace transformer (#3583).
...
} = require('../ferrox-core/bin/lib/command-roster.cjs');
const {
  resolveAntigravityGlobalDir,
  getGlobalConfigDir,
  getGlobalSkillsBase,
  resolveKimiHooksTomlDir,
} = require('../ferrox-core/bin/lib/runtime-homes.cjs');
...
const { getDirName, getRuntimeLabel, getGlobalConfigHomeFragment, runtimeFlags, getRuntimeNewProjectCommand } = require('../ferrox-core/bin/lib/runtime-name-policy.cjs');
const {
  applyWorktreeBaseRef,
  readBaseRefFromSettings,
} = require('../ferrox-core/bin/lib/worktree-base-ref.cjs');
const { resolveInstallPlan } = require('../ferrox-core/bin/lib/runtime-config-adapter-registry.cjs');
const { createImperativeAdapter } = require('../ferrox-core/bin/lib/adapter-imperative.cjs');
const runtimeArtifactConversion = require('../ferrox-core/bin/lib/runtime-artifact-conversion.cjs');
...
const hooksSurface = require('../ferrox-core/bin/lib/runtime-hooks-surface.cjs');

/**
 * #3677 predicate — true when an agent body needs `/ferrox:<cmd>` → `/ferrox-<cmd>`
...
const FERROX_CODEX_MARKER = '# Ferrox Agent Configuration \u2014 managed by ferrox-core installer';
const FERROX_CODEX_HOOKS_OWNERSHIP_PREFIX = '# Ferrox codex_hooks ownership: ';
...
// `[agents.<name>]` sub-tables, a separate `AgentRoleToml`). Ferrox writes only
...
// Ferrox's managed dispatch-depth value. Codex's implicit default is also 1 (root
...
// (ADR-1239 upgrade 2 / #2088). Per the negotiated capability, Ferrox-hosted Codex
// dispatch is single-level (maxDepth === 1 \u2192 `degradationFor` flattens waves).
const FERROX_CODEX_AGENTS_MAX_DEPTH = 1;
// Codex hooks.json lifecycle events Ferrox registers beyond SessionStart (which has
...
// codex-hooks-json). All route through ferrox-context-monitor.js. #772 wired the
// first three; #2088 adds the remaining six documented events so Ferrox's monitor
...
const FERROX_CLAUDE_DENY_PERMISSIONS = Object.freeze([
  'Read(.env)',
  'Read(.env.*)',
  'Read(.secrets)',
]);
...
/**
 * Merge Ferrox-owned permission entries into a Claude Code settings object.
 *
 * Additive and idempotent: existing allow/deny entries are preserved; Ferrox
...
function mergeClaudePermissions(settings) {
  ...
  for (const entry of FERROX_CLAUDE_ALLOW_PERMISSIONS) {
    if (!settings.permissions.allow.includes(entry)) {
      settings.permissions.allow.push(entry);
    }
  }
  for (const entry of FERROX_CLAUDE_DENY_PERMISSIONS) {
...
const FERROX_COPILOT_INSTRUCTIONS_MARKER = '<!-- Ferrox Configuration \u2014 managed by ferrox-core installer -->';
const FERROX_COPILOT_INSTRUCTIONS_CLOSE_MARKER = '<!-- /Ferrox Configuration -->';
...
// hook runs an INLINE shell command (bash / powershell), so the Ferrox hook is fully
...
const FERROX_COPILOT_HOOK_FILE = 'ferrox-session.json';
...
const FERROX_COPILOT_SESSION_MSG_PRESENT =
  'Ferrox: .planning/STATE.md present - review the current phase and any blockers before acting.';
const FERROX_COPILOT_SESSION_MSG_ABSENT =
  'Ferrox: no .planning/ workflow found - run /ferrox-new-project to start a tracked workflow.';
const FERROX_COPILOT_SESSION_HOOK_BASH =
  'if [ -f .planning/STATE.md ]; then ' +
  `printf '%s' '{"additionalContext":"${FERROX_COPILOT_SESSION_MSG_PRESENT}"}'; else ` +
  `printf '%s' '{"additionalContext":"${FERROX_COPILOT_SESSION_MSG_ABSENT}"}'; fi`;
const FERROX_COPILOT_SESSION_HOOK_PWSH =
  'if (Test-Path .planning/STATE.md) ' +
  `{ '{"additionalContext":"${FERROX_COPILOT_SESSION_MSG_PRESENT}"}' } ` +
  `else { '{"additionalContext":"${FERROX_COPILOT_SESSION_MSG_ABSENT}"}' }`;
...
// A `command` hook entry runs an external script. Ferrox registers six managed hooks
// (AC4a upgrade, #2089 — ADR-1239):
//   sessionStart   → ferrox-cursor-session-start.js   (context injection)
//   postToolUse    → ferrox-cursor-post-tool.js        (STATE.md update monitor)
//   preToolUse     → ferrox-cursor-pre-tool.js         (write-path guard)
//   stop           → ferrox-cursor-stop.js             (verify-work reminder)
//   subagentStart  → ferrox-cursor-subagent-start.js   (subagent context injection)
//   subagentStop   → ferrox-cursor-subagent-stop.js    (subagent completion reminder)
// Cursor docs: https://cursor.com/docs/hooks
const FERROX_CURSOR_SESSION_HOOK_SCRIPT = 'ferrox-cursor-session-start.js';
const FERROX_CURSOR_POST_TOOL_HOOK_SCRIPT = 'ferrox-cursor-post-tool.js';
const FERROX_CURSOR_PRE_TOOL_HOOK_SCRIPT = 'ferrox-cursor-pre-tool.js';
const FERROX_CURSOR_STOP_HOOK_SCRIPT = 'ferrox-cursor-stop.js';
const FERROX_CURSOR_SUBAGENT_START_HOOK_SCRIPT = 'ferrox-cursor-subagent-start.js';
const FERROX_CURSOR_SUBAGENT_STOP_HOOK_SCRIPT = 'ferrox-cursor-subagent-stop.js';
// All Ferrox-managed Cursor hook scripts (used by uninstall cleanup).
const FERROX_CURSOR_HOOK_SCRIPTS = [
  FERROX_CURSOR_SESSION_HOOK_SCRIPT,
  FERROX_CURSOR_POST_TOOL_HOOK_SCRIPT,
  FERROX_CURSOR_PRE_TOOL_HOOK_SCRIPT,
  FERROX_CURSOR_STOP_HOOK_SCRIPT,
  FERROX_CURSOR_SUBAGENT_START_HOOK_SCRIPT,
  FERROX_CURSOR_SUBAGENT_STOP_HOOK_SCRIPT,
];
// Marker comment embedded in managed hook entries so Ferrox can find+remove them.
const FERROX_CURSOR_HOOK_MARKER = 'ferrox-managed';
...
// field), unlike Cursor's hooks.json. Ferrox registers two managed BLOCKING
// hooks (exit code 2 to block, vs. Cursor's stdout-JSON form):
//   pre_write_code   → ferrox-windsurf-pre-write.js    (write-path guard)
//   pre_run_command  → ferrox-windsurf-pre-command.js  (destructive-command guard)
// Cascade has no context-injection channel, so the 4 advisory hooks Ferrox
...
const FERROX_WINDSURF_PRE_WRITE_HOOK_SCRIPT = 'ferrox-windsurf-pre-write.js';
const FERROX_WINDSURF_PRE_COMMAND_HOOK_SCRIPT = 'ferrox-windsurf-pre-command.js';
// All Ferrox-managed Windsurf hook scripts (used by uninstall cleanup).
const FERROX_WINDSURF_HOOK_SCRIPTS = [
  FERROX_WINDSURF_PRE_WRITE_HOOK_SCRIPT,
  FERROX_WINDSURF_PRE_COMMAND_HOOK_SCRIPT,
];

// Ferrox-managed files under hooks/lib/ (helpers required by ferrox-*.sh hooks).
...
const CODEX_AGENT_SANDBOX = {
  ...
  'ferrox-phase-researcher': 'workspace-write',
  'ferrox-project-researcher': 'workspace-write',
  'ferrox-research-synthesizer': 'workspace-write',
  'ferrox-verifier': 'workspace-write',
  'ferrox-codebase-mapper': 'workspace-write',
  'ferrox-roadmapper': 'workspace-write',
  'ferrox-debugger': 'workspace-write',
  'ferrox-plan-checker': 'read-only',
  'ferrox-integration-checker': 'read-only',
};
...
// Hoisted to top with absolute __dirname-based paths so `ferrox install codex` works
// when invoked via npm global install (cwd is the user's project, not the ferrox repo
// root). Inline `require('../ferrox-core/...')` from inside install functions
...
const _ferroxLibDir = path.join(__dirname, '..', 'ferrox-core', 'bin', 'lib');
const { MODEL_PROFILES: FERROX_MODEL_PROFILES } = require(path.join(_ferroxLibDir, 'model-profiles.cjs'));
const {
  RUNTIME_PROFILE_MAP: FERROX_RUNTIME_PROFILE_MAP,
} = require(path.join(_ferroxLibDir, 'model-catalog.cjs'));
const {
  resolveTierEntry: ferroxResolveTierEntry,
} = require(path.join(_ferroxLibDir, 'model-resolver.cjs'));

// #2071 — install-time effort resolution (readFerroxEffectiveEffortConfig /
// resolveInstallTimeEffort, plus their _getFerroxEffortCatalog + _readFerroxConfigFile
// helpers) was extracted into the shipped ferrox-core/bin/lib/install-effort-resolver.cjs
// so `ferrox-tools effort sync` can require it from the installed runtime instead of this
...
const {
  readFerroxEffectiveEffortConfig,
  resolveInstallTimeEffort,
  _getFerroxEffortCatalog,
  _readFerroxConfigFile,
} = require(path.join(_ferroxLibDir, 'install-effort-resolver.cjs'));
...
} = require(path.join(_ferroxLibDir, 'install-profiles.cjs'));
// ADR-857 phase 4c: load capability registry (optional; missing → falls back to undefined)
let _capabilityRegistry;
try {
  _capabilityRegistry = require(path.join(_ferroxLibDir, 'capability-registry.cjs'));
} catch (_) {
...
const FALLBACK_HOST_BEHAVIORS = Object.freeze({
  claude: Object.freeze({
    ...
    sourceMarkerFile: '.ferrox-source',
    hyphenNameAgentBody: true,
    legacyCommandsFerroxInstallMigration: true,
    legacyCommandsFerroxUninstall: 'global',
  }),
...
} = require(path.join(_ferroxLibDir, 'installer-migrations.cjs'));
const {
  assertInstallerMigrationsUnblocked,
  resolveInstallerMigrationPromptsForNonTty,
  summarizeInstallerMigrationResult,
} = require(path.join(_ferroxLibDir, 'installer-migration-report.cjs'));
const {
  resolveRuntimeArtifactLayout,
} = require(path.join(_ferroxLibDir, 'runtime-artifact-layout.cjs'));
const {
  assertDestWithinConfigHome,
  createRuntimeArtifactInstallPlan,
  createRuntimeArtifactUninstallPlan,
} = require(path.join(_ferroxLibDir, 'runtime-artifact-install-plan.cjs'));
const {
  planLegacyCleanup,
  applyLegacyCleanup,
} = require(path.join(__dirname, '..', 'ferrox-core', 'bin', 'lib', 'legacy-cleanup.cjs'));
const {
  updateCacheFileName,
  PACKAGE_NAME,
} = require(path.join(__dirname, '..', 'ferrox-core', 'bin', 'lib', 'package-identity.cjs'));
...
const installEngine = require(path.join(_ferroxLibDir, 'install-engine.cjs'));
const {
...
const hasPortableHooks = args.includes('--portable-hooks') || process.env.FERROX_PORTABLE_HOOKS === '1';
...
// Note: when re-running as `ferrox update` the marker is read later (after
...
function selectRuntimesFromArgs(runtimeArgs) {
  ...
  // REACH-WCORE-01 / FF-B25: a `--wayland-core` install flag is intentionally NOT
  // wired yet. wayland-core is a recognized capability-registry runtime, but the
  // install ENGINE lacks a clean plan/label mapping for it: an isolated
  // `--wayland-core` install mislabels as "Claude Code", skips AGENTS.md, and runs
  // a confused Claude/Codex hybrid. Wiring getRuntimeLabel + a clean install plan
  // (and verifying against a real wcore runtime) is FF-B25 — shipping a broken
  // flag would be worse than none.
  return selected;
}
...
if (args.includes('--gemini')) {
  ...
  console.error('Ferrox now supports Antigravity CLI (the official successor). Re-run with: --antigravity');
  if (hasUninstall) {
    ...
    console.error('To remove a prior Gemini install, delete Ferrox files under your Gemini config dir');
    console.error('(e.g. ~/.gemini/commands/ferrox) and Ferrox hook entries in ~/.gemini/settings.json.');
    process.exit(1);
  }
...
const bannerArt =
  cyan +
  String.raw`___________                              ___________              __
\_   _____/_________________  _______  __\_   _____/____    _____/  |_  ___________ ___.__.
 |    __)/ __ \_  __ \_  __ \/  _ \  \/  /|    __) \__  \ _/ ___\   __\/  _ \_  __ <   |  |
 |     \\  ___/|  | \/|  | \(  <_> >    < |     \   / __ \\  \___|  | (  <_> )  | \/\___  |
 \___  / \___  >__|   |__|   \____/__/\_ \\___  /  (____  /\___  >__|  \____/|__|   / ____|
     \/      \/                         \/    \/        \/     \/                   \/     ` +
  reset;
const banner = '\n' +
  bannerArt + '\n' +
...
// applySettingsJsonHooks: mutates settings.hooks.* in place with all Ferrox-managed
...
/**
 * Build the Ferrox-managed Codex SessionStart hook block for config.toml.
 *
...
/**
 * Generic reconcile helper: ensure hooks.json contains exactly one managed Ferrox
...
/**
 * Reconcile the Ferrox-managed SessionStart hook entry in hooks.json.
 ...
 *
 *   "C:/Program Files/nodejs/node.exe" "C:/path/.codex/hooks/ferrox-check-update.js"
 *
 ...
 * ferrox-check-update hook entry, while preserving user-owned entries.
 *
 * Codex accepts hook config from hooks.json and config.toml. To avoid the
 * startup warning for mixed representations in the same layer, Ferrox now stores
...
/**
 * Ensure hooks.json contains exactly one managed Ferrox hook entry for the given
 * Codex event, wired to ferrox-context-monitor.js. Preserves user-owned entries.
 *
 * Used for the new Codex events added in #772:
 *   SubagentStart — inject context / FERROX_AGENT_NAME awareness at subagent open
 *   Stop          — post-session context headroom tracking
 *   PostToolUse   — mirror the Claude Code PostToolUse context monitor
 *
 * All three events are routed through ferrox-context-monitor.js — the same hook
 ...
 *
 * On Windows (#3426): writes a ferrox-context-monitor.cmd shim alongside the .js
 * file and uses the .cmd path as the hook command — exactly the same fix as
 * SessionStart uses for ferrox-check-update — to avoid the bash.exe POSIX-exec
...
/**
 * Remove a Ferrox-managed event entry from hooks.json. Called during uninstall.
 *
 * @param {string} targetDir
 ...
 * @param {string} hookName - Hook filename (e.g. 'ferrox-statusline.js')
...
/**
 * Read model_overrides from ~/.ferrox/defaults.json at install time.
 ...
 */
function readFerroxGlobalModelOverrides(options = {}) {
  try {
    const home = options.homedir ? options.homedir() : os.homedir();
    const defaultsPath = path.join(home, '.ferrox', 'defaults.json');
 ...
 *
 * Merges `~/.ferrox/defaults.json` (global) with per-project
 ...
 * described — a per-project override for `ferrox-codebase-mapper` in
 ...
 */
function readFerroxEffectiveModelOverrides(targetDir = null, options = {}) {
  const global = readFerroxGlobalModelOverrides(options);
    ...
    for (let depth = 0; depth < 8; depth += 1) {
      ...
      if (fs.existsSync(candidate)) {
        try {
          ...
          // Malformed config.json — fall back to global; readFerroxRuntimeProfileResolver
          // surfaces a parse warning via _readFerroxConfigFile already.
        }
        break;
      }
...
// output file so they deny only Edit,MultiEdit. ferrox-nyquist-auditor is intentionally
// excluded (it legitimately uses Write AND Edit to create/patch test files).
const READONLY_AGENT_DISALLOWED_TOOLS = {
  'ferrox-plan-checker': 'Write, Edit, MultiEdit',
  'ferrox-integration-checker': 'Write, Edit, MultiEdit',
  'ferrox-ui-checker': 'Write, Edit, MultiEdit',
  'ferrox-verifier': 'Edit, MultiEdit',
 ...
 * `~/.ferrox/defaults.json`, with per-project keys winning over global. This
 ...
 */
function readFerroxRuntimeProfileResolver(targetDir = null) {
  const homeDefaults = _readFerroxConfigFile(
    path.join(os.homedir(), '.ferrox', 'defaults.json'),
    '~/.ferrox/defaults.json'
  );
  ...
  if (targetDir) {
    ...
    for (let depth = 0; depth < 8; depth += 1) {
      const candidate = path.join(probeDir, '.planning', 'config.json');
      if (fs.existsSync(candidate)) {
        projectConfig = _readFerroxConfigFile(candidate, '.planning/config.json');
        break;
      }
      const parent = path.dirname(probeDir);
      if (parent === probeDir) break;
  ...
  // Per-project wins. Only fall back to ~/.ferrox/defaults.json when the project
  ...
  return {
    ...
    resolve(agentName) {
      const agentModels = FERROX_MODEL_PROFILES[agentName];
      if (!agentModels) return null;
      const tier = agentModels[profile] || agentModels.balanced;
      if (!tier) return null;
      return ferroxResolveTierEntry({
        runtime: merged.runtime,
        tier,
...
// Tool name mapping from Claude/Ferrox agents to Kimi CLI module paths.
...
/**
 * Convert a Claude/Ferrox tool name to a Kimi CLI module path.
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.github/');
  c = c.replace(/\.claude\//g, '.github/');
  // CONV-07: Command name conversion (all ferrox: references → ferrox-)
  c = c.replace(/ferrox:/g, 'ferrox-');
...
/**
 * Map a skill directory name (ferrox-<cmd>) to the frontmatter `name:` used
 * by Claude Code as the skill identity. Emits the hyphen form (ferrox-<cmd>)
 * so Claude Code autocomplete shows the canonical invocation form, not the
 * deprecated colon form. See #2808.
 *
 * Historical note: this previously returned `ferrox:<cmd>` (colon) because
 * workflows called Skill(skill="ferrox:<cmd>"). Those calls have been updated
 * to use hyphen form (#2808) so the colon rewrite is no longer needed.
 *
 * Codex must NOT use this helper: its adapter invokes skills as `$ferrox-<cmd>`
...
function skillFrontmatterName(skillDirName) {
  ...
  // Return the hyphen form as-is (ferrox-<cmd>) — canonical since #2808.
  return skillDirName;
}
 ...
 *
 * We assign descending priorities to Ferrox's main-loop commands so the most-used
 ...
 * Keyed by command stem (skill dir is `ferrox-<stem>`).
 */
 ...
 * Emits `name: ferrox-<cmd>` (hyphen) so Skill(skill="ferrox-<cmd>") calls and
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
  ...
  // #3583: rewrite any /ferrox:<cmd> or ferrox:<cmd> in the body to the canonical
  // hyphen form (ferrox-<cmd>) so installed SKILL.md bodies match the hyphen
  ...
  const names = cmdNames || readFerroxCommandNames();
  ...
  // Track Ferrox's package version so Hermes' skill_view() reports a stable
  ...
  // skillName is the `ferrox-<stem>` dir name. (ADR-1239 / #2086)
  if (_hostBehaviors(runtime).skillPriorityFrontmatter) {
    const stem = typeof skillName === 'string' && skillName.startsWith('ferrox-')
      ? skillName.slice(4)
      : skillName;
...
function normalizeKimiSkillName(skillName) {
  ...
  if (text.startsWith('$')) text = text.slice(1);
  text = text.replace(/^ferrox:/, 'ferrox-');
  if (!text.startsWith('ferrox-')) text = `ferrox-${text}`;
  text = text.replace(/[^a-z0-9-]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
  return text || 'ferrox-command';
}

function convertFerroxCommandReferencesToKimiSkillInvocations(content, cmdNames) {
  ...
  const colonPattern = new RegExp(`(?<![A-Za-z0-9_/:.-])/?ferrox:(${commandGroup})(?=[^A-Za-z0-9_-]|$)`, 'g');
  const hyphenPattern = new RegExp(`(?:/|\\$)ferrox-(${commandGroup})(?=[^A-Za-z0-9_-]|$)`, 'g');

  return content
    .replace(colonPattern, (_, cmd) => `/skill:ferrox-${cmd}`)
    .replace(hyphenPattern, (_, cmd) => `/skill:ferrox-${cmd}`);
}
...
function convertClaudeCommandToKimiSkill(content, skillName, _runtime = null, cmdNames = null) {
  ...
  const description = frontmatter
    ? extractFrontmatterField(frontmatter, 'description') || `Run Ferrox workflow ${kimiSkillName}.`
    : `Run Ferrox workflow ${kimiSkillName}.`;
  const normalizedBody = convertFerroxCommandReferencesToKimiSkillInvocations(
...
const KIMI_CANONICAL_FERROX_AGENT_RE = /^ferrox-[a-z0-9-]+$/;
...
function neutralizeKimiAgentPrompt(content) {
  ...
  prompt = prompt.replace(/~\/\.claude\/ferrox-core\b/g, 'Ferrox core');
  prompt = prompt.replace(/\$HOME\/\.claude\/ferrox-core\b/g, 'Ferrox core');
...
function buildKimiRootAgentYaml({ description, tools, subagents }) {
  const lines = [
    ...
    'agent:',
    '  name: ferrox',
    `  description: ${yamlQuote(toSingleLine(description || 'Run Ferrox workflows in Kimi CLI.'))}`,
    '  extend: default',
    '  system_prompt_path: ./ferrox.md',
  ];
...
function buildKimiAgentArtifacts({
  ...
  const rootDescription = rootFrontmatter
    ? extractFrontmatterField(rootFrontmatter, 'description') || 'Run Ferrox workflows in Kimi CLI.'
    : 'Run Ferrox workflows in Kimi CLI.';
  ...
  for (const source of subagentSources) {
    ...
    const name = frontmatter
      ...
      : fallbackName;
    if (!name || !KIMI_CANONICAL_FERROX_AGENT_RE.test(name)) {
      addKimiAgentDiagnostic(
        diagnostics,
        'kimi_invalid_subagent_name',
        'Subagent source does not use a canonical ferrox-* Kimi agent name.',
        name || '(missing)',
        parsed.path
      );
      continue;
  ...
  const rootTools = mapKimiAgentContractTools(parseFrontmatterTools(rootFrontmatter), diagnostics, 'ferrox');
  ...
  return {
    root: {
      name: 'ferrox',
      yamlPath: 'agents/ferrox.yaml',
      promptPath: 'agents/ferrox.md',
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.agents/');
  c = c.replace(/\.claude\//g, '.agents/');
  // Command name conversion (all ferrox: references → ferrox-)
  c = c.replace(/ferrox:/g, 'ferrox-');
  // Runtime-neutral agent name replacement (#766)
...
function convertSlashCommandsToCursorSkillMentions(content) {
  // Keep leading "/" for slash commands; only normalize ferrox: -> ferrox-.
  // This preserves rendered "next step" commands like "/ferrox-execute-phase 17".
  return content.replace(/ferrox:/gi, 'ferrox-');
}
...
function convertClaudeToCursorMarkdown(content) {
  ...
  converted = converted.replace(/\$ARGUMENTS\b/g, '{{FERROX_ARGS}}');
...
- Treat all user text after the skill mention as \`{{FERROX_ARGS}}\`.
- If no arguments are present, treat \`{{FERROX_ARGS}}\` as empty.
...
## C. Tool Usage
Use these Cursor tools when executing Ferrox workflows:
...
function convertClaudeCommandToCursorSkill(content, skillName) {
  ...
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  let description = `Run Ferrox workflow ${skillName}.`;
  if (frontmatter) {
 ...
 * becomes the command name (e.g. `ferrox-help.md` → `/ferrox-help`).
 *
...
## C. Tool Usage
Use these Windsurf tools when executing Ferrox workflows:
- \`Shell\` for running commands (terminal operations)
- \`StrReplace\` for editing existing files
- \`Read\`, \`Write\`, \`Glob\`, \`Grep\`, \`Task\`, \`WebSearch\`, \`WebFetch\`, \`TodoWrite\` as needed
...
function convertClaudeCommandToWindsurfWorkflow(content, commandName) {
  ...
  // Pattern: optional ferrox- prefix + lowercase alphanumeric + dashes; rejects
  // everything else. See DEFECT.PROMPT-INJECTION-SCAN-COLLISION and the
  // PR #1622 security review.
  if (typeof commandName !== 'string' || !/^(?:ferrox-)?[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/.test(commandName)) {
    ...
    throw new Error(
      ...
      'must match /^(?:ferrox-)?[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/ (no slashes, backslashes, spaces, dots, trailing dash, or control chars — prevents prompt injection and path-component injection into the workflow body)'
    );
  }
  const converted = convertClaudeToWindsurfMarkdown(content);
  ...
  const stem = commandName.startsWith('ferrox-') ? commandName.slice(4) : commandName;
  const workflow = `# ${commandName}\n\n${toSingleLine(description || `Run ${commandName}.`)}\n\nRead and execute the Ferrox command at @~/.claude/ferrox-core/commands/ferrox/${stem}.md end-to-end. Treat the user's message after /${commandName} as the command arguments.`;
...
function convertSlashCommandsToTraeSkillMentions(content) {
  return content.replace(/\/ferrox:([a-z0-9-]+)/g, (_, commandName) => {
    return `/ferrox-${commandName}`;
  });
}
...
function convertClaudeCommandToTraeSkill(content, skillName) {
  ...
  // frontmatter parser.
  let fm = `---\nname: ${yamlIdentifier(skillName)}\ndescription: ${yamlQuote(shortDescription)}\n`;
  // #2094: emit `stage:` so Trae's SOLO agent can auto-invoke Ferrox skills at
  ...
  // fixed Ferrox-side value (runtime.hostBehaviors.soloStageMetadata), inferred/
  // best-effort.
...
function convertClaudeCommandToCodebuddySkill(content, skillName) {
  ...
  // avoiding a duplicated /ferrox-* entry per workflow.
 ...
 * command name (ferrox-help.md → /ferrox-help), so the Claude-specific `name: ferrox:<x>`
 ...
 * @param {string} commandName  installed command name (e.g. 'ferrox-help')
 * @returns {string}
 */
function convertClaudeCommandToCodebuddyCommand(content, commandName) {
  const converted = convertClaudeToCodebuddyMarkdown(content);
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  let description = `Run Ferrox workflow ${commandName}.`;
  let argumentHint = '';
  if (frontmatter) {
    const maybeDescription = extractFrontmatterField(frontmatter, 'description');
...
function convertClaudeCommandToClineSkill(content, skillName, runtime = null, cmdNames = null) {
  ...
  // Hyphen-normalise /ferrox:<cmd> → ferrox-<cmd> references in the body, then
  ...
  if (!description) description = `Run Ferrox workflow ${skillName}.`;
  description = toSingleLine(description);
...
function convertSlashCommandsToCodexSkillMentions(content) {
  // Colon-style /ferrox: never appears as a filesystem path segment, so no boundary guard is needed (unlike the hyphen-style below).
  let converted = content.replace(/\/ferrox:([a-z0-9-]+)/gi, (_, commandName) => {
    return `$ferrox-${String(commandName).toLowerCase()}`;
  });
  // Convert hyphen-style command references (workflow output) to Codex $ prefix.
  // A real /ferrox-<cmd> MENTION is defined positively by two boundaries, so any
  ...
  //      delimiter (backtick/quote/paren/bracket) — e.g. `/ferrox-execute-phase`.
  //   2. Right boundary: the command token is NOT followed by a path separator
  //      `/` (a path continues: `/ferrox-core/bin/...`; a command does not). The
  //      `(?![a-z0-9/-])` also blocks regex backtracking to a shorter command.
  // This converts backtick-wrapped MENTIONS (`/ferrox-foo`) while leaving backtick-
  // wrapped PATHS (`/ferrox-core/workflows/update.md`) untouched (#712).
  converted = converted.replace(/(?<=^|[\s`"'([])\/ferrox-([a-z0-9-]+)(?![a-z0-9/-])/gi, (_, commandName) => {
    return `$ferrox-${String(commandName).toLowerCase()}`;
  });
  return converted;
}

const CODEX_FERROX_TOOLS_INVOCATION = 'node "$HOME/.codex/ferrox-core/bin/ferrox-tools.cjs"';

function rewriteBareFerroxToolsCommandsForCodex(content) {
  return content
    .replace(/(^[ \t]*)ferrox-tools(?=\s)/gm, `$1${CODEX_FERROX_TOOLS_INVOCATION}`)
    .replace(/(\$\(\s*)ferrox-tools(?=\s)/g, `$1${CODEX_FERROX_TOOLS_INVOCATION}`)
    .replace(/(`\s*)ferrox-tools(?=\s)/g, `$1${CODEX_FERROX_TOOLS_INVOCATION}`)
    .replace(/((?:&&|\|\||[;|])\s*)ferrox-tools(?=\s)/g, `$1${CODEX_FERROX_TOOLS_INVOCATION}`);
}
...
function convertClaudeToCodexMarkdown(content) {
  ...
  // bare `ferrox-tools` binary on PATH. Keep resolver probes such as
  // `command -v ferrox-tools` intact; rewrite only command invocations.
  converted = rewriteBareFerroxToolsCommandsForCodex(converted);
  // Runtime-neutral agent name replacement (#766)
...
- Treat all user text after \`${invocation}\` as \`{{FERROX_ARGS}}\`.
- If no arguments are present, treat \`{{FERROX_ARGS}}\` as empty.

## B. AskUserQuestion → request_user_input Mapping
Ferrox workflows use \`AskUserQuestion\` (Claude Code syntax). Translate to Codex \`request_user_input\`:
...
Ferrox workflows use \`Task(...)\` (Claude Code syntax). Translate to Codex collaboration tools:

**Schema detection (required first step):** Codex exposes two \`spawn_agent\` schemas:
- **agent_type-capable schema** (e.g. \`multi_agent_v2\`): \`spawn_agent\` accepts \`agent_type\`, \`message\`, \`reasoning_effort\`, \`fork_context\`, etc. — typed Ferrox agent dispatch is available.
- **Generic schema** (\`multi_agent_v1\`): \`spawn_agent\` accepts only \`message\`, \`items\`, \`fork_context\` — there is **no \`agent_type\` field**. Typed Ferrox agent dispatch is unavailable in this session.
...
- \`Task(model="...")\` → omit. \`spawn_agent\` has no inline \`model\` parameter;
  Ferrox embeds the resolved per-agent model directly into each agent's \`.toml\`
  at install time so \`model_overrides\` from \`.planning/config.json\` and
  \`~/.ferrox/defaults.json\` are honored automatically by Codex's agent router.
- Resolved \`reasoning_effort="low|medium|high|xhigh"\` (\`xhigh\` is a Ferrox/Codex tier, not a generic runtime enum) → pass \`reasoning_effort\`
  to \`spawn_agent\` when the runtime/tool supports it. Omit missing, empty,
  inherited, or unsupported values; do not invent one-off effort literals in
  workflow prose.
- \`fork_context: false\` by default — Ferrox agents load their own context via \`<files_to_read>\` blocks
...
When only the generic \`multi_agent_v1\` schema is available, typed Ferrox agent dispatch
(\`ferrox-planner\`, \`ferrox-executor\`, etc.) is NOT possible. This is a known Codex limitation
(openai/codex#15250). **This workaround is NOT equivalent to typed ferrox-planner/ferrox-executor
execution** — Ferrox agents carry project-aware prompts, audit logging, and workflow context
 ...
 * @param {object|null} runtimeResolver  — runtime-aware tier resolver from readFerroxRuntimeProfileResolver
 * @param {object|null} effortCfg        — #443: merged effort config from readFerroxEffectiveEffortConfig
 */
function generateCodexAgentToml(agentName, agentContent, modelOverrides = null, runtimeResolver = null, effortCfg = null, sandboxTier = 'codex-agent-sandbox') {
  ...
  const resolvedDescription = toSingleLine(
    extractFrontmatterField(frontmatterText, 'description') || `Ferrox agent ${resolvedName}`
  );
  ...
  // Embed model override when configured in ~/.ferrox/defaults.json so that
  ...
  // ferroxRenderEffortForRuntime('codex', ...).
  ...
  // follows Ferrox. Keep those knobs coupled unless Ferrox also pins the model.
  if (hasPinnedModel) {
    const _universalEffortCodex = resolveInstallTimeEffort(effortCfg, resolvedName !== agentName ? resolvedName : agentName);
    const _renderedEffortCodex = _getFerroxEffortCatalog().renderEffortForRuntime('codex', _universalEffortCodex).value;
  ...
  const { AGENT_DEFAULT_TIERS: _agentTiers } = _getFerroxEffortCatalog();
...
/**
 * Remove stale agents/openai.yaml sidecar files from Ferrox-managed Codex skill dirs.
 *
 * Prior to #1326, Ferrox's Codex install path wrote an agents/openai.yaml file
 * alongside each ferrox-* SKILL.md. Recent Codex builds index BOTH SKILL.md and
 * the sidecar, causing each Ferrox skill to appear twice in autocomplete. This
 ...
 *   - Only touches directories whose names start with "ferrox-".
 *   - Skips user-owned dirs (ferrox-dev-preferences) — their agents/ content is
 *     never modified, mirroring the same USER_OWNED_SKILL_DIRS guard used by
 *     installOpencodeFamilySkills.
 *   - For each managed ferrox-* dir, if agents/openai.yaml exists, deletes it.
 *   - If agents/ is now empty, removes the directory; if it still contains
 *     other files (e.g. user-added content), leaves it in place.
 *   - Non-ferrox-* dirs and their agents/ content are never touched.
...
function cleanupCodexSkillMetadataSidecars(skillsDir) {
  ...
  // never be modified by Ferrox's install path.
  const _userOwnedSkillDirs = new Set(['ferrox-dev-preferences']);
  for (const entry of fs.readdirSync(skillsDir, { withFileTypes: true })) {
    if (!entry.isDirectory() || !entry.name.startsWith('ferrox-')) continue;
...
/**
 * Remove legacy Windsurf skill artifacts from .devin/skills/ferrox- directories.
 *
 ...
 * Old .devin/skills/ferrox- dirs linger on disk indefinitely and confuse
 * users who see two Ferrox trees.
 *
 * Preserves user-owned content:
 *   - non-ferrox-* dirs under .devin/skills/ (user-authored skills)
 *   - ferrox-dev-preferences/ (user-owned per #2973)
 *   - any files (not dirs) under .devin/skills/
 *
 * @param {string} workspaceDir - workspace root (process.cwd() for local installs)
 * @returns {number} count of removed legacy ferrox-* skill directories
 */
function cleanupWindsurfLegacyDevinSkills(workspaceDir) {
  ...
  for (const entry of fs.readdirSync(legacySkillsDir, { withFileTypes: true })) {
    ...
    try {
      // Symlink guard: if the ferrox-* dir is itself a symlink pointing outside
 ...
 * (non-prefixed dirs, ferrox-dev-preferences, symlinks) is preserved. Fail-open.
 * @param {string} oldSkillsDir absolute path to the pre-move skills location
 * @param {string} prefix managed skill-dir prefix (e.g. 'ferrox-')
...
function cleanupMovedSkillsOldLocation(oldSkillsDir, prefix) {
  ...
  for (const entry of fs.readdirSync(oldSkillsDir, { withFileTypes: true })) {
    ...
    try {
      ...
      // through a symlinked ferrox-* dir — it could escape the tree.
      const stat = fs.lstatSync(dirToRemove);
      if (stat.isSymbolicLink()) continue;
...
/**
 * Generate the Ferrox config block for Codex config.toml.
...
function generateCodexConfigBlock(agents, targetDir) {
  ...
  lines.push('[agents]');
  lines.push(`max_depth = ${FERROX_CODEX_AGENTS_MAX_DEPTH}`);
  lines.push('');
 ...
 * `[agents]` table — every known scalar EXCEPT `max_depth` (which Ferrox manages
...
function extractCodexUserAgentsScalars(content) {
  ...
  for (const record of getTomlLineRecords(body)) {
    ...
    if (key === 'max_depth') continue; // Ferrox-managed — Ferrox's value wins.
...
/**
 * Splice preserved user AgentsToml scalar lines into the managed Ferrox config
 * block, immediately after the `[agents]` header and before Ferrox's `max_depth`
...
/**
 * Strip any managed Ferrox agent sections from a TOML string.
 *
 * Used by the uninstall path (`stripFerroxFromCodexConfig`). Removes only what Ferrox
 * owns; user-authored `[agents.<name>]` and `[[agents]]` entries are preserved
 * so uninstall returns the file to its pre-Ferrox shape.
 *
 * Handles BOTH shapes so reinstall self-heals configs from all Ferrox versions:
 *   - Current (#2727): `[agents.ferrox-*]` struct tables (Codex 0.120.0+).
 *   - Legacy (#2645): `[[agents]]` array-of-tables whose `name = "ferrox-*"`.
 *
 * A section runs from its header to the next `[` header or EOF.
 */
function stripCodexFerroxAgentSections(content) {
  ...
  const sections = getTomlTableSections(content).filter((section) => {
    // Current `[agents.ferrox-<name>]` struct tables (#2727, Codex 0.120.0+).
    if (!section.array && /^agents\.ferrox-/.test(section.path)) {
      return true;
    }

    // Ferrox's managed `[agents]` scalar block (ADR-1239 upgrade 2 / #2088 — the
    ...
    // Ferrox-owned; strip it on uninstall. (The marker path already removes it via
    ...
    // `name = "ferrox-..."`, preserving user-authored [[agents]] entries.
    if (section.array && section.path === 'agents') {
      const body = content.slice(section.headerEnd, section.end);
      const nameMatch = body.match(/^[ \t]*name[ \t]*=[ \t]*["']([^"']+)["']/m);
      return Boolean(nameMatch && /^ferrox-/.test(nameMatch[1]));
    }
...
/**
 * Strip Ferrox sections from Codex config.toml content.
 * Returns cleaned content, or null if file would be empty.
 */
function stripFerroxFromCodexConfig(content) {
  const eol = detectLineEnding(content);
  const markerIndex = content.indexOf(FERROX_CODEX_MARKER);
  const codexHooksOwnership = getManagedCodexHooksOwnership(content);

  if (markerIndex !== -1) {
    // Has Ferrox marker — remove everything from marker to EOF. First recover the
    ...
    // round-trips the user's tuning. Ferrox-managed max_depth is dropped.
    ...
    // Also strip Ferrox-injected feature keys above the marker (Case 3 inject)
  ...
  // No marker but may have Ferrox-injected feature keys
  let cleaned = content;
  ...
  // Remove [agents.ferrox-*] sections + the managed known-scalar [agents] table.
  cleaned = stripCodexFerroxAgentSections(cleaned);
...
function getManagedCodexHooksOwnership(content) {
  ...
  const afterMarker = content.slice(markerIndex + FERROX_CODEX_MARKER.length);
  const match = afterMarker.match(/^\r?\n# Ferrox codex_hooks ownership: (section|root_dotted)\r?\n/);
...
function setManagedCodexHooksOwnership(content, ownership) {
  ...
  const eol = detectLineEnding(content);
  const markerEnd = markerIndex + FERROX_CODEX_MARKER.length;
  ...
  return content.slice(0, markerEnd) +
    eol +
    `${FERROX_CODEX_HOOKS_OWNERSHIP_PREFIX}${ownership}${eol}` +
    remainder;
}

function isLegacyFerroxAgentsSection(body) {
...
function stripLeakedFerroxCodexSections(content) {
  // Defensive precedence (#2760): we own the `agents` namespace under our
  // managed `ferrox-*` names, and the legacy bare-table and sequence forms
  ...
  // purges them regardless of Ferrox marker presence. Users who had legitimate
  ...
  const leakedSections = getTomlTableSections(content)
    .filter((section) => {
      // Legacy [agents.ferrox-<name>] map tables (pre-#2645).
      if (!section.array && section.path.startsWith('agents.ferrox-')) return true;

      // ANY bare [agents] single-bracket table — invalid in current Codex
      // schema, always purged at install time (#2760). Previously gated
      // on `isLegacyFerroxAgentsSection`, which missed bare tables holding
      ...
      // `name = "ferrox-..."` which preserved user-authored entries that are
...
/**
 * Strip Ferrox-managed legacy Codex hook blocks from a config.toml string
 ...
 * `command = "...ferrox-(check-update|update-check).js"` value, remove its
 ...
 * The leading `# Ferrox Hooks` header line is swallowed by extending the
 ...
 * if no Ferrox-managed hook section is present.
 */
function stripStaleFerroxHookBlocks(configContent) {
  ...
  // A section is Ferrox-managed if any structural `command` key inside its
  // body parses to a string whose basename matches `ferrox-(check-update|
  ...
  // preceding line is the Ferrox marker comment `# Ferrox Hooks` (and is not part
  ...
  for (const section of stale) {
    ...
    if (prev && !prev.startsInMultilineString && prev.text.trim() === '# Ferrox Hooks') {
      start = prev.start;
    }
...
function migrateCodexHooksMapFormat(content) {
  ...
  // Insert flat-AoT conversions before the Ferrox managed marker (if present) so
  // the migrated user hooks stay in the "user" portion of the file and are not
  // swept away when stripFerroxFromCodexConfig strips from the marker to EOF.
  // If no marker exists, append at the end of the file.
  if (flatAotBlocks.length > 0) {
    const insertionText = flatAotBlocks.join('');
    const markerIdx = result.indexOf(FERROX_CODEX_MARKER);
    if (markerIdx !== -1) {
 ...
 * the Ferrox-managed hook block must be emitted in the same shape so it
 ...
 *
 * Supports the value forms Ferrox emits or that real Codex configs commonly use:
  ...
  if (ch === '"') {
    ...
    while (j < text.length) {
      ...
      if (c === '\\') {
        ...
        if (next === '"') { out += '"'; j += 2; continue; }
        if (next === '/') { out += '/'; j += 2; continue; }
        // Pass-through unrecognized escape (Codex/Ferrox don't use these).
 ...
 * empty body is trivially valid. Mirrors isLegacyFerroxAgentsSection's line scan.
 */
...
function validateCodexConfigSchema(content) {
  ...
  for (const section of sections) {
    ...
    if (!section.array && section.path === 'agents') {
      ...
      // Ferrox's managed `max_depth` dispatch-tuning block (ADR-1239 upgrade 2 /
...
/**
 * Merge Ferrox config block into an existing or new config.toml.
 * Three cases: new file, existing with Ferrox marker, existing without marker.
 *
 * All writes go through atomicWriteFileSync so a mid-write failure leaves
 * the original config.toml untouched (#2760 fix 4).
 */
function mergeCodexConfig(configPath, ferroxBlock) {
  // Case 1: No config.toml — create fresh
  if (!fs.existsSync(configPath)) {
    atomicWriteFileSync(configPath, ferroxBlock + '\n');
    return;
  }
  ...
  // stripLeakedFerroxCodexSections) to keep a single managed `[agents]`. Preserve
  // the user's own AgentsToml scalar tuning (max_threads, job_max_runtime_seconds,
  // interrupt_message — everything except Ferrox-managed max_depth) by re-emitting
  // it inside the managed block, so install never silently drops it.
  const mergedFerroxBlock = spliceCodexAgentsScalars(ferroxBlock, extractCodexUserAgentsScalars(existing));
  const normalizedFerroxBlock = mergedFerroxBlock.replace(/\r?\n/g, eol);
  const markerIndex = existing.indexOf(FERROX_CODEX_MARKER);

  // Case 2: Has Ferrox marker — truncate and re-append
  if (markerIndex !== -1) {
    let before = existing.substring(0, markerIndex).trimEnd();
    if (before) {
      // Strip any Ferrox-managed sections that leaked above the marker from previous installs
      before = stripLeakedFerroxCodexSections(before).trimEnd();

      atomicWriteFileSync(configPath, before + eol + eol + normalizedFerroxBlock + eol);
    } else {
      atomicWriteFileSync(configPath, normalizedFerroxBlock + eol);
    }
    return;
  }

  // Case 3: No marker — append Ferrox block
  let content = stripLeakedFerroxCodexSections(existing).trimEnd();
  if (content) {
    content = content + eol + eol + normalizedFerroxBlock + eol;
  } else {
    content = normalizedFerroxBlock + eol;
  }
...
/**
 * Repair config.toml files corrupted by pre-#1346 Ferrox installs.
...
/**
 * Merge Ferrox instructions into copilot-instructions.md.
 ...
 * @param {string} ferroxContent - Template content (without markers)
 */
function mergeCopilotInstructions(filePath, ferroxContent) {
  const ferroxBlock = FERROX_COPILOT_INSTRUCTIONS_MARKER + '\n' +
    ferroxContent.trim() + '\n' +
    FERROX_COPILOT_INSTRUCTIONS_CLOSE_MARKER;

  // Case 1: No file — create fresh
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, ferroxBlock + '\n');
    return;
  }

  const existing = fs.readFileSync(filePath, 'utf8');
  const openIndex = existing.indexOf(FERROX_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = existing.indexOf(FERROX_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  // Case 2: Has Ferrox markers — replace between markers
  if (openIndex !== -1 && closeIndex !== -1) {
    const before = existing.substring(0, openIndex).trimEnd();
    const after = existing.substring(closeIndex + FERROX_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
    let newContent = '';
    if (before) newContent += before + '\n\n';
    newContent += ferroxBlock;
    if (after) newContent += '\n\n' + after;
    newContent += '\n';
...
/**
 * Strip Ferrox section from copilot-instructions.md content.
 * Returns cleaned content, or null if file should be deleted (was Ferrox-only).
 * @param {string} content - File content
 * @returns {string|null} - Cleaned content or null if empty
 */
function stripFerroxFromCopilotInstructions(content) {
  const openIndex = content.indexOf(FERROX_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = content.indexOf(FERROX_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  if (openIndex !== -1 && closeIndex !== -1) {
    const before = content.substring(0, openIndex).trimEnd();
    const after = content.substring(closeIndex + FERROX_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
...
// form (`.clinerules/ferrox.md`). Sources adjudicated:
//   - https://cline.bot/blog/cline-v3-36-hooks
//   - https://docs.cline.bot/customization/cline-rules

const FERROX_AGENTS_MD_MARKER = '<!-- Ferrox Configuration — managed by ferrox-core installer -->';
const FERROX_AGENTS_MD_CLOSE_MARKER = '<!-- End Ferrox Configuration -->';

/**
 * The Ferrox instruction body shared by the Cline directory-form rules file and
 * the cross-tool AGENTS.md block. Self-contained — references only the ferrox-core
 ...
 * tool calls that target `.planning/` (Ferrox-owned artifacts), and otherwise
...
/**
 * Merge the Ferrox AGENTS.md block into an existing file (or create it), preserving
 * any user content. Mirrors mergeCopilotInstructions: marker-delimited, idempotent.
 */
function mergeFerroxAgentsMd(filePath, ferroxContent) {
  return hooksSurface.mergeFerroxAgentsMd(filePath, ferroxContent);
}

/**
 * Strip the Ferrox block from AGENTS.md content. Returns null if the file became
 * empty (was Ferrox-only), the unchanged content if no markers were found, or the
 * cleaned content otherwise.
 */
function stripFerroxFromAgentsMd(content) {
  const openIndex = content.indexOf(FERROX_AGENTS_MD_MARKER);
  const closeIndex = content.indexOf(FERROX_AGENTS_MD_CLOSE_MARKER);
  if (openIndex !== -1 && closeIndex !== -1) {
    const before = content.substring(0, openIndex).trimEnd();
    const after = content.substring(closeIndex + FERROX_AGENTS_MD_CLOSE_MARKER.length).trimStart();
    const cleaned = (before + (before && after ? '\n\n' : '') + after).trim();
    if (!cleaned) return null;
    return cleaned + '\n';
  }
...
//
// Cursor v2.4+ supports a hooks.json lifecycle hook system. Ferrox registers two
...
//
// Ferrox entries are identified by a top-level `"ferrox-managed": true` field on
// each hook entry. Non-Ferrox entries are preserved. The reconciler is idempotent
...
/**
 * Return true if a Cursor hook entry is Ferrox-managed.
 * Detection: presence of the FERROX_CURSOR_HOOK_MARKER sentinel field.
 *
...
/**
 * Reconcile the Ferrox-managed entries in a Cursor hooks.json file.
 *
 * Supports both known hooks.json shapes:
 *   1) { "version": 1, "hooks": { "sessionStart": [...], "postToolUse": [...] } }
 *   2) { "sessionStart": [...], "postToolUse": [...] }   (no wrapper object)
 *
 * Managed entries (those with FERROX_CURSOR_HOOK_MARKER) are removed then
...
/**
 * #777 — Write Ferrox-managed Cursor lifecycle hooks into <targetDir>/hooks.json.
 *
 * Both managed hook scripts (ferrox-cursor-session-start.js, ferrox-cursor-post-tool.js)
 * are copied from the Ferrox hooks/ source to <targetDir>/hooks/ first, so the
 ...
 * @param {string} src       - The Ferrox install source root (for copying hook scripts)
...
/**
 * Remove all Ferrox-managed Cursor lifecycle hook entries from hooks.json.
...
/**
 * #2100 Stage 2 — Write Ferrox-managed Windsurf/Cascade lifecycle hooks into
 * <targetDir>/hooks.json. Both managed hook scripts
 * (ferrox-windsurf-pre-write.js, ferrox-windsurf-pre-command.js) are copied from
 * the Ferrox hooks/ source to <targetDir>/hooks/ first, so the hooks.json
...
/**
 * Remove all Ferrox-managed Windsurf/Cascade lifecycle hook entries from
...
/**
 * #786 — Build the Ferrox-managed GitHub Copilot lifecycle hook config object.
 *
 ...
 * the agent toward the project's Ferrox planning state at session start.
 *
...
/**
 * #786 — Write the Ferrox-managed Copilot lifecycle hook config under the runtime
 * config dir (`<targetDir>/hooks/ferrox-session.json`). For local installs
 * targetDir is `.github` (→ `.github/hooks/`); for global installs it is
 * `~/.copilot` (→ `~/.copilot/hooks/`) — both are valid Copilot hook locations.
 *
 * The managed file is fully owned by Ferrox, so it is overwritten wholesale on
...
function installCodexConfig(targetDir, agentsSrc, sandboxTier = 'codex-agent-sandbox') {
  ...
  const agentEntries = fs.readdirSync(agentsSrc).filter(f => f.startsWith('ferrox-') && f.endsWith('.md'));
  const agents = [];

  // Compute the Codex Ferrox install path (absolute, so subagents with empty $HOME work — #820)
  const codexFerroxPath = `${path.resolve(targetDir, 'ferrox-core').replace(/\\/g, '/')}/`;

  for (const file of agentEntries) {
    let content = fs.readFileSync(path.join(agentsSrc, file), 'utf8');
    // Replace full .claude/ferrox-core prefix so path resolves to the Codex
    // Ferrox install before generic .claude → .codex conversion rewrites it.
    content = content.replace(/~\/\.claude\/ferrox-core\//g, codexFerroxPath);
    content = content.replace(/\$HOME\/\.claude\/ferrox-core\//g, codexFerroxPath);
    ...
    // $ARGUMENTS → {{FERROX_ARGS}}, /clear removal, anchored and bare .claude/
    ...
    // `~/.ferrox/defaults.json` (project wins on conflict) so Codex TOML files
    ...
    // per-project override the reporter had set for ferrox-codebase-mapper.
    ...
    const modelOverrides = readFerroxEffectiveModelOverrides(targetDir);
    // Pass `targetDir` so per-project .planning/config.json wins over global
    // ~/.ferrox/defaults.json — without this, the PR's headline claim that
    // setting runtime in the project config reaches the Codex emit path is
    // false (review finding #1).
    const runtimeResolver = readFerroxRuntimeProfileResolver(targetDir);
    ...
    const effortCfg = readFerroxEffectiveEffortConfig(targetDir);
  ...
  const ferroxBlock = generateCodexConfigBlock(agents, targetDir);
  mergeCodexConfig(configPath, ferroxBlock);
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
  ...
  // Replace /ferrox-command colon variant with /ferrox-command for opencode (flat command structure)
  convertedContent = convertedContent.replace(/\/ferrox:/g, '/ferrox-');
  ...
  if (isAgent) {
    ...
    // Embed model override from ~/.ferrox/defaults.json so model_overrides is
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
  ...
  convertedContent = convertedContent.replace(/\bSlashCommand\b/g, 'skill');
  convertedContent = convertedContent.replace(/\bTodoWrite\b/g, 'todowrite');
  // Replace /ferrox-command colon variant with /ferrox-command for Kilo (flat command structure)
  convertedContent = convertedContent.replace(/\/ferrox:/g, '/ferrox-');
...
function listCodexSkillNames(skillsDir, prefix = 'ferrox-') {
  if (!fs.existsSync(skillsDir)) return [];
 ...
 * @param {string} prefix          skill name prefix without trailing dash (e.g. 'ferrox')
 ...
 * Claude Code 2.1.88+ uses skills/xxx/SKILL.md instead of commands/ferrox/xxx.md.
 ...
 * @param {string} prefix - Skill name prefix (e.g. 'ferrox')
...
/**
 * Write the Hermes "ferrox" category DESCRIPTION.md.
 ...
 * reach for that category. Per spec in #2841 we collapse all 86 Ferrox commands
 * under a single "ferrox" category to keep system-prompt overhead bounded.
 */
function writeHermesCategoryDescription(categoryDir) {
  ...
  const body = [
    ...
    'name: ferrox',
    `version: ${pkg.version}`,
    'description: Ferrox Factory — Go Build Something Awesome. Disciplined planning, execution, and shipping workflows. Use any ferrox-* skill in this category to drive a project through new-project → discuss-phase → plan-phase → execute-phase → ship.',
    '---',
    '',
    '# Ferrox Factory',
    '',
    'Ferrox is a structured development workflow. Skills in this category cover',
    'project initialization, phase planning, execution, code review, and shipping.',
    '',
    'Invoke any `ferrox-*` skill in this category to drive the corresponding step.',
    '',
  ].join('\n');
...
/**
 * Recursively install Ferrox commands as Antigravity skills.
...
// migrateLegacyDevPreferencesToSkill, _copyStaged, _removeFerroxEntries,
...
const RUNTIME_CONTENT_DISPATCH = {
  ...
  cursor: {
    ...
    js: (content) => {
      content = content.replace(/ferrox:/gi, 'ferrox-');
  ...
  trae: {
    ...
    js: (content) => {
      content = content.replace(/\/ferrox:([a-z0-9-]+)/g, (_, commandName) => {
        return `/ferrox-${commandName}`;
      });
...
function copyWithPathReplacement(srcDir, destDir, pathPrefix, runtime, isCommand = false, isGlobal = false, confinementRoot) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // copyWithPathReplacement is the emit path for ferrox-core/workflows/*.md;
      ...
      // #3683 — normalize /ferrox:<cmd> → /ferrox-<cmd> in any body passing through
      ...
      content = normalizeAgentBodyForRuntime(content, runtime, readFerroxCommandNames());
...
function cleanupOrphanedHooks(settings) {
  const orphanedHookPatterns = [
    'ferrox-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to ferrox-statusline.js in v1.9.0
    'ferrox-intel-index.js',  // Removed in v1.9.2
    'ferrox-intel-session.js',  // Removed in v1.9.2
    'ferrox-intel-prune.js',  // Removed in v1.9.2
  ];
  ...
  // Fix #330: Update statusLine if it points to old Ferrox statusline.js path
  // Only match the specific old Ferrox path pattern (hooks/statusline.js),
  ...
  if (settings.statusLine && settings.statusLine.command &&
    ...
    settings.statusLine.command = settings.statusLine.command.replace(
      /hooks([\/\\])statusline\.js/,
      'hooks$1ferrox-statusline.js'
    );
    console.log(`  ${green}✓${reset} Updated statusline path (hooks/statusline.js → hooks/ferrox-statusline.js)`);
  }
...
/**
 * Ferrox hook filenames removed during uninstall.
 ...
 * ferrox-check-update-worker.js, ferrox-ensure-canonical-path.js,
 * managed-hooks-registry.cjs, ferrox-cursor-pre-tool.js, ferrox-cursor-stop.js,
 * ferrox-cursor-subagent-start.js, ferrox-cursor-subagent-stop.js, and
 * ferrox-worktree-path-guard.js — so every one of those files (and the hooks/ dir
 * itself, via the non-empty-dir rmdir guard) was left behind on uninstall for
 * every settings-json-hook runtime. `ferrox-check-update.cmd` is added on top: a
...
function uninstall(isGlobal, runtime = DEFAULT_RUNTIME) {
  ...
  console.log(`  Uninstalling Ferrox from ${cyan}${runtimeLabel}${reset} at ${cyan}${locationLabel}${reset}\n`);
  ...
  if (resolveInstallPlan(runtime).installSurface === 'copilot-instructions' && !isGlobal) {
    ...
    if (fs.existsSync(agentsMdPath)) {
      ...
      const cleaned = stripFerroxFromCopilotInstructions(content);
      if (cleaned === null) {
        fs.unlinkSync(agentsMdPath);
        console.log(`  ${green}✓${reset} Removed AGENTS.md (was Ferrox-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(agentsMdPath, cleaned);
        console.log(`  ${green}✓${reset} Cleaned Ferrox section from AGENTS.md`);
      }
    }
  }
  ...
  try {
    fs.unlinkSync(path.join(targetDir, '.ferrox-profile'));
    removedCount++;
  } catch {}

  // 1. Remove Ferrox commands/skills (layout-driven)
  ...
  // BEFORE the move and never reinstalled still has managed ferrox-* skill dirs at
  ...
  // uninstall leaves nothing behind. User-owned content is preserved.
  {
    ...
    if (_movedOldSkillsDir) {
      const migrated = cleanupMovedSkillsOldLocation(_movedOldSkillsDir, 'ferrox-');
      if (migrated > 0) {
        removedCount++;
  ...
  if (_hostBehaviors(runtime).tomlConfigInstall) {
    ...
    if (fs.existsSync(codexAgentsDir)) {
      ...
      for (const file of tomlFiles) {
        if (file.startsWith('ferrox-') && file.endsWith('.toml')) {
    ...
    // Codex: clean Ferrox sections from config.toml
    const codexConfigPath = path.join(targetDir, 'config.toml');
    if (fs.existsSync(codexConfigPath)) {
      const content = fs.readFileSync(codexConfigPath, 'utf8');
      const cleaned = stripFerroxFromCodexConfig(content);
      if (cleaned === null) {
        fs.unlinkSync(codexConfigPath);
        removedCount++;
        console.log(`  ${green}✓${reset} Removed config.toml (was Ferrox-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(codexConfigPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned Ferrox sections from config.toml`);
      }
    }
  ...
  if (resolveInstallPlan(runtime).hooksSurface === 'kimi-hooks-toml') {
    ...
    if (kimiHooksCleanup.changed) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed Ferrox hooks from ${kimiHooksTomlPath}`);
    }
    ...
    // Remove Ferrox hooks" / "5. Remove Ferrox package.json" below, but scoped to
    ...
    // Ferrox-owned filenames are removed, and directories are pruned only if left
    // empty by that removal.
    const kimiHooksDir = path.join(kimiHooksRoot, 'hooks');
    if (fs.existsSync(kimiHooksDir)) {
      let kimiHookCount = 0;
      for (const hook of FERROX_UNINSTALL_HOOKS) {
      ...
      if (kimiHookCount > 0) {
        removedCount++;
        console.log(`  ${green}✓${reset} Removed ${kimiHookCount} Ferrox hooks from ${kimiHooksDir}`);
      }

      const kimiHooksLibDir = path.join(kimiHooksDir, 'lib');
      if (fs.existsSync(kimiHooksLibDir)) {
        let removedKimiLibFiles = 0;
        for (const file of FERROX_HOOK_LIB_FILES) {
          try {
    ...
    if (fs.existsSync(kimiPkgJsonPath)) {
      try {
        ...
        if (content === '{"type":"commonjs"}') {
          ...
          removedCount++;
          console.log(`  ${green}✓${reset} Removed Ferrox package.json from ${kimiHooksRoot}`);
        }
      } catch (e) {
        // Ignore read errors
      }
  ...
  if (resolveInstallPlan(runtime).installSurface === 'copilot-instructions') {
    ...
    if (fs.existsSync(instructionsPath)) {
      ...
      if (cleaned === null) {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed copilot-instructions.md (was Ferrox-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(instructionsPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned Ferrox section from copilot-instructions.md`);
      }
    }

    // #786: remove the Ferrox-managed Copilot lifecycle hook config and prune the
    // hooks dir if we left it empty.
    const hookPath = path.join(targetDir, 'hooks', FERROX_COPILOT_HOOK_FILE);
    if (fs.existsSync(hookPath)) {
      fs.unlinkSync(hookPath);
      removedCount++;
      console.log(`  ${green}✓${reset} Removed Copilot lifecycle hook (${FERROX_COPILOT_HOOK_FILE})`);
      try {
  ...
  // directory-form rules + PreToolUse hook, and strip the Ferrox block from the
  ...
  if (_hostBehaviors(runtime).clineRulesSurface) {
    ...
    for (const rel of ['ferrox.md', path.join('hooks', 'PreToolUse')]) {
    ...
    // Prune now-empty Ferrox-created directories (leave any user-added rule files).
    ...
    if (isGlobal) {
      ...
      try {
        if (fs.existsSync(agentsPath)) {
          ...
          const cleaned = stripFerroxFromAgentsMd(content);
          if (cleaned === null) {
            fs.unlinkSync(agentsPath);
            removedCount++;
            console.log(`  ${green}✓${reset} Removed ~/.agents/AGENTS.md (was Ferrox-only)`);
          } else if (cleaned !== content) {
            fs.writeFileSync(agentsPath, cleaned);
            removedCount++;
            console.log(`  ${green}✓${reset} Cleaned Ferrox section from ~/.agents/AGENTS.md`);
          }
        }
      } catch { /* best-effort */ }
    }
  ...
  // Ferrox-managed hook entries from hooks.json and clean up the managed hook
  ...
  if (_hostBehaviors(runtime).hooksJsonSurface) {
    ...
    if (hooksJsonCleanup.changed) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed Ferrox-managed Cursor hooks from hooks.json`);
    }
    // Remove all Ferrox-managed hook scripts (sessionStart, postToolUse, preToolUse,
    // stop, subagentStart, subagentStop — AC4a, #2089).
    const hooksDir = path.join(targetDir, 'hooks');
    for (const script of FERROX_CURSOR_HOOK_SCRIPTS) {
  ...
  // remove Ferrox-managed Cascade hook entries from hooks.json and clean up the
  ...
  if (resolveInstallPlan(runtime).hooksSurface === 'windsurf-hooks-json') {
    ...
    if (windsurfHooksJsonCleanup.changed) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed Ferrox-managed Windsurf hooks from hooks.json`);
    }
    // Remove all Ferrox-managed hook scripts (pre_write_code, pre_run_command).
    const windsurfHooksDir = path.join(targetDir, 'hooks');
    for (const script of FERROX_WINDSURF_HOOK_SCRIPTS) {
  ...
  // 1c. Claude local: remove flat ferrox-*.md commands from commands/ (current layout,
  //     #1367 fix). Also remove legacy commands/ferrox/ subdirectory from prior installs.
  if (!isGlobal && _hostBehaviors(runtime).localInstallStyle === 'legacy-flat') {
    const commandsDir = path.join(targetDir, 'commands');
    // Remove flat ferrox-*.md files (current layout after #1367 fix)
    if (fs.existsSync(commandsDir)) {
      let removed = 0;
      for (const f of fs.readdirSync(commandsDir)) {
        if (f.startsWith('ferrox-') && f.endsWith('.md')) {
      ...
      if (removed > 0) {
        removedCount++;
        console.log(`  ${green}✓${reset} Removed ${removed} flat ferrox-*.md commands from commands/`);
      }
    }
    // Remove legacy commands/ferrox/ subdirectory if it still exists (pre-#1367 layout).
    // Preserve user-owned dev-preferences.md if present (#1423 parity).
    const legacyFerroxCommandsDir = path.join(targetDir, 'commands', 'ferrox');
    if (fs.existsSync(legacyFerroxCommandsDir)) {
      const legacyDevPrefsPath = path.join(legacyFerroxCommandsDir, 'dev-preferences.md');
      const savedDevPrefs = fs.existsSync(legacyDevPrefsPath) ? fs.readFileSync(legacyDevPrefsPath, 'utf-8') : null;
      fs.rmSync(legacyFerroxCommandsDir, { recursive: true });
      removedCount++;
      console.log(`  ${green}✓${reset} Removed legacy commands/ferrox/`);
      if (savedDevPrefs) {
        try {
          fs.mkdirSync(legacyFerroxCommandsDir, { recursive: true });
          fs.writeFileSync(legacyDevPrefsPath, savedDevPrefs);
          console.log(`  ${green}✓${reset} Preserved commands/ferrox/dev-preferences.md`);
        } catch (err) {
  ...
  // 1d. Qwen/Hermes: migrate dev-preferences.md from legacy commands/ferrox/ location
  ...
  if (_hostBehaviors(runtime).legacyCommandsFerroxCleanup === true) {
    // dev-preferences may have survived in skills/ as SKILL.md — nothing to do for
    // that case. If a stale commands/ferrox/ still exists (e.g. legacy was not removed),
    // attempt migration. In practice _runLegacyUninstallCleanup removes it first,
    // so this is a best-effort guard.
    const legacyDir = path.join(targetDir, 'commands', 'ferrox');
    if (fs.existsSync(legacyDir)) {
      ...
      if (migrateLegacyDevPreferencesToSkill(targetDir, savedLegacyArtifacts, runtime, _uninstallScope)) {
        ...
        const _stem = _sk && _sk.prefix === '' ? 'dev-preferences' : 'ferrox-dev-preferences';
        const _skillRelPath = _sk ? `${_sk.destSubpath}/${_stem}/SKILL.md` : 'skills/ferrox-dev-preferences/SKILL.md';
  ...
  // 2. Remove ferrox-core directory
  const ferroxDir = path.join(targetDir, 'ferrox-core');
  if (fs.existsSync(ferroxDir)) {
    // Preserve user-generated files before wipe (#1423)
    const userProfilePath = path.join(ferroxDir, 'USER-PROFILE.md');
    const preservedProfile = fs.existsSync(userProfilePath) ? fs.readFileSync(userProfilePath, 'utf-8') : null;

    fs.rmSync(ferroxDir, { recursive: true });
    removedCount++;
    console.log(`  ${green}✓${reset} Removed ferrox-core/`);

    // Restore user-generated files
    if (preservedProfile) {
      try {
        fs.mkdirSync(ferroxDir, { recursive: true });
        fs.writeFileSync(userProfilePath, preservedProfile);
        console.log(`  ${green}✓${reset} Preserved ferrox-core/USER-PROFILE.md`);
      } catch (err) {
  ...
  // 3. Remove Ferrox agents (ferrox-*.md files only)
  const agentsDir = path.join(targetDir, 'agents');
  if (fs.existsSync(agentsDir)) {
    const files = fs.readdirSync(agentsDir);
    let agentCount = 0;
    for (const file of files) {
      if (file.startsWith('ferrox-') && file.endsWith('.md')) {
    ...
    if (agentCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${agentCount} Ferrox agents`);
    }
  }

  // 4. Remove Ferrox hooks
  const hooksDir = path.join(targetDir, 'hooks');
  if (fs.existsSync(hooksDir)) {
    ...
    if (hookCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${hookCount} Ferrox hooks`);
    }

    // Remove only the Ferrox-managed files from hooks/lib/ (git-cmd.js + ferrox-graphify-rebuild.sh).
  ...
  // Ferrox's own plugin file is removed; the plugins/ dir is pruned only if it
  ...
  // Ferrox-managed files only: enumerate the exact set the installer writes.
  // Any file NOT in this set is user-owned and must survive uninstall.
  // After removing Ferrox files, attempt to rmdir — if the directory is still
  // non-empty (user has custom helpers) it stays; otherwise it goes cleanly.
  const FERROX_CHANGESET_FILES = [
  ...
  const FERROX_SCRIPTS_LIB_FILES = ['cli-exit.cjs', 'allowlist-ratchet.cjs'];
  ...
  if (fs.existsSync(changesetUninstallDir)) {
    ...
    for (const file of FERROX_CHANGESET_FILES) {
    ...
    if (removedChangeset > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed scripts/changeset/ Ferrox files`);
    }
  }
  ...
  if (fs.existsSync(scriptsLibUninstallDir)) {
    ...
    for (const file of FERROX_SCRIPTS_LIB_FILES) {
    ...
    if (removedScriptsLib > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed scripts/lib/ Ferrox files`);
    }
  }
  ...
  // 5. Remove Ferrox package.json (CommonJS mode marker)
  ...
  if (fs.existsSync(pkgJsonPath)) {
    try {
      ...
      if (content === '{"type":"commonjs"}') {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed Ferrox package.json`);
      }
    } catch (e) {
      // Ignore read errors
    }
  ...
  if (resolveInstallPlan(runtime).finishPermissionWriter === 'opencode') {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        // Remove Ferrox permission entries
        if (config.permission) {
          for (const permType of ['read', 'external_directory']) {
            if (config.permission[permType]) {
              const keys = Object.keys(config.permission[permType]);
              for (const key of keys) {
                if (key.includes('ferrox-core')) {
        ...
        if (modified) {
          ...
          removedCount++;
          console.log(`  ${green}✓${reset} Removed Ferrox permissions from ${path.basename(configPath)}`);
        }
      } catch (e) {
  ...
  // (#2096 Phase B Upgrade 2). Only the Ferrox-owned mcpServers.ferrox key is
  ...
  if (resolveInstallPlan(runtime).finishPermissionWriter === 'antigravity') {
    ...
    if (fs.existsSync(mcpConfigPath)) {
      try {
        ...
        if (mcpConfig && typeof mcpConfig === 'object' && mcpConfig.mcpServers && mcpConfig.mcpServers.ferrox !== undefined) {
          delete mcpConfig.mcpServers.ferrox;
          if (Object.keys(mcpConfig.mcpServers).length === 0) {
            delete mcpConfig.mcpServers;
          }
          fs.writeFileSync(mcpConfigPath, JSON.stringify(mcpConfig, null, 2) + '\n');
          removedCount++;
          console.log(`  ${green}✓${reset} Removed Ferrox MCP companion server from mcp_config.json`);
        }
      } catch (e) {
        // Ignore JSON parse errors
      }
  ...
  if (removedCount === 0) {
    console.log(`  ${yellow}⚠${reset} No Ferrox files found to remove.`);
  }

  console.log(`
  ${green}Done!${reset} Ferrox has been uninstalled from ${runtimeLabel}.
...
/**
 * Configure OpenCode permissions to allow reading Ferrox reference docs
 * This prevents permission prompts when Ferrox accesses the ferrox-core directory
...
function configureOpencodePermissions(isGlobal = true, configDir = null) {
  ...
  // Build the Ferrox path using the actual config directory
  // Use ~ shorthand if it's in the default location, otherwise use full path
  const defaultConfigDir = path.join(os.homedir(), '.config', 'opencode');
  const ferroxPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/ferrox-core/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/ferrox-core/*`;
  ...
  if (config.permission.read[ferroxPath] !== 'allow') {
    config.permission.read[ferroxPath] = 'allow';
    modified = true;
  }
  ...
  if (config.permission.external_directory[ferroxPath] !== 'allow') {
    config.permission.external_directory[ferroxPath] = 'allow';
    modified = true;
  }

  // ADR-1239 Phase D / #1682 — register the companion MCP server (Phase 4) so
  // OpenCode connects to Ferrox's command (point 1) + state-IO (point 5) surface
  // with NO bespoke plugin. Idempotent + non-clobbering: only added when
  // `mcp.ferrox` is absent (a user-defined `mcp.ferrox` is respected — Hyrum's Law).
  // Local-stdio schema per OpenCode config (packages/core/src/config/mcp.ts).
  // `-p ferrox-core` resolves the `ferrox-mcp-server` bin from this package
  // (bin name != package name) regardless of global-install state.
  if (!config.mcp || typeof config.mcp !== 'object') {
    config.mcp = {};
  }
  if (config.mcp.ferrox === undefined) {
    config.mcp.ferrox = {
      type: 'local',
      command: ['npx', '-y', '-p', PACKAGE_NAME, 'ferrox-mcp-server'],
      enabled: true,
    };
    modified = true;
  }
  ...
  // Write config back
  fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n');
  console.log(`  ${green}✓${reset} Configured read permission for Ferrox docs`);
}

/**
 * Configure Kilo permissions to allow reading Ferrox reference docs
 * This prevents permission prompts when Ferrox accesses the ferrox-core directory
 * @param {boolean} isGlobal - Whether this is a global or local install
 * @param {string|null} configDir - Resolved config directory when already known
 */
function configureKiloPermissions(isGlobal = true, configDir = null) {
  ...
  const ferroxPath = kiloConfigDir === defaultConfigDir
    ? '~/.config/kilo/ferrox-core/*'
    : `${kiloConfigDir.replace(/\\/g, '/')}/ferrox-core/*`;
 ...
 * strings in permissions.{allow,deny,ask}, evaluated deny > ask > allow. Ferrox
...
function buildAntigravityAllowRules(configDir) {
  const ferroxPath = toTildePosixPath(configDir);
  return [
    `read_file(${ferroxPath}/ferrox-core/*)`,
    `read_file(${ferroxPath}/agents/ferrox-*)`,
    `read_file(${ferroxPath}/skills/ferrox-*)`,
    `command(node ${ferroxPath}/hooks/*)`,
  ];
}

/**
 * Configure Antigravity permissions to allow reading/executing Ferrox's installed
 ...
 * SAME settings.json Ferrox's own hook registration writes for this runtime
 ...
 * re-reads the file (already containing Ferrox's hooks by the time finishInstall
...
function configureAntigravityPermissions(isGlobal = true, configDir = null) {
  // For local installs, use ./.agents/ (Ferrox's antigravity localConfigDir)
  ...
  console.log(`  ${green}✓${reset} Configured Antigravity permissions for Ferrox paths`);
}
 ...
 * because (1) Ferrox's own antigravity configDir resolution already varies
 ...
 * Ferrox's local configDir for antigravity), keeping global/local symmetric and
 * consistent with the configDir-relative convention every other Ferrox
 * permission writer (kilo/opencode) already uses.
 *
 * Non-destructive + idempotent: only adds mcpServers.ferrox when entirely absent;
 * any other user-configured mcpServers entries (or a user's OWN "ferrox" override)
 * are preserved untouched (Hyrum's Law — mirrors OpenCode's config.mcp.ferrox guard).
 *
 * @param {boolean} isGlobal - Whether this is a global or local install
 * @param {string|null} configDir - Resolved config directory when already known
 */
function configureAntigravityMcpConfig(isGlobal = true, configDir = null) {
  ...
  if (config.mcpServers.ferrox !== undefined) {
    return; // Already configured (or a user-owned override) — never clobber.
  }

  config.mcpServers.ferrox = {
    command: 'npx',
    args: ['-y', '-p', PACKAGE_NAME, 'ferrox-mcp-server'],
  };

  fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n');
  console.log(`  ${green}✓${reset} Configured Antigravity MCP companion server (ferrox)`);
}

/**
 * #2097 (ADR-1239 transport:mcp): register the Ferrox companion MCP server inside a
 ...
 * mcpServers.ferrox, preserving any user-defined servers (a user's own `ferrox` override
 * is respected — Hyrum's Law).
 * @param {object} settings - the in-memory settings object finishInstall will write
 */
function mergeFerroxMcpServerIntoSettings(settings) {
  if (!settings.mcpServers || typeof settings.mcpServers !== 'object' || Array.isArray(settings.mcpServers)) {
    settings.mcpServers = {};
  }
  if (settings.mcpServers.ferrox === undefined) {
    settings.mcpServers.ferrox = {
      command: 'npx',
      args: ['-y', '-p', PACKAGE_NAME, 'ferrox-mcp-server'],
    };
  }
...
const PATCHES_DIR_NAME = 'ferrox-local-patches';
const MANIFEST_NAME = 'ferrox-file-manifest.json';
...
function writeManifest(configDir, runtime = DEFAULT_RUNTIME, options = {}) {
  ...
  // above, now covered by hostBehaviors.skipSharedHooksInstall.
  const { isOpencode, isCodex, isCursor, isAugment, isQwen, isHermes, isCline } = runtimeFlags(runtime);
  const ferroxDir = path.join(configDir, 'ferrox-core');
  // #1367: Claude local now writes flat ferrox-*.md files at commands/ (not commands/ferrox/).
  ...
  // Hermes nests Ferrox skills under skills/ferrox/ as a single category (#2841) —
  // already encoded in its layout descriptor's destSubpath ('skills/ferrox').
  ...
  // resolves destSubpath (which includes hermes's 'skills/ferrox' nesting) — do not
  // re-append 'ferrox' or the hermes dir gets double-nested to skills/ferrox/ferrox.
  ...
  const ferroxHashes = generateManifest(ferroxDir);
  for (const [rel, hash] of Object.entries(ferroxHashes)) {
    ...
    manifest.files['ferrox-core/' + rel] = hash;
  }
  // Record commands surface for runtimes that emit it:
  //   Claude local (#1367 fix): flat ferrox-<cmd>.md at commands/ level
  ...
  // Claude local (#1367): flat ferrox-*.md files at commands/ level.
  // Only claude local writes ferrox-*.md here; global installs don't emit commands,
  ...
  if (!_hostBehaviors(runtime).skipCodexSkillsManifest && fs.existsSync(codexSkillsDir)) {
    // All runtimes (including Hermes post-#947) use the canonical 'ferrox-' prefix.
    const skillListPrefix = 'ferrox-';
    ...
    if (_hostBehaviors(runtime).trackCategoryDescription) {
      ...
      if (fs.existsSync(descPath)) {
        manifest.files['skills/ferrox/DESCRIPTION.md'] = fileHash(descPath);
      }
    }
  }
  if (_hostBehaviors(runtime).agentManifestStyle === 'kimi-nested' && fs.existsSync(agentsDir)) {
    ...
    for (const [rel, hash] of Object.entries(agentHashes)) {
      const isRootAgent = rel === 'ferrox.yaml' || rel === 'ferrox.md';
      const isSubagent = /^subagents\/ferrox-[^/]+\.(yaml|md)$/.test(rel);
    ...
    for (const file of fs.readdirSync(agentsDir)) {
      if (file.startsWith('ferrox-') && (file.endsWith('.md') || file.endsWith('.toml'))) {
  ...
  if (_hostBehaviors(runtime).clineRulesSurface) {
    for (const rel of ['.clinerules/ferrox.md', '.clinerules/hooks/PreToolUse']) {
  ...
  if (!isCodex && _hostBehaviors(runtime).skipSharedHooksInstall !== true) {
    ...
    if (fs.existsSync(hooksDir)) {
      ...
      // `file.startsWith('ferrox-') && (file.endsWith('.js') || file.endsWith('.sh'))`
      ...
      // on every /ferrox-update. See #941.
      ...
      // to git-cmd.js (validate-commit classifier) and ferrox-graphify-rebuild.sh.
      const hooksLibDir = path.join(hooksDir, 'lib');
      if (fs.existsSync(hooksLibDir)) {
        for (const file of fs.readdirSync(hooksLibDir)) {
          if (FERROX_HOOK_LIB_FILES.includes(file)) {
...
/**
 * Populate ferrox-pristine/ with the transformed pristine versions of every
 ...
 * /ferrox-reapply-patches Step 5 verifier (#2972) uses as the diff base
...
function populatePristineDir({ packageSrc, pristineDir, modified, runtime, pathPrefix, isGlobal }) {
  ...
  // install roots: ferrox-core/, commands/ferrox/, command/, skills/, agents/,
  ...
  // path replacement).
  const stageRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-pristine-stage-'));
  let written = 0;
  try {
...
/**
 * Detect user-modified Ferrox files by comparing against install manifest.
 * Backs up modified files to ferrox-local-patches/ for reapply after update.
 * Also saves pristine copies (from manifest) to ferrox-pristine/ to enable
 ...
 * needed to populate ferrox-pristine/. If omitted (legacy callers), pristine
...
function saveLocalPatches(configDir, pristineCtx) {
  ...
  if (manifest.files) {
    for (const artifact of USER_OWNED_ARTIFACTS) {
      delete manifest.files[`ferrox-core/${artifact}`];
    }
  }

  const patchesDir = path.join(configDir, PATCHES_DIR_NAME);
  const pristineDir = path.join(configDir, 'ferrox-pristine');
  const modified = [];
  ...
  // if the user had not modified it" — used by /ferrox-reapply-patches Step 5
  ...
  if (modified.length > 0) {
    ...
    console.log('  ' + yellow + 'i' + reset + '  Found ' + modified.length + ' locally modified Ferrox file(s) — backed up to ' + PATCHES_DIR_NAME + '/');
    for (const f of modified) {
      console.log('     ' + dim + f + reset);
    }

    // #2998 / #3407: maintain ferrox-pristine/ as the diff baseline for the
    // reapply-patches verifier (#2972).
    //
    // #3407 root-cause fix: the prior approach (#3004 CR) wiped ferrox-pristine/
    ...
    //
    // Correct approach: `ferrox-pristine/` is populated lazily by saveLocalPatches'
    ...
    //
    // Per-file decision:
    //   - sha256(ferrox-pristine/X) === originalHash  →  correct; keep it
    //   - ferrox-pristine/X exists but hash mismatch  →  stale from a previous
    //     buggy run (#3407); remove so verifier falls back cleanly
    //   - ferrox-pristine/X absent                    →  attempt hash-validated
    ...
    if (pristineCtx) {
      ...
      for (const relPath of modified) {
        ...
        // File absent from ferrox-pristine/ (or just removed above as stale):
      ...
      if (missingPaths.length > 0) {
        ...
        try {
          tempPristineDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-pristine-regen-'));
          populatePristineDir({
          ...
          console.warn(`ferrox-pristine regen skipped: ${err.message}`);
        } finally {
          if (tempPristineDir) {
      ...
      if (preserved > 0) {
        console.log('  ' + green + '✓' + reset + '  Preserved ' + cyan + 'ferrox-pristine/' + reset + ' (' + preserved + ' file(s)) for three-way merge');
      }
      if (regenerated > 0) {
        console.log('  ' + green + '✓' + reset + '  Regenerated ' + cyan + 'ferrox-pristine/' + reset + ' (' + regenerated + ' file(s)) via hash-validated new-release source');
      }
      if (removed > 0) {
        console.log('  ' + yellow + 'i' + reset + '  Removed ' + removed + ' stale ferrox-pristine/ snapshot(s); regenerated ' + regenerated + ' of those — falls back to over-broad verify heuristic for the rest');
      }
    }
  }
  return modified;
...
function reportLocalPatches(configDir, runtime = DEFAULT_RUNTIME) {
  ...
  if (meta.files && meta.files.length > 0) {
    const reapplyCommand = _hostBehaviors(runtime).reapplyCommand || '/ferrox-update --reapply';
    console.log('');
...
function install(isGlobal, runtime = DEFAULT_RUNTIME, options = {}) {
  ...
  // (legacy .devin/skills/ferrox-* cleanup, the #1629 command-bodies copy, the
  ...
  // Reusable helper to copy hooks/lib/ (git-cmd.js + ferrox-graphify-rebuild.sh).
  ...
  // keeping install scope aligned with FERROX_HOOK_LIB_FILES (which uninstall/manifest manage).
  const copyLibDir = (sDir, dDir, allowlist = []) => {
    ...
    for (const entry of fs.readdirSync(sDir)) {
      ...
      if (st.isDirectory()) {
        ...
        content = content.replace(/\{\{FERROX_VERSION\}\}/g, pkg.version);
  ...
  // but NOT auto-removed here; legacy .agent/ ferrox artifacts are recognized but not
  ...
  // Path prefix for file references in markdown content (e.g. ferrox-tools.cjs).
  // Replaces $HOME/.claude/ or ~/.claude/ so the result is <pathPrefix>ferrox-core/bin/...
  ...
  // ferrox update marker re-application (ADR-0010 Deviation 2):
  ...
  const _commandsDir = path.join(src, 'commands', 'ferrox');
  ...
  function _stageSkills(commandsFerroxDir) {
    return stageSkillsForProfile(commandsFerroxDir, _resolvedProfile);
  }
  ...
  // Save any locally modified Ferrox files before they get wiped.
  // The pristine context lets saveLocalPatches populate ferrox-pristine/ via
  ...
  // directories and files Ferrox will mutate so that any post-install validation
  ...
  // reflect the true pre-Ferrox state. Non-Codex runtimes skip this block.
  //
  // Snapshot contents:
  //   codexPreInstallSkillNames  — Set of ferrox-* skill dir names that existed
  //   codexPreInstallSkillContents — Map<skillName, Map<relPath, Buffer>> of
  //       the full file tree of each pre-existing ferrox-* skill dir, so that
  //       overwritten dirs can be fully restored on rollback (not just removed).
  //   codexPreInstallAgentFiles  — Set of ferrox-*.{md,toml} filenames in agents/
  ...
  //   codexPreInstallVersionBytes — Buffer (or null) of ferrox-core/VERSION
  //
  ...
  // pre-existing ferrox-* skill directory. Best-effort: read errors are silently
  ...
  // Map<filename, Buffer> — content snapshot of each pre-existing ferrox-* agent file.
  ...
  if (_hostBehaviors(runtime).tomlConfigInstall && !isMinimalMode(_effectiveInstallMode)) {
    ...
    if (fs.existsSync(_preSkillsDir)) {
      for (const entry of fs.readdirSync(_preSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory() && entry.name.startsWith('ferrox-')) {
    ...
    const _preVersionPath = path.join(targetDir, 'ferrox-core', 'VERSION');
  ...
  // config.toml is touched: skills/, agents/, ferrox-core/VERSION, and orphaned
  ...
  const _codexPreConfigRollback = !_hostBehaviors(runtime).tomlConfigInstall || isMinimalMode(_effectiveInstallMode) ? null : () => {
    rollbackInstallerMigrations();
    // skills/ferrox-* — pass 1: restore snapshot entries (may be absent if deleted mid-install).
    ...
    // skills/ferrox-* — pass 2: remove any newly-created dirs not in the snapshot.
    if (fs.existsSync(_earlySkillsDir)) {
      try {
        for (const entry of fs.readdirSync(_earlySkillsDir, { withFileTypes: true })) {
          if (entry.isDirectory() && entry.name.startsWith('ferrox-') && !codexPreInstallSkillNames.has(entry.name)) {
    ...
    // agents/ferrox-* — pass 1: restore snapshot entries.
    ...
    // agents/ferrox-* — pass 2: remove any newly-created files not in the snapshot.
    if (fs.existsSync(_earlyAgentsDir)) {
      try {
        for (const file of fs.readdirSync(_earlyAgentsDir)) {
          if (file.startsWith('ferrox-') && (file.endsWith('.md') || file.endsWith('.toml')) && !codexPreInstallAgentFiles.has(file)) {
    ...
    // ferrox-core/VERSION
    const _earlyVersionPath = path.join(targetDir, 'ferrox-core', 'VERSION');
  ...
  const priorInstallExisted = fs.existsSync(path.join(targetDir, 'ferrox-core', 'VERSION'));
  try {
  ...
  // #3541: non-interactive runs (typical /ferrox-update via Claude Code) have
  ...
  // artifacts → remove; user-facing skills → keep; bundled Ferrox hooks →
  ...
  // For unambiguous categories (e.g. `hooks/ferrox-*` bundled hooks left
  ...
  if (_isSkillsRuntime) {
    ...
    // ferrox-* skill dirs. Prior installs wrote these files so Codex would show a
    // display name and description in the /skills TUI popup. Recent Codex builds
    // index BOTH SKILL.md and the sidecar, causing each Ferrox skill to appear twice
    ...
    // managed ferrox-* skill dirs at the old configDir-rooted location
    // (~/.codex/skills). Reinstalling here writes the new location but would
    // otherwise orphan the old one — clean up the stale ferrox-* dirs.
    {
      const _movedOldSkillsDir = _resolveMovedSkillsOldDir(runtime, targetDir, scope);
      if (_movedOldSkillsDir) {
        const migrated = cleanupMovedSkillsOldLocation(_movedOldSkillsDir, 'ferrox-');
    ...
    // #1629 Finding B: Windsurf local only — remove legacy .devin/skills/ferrox-*
    ...
    // content is preserved (non-ferrox- dirs, ferrox-dev-preferences, symlinks).
    ...
    if (_hostBehaviors(runtime).legacyDevinSkillsCleanup && !isGlobal) {
      ...
      if (removedCount > 0) {
        console.log(`  ${green}✓${reset} Removed ${removedCount} legacy .devin/skills/ferrox-* dir(s) (pre-#1615 Windsurf layout)`);
      }
    }

    // Descriptor-driven (#2090): write DESCRIPTION.md for the ferrox/ category after layout install
    if (_hostBehaviors(runtime).writeCategoryDescription) {
      writeHermesCategoryDescription(path.join(targetDir, 'skills', 'ferrox'));
    }

    // Verify installed artifacts and report
    if (_hostBehaviors(runtime).reportSkillsCount) {
      const hermesSkillsDir = path.join(targetDir, 'skills', 'ferrox');
      if (fs.existsSync(hermesSkillsDir)) {
        // Hermes layout uses prefix: 'ferrox-' (#947) — skill dirs have ferrox-<stem> names
        const count = fs.readdirSync(hermesSkillsDir, { withFileTypes: true })
          .filter(e => e.isDirectory() && e.name.startsWith('ferrox-')).length;
        if (count > 0) {
          console.log(`  ${green}✓${reset} Installed ${count} skills to skills/ferrox/`);
        } else {
          failures.push('skills/ferrox/*');
        }
      } else {
        failures.push('skills/ferrox/*');
      }
      ...
      const rootAgentPath = path.join(targetDir, 'agents', 'ferrox.yaml');
      ...
      if (fs.existsSync(rootAgentPath)) {
        ...
        failures.push('agents/ferrox.yaml');
      }
    // Descriptor-driven (ADR-1239 / #2100): folded from `isWindsurf` into
      ...
      if (isGlobal) {
        ...
        if (fs.existsSync(workflowsDir)) {
          const workflowCount = fs.readdirSync(workflowsDir)
            .filter(f => f.startsWith('ferrox-') && f.endsWith('.md')).length;
          if (workflowCount > 0) {
            console.log(`  ${green}✓${reset} Installed ${workflowCount} workflows to workflows/`);
          } else {
            failures.push('workflows/ferrox-*');
          }
        } else {
          failures.push('workflows/ferrox-*');
        }
    ...
    // pi (ADR-1239 / #2102 Stage 1): plugin-only install — pi's /ferrox command is
    // registered programmatically by the native extension (pi/ferrox.cjs →
    // extensions/ferrox.cjs, staged separately below) and dispatches in-process
    // through the embedded ferrox-core command-routing hub. pi has no host-read
    // markdown surface (unlike Claude/OpenCode/etc., which scan commands/ or
    // command/ directories), so writing flat ferrox-<cmd>.md files here would be
    // dead weight the extension never reads. Skip the flat-commands fallback
    // entirely for pluginOnlyInstall runtimes.
    console.log(`  ${green}✓${reset} pi: /ferrox registered via native extension (no declarative command files)`);
  } else {
    // Claude Code local: flat ferrox-<cmd>.md layout — Claude Code registers
    // commands from .claude/commands/ using the filename stem as the command
    // name, so ferrox-<cmd>.md produces the /ferrox-<cmd> hyphen form used everywhere
    // in the framework. The old commands/ferrox/<cmd>.md subdirectory layout caused
    // Claude Code to namespace commands as /ferrox:<cmd> (colon form). (#1367)
    const commandsDir = path.join(targetDir, 'commands');
    fs.mkdirSync(commandsDir, { recursive: true });
    const ferroxSrc = _stageSkills(_commandsDir);
    const cmdNames = readFerroxCommandNames();

    // Remove stale ferrox-*.md files before writing new ones (clean install)
    if (fs.existsSync(commandsDir)) {
      for (const f of fs.readdirSync(commandsDir)) {
        if (f.startsWith('ferrox-') && f.endsWith('.md')) {
    ...
    // Write each command as ferrox-<stem>.md (flat, hyphen-prefixed)
    let cmdCount = 0;
    if (fs.existsSync(ferroxSrc)) {
      for (const entry of fs.readdirSync(ferroxSrc, { withFileTypes: true })) {
        if (!entry.isFile() || !entry.name.endsWith('.md')) continue;
        const stem = entry.name.slice(0, -3);
        let content = fs.readFileSync(path.join(ferroxSrc, entry.name), 'utf8');
        ...
        fs.writeFileSync(path.join(commandsDir, `ferrox-${stem}.md`), content);
        cmdCount++;
      }
    }

    if (cmdCount > 0) {
      console.log(`  ${green}✓${reset} Installed ${cmdCount} commands to commands/ (ferrox-<cmd>.md flat form)`);
    } else {
      failures.push('commands/ferrox-*');
    }

    // Legacy cleanup: remove old commands/ferrox/ subdirectory from prior installs
    // that used the namespaced layout (wrote bare-name files under commands/ferrox/).
    const legacyFerroxDir = path.join(commandsDir, 'ferrox');
    if (fs.existsSync(legacyFerroxDir)) {
      // Preserve user-owned dev-preferences.md before wiping
      const devPrefsPath = path.join(legacyFerroxDir, 'dev-preferences.md');
      const preservedDevPrefs = fs.existsSync(devPrefsPath) ? fs.readFileSync(devPrefsPath, 'utf-8') : null;
      fs.rmSync(legacyFerroxDir, { recursive: true });
      console.log(`  ${green}✓${reset} Removed legacy commands/ferrox/ (migrated to flat ferrox-<cmd>.md layout)`);
      if (preservedDevPrefs) {
        // Migrate dev-preferences to the new flat form
        fs.writeFileSync(path.join(commandsDir, 'ferrox-dev-preferences.md'), preservedDevPrefs);
        console.log(`  ${green}✓${reset} Migrated dev-preferences.md to commands/ferrox-dev-preferences.md`);
      }
    }
    ...
    if (fs.existsSync(staleSkillsDir)) {
      const staleFerrox = fs.readdirSync(staleSkillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('ferrox-'));
      for (const e of staleFerrox) {
        fs.rmSync(path.join(staleSkillsDir, e.name), { recursive: true });
      }
      if (staleFerrox.length > 0) {
        console.log(`  ${green}✓${reset} Removed ${staleFerrox.length} stale Ferrox skill(s) from skills/`);
      }
    }
  }
  ...
  // Copy ferrox-core skill with path replacement
  // Preserve user-generated files before the wipe-and-copy so they survive re-install
  const skillSrc = path.join(src, 'ferrox-core');
  const skillDest = path.join(targetDir, 'ferrox-core');
  const savedFerroxArtifacts = preserveUserArtifacts(skillDest, USER_OWNED_ARTIFACTS);
  copyWithPathReplacement(skillSrc, skillDest, pathPrefix, runtime, false, isGlobal, targetDir);
  restoreUserArtifacts(skillDest, savedFerroxArtifacts);
  if (verifyInstalled(skillDest, 'ferrox-core')) {
    console.log(`  ${green}✓${reset} Installed workflow assets`);
  } else {
    failures.push('ferrox-core');
  }

  // Write the .ferrox-source marker so runtime source resolution succeeds at
  // runtime (#1477). The Claude-global skills layout ships ferrox-core/{bin,
  // contexts,references,templates,workflows} but NOT the commands/ferrox source
  // tree, and _runLegacyUninstallCleanup actively removes any commands/ferrox/
  // for that scope — so findInstallSourceRoot's walk-up has nothing to find
  // and /ferrox-surface (list/status) throws. This is the writer half of the
  // marker that runtime-artifact-layout.cjs's finders already read (the reader
  // landed in #1476). It points at the package's own commands/ferrox source.
  // Scoped to the Claude-global layout (issue #1477) — the only install path
  // that ships the skills layout without a commands/ferrox source tree; every
  // other runtime/scope deploys commands/ferrox, so its walk-up already resolves
  ...
  if (_hostBehaviors(runtime).sourceMarkerFile && isGlobal) {
    const ferroxSourceCommands = path.join(src, 'commands', 'ferrox');
    if (fs.existsSync(ferroxSourceCommands)) {
      try {
        ...
        fs.writeFileSync(_markerPath, ferroxSourceCommands + '\n', 'utf8');
      } catch (err) {
        // Non-fatal: install proceeds. But on the Claude-global layout walk-up
        // also fails (no commands/ferrox source tree), so a silent write failure
        // still leaves /ferrox-surface broken at runtime — warn so it's diagnosable.
        console.warn(`  ${yellow}!${reset} Could not write .ferrox-source marker (${err.message}); /ferrox-surface list/status may fail`);
      }
    }
  }

  // #1629 critical fix: Windsurf workflow wrappers (convertClaudeCommandToWindsurfWorkflow)
  // delegate to command bodies at <targetDir>/ferrox-core/commands/ferrox/${stem}.md via a
  // hardcoded @~/.claude/ferrox-core/commands/ferrox/ path that _applyRuntimeRewrites rewrites
  // to the install target. The source ferrox-core/ dir does NOT ship with commands/ —
  // the canonical command source lives at the package root (commands/ferrox/). Without
  // this copy, every /ferrox-* workflow in Cascade references a missing file and the LLM
  ...
  if (_hostBehaviors(runtime).installsCommandBodiesForWorkflowDelegation && !isGlobal) {
    const commandsSrc = path.join(src, 'commands', 'ferrox');
    const commandsDest = path.join(skillDest, 'commands', 'ferrox');
    if (fs.existsSync(commandsSrc)) {
      copyWithPathReplacement(commandsSrc, commandsDest, pathPrefix, runtime, true, isGlobal, targetDir);
      console.log(`  ${green}✓${reset} Installed command bodies to ferrox-core/commands/ferrox/ (workflow delegation targets)`);
    }
  }

  // Copy shared manifests into the ferrox-core payload
  // at the co-located path that CJS modules resolve first:
  //   ferrox-core/bin/shared/*.json
  //
  // This source now lives under ferrox-core/bin/shared in-repo.
  ...
  for (const fileName of sharedPayloadFiles) {
    const sharedSrc = path.join(src, 'ferrox-core', 'bin', 'shared', fileName);
    const sharedDest = path.join(skillDest, 'bin', 'shared', fileName);
    const displayPath = `ferrox-core/bin/shared/${fileName}`;
    if (fs.existsSync(sharedSrc)) {
      ...
      failures.push(`ferrox-core/bin/shared/${fileName} (source missing)`);
    }
  }

  // Copy agents to agents directory.
  // Skipped under --minimal: ferrox-* subagent descriptions are eagerly loaded
  // into the runtime's Agent tool schema, costing ~6k tokens per turn even
  // when no Ferrox workflow is active. See ferroxfactory/ferrox-core#2762.
  ...
  // Always remove stale ferrox-* agents first so re-installing with
  ...
  if (_hostBehaviors(runtime).pluginOnlyInstall) {
    ...
    // Codex registers agents in `config.toml` via `[agents.ferrox-*]` sections.
    ...
    if (_hostBehaviors(runtime).tomlConfigInstall) {
      ...
      if (fs.existsSync(codexConfigPath)) {
        ...
        const cleaned = stripFerroxFromCodexConfig(existing);
        if (cleaned === null) {
          fs.unlinkSync(codexConfigPath);
    ...
    console.log(`  ${dim}↳${reset} Skipping agents (minimal install — run \`ferrox update\` without \`--minimal\` to add full surface)`);
    ...
    for (const entry of agentEntries) {
      if (entry.isFile() && entry.name.endsWith('.md')) {
        ...
        if (_hostBehaviors(runtime).frontmatterDialect === 'opencode') {
          ...
          const _ocModelOverrides = readFerroxEffectiveModelOverrides(targetDir);
          ...
          if (!_ocModelOverride) {
            ...
            const _ocRuntimeResolver = readFerroxRuntimeProfileResolver(targetDir);
          ...
          const _kiloModelOverrides = readFerroxEffectiveModelOverrides(targetDir);
          ...
          if (!_kiloModelOverride) {
            ...
            const _kiloRuntimeResolver = readFerroxRuntimeProfileResolver(targetDir);
        ...
        if ((_hostBehaviors(runtime).agentFrontmatterExtensions || []).includes('effort')) {
          const _effortCfg = readFerroxEffectiveEffortConfig(targetDir);
          const _agentName = entry.name.replace(/\.md$/, '');
          const _universalEffort = resolveInstallTimeEffort(_effortCfg, _agentName);
          const _renderedEffort = _getFerroxEffortCatalog().renderEffortForRuntime(runtime, _universalEffort).value;
        ...
        // #3677 — normalize retired `/ferrox:<cmd>` colon refs in the agent body
        // to the canonical hyphen form `/ferrox-<cmd>` for hyphen-`name:`
  ...
  const changelogDest = path.join(targetDir, 'ferrox-core', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'ferrox-core', 'VERSION');
  ...
  function installSharedHooksBundle(destRootDir) {
    ...
    // Write package.json to force CommonJS mode for Ferrox scripts
    ...
    if (fs.existsSync(hooksSrc)) {
      ...
      // Managed-hooks whitelist. A dev tree's hooks/dist may contain orphaned
      // files from a prior build (e.g. pre-rename hooks from an earlier brand)
      // because the build historically did not prune dist. Installing via a
      // blind readdir-all would ship those orphans — unstamped {{FERROX_VERSION}}
      // placeholders that require() paths absent from a Ferrox install. Filter
      // to the managed set so we never ship unmanaged artifacts, even from a
      // dirty tree. Source of truth is scripts/build-hooks.js's exported
      // whitelist (kept in lockstep with what the build copies); fall back to
      // a ferrox-* prefix filter if that module is unavailable in the package.
      let MANAGED_HOOK_FILES = null;
      let MANAGED_HOOK_SUBDIRS = null;
      try {
        const bh = require(path.join(src, 'scripts', 'build-hooks.js'));
        if (Array.isArray(bh.HOOKS_TO_COPY)) MANAGED_HOOK_FILES = new Set(bh.HOOKS_TO_COPY);
        if (Array.isArray(bh.HOOKS_SUBDIRS_TO_COPY)) MANAGED_HOOK_SUBDIRS = new Set(bh.HOOKS_SUBDIRS_TO_COPY);
      } catch (_) { /* fall back to prefix filter below */ }
      const isManagedHookFile = (name) => {
        if (MANAGED_HOOK_FILES) return MANAGED_HOOK_FILES.has(name);
        // Fallback: managed hooks are ferrox-* plus the registry helper they require.
        return name.startsWith('ferrox-') || name === 'managed-hooks-registry.cjs';
      };
      const isManagedHookSubdir = (name) => {
        if (MANAGED_HOOK_SUBDIRS) return MANAGED_HOOK_SUBDIRS.has(name);
        return name === 'lib';
      };
      // For whitelisted subdirs (lib/), the authoritative file set is the
      // package's SOURCE hooks/<subdir>/ — copy only files present there, so a
      // stale pre-rename lib orphan is never shipped while legitimate helpers
      // (git-cmd.js, ferrox-graphify-rebuild.sh) still install.
      const managedSubdirFileSet = (subdir) => {
        const srcSub = path.join(src, 'hooks', subdir);
        try { return new Set(fs.readdirSync(srcSub)); } catch (_) { return null; }
      };

      for (const entry of hookEntries) {
        const srcFile = path.join(hooksSrc, entry);
        if (fs.statSync(srcFile).isFile()) {
          if (!isManagedHookFile(entry)) continue; // skip orphaned/unmanaged files
          ...
          if (entry.endsWith('.js') || entry.endsWith('.cjs')) {
            ...
            // #376: rewrite ferrox: → ferrox- for hyphen-namespace runtimes
            ...
            // non-.js: .sh hooks need {{FERROX_VERSION}} stamped; others are copied as-is
          ...
          // hooks/dist/lib/ferrox-graphify-rebuild.sh and must land at the
          // mirrored target path so the hook's REBUILD_SCRIPT lookup resolves.
          if (!isManagedHookSubdir(entry)) continue; // skip unmanaged subdirs
          const subDest = path.join(hooksDest, entry);
          fs.mkdirSync(subDest, { recursive: true });
          const subEntries = fs.readdirSync(srcFile);
          const allowedSubFiles = managedSubdirFileSet(entry);
          for (const subEntry of subEntries) {
            // Only ship files that exist in the package's source subdir — this
            // drops stale pre-rename orphans while keeping legitimate helpers.
            // If the source subdir is unreadable, fall back to copying all
            // (preserves prior behavior).
            if (allowedSubFiles && !allowedSubFiles.has(subEntry)) continue;
      ...
      if (verifyInstalled(hooksDest, 'hooks')) {
        ...
        const expectedShHooks = ['ferrox-session-state.sh', 'ferrox-validate-commit.sh', 'ferrox-phase-boundary.sh', 'ferrox-graphify-update.sh'];
    ...
    if (fs.existsSync(hooksLibSrc)) {
      ...
      copyLibDir(hooksLibSrc, hooksLibDest, FERROX_HOOK_LIB_FILES);
  ...
  // its native plugin adapter (#1914, installed above under plugins/ferrox-core.js)
  ...
  // Agent-Skills configDir Ferrox installs skills/agents into for kimi.
  // #2099: Copilot's exclusion is likewise descriptor-driven (copilot declares
  // skipSharedHooksInstall:true) — the redundant `&& !isCopilot` was removed.
  // #2100: Windsurf's exclusion is likewise descriptor-driven (windsurf declares
  // skipSharedHooksInstall:true) — the redundant `&& !isWindsurf` was removed.
  ...
  // so that `node "$FERROX_DIR/scripts/changeset/cli.cjs"` resolves at runtime.
  //
  // The changeset CLI (scripts/changeset/cli.cjs) is invoked by the update
  // workflow (ferrox-core/workflows/update.md) to extract changelog ranges for
  // the /ferrox-update preview step. It was previously only present in the npm
  ...
  //   - ../../ferrox-core/bin/lib/semver-compare.cjs  (already installed under ferrox-core/)
  //   - ../../ferrox-core/bin/lib/package-identity.cjs (already installed under ferrox-core/)
  //
  // All runtimes that use the update workflow need this, so we copy unconditionally
  // (same scope as ferrox-core/ itself — every runtime that installs workflows gets it).
  ...
  // Copy scripts/fix-slash-commands.cjs — required by ferrox-core/bin/lib/command-roster.cjs
  // at load time via require('../../../scripts/fix-slash-commands.cjs'). Without this file
  // every ferrox-tools command crashes with MODULE_NOT_FOUND (#1223).
  ...
  // required by ferrox-core/bin/lib/capability-loader.cjs at overlay-composition time via
  ...
  // any runtime that installs ferrox-core/ needs the capability system to compose.
  {
  ...
  try {
    cleanupLegacyFerroxCc({ dryRun: false });
  } catch (cleanupErr) {
  ...
  if (plan.installSurface === 'codex-toml' && !isMinimalMode(_effectiveInstallMode)) {
    // Capture pre-install snapshots before ANY Ferrox mutation
    ...
    //   skills/ferrox-* — restore pre-existing dirs from content snapshot; remove
    //                   newly-created dirs (i.e. those not in the pre-install Set)
    //   agents/ferrox-* — restore pre-existing files from content snapshot; remove
    //                   newly-created files
    //   ferrox-core/VERSION — restore or remove
    ...
    // default to empty Set / null). Does NOT touch non-ferrox-* user content.
    const restoreCodexSnapshot = () => {
      ...
      //     (copyCommandsAsCodexSkills removes pre-existing ferrox-* dirs before re-writing)
      ...
      // Pass 2 — remove any newly-created ferrox-* dirs (not in the pre-install snapshot).
      ...
      // Pass 2 — remove any newly-created ferrox-* agent files (not in the pre-install snapshot).
      if (fs.existsSync(_rollbackAgentsDir)) {
        try {
          for (const file of fs.readdirSync(_rollbackAgentsDir)) {
            if (!file.startsWith('ferrox-') || (!file.endsWith('.md') && !file.endsWith('.toml'))) continue;
      ...
      // 4. ferrox-core/VERSION
      const _rollbackVersionPath = path.join(targetDir, 'ferrox-core', 'VERSION');
      if (codexPreInstallVersionBytes !== null) {
    ...
    if (!isMinimalMode(_effectiveInstallMode)) {
      ...
      // not include agents/ferrox-*.toml because those files did not yet exist.
      writeManifest(targetDir, runtime, { mode: _effectiveInstallMode, scope: isGlobal ? 'global' : 'local' });
    } else {
    ...
    // #772: added ferrox-context-monitor.js for the new SubagentStart/Stop/PostToolUse events.
    // We deliberately do *not* copy ferrox-graphify-update.sh or hooks/lib/ for Codex
    // in this change (graphify auto-update support for Codex is out of scope for #3579).
    // REACH-CODEX-01: ferrox-merge-gate-guard.js is copied so the strength
    // merge-gate can be enforced at Codex's PreToolUse layer (parity with Claude
    // Code's FF-B10 guard). Registration on PreToolUse follows below; the guard's
    // shell-command extraction is runtime-agnostic (handles Codex 'shell'/argv).
    const CODEX_HOOKS_TO_COPY = ['ferrox-check-update.js', 'ferrox-context-monitor.js', 'ferrox-merge-gate-guard.js'];
    ...
    if (fs.existsSync(codexHooksSrc)) {
      ...
      for (const entry of fs.readdirSync(codexHooksSrc)) {
        ...
        if (entry.endsWith('.js')) {
          ...
          // #2136: any .sh hook reaching this loop must have {{FERROX_VERSION}}
          ...
          // extended later (e.g. to ship ferrox-graphify-update.sh for Codex).
          let content = fs.readFileSync(srcFile, 'utf8');
          content = content.replace(/\{\{FERROX_VERSION\}\}/g, pkg.version);
          fs.writeFileSync(destFile, content);
          try { fs.chmodSync(destFile, 0o755); } catch (e) { /* Windows */ }
    ...
    // restore returns the file to its true pre-Ferrox state on validation
    ...
    try {
      ...
      // Strip ALL prior Ferrox-managed hook blocks BEFORE migration so the migration
      // only touches user-authored hooks, not Ferrox-owned stale entries. Running
      // strip after migration causes Shape 1 (legacy ferrox-update-check filename)
      // to be converted by migration before the strip regex can match it (#2698).
      //
      // Historical shapes stripped, in order:
      //   Shape 1 — legacy ferrox-update-check filename (pre-#1755): flat [[hooks]] + event
      ...
      configContent = stripStaleFerroxHookBlocks(configContent);

      // Migrate legacy [hooks] map format and flat [[hooks]] AoT entries to the
      // namespaced [[hooks.<EVENT>]] form after stripping Ferrox-managed stale blocks.
      // Running migration after strip ensures only user-authored hooks are migrated
      // (#2698 regression: migration before strip converts stale Ferrox blocks before
      ...
      // Ferrox-managed Codex hook payloads now live in hooks.json to avoid mixed
      ...
      if (hasEnabledCodexHooksFeature(configContent)) {
        const checkUpdateFile = path.join(targetDir, 'hooks', 'ferrox-check-update.js');
        if (!fs.existsSync(checkUpdateFile)) {
          console.warn(`  ${yellow}⚠${reset}  Skipped Codex SessionStart hook registration — ferrox-check-update.js not found at target`);
        ...
        // Codex CLI stabilised a full hook-event set in rust-v0.137.0. Ferrox
        // registers CODEX_EXTENDED_HOOK_EVENTS (#2088 adds the 6 documented
        // events beyond the original #772 three) — all routed through
        // ferrox-context-monitor.js so context-headroom warnings surface at each
        ...
        const contextMonitorFile = path.join(targetDir, 'hooks', 'ferrox-context-monitor.js');
        ...
        // ── REACH-CODEX-01 / FF-B24 — merge-gate guard on PreToolUse ──────────
        // Register ferrox-merge-gate-guard.js on PreToolUse ALONGSIDE the
        // context-monitor (preserveOtherManaged keeps both). Runs AFTER the
        // context-monitor loop so both survive; the guard's shell-command
        // extraction is runtime-agnostic (handles Codex 'shell'/argv). The guard
        // self-filters to merge/ship/release ops, so an unmatched (all-tools)
        // registration is a no-op for ordinary work and blocks (exit 2) merges
        // lacking gate evidence.
        const mergeGateGuardFile = path.join(targetDir, 'hooks', 'ferrox-merge-gate-guard.js');
        if (codexNodeRunner && fs.existsSync(mergeGateGuardFile)) {
          const guardWrite = ensureCodexHooksJsonEvent(targetDir, 'PreToolUse', {
            absoluteRunner: codexNodeRunner,
            platform: process.platform,
            scriptName: 'ferrox-merge-gate-guard.js',
            preserveOtherManaged: true,
            timeout: 10,
          });
          if (guardWrite.wrote) {
            console.log(`  ${green}✓${reset} Configured Codex merge-gate guard (PreToolUse via hooks.json)`);
          } else if (guardWrite.changed) {
            console.log(`  ${green}✓${reset} Verified Codex merge-gate guard (PreToolUse via hooks.json)`);
          }
        }
  ...
  if (plan.installSurface === 'copilot-instructions') {
    ...
    const templatePath = path.join(targetDir, 'ferrox-core', 'templates', 'copilot-instructions.md');
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
  ...
  if (plan.installSurface === 'cursor-hooks-json') {
    ...
    // produced. NOTE: hooks.json and the ferrox-cursor-*.js scripts are NOT
  ...
  if (plan.installSurface === 'profile-marker-only') {
    ...
    if (plan.hooksSurface === 'kimi-hooks-toml' && isGlobal) {
      ...
      if (!installSharedHooksBundle(kimiHooksRoot)) {
        console.warn(`  ${yellow}⚠${reset}  Kimi hook bundle did not verify at ${path.join(kimiHooksRoot, 'hooks')} — Ferrox lifecycle hooks may be incomplete`);
      }
      ...
      if (kimiHooksResult.changed) {
        console.log(`  ${green}✓${reset} Configured ${kimiHooksResult.entryCount} Ferrox hook(s) in ${kimiHooksTomlPath}`);
      }
    }
    ...
    if (plan.hooksSurface === 'windsurf-hooks-json') {
      ...
      // ferrox-windsurf-*.js scripts (same as cursor): uninstall removes them
  ...
  if (plan.installSurface === 'cline-rules') {
    // Cline uses the `.clinerules/` directory form (issue #787): Ferrox rules live
    // at .clinerules/ferrox.md and a PreToolUse lifecycle hook at
  ...
  // #338 migration: if a prior local Claude install wrote Ferrox-shaped entries to settings.json,
  ...
  if (isLocalClaude) {
    ...
    if (sharedRaw && typeof sharedRaw === 'object') {
      const hasFerroxHooks = sharedRaw.hooks && Object.values(sharedRaw.hooks).some(
      ...
      const hasFerroxStatusline = sharedRaw.statusLine && sharedRaw.statusLine.command &&
        isManagedHookCommand(sharedRaw.statusLine.command, { surface: 'settings-json' });
      if (hasFerroxHooks || hasFerroxStatusline) {
        // Merge Ferrox entries into settings.local.json
        const localRaw = readSettings(settingsPath) || {};
        if (hasFerroxStatusline && !localRaw.statusLine) {
        ...
        // Remove Ferrox entries from shared settings.json
        if (hasFerroxStatusline) {
        ...
        console.log(`  ${green}✓${reset} Migrated Ferrox hook entries from settings.json to settings.local.json (#338)`);
      }
    }
  }
  ...
  const statuslineCommand = isGlobal
    ? buildHookCommand(targetDir, 'ferrox-statusline.js', hookOpts)
    : localCmd('ferrox-statusline.js');
  const updateCheckCommand = isGlobal
    ? buildHookCommand(targetDir, 'ferrox-check-update.js', hookOpts)
    : localCmd('ferrox-check-update.js');
  const contextMonitorCommand = isGlobal
    ? buildHookCommand(targetDir, 'ferrox-context-monitor.js', hookOpts)
    : localCmd('ferrox-context-monitor.js');
  const promptGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'ferrox-prompt-guard.js', hookOpts)
    : localCmd('ferrox-prompt-guard.js');
  const readGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'ferrox-read-guard.js', hookOpts)
    : localCmd('ferrox-read-guard.js');
  const readInjectionScannerCommand = isGlobal
    ? buildHookCommand(targetDir, 'ferrox-read-injection-scanner.js', hookOpts)
    : localCmd('ferrox-read-injection-scanner.js');
  ...
  // Register all Ferrox-managed hook entries into settings.hooks.* for runtimes
  ...
  const updateBannerCommand = _hostBehaviors(runtime).skipUpdateBannerCommand
    ? null
    : (isGlobal
      ? buildHookCommand(targetDir, 'ferrox-update-banner.js', hookOpts)
      : localCmd('ferrox-update-banner.js'));
  ...
  if (isLocalClaude && settings !== null && typeof settings === 'object' && !Array.isArray(settings)) {
    ...
    // targetDir (same walk-up pattern as readFerroxRuntimeProfileResolver). Defaults
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = DEFAULT_RUNTIME, isGlobal = true, configDir = null, bannerOpts = {}) {
  ...
  if (shouldInstallBanner && settings && plan.writesSharedSettings && !_hostBehaviors(runtime).skipSettingsUi) {
    if (!bannerCommand) {
      ...
      const alreadyRegistered = settings.hooks.SessionStart.some(entry =>
        entry && entry.hooks && entry.hooks.some(h => h && referencesHook(h, 'ferrox-update-banner'))
      );
      const bannerHookFile = configDir ? path.join(configDir, 'hooks', 'ferrox-update-banner.js') : null;
      ...
      if (alreadyRegistered) {
        ...
        console.warn(`  ${yellow}⚠${reset}  Skipped update banner — ferrox-update-banner.js not found at target`);
      } else {
  ...
  // Merges Ferrox-owned entries non-destructively (preserves existing user permissions).
  ...
  if (_hostBehaviors(runtime).mcpCompanion === 'settings-json' && settings && plan.writesSharedSettings) {
    mergeFerroxMcpServerIntoSettings(settings);
  }
  ...
  if (plan.finishPermissionWriter === 'opencode' && !process.env.FERROX_TEST_MODE) {
  ...
  // Upgrades 1+2). Not FERROX_TEST_MODE-gated — mirrors Kilo's dispatch exactly;
  ...
  // For non-Claude runtimes, DEFAULT resolve_model_ids to "omit" in ~/.ferrox/defaults.json
  ...
  if (!_hostBehaviors(runtime).nativeModelAliases && !process.env.FERROX_TEST_MODE) {
    const ferroxDir = path.join(os.homedir(), '.ferrox');
    const defaultsPath = path.join(ferroxDir, 'defaults.json');
    try {
      fs.mkdirSync(ferroxDir, { recursive: true });
      let defaults = {};
      ...
      if (shouldDefaultToOmit) {
        ...
        console.log(`  ${green}✓${reset} Set resolve_model_ids: "omit" in ~/.ferrox/defaults.json`);
      }
    } catch (e) {
      console.log(`  ${yellow}⚠${reset} Could not write ~/.ferrox/defaults.json: ${e.message}`);
    }
  }

  // program + command are now single-source lookups (ADR-1239 Phase B / #1679):
  // program is the runtime display label; command is the per-host /ferrox-new-project
  // invocation syntax.
...
function handleStatusline(settings, isInteractive, callback) {
  ...
  Ferrox includes a statusline showing:
    • Model name
    • Current task (from todo list)
    • Context window usage (color-coded)

  ${cyan}1${reset}) Keep existing
  ${cyan}2${reset}) Replace with Ferrox statusline
`);
...
function buildUpdateBannerPromptText() {
  return `
  ${yellow}Optional: Ferrox update banner${reset}
  Without Ferrox's statusline, update notifications won't be visible. You can
  install a SessionStart banner that surfaces a one-line message when a new
  Ferrox release is available. The banner appears only at session start and
 ...
 * update banner. Used by `installAllRuntimes` only when Ferrox's statusline
 ...
 */
function cleanupLegacyFerroxCc({ homeDir = os.homedir(), dryRun = false, logger = console } = {}) {
  ...
  // shared cache (ferrox-update-check.json) as a plan entry.
  ...
  const perPkgCacheFile = path.join(homeDir, '.cache', 'ferrox', updateCacheFileName);
  if (dryRun) {
...
/**
 * Install Ferrox for all selected runtimes
 */
function installAllRuntimes(runtimes, isGlobal, isInteractive) {
  const results = [];
  const installerMigrations = discoverInstallerMigrations({
    migrationsDir: path.join(_ferroxLibDir, 'installer-migrations'),
  });
...
// The main() block below is gated on !FERROX_TEST_MODE, as before.
module.exports = {
    ...
    stripFerroxFromCodexConfig,
    migrateCodexHooksMapFormat,
    stripStaleFerroxHookBlocks,
    hasUserNamespacedAotHooks,
    parseTomlToObject,
    validateCodexConfigSchema,
    mergeCodexConfig,
    installCodexConfig,
    readFerroxRuntimeProfileResolver,
    readFerroxEffectiveModelOverrides,
    readFerroxEffectiveEffortConfig,
    resolveInstallTimeEffort,
    injectEffortFrontmatter,
    get _FERROX_EFFORT_MANIFEST_TIER_DEFAULTS() { return _getFerroxEffortCatalog().EFFORT_MANIFEST_TIER_DEFAULTS; },
    get _FERROX_EFFORT_MANIFEST_DEFAULT() { return _getFerroxEffortCatalog().EFFORT_MANIFEST_DEFAULT; },
    install,
    installAllRuntimes,
    uninstall,
    ...
    FERROX_CLAUDE_ALLOW_PERMISSIONS,
    FERROX_CLAUDE_DENY_PERMISSIONS,
    FERROX_CODEX_MARKER,
    CODEX_AGENT_SANDBOX,
    getDirName,
    getGlobalDir,
    ...
    mergeFerroxMcpServerIntoSettings,
    claudeToCopilotTools,
    convertCopilotToolName,
    convertClaudeToCopilotContent,
    convertClaudeCommandToCopilotSkill,
    convertClaudeAgentToCopilotAgent,
    FERROX_COPILOT_INSTRUCTIONS_MARKER,
    FERROX_COPILOT_INSTRUCTIONS_CLOSE_MARKER,
    mergeCopilotInstructions,
    stripFerroxFromCopilotInstructions,
    FERROX_COPILOT_HOOK_FILE,
    ...
    FERROX_CURSOR_SUBAGENT_START_HOOK_SCRIPT,
    FERROX_CURSOR_SUBAGENT_STOP_HOOK_SCRIPT,
    FERROX_CURSOR_HOOK_SCRIPTS,
    FERROX_CURSOR_HOOK_MARKER,
    ...
    FERROX_WINDSURF_PRE_WRITE_HOOK_SCRIPT,
    FERROX_WINDSURF_PRE_COMMAND_HOOK_SCRIPT,
    FERROX_WINDSURF_HOOK_SCRIPTS,
    writeWindsurfHooksJson,
    removeWindsurfHooksJson,
    stripFerroxFromAgentsMd,
    FERROX_AGENTS_MD_MARKER,
    FERROX_AGENTS_MD_CLOSE_MARKER,
    writeManifest,
    saveLocalPatches,
    reportLocalPatches,
    validateHookFields,
...
if (require.main === module && !process.env.FERROX_TEST_MODE) {
  if (hasDryRun) {
    ...
    // cleanupLegacyFerroxCc with dryRun:true is the single source of truth for
    // both the legacy artifacts and the per-package cache path — no duplicate
    // printing here.
    const { plan } = cleanupLegacyFerroxCc({ dryRun: true });
    if (plan.length === 0) {
      console.log('  (no legacy get-shit-done-cc artifacts found)');
    }
    process.exit(0);
  } else if (hasSkillsRoot) {
    // Print the skills root directory for a given runtime (used by /ferrox-sync-skills).
...
} // end of !FERROX_TEST_MODE main logic block
