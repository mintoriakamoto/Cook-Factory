#!/usr/bin/env bash
# ferrox-graphify-rebuild.sh — detached rebuild runner for hooks/ferrox-graphify-update.sh.
#
# Usage:
#   ferrox-graphify-rebuild.sh <STATUS_FILE> <LOCK_FILE> <HEAD_SHA> <MS_START> <GRAPHIFY_BIN>
#
...
FERROX_STATUS_TS="$TS_END" \
FERROX_STATUS_NAME="$STATUS_NAME" \
FERROX_EXIT_CODE="$EXIT_CODE" \
FERROX_DURATION="$DURATION" \
FERROX_HEAD_SHA="$HEAD_SHA" \
FERROX_STATUS_FILE="$STATUS_FILE" \
node -e '
  const fs = require("node:fs");
  const status = {
    ts: process.env.FERROX_STATUS_TS,
    status: process.env.FERROX_STATUS_NAME,
    exit_code: parseInt(process.env.FERROX_EXIT_CODE, 10),
    duration_ms: parseInt(process.env.FERROX_DURATION, 10),
    head_at_build: process.env.FERROX_HEAD_SHA,
    graphify_version: null,
  };
  fs.writeFileSync(process.env.FERROX_STATUS_FILE, JSON.stringify(status, null, 2) + "\n");
' 2>/dev/null || true
