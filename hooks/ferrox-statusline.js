...
// Claude Code Statusline - Ferrox Edition
// Shows: model | current task (or Ferrox state) | directory | context usage
...
const { isSemverNewer } = require('../ferrox-core/bin/lib/semver-compare.cjs');
const { PACKAGE_NAME, updateCacheFileName } = require('../ferrox-core/bin/lib/package-identity.cjs');
...
function readLastSlashCommand(transcriptPath) {
  ...
  // Command names in Claude Code transcripts are plain identifiers like "ferrox-plan-phase"
 ...
 *
 * All new fields default to undefined when absent — formatFerroxState() degrades
...
/**
 * Format Ferrox state into display string.
 *
...
function runStatusline() {
  ...
  try {
    ...
    let cfg = {};
    try { cfg = readFerroxConfig(dir); } catch (e) {}
    ...
    // Ferrox state (milestone · status · phase) — shown when no todo task
    const ferroxStateStr = task ? '' : formatFerroxState(readFerroxState(dir) || {});
    ...
    let ferroxUpdate = '';
    const cacheFile = path.join(homeDir, '.cache', 'ferrox', updateCacheFileName);
    if (fs.existsSync(cacheFile)) {
      try {
        const cache = JSON.parse(fs.readFileSync(cacheFile, 'utf8'));
        const { showUpdate, staleWarning } = evaluateUpdateCache(cache);
        if (showUpdate) {
          ferroxUpdate = '\x1b[33m⬆ /ferrox:update\x1b[0m │ ';
        }
        if (staleWarning === 'dev') {
          ferroxUpdate += '\x1b[33m⚠ dev install — re-run installer to sync hooks\x1b[0m │ ';
        } else if (staleWarning === 'stale') {
          ferroxUpdate += '\x1b[31m⚠ stale hooks — run /ferrox:update\x1b[0m │ ';
        }
      } catch (e) {}
    }
    ...
    const middle = task
      ? `\x1b[1m${task}\x1b[0m`
      : ferroxStateStr
        ? `\x1b[2m${ferroxStateStr}\x1b[0m`
        : null;

    process.stdout.write(composeStatusline({ ferroxUpdate, model, ctx, middle, dirname, lastCmdSuffix, gitSuffix, position }));
  } catch (e) {
 ...
 *
 * @param {object} opts
 * @param {string} [opts.ferroxUpdate='']      - leading update/stale-hooks warning (already formatted)
 ...
 * @param {string|null} [opts.middle=null]  - middle segment (todo task or Ferrox state), null = absent
...
function composeStatusline({
  ...
  if (pos === 'front') {
    if (middle) return `${ferroxUpdate}${modelSeg}${ctx} │ ${middle} │ ${dirSeg}${gitSuffix}${lastCmdSuffix}`;
    return `${ferroxUpdate}${modelSeg}${ctx} │ ${dirSeg}${gitSuffix}${lastCmdSuffix}`;
  }
  // 'end' — preserved byte-for-byte relative to original inline templates
  if (middle) return `${ferroxUpdate}${modelSeg} │ ${middle} │ ${dirSeg}${gitSuffix}${ctx}${lastCmdSuffix}`;
  return `${ferroxUpdate}${modelSeg} │ ${dirSeg}${gitSuffix}${ctx}${lastCmdSuffix}`;
}
...
module.exports = {
  readFerroxState, parseStateMd, formatFerroxState,
  readFerroxConfig, getConfigValue, readLastSlashCommand,
  composeStatusline,
...
function renderStatusline(data) {
  ...
  let gitSuffix = '';
  try {
    const cfg = readFerroxConfig(dir);
    if (getConfigValue(cfg, 'statusline.show_last_command') === true) {
  ...
  const ferroxStateStr = formatFerroxState(readFerroxState(dir) || {});
  const middle = ferroxStateStr ? `\x1b[2m${ferroxStateStr}\x1b[0m` : null;
