...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Detect if Ferrox is active (has .planning/STATE.md in working directory)
    const isFerroxActive = fs.existsSync(path.join(cwd, '.planning', 'STATE.md'));

    // On CRITICAL with active Ferrox project, auto-record session state as a
    // breadcrumb for /ferrox:resume-work (#1974). Fire-and-forget subprocess —
    ...
    if (isCritical && isFerroxActive && !warnData.criticalRecorded) {
      try {
        // Runtime-agnostic path: this hook lives at <runtime-config>/hooks/
        // and ferrox-tools.cjs lives at <runtime-config>/ferrox-core/bin/.
        // Using __dirname makes this work on Claude Code, OpenCode, Gemini,
        // Kilo, etc. without hardcoding ~/.claude/.
        const ferroxTools = path.join(__dirname, '..', 'ferrox-core', 'bin', 'ferrox-tools.cjs');
        ...
        spawn(
          process.execPath,
          [ferroxTools, 'state', 'record-session', '--stopped-at', stoppedAt],
    ...
    let message;
    if (isCritical) {
      message = isFerroxActive
        ? `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
          'Context is nearly exhausted. Do NOT start new complex work or write handoff files — ' +
          'Ferrox state is already tracked in STATE.md. Inform the user so they can run ' +
          '/ferrox:pause-work at the next natural stopping point.'
        : `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
