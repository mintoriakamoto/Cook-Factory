...
// SessionStart banner that surfaces Ferrox update availability when Ferrox's
// statusline isn't installed. Reads the cache that
// ferrox-check-update-worker.js writes to ~/.cache/ferrox/<updateCacheFileName> (per-package).
//
// Opt-in by design: bin/install.js only registers this hook when the user
// declines to install (or replace) the Ferrox statusline. The presence of the
...
function buildBannerOutput(state) {
  ...
  if (parseError) {
    ...
    return { systemMessage: 'Ferrox update check failed.' };
  }
  ...
  return {
    systemMessage: `Ferrox update available: ${installed} → ${latest}. Run /ferrox:update.`,
  };
}
...
function main() {
  const cacheDir = path.join(os.homedir(), '.cache', 'ferrox');
  ...
  if (parseError && !suppressFailureWarning) {
    ...
    // where ~/.cache/ferrox was created by check-update but the parent dir got
    // wiped between runs.
    try {
