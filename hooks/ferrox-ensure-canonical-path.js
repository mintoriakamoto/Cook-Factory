...
//
// ferrox-ensure-canonical-path — SessionStart hook (#997)
//
// PROBLEM: Ferrox agents/commands/templates use markdown `@`-file-includes that
// hardcode the canonical path `@~/.claude/ferrox-core/...` (references, workflows,
...
// `~/.claude/ferrox-core/` is never created and every @-include resolves to
...
// set and a bundled `ferrox-core/` tree found beneath it), ensure
// `~/.claude/ferrox-core/` exists and its immutable subdirs (bin, contexts,
...
// are the directories `@~/.claude/ferrox-core/<subdir>/...` includes point into.
 ...
 * ferrox-check-update.js detectConfigDir), else falls back to ~/.claude. The
 * canonical Ferrox tree always lives at `<configDir>/ferrox-core`.
 */
...
/**
 * Locate the bundled `ferrox-core/` tree beneath a plugin root.
 *
 * Claude Code unpacks the package so the bundled tree sits at
 * `<pluginRoot>/ferrox-core/`. Returns the absolute, realpath-normalised path to
...
function resolveBundledTree(pluginRoot) {
  ...
  const bundled = path.join(root, 'ferrox-core');
  let bundledReal;
  try {
    ...
    // ENOTDIR if <pluginRoot>/ferrox-core is absent, so no separate existence
  ...
  // root. A crafted symlink at <pluginRoot>/ferrox-core pointing outside the root
...
/**
 * Ensure the canonical `~/.claude/ferrox-core/` path exposes the bundled subdirs.
 *
...
function ensureCanonicalPath(opts = {}) {
  ...
  const canonicalDir = path.join(configDir, 'ferrox-core');
  ...
  for (const sub of MANAGED_SUBDIRS) {
    ...
    if (existing) {
      ...
      // junctions, so this single predicate identifies every Ferrox-managed link.
      if (existing.isSymbolicLink()) {
        // A Ferrox-managed link that is stale or points elsewhere (e.g. previous
