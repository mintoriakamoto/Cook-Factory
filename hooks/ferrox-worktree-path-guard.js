...
// Ferrox Worktree Path Guard — PreToolUse hook
// Blocks Edit/Write/MultiEdit tool calls that target absolute paths outside the worktree root.
//
// Problem: ferrox-executor agents spawned with isolation="worktree" sometimes issue
// Edit/Write calls with absolute paths rooted at the MAIN repository instead of
// the worktree (issue #260). The prose guard in agents/ferrox-executor.md step 0b
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // #1342: Only enforce inside a Ferrox-managed isolated executor worktree. Those
    ...
    // non-Ferrox work, e.g. Claude Code plan-mode) is on the user's own branch, so the
    // guard must be a no-op there. Detached HEAD / error → not Ferrox-managed → no-op.
    ...
    if (!/^worktree-agent-[A-Za-z0-9._/-]+$/.test(branch)) {
      process.exit(0); // not a Ferrox-managed executor worktree — no-op
    }
