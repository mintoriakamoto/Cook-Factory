...
# ferrox-session-state.sh — SessionStart hook: inject project state reminder
...
node -e '
  ...
  if (statePresent === "true") {
    ...
    headerLines.push("No .planning/ found - suggest /ferrox-new-project if starting new work.");
  }
  headerLines.push("");
