...
// ferrox-config-reload.js — FileChanged hook: hot-reload Ferrox config context
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Only handle the Ferrox planning config — verify both basename and that the
    ...
    if (event === 'unlink') {
      process.stdout.write(JSON.stringify({
        hookSpecificOutput: {
          ...
          additionalContext:
            'Ferrox config (.planning/config.json) was deleted. ' +
    ...
    try {
      ...
      process.stdout.write(JSON.stringify({
        hookSpecificOutput: {
          hookEventName: 'FileChanged',
          additionalContext:
            'Ferrox config (.planning/config.json) was modified but could not be parsed. ' +
    ...
    const lines = ['Ferrox config reloaded (.planning/config.json updated):'];
