...
# ferrox-graphify-update.sh — PostToolUse hook (Bash matcher) that auto-rebuilds
...
#      the exact `ferrox-tools query commit` command shape; the SDK command invokes
...
#   - Detaches hooks/lib/ferrox-graphify-rebuild.sh which copies graphify-out/* to
...
# Gate 2 — HEAD-advancing git op (shell-direct or exact `ferrox-tools query commit`)
case "$COMMAND" in
  *"git commit"*|*"git merge"*|*"git pull"*|*"git rebase --continue"*|*"git cherry-pick"*) ;;
  *"ferrox-tools query commit"|*"ferrox-tools query commit "*) ;;
  *) exit 0 ;;
esac
...
FERROX_TS="$TS_START" \
FERROX_HEAD="$HEAD_SHA" \
FERROX_STATUS_FILE="$STATUS_FILE" \
node -e '
  const fs = require("node:fs");
  const status = {
    ts: process.env.FERROX_TS,
    status: "running",
    exit_code: null,
    duration_ms: null,
    head_at_build: process.env.FERROX_HEAD,
...
REBUILD_SCRIPT="$HOOK_DIR/lib/ferrox-graphify-rebuild.sh"
