...
// Ferrox Workflow Guard — PreToolUse hook
// Detects when Claude attempts file edits outside a Ferrox workflow context
// (no active /ferrox- skill or Task subagent) and injects an advisory warning.
//
// This is a SOFT guard — it advises, not blocks. The edit still proceeds.
// The warning nudges Claude to use /ferrox:quick or /ferrox:fast instead of
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Check if we're inside a Ferrox workflow (Task subagent or /ferrox- skill)
    ...
    // Allow edits to .planning/ files (Ferrox state management)
    if (filePath.includes('.planning/') || filePath.includes('.planning\\')) {
      process.exit(0);
    }

    // Allow edits to common config/docs files that don't need Ferrox tracking
    ...
    if (!isWorkflowGuardEnabled) {
      process.exit(0); // Guard disabled (default) or no Ferrox project
    }

    // If we get here: Ferrox project, guard enabled, file edit outside .planning/,
    // not in a subagent context. Inject advisory warning.
    const output = {
      hookSpecificOutput: {
        hookEventName: "PreToolUse",
        additionalContext: `⚠️ WORKFLOW ADVISORY: You're editing ${path.basename(filePath)} directly without a Ferrox command. ` +
          'This edit will not be tracked in STATE.md or produce a SUMMARY.md. ' +
          'Consider using /ferrox:fast for trivial fixes or /ferrox:quick for larger changes ' +
