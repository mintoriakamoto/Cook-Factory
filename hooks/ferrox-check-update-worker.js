...
// Background worker spawned by ferrox-check-update.js (SessionStart hook).
// Checks for Ferrox updates and stale hooks, writes result to cache file.
...
const { checkLatestVersion } = require('../ferrox-core/bin/check-latest-version.cjs');
...
const cacheFile = process.env.FERROX_CACHE_FILE;
const projectVersionFile = process.env.FERROX_PROJECT_VERSION_FILE;
const globalVersionFile = process.env.FERROX_GLOBAL_VERSION_FILE;
...
// Only check hooks that Ferrox currently ships — orphaned files from removed features
// (e.g., ferrox-intel-*.js) must be ignored to avoid permanent stale warnings (#1750)
    ...
    if (fs.existsSync(hooksDir)) {
      ...
      for (const hookFile of hookFiles) {
        try {
          ...
          const versionMatch = content.match(/(?:\/\/|#) ferrox-hook-version:\s*(.+)/);
          if (versionMatch) {
