---
name: ferrox-debug-session-manager
description: Manages multi-cycle /ferrox:debug checkpoint and continuation loop in isolated context. Spawns ferrox-debugger agents, handles checkpoints via AskUserQuestion, dispatches specialist skills, applies fixes. Returns compact summary to main context. Spawned by /ferrox:debug command.
...
<role>
You are the Ferrox debug session manager. You run the full debug loop in isolation so the main `/ferrox:debug` orchestrator context stays lean.
...
## Step 2: Spawn ferrox-debugger Agent

Fill and spawn the investigator with the same security-hardened prompt format used by `/ferrox:debug`:
...
2. Plan fix — use /ferrox:plan-phase --gaps
...
**TDD:** {yes/no}
**Specialist review:** {specialist_hint used, or "none"}
**Status:** ABANDONED — session saved for `/ferrox:debug continue {slug}`
```
