...
<required_reading>
@~/.claude/ferrox-core/references/verification-overrides.md
...
@~/.claude/ferrox-core/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@~/.claude/ferrox-core/references/few-shot-examples/verifier.md
...
ferrox_run query roadmap.get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(ferrox_run query roadmap.get-phase "$PHASE_NUM" --raw)
```
...
Use `ferrox-tools query` for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(ferrox_run query verify.artifacts "$PLAN_PATH")
```
...
Use `ferrox-tools query` for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(ferrox_run query verify.key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(ferrox_run query summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(ferrox_run query verify.commits $COMMIT_HASHES)
fi
...
> **Shared status seam**: the status vocabulary (`passed`, `gaps_found`, `human_needed`) and the per-status routing (next action and next command for each value) are owned by `src/verification.cts` via `ferrox_run query verification.status`. This agent is the single emitter of the frontmatter status field; consumers (ship.md, execute-phase.md) read routing from that query instead of re-deriving it.
...
```bash
ROADMAP_DATA=$(ferrox_run query roadmap.analyze --raw)
```
...
Structure gaps in YAML frontmatter for `/ferrox:plan-phase --gaps`:
...
**When the phase under verification has `mode: mvp` in ROADMAP.md (resolved by the verify-work workflow):** Apply the goal-backward methodology, narrowed to the phase's user-story goal. Required reading: `@~/.claude/ferrox-core/references/verify-mvp-mode.md`.
...
If `valid != true`, refuse to verify. Surface the discrepancy and ask the user to run `/ferrox mvp-phase ${PHASE}` to set a proper User Story goal. The verb owns the canonical regex `/^As a .+, I want to .+, so that .+\.$/` and surfaces per-error guidance in `errors[]` plus slot extractions in `slots`. Do NOT attempt to verify against a non-User Story goal under MVP mode — the User Flow Coverage section would be low-quality.
...
Structured gaps in VERIFICATION.md frontmatter for `/ferrox:plan-phase --gaps`.
