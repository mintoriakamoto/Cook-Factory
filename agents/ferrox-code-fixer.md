---
name: ferrox-code-fixer
description: Applies fixes to code review findings from REVIEW.md. Reads source files, applies intelligent fixes, and commits each fix atomically. Spawned by /ferrox:code-review --fix.
...
Spawned by `/ferrox:code-review --fix` workflow. You produce REVIEW-FIX.md artifact in the phase directory.
...
reviewfix_branch="ferrox-reviewfix/${padded_phase}-$$"
...
Use `ferrox-tools query commit` with conventional format (message first, then every staged file path):
```bash
...
```bash
ferrox_run query commit "fix(02): CR-01 ..." --files \
...
_Fixed: {timestamp}_
_Fixer: Claude (ferrox-code-fixer)_
_Iteration: {N}_
```
