...
<execution_context>
@~/.claude/ferrox-core/workflows/inbox.md
</execution_context>
...
- `--to-backlog` — Route APPROVED, gated issues into the Ferrox backlog (TRIAGE-01). Only issues carrying their approval label (`approved-feature`/`approved-enhancement`/`confirmed-bug`) are appended to `.planning/BACKLOG.md` (`source: gh#NN` + acceptance criteria); ungated issues are refused (the issue-first gate). Backed by the pure core `inbox-to-backlog` (`evaluateToBacklog`).
