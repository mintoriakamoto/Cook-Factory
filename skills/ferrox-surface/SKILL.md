...
<objective>
Manage the runtime skill surface without reinstall. Reads/writes `~/.claude/.ferrox-surface.json`
(sibling to `~/.claude/.ferrox-profile`) and re-stages the active skills directory in place.
Skill dirs live at `~/.claude/skills/ferrox-*/`.
...
| `reset` | Delete `.ferrox-surface.json`, return to install-time profile |
| *(none)* | Treat as `list` |
...
the engine module at `${runtimeConfigDir}/ferrox-core/bin/lib/surface.cjs`. The registry is loaded via:
```js
const registry = require(runtimeConfigDir + '/ferrox-core/bin/lib/capability-registry.cjs');
```
Display:
...
```
Base profile:   standard  (from .ferrox-surface.json)
Install profile: standard  (from .ferrox-profile)
```
...
1. Check if `.ferrox-surface.json` exists.
2. Delete it.
...
`~/.claude/skills/ferrox-*/` because the `claude global` layout has `destSubpath =
...
Surface state is stored at `${RUNTIME_CONFIG_DIR}/.ferrox-surface.json`
(i.e. `~/.claude/.ferrox-surface.json`).
...
- Missing `surface.cjs` → prompt: "Run `npm i -g ferrox-core` to reinstall Ferrox."

<execution_context>
Surface state file: `~/.claude/.ferrox-surface.json`
Install profile marker: `~/.claude/.ferrox-profile`
Skill dirs: `~/.claude/skills/ferrox-*/`
Engine module: `~/.claude/ferrox-core/bin/lib/surface.cjs`
Cluster definitions: `~/.claude/ferrox-core/bin/lib/clusters.cjs`
</execution_context>
