...
<objective>
Manage Ferrox workspaces with a single consolidated command.

Mode routing:
- **--new**: Create an isolated workspace with repo copies and independent .planning/ → new-workspace workflow
- **--list**: List active Ferrox workspaces and their status → list-workspaces workflow
- **--remove**: Remove a Ferrox workspace and clean up worktrees → remove-workspace workflow
</objective>
...
| --list | Scan ~/ferrox-workspaces/, show summary table | list-workspaces |
| --remove | Confirm and remove workspace directory | remove-workspace |

</routing>

<execution_context>
@~/.claude/ferrox-core/workflows/new-workspace.md
@~/.claude/ferrox-core/workflows/list-workspaces.md
@~/.claude/ferrox-core/workflows/remove-workspace.md
