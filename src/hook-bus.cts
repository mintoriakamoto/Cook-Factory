 ...
 *
 *   - `engine` — Ferrox owns the bus internally (in-process pub/sub). Used by
 *     hosts that have no event bus (VS Code). Full subscribe + emit.
 *   - `host`   — the host fires events; Ferrox subscribes. Handlers register
 *     locally for a Phase-5 host binding to dispatch to; `emit` delegates to a
 *     host-supplied emitter (fail-closed until bound — Ferrox does not drive a
 *     host-owned bus).
...
export interface CreateHookBusOptions {
  /** Required for `host`: the host's emit primitive (Ferrox emits → host bus). */
...
export function createHookBus(
  ...
  // host: Ferrox subscribes; emits go to the host-supplied emitter (fail-closed until bound).
  const hostEmit = options.hostEmit;
  return Object.freeze({
    bus: 'host',
    subscribe(_event: string, _handler: (payload?: unknown) => void) {
      // Host owns the bus; Ferrox's subscriptions are dispatched by a Phase-5 host
