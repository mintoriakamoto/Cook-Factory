'use strict';
/**
 * Graphify command router — CLI subcommand dispatcher for `ferrox-tools graphify`.
 *
 * ADR-959 (phase 4d-impl-2) pilot: first real capability command cutover.
 * Extracted from the hardcoded `case 'graphify':` arm in ferrox-tools.cjs.
  ...
  routeHubCommandFamily({
    ...
    handlers: {
      query: () => {
        const term = args[2];
        if (!term) {
          return makeInvalidArgs('term', 'Usage: ferrox-tools graphify query <term>', ERROR_REASON.USAGE);
        }
        ...
        if (budgetIdx !== -1) {
          ...
          if (rawBudget === undefined || Number.isNaN(parseInt(rawBudget, 10))) {
            return makeInvalidArgs(
              '--budget',
              'Usage: ferrox-tools graphify query <term> [--budget <N>]',
              ERROR_REASON.USAGE,
            );
          }
