...
function versionFile(dir: string): string { return path.join(dir, 'ferrox-core', 'VERSION'); }
function markerFile(dir: string): string { return path.join(dir, 'ferrox-core', 'workflows', 'update.md'); }
...
// Detection: a dir "has Ferrox" if it carries a VERSION file or the update.md
// workflow marker.
...
/**
 * Pure resolver. Returns { installedVersion, scope, runtime, ferroxDir }.
 */
export function resolveUpdateContext({
  ...
  if (isLocal) {
    return { installedVersion: localValid, scope: 'LOCAL', runtime: localRuntime, ferroxDir: localDir };
  }
  const globalValid = trustedVersionAt(fs, globalDir);
  if (globalValid) {
    return { installedVersion: globalValid, scope: 'GLOBAL', runtime: globalRuntime, ferroxDir: globalDir };
  }
  ...
  if (localRuntime && (!globalDir || localDir !== globalDir)) {
    return { installedVersion: '0.0.0', scope: 'LOCAL', runtime: localRuntime, ferroxDir: localDir };
  }
  if (globalRuntime) {
    return { installedVersion: '0.0.0', scope: 'GLOBAL', runtime: globalRuntime, ferroxDir: globalDir };
  }
  return { installedVersion: '0.0.0', scope: 'UNKNOWN', runtime: 'claude', ferroxDir: '' };
}
