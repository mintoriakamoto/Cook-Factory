 ...
 *
 * Compatibility routing for ferrox-tools.cjs command families. Uses generated
...
function routeHubCommandFamily({
  ...
  if (result.kind === ERROR_KINDS.UnknownCommand) {
    ...
    // JSON-error envelope (FERROX_JSON_ERRORS=1) preserves the typed reason
  ...
  if (result.kind === ERROR_KINDS.InvalidArgs) {
    ...
    // (FERROX_JSON_ERRORS=1) preserves the typed ERROR_REASON value for
