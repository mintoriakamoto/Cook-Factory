...
/**
 * #488 — Re-sync effort: frontmatter in all installed ferrox-*.md agent files to
 * match the current effort config, without requiring a full reinstall.
 *
 * Uses install-time resolution (readFerroxEffectiveEffortConfig + resolveInstallTimeEffort
 ...
 * The runtime resolver (loadConfig) does not merge ~/.ferrox/defaults.json when a project
...
function cmdEffortSync(cwd: string, raw: boolean, opts?: { dryRun?: boolean; configDir?: string; runtime?: string }): void {
  ...
  // Use install-time resolvers: they merge ~/.ferrox/defaults.json with project config,
  ...
  // eslint-disable-next-line @typescript-eslint/no-require-imports, @typescript-eslint/unbound-method
  const { readFerroxEffectiveEffortConfig, resolveInstallTimeEffort } = require('./install-effort-resolver.cjs') as {
    readFerroxEffectiveEffortConfig(cwd: string): Record<string, unknown>;
    resolveInstallTimeEffort(cfg: Record<string, unknown>, agentName: string): string;
  };
  const effortCfg = readFerroxEffectiveEffortConfig(cwd);
  ...
  const files = fs.readdirSync(agentsDir).filter(f => {
    if (!f.startsWith('ferrox-') || !f.endsWith('.md')) return false;
...
async function cmdWebsearch(query: string | undefined, options: WebsearchOptions, raw: boolean): Promise<void> {
  ...
  const rawTimeout = parseInt(process.env['FERROX_WEBSEARCH_TIMEOUT_MS'] as string, 10);
