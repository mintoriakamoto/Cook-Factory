 ...
 * migrations for Ferrox runtime config directories.
 *
...
const INSTALL_STATE_NAME = 'ferrox-install-state.json';
const INSTALL_MIGRATION_LOCK_NAME = 'ferrox-install-migration.lock';
...
function applyInstallerMigrationPlan({
  ...
  const journalRelPath = path.posix.join('ferrox-migration-journal', `${runId}.json`);
  const journalPath = path.join(configDir, journalRelPath);
  const rollbackRootRelPath = path.posix.join('ferrox-migration-journal', `${runId}-rollback`);
  const rollbackRoot = path.join(configDir, rollbackRootRelPath);
  const backupRootRelPath = path.posix.join('ferrox-migration-journal', `${runId}-backups`);
