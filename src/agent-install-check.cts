 ...
 *
 * Priority:
 *   1. FERROX_AGENTS_DIR env var (explicit override, any runtime)
 *   2. For claude runtime: __dirname-relative path (agents/ sibling of ferrox-core/)
 *      This is correct for both repo runs and real installs (the runtime config dir's
 *      agents/ folder) because ferrox-tools.cjs lives inside ferrox-core/bin/ in both cases.
 *   3. For non-claude runtimes: getGlobalConfigDir(runtime)/agents
 *
 * @param runtime - the active runtime name; defaults to FERROX_RUNTIME env, then 'claude'
 */
function getAgentsDir(runtime?: string): string {
  if (process.env['FERROX_AGENTS_DIR']) {
    return process.env['FERROX_AGENTS_DIR'];
  }
  const resolved = runtime ?? (process.env['FERROX_RUNTIME'] || 'claude');
  if (resolved === 'claude') {
...
/**
 * Check which Ferrox agents are installed on disk.
 *
 * @param runtime - the active runtime name; defaults to FERROX_RUNTIME env, then 'claude'
 */
function checkAgentsInstalled(runtime?: string): AgentsInstalledResult {
  const resolvedRuntime = runtime ?? (process.env['FERROX_RUNTIME'] || 'claude');
  ...
  // If a ferrox-file-manifest.json exists alongside the agents dir (parent dir),
  ...
  const manifestPath = path.join(path.dirname(agentsDir), 'ferrox-file-manifest.json');
