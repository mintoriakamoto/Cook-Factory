 ...
 *   - integrity + engines.ferrox are verified by the resolver BEFORE staging finalizes;
...
const consentMod = require('./capability-consent.cjs') as {
  recordProjectConsent: (args: { ferroxHome?: string; projectRoot: string; id: string; integrity: string; disclosureSignature: string; contentHash: string }) => void;
  revokeProjectConsent: (args: { ferroxHome?: string; projectRoot: string; id: string }) => void;
  ...
  consentStorePath: (ferroxHome?: string) => string;
};
...
interface LifecycleOptions {
  /** Scope root: holds .ferrox/capabilities/<id>, the ledger, and shared config files. */
  runtimeDir: string;
  hostVersion: string;
  /**
   ...
   * `ferrox capability update <id>` passes the requested id so a source that has been retargeted or
...
const CAP_MARKER = '_ferroxCapability';
...
function capabilitiesRoot(runtimeDir: string): string {
  return path.join(runtimeDir, '.ferrox', 'capabilities');
}
...
function capDataDir(runtimeDir: string, id: string): string {
  return path.join(runtimeDir, '.ferrox', 'capability-data', id);
}
...
/** A `.ferrox-capabilities.json.tmp.*` temp younger than this may belong to an in-flight write; spare it (W-3/DUR-5). */
...
/**
 * Acquire the capability-mutation lock (the single `.ferrox/capabilities/.lock` under runtimeDir),
 ...
 *
 * Lexical containment alone is insufficient: a tampered ledger could record `.ferrox/link/victim`
 * where `.ferrox/link` is a symlink to `/`, and a lexical check would pass while the delete escapes
 ...
 * serializes Ferrox's own operations, and the realpath check defeats the realistic persistent-symlink
 * vector.
 */
...
function bindProjectConsent(opts: LifecycleOptions, id: string, integrity: string, manifest: Record<string, unknown>): void {
  ...
  try {
    consentMod.recordProjectConsent({
      ferroxHome: opts.consentStoreDir,
    ...
    // `ferrox capability trust`), and let the op SUCCEED. The cap is simply inactive until consent writes.
    const storePath = (() => {
...
async function installCapability(spec: string, opts: LifecycleOptions): Promise<InstallResult> {
  ...
  // reason. The previous order called _resolve first (creating .ferrox/capabilities/.staging) and
  ...
  try {
    ...
    // A .ferrox/capabilities/constructor (or __proto__, prototype) bundle must never be promoted —
...
interface RemoveResult {
   ...
   * so the caller must report a NON-CLEAN removal and tell the user to clear it (`ferrox capability trust revoke`).
   */
...
function removeCapability(id: string, opts: LifecycleOptions): RemoveResult {
  ...
  // .ferrox/capabilities and a .lock file; doing it before detecting corruption pollutes the scope
  ...
  try {
    ...
    // longer exist — the user can re-run `ferrox capability remove <id>` to retry the commit (the
    ...
    try {
      ...
      return {
        ...
        blockReasons: [
          ...
          `To recover: run 'ferrox capability remove ${id}' again, or manually inspect and restore ` +
    ...
    if (shouldBindConsent(opts)) {
      try {
        ...
        consentMod.revokeProjectConsent({ ferroxHome: opts.consentStoreDir, projectRoot: projectRootMod.consentProjectRoot(runtimeDir), id });
      } catch (err) {
        ...
        consentRevokeWarning =
          ...
          `Clear it manually: ferrox capability trust revoke ${id}`;
...
function reconcileCapabilities(opts: { runtimeDir: string; scope?: 'global' | 'project'; consentStoreDir?: string }): ReconcileReport {
  ...
  try {
    ...
    if (ledger === null) {
      const ledgerFilePath = path.join(runtimeDir, '.ferrox-capabilities.json');
    ...
    // W-3 / DUR-5: sweep STALE ledger temp orphans (`.ferrox-capabilities.json.tmp.<pid>-<nonce>`) from
 ...
 *
 * @param opts.runtimeDir   the scope root holding `.ferrox-capabilities.json`.
...
export = {
  ...
  // re-export it (acquireLock here still takes a runtimeDir and computes the `.ferrox/capabilities/.lock`
