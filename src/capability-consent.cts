 ...
 *
 * A USER-OWNED store, living OUTSIDE any repository at `${FERROX_HOME||homedir()}/.ferrox/consent.json`,
 ...
 *
 * Exports:
 *   consentStorePath(ferroxHome?)            — resolve the store path (FERROX_HOME||homedir() rule).
 *   bundleContentHash(capDir)            — recomputed sha512 over the whole bundle (the binding).
 *   readConsentStore(ferroxHome?)            — bounded, NON-THROWING read; bad input → { records: {} }.
...
/**
 * Resolve the consent store path. Uses the SAME `ferroxHome || FERROX_HOME || homedir()` rule the loader
 ...
 */
function consentStorePath(ferroxHome?: string): string {
 ...
 */
function readConsentStore(ferroxHome?: string): ConsentStore {
  const empty: ConsentStore = { records: {} };
  const filePath = consentStorePath(ferroxHome);
  let raw: string | null;
  try {
...
function consentLockPath(ferroxHome?: string): string {
  return path.join(path.dirname(consentStorePath(ferroxHome)), '.consent.lock');
}
 ...
 */
function writeConsentStore(ferroxHome: string | undefined, store: ConsentStore): void {
  const filePath = consentStorePath(ferroxHome);
...
function recordProjectConsent(args: {
  ...
  const { ferroxHome, projectRoot, id, integrity, disclosureSignature, contentHash } = args;
  ...
  const lockDir = path.dirname(consentStorePath(ferroxHome));
  ...
  if (lock === null) {
    throw new Error(
      `recordProjectConsent: could not acquire the consent-store lock at ${consentLockPath(ferroxHome)} ` +
  ...
  try {
    ...
    if (!Object.prototype.hasOwnProperty.call(store.records, key) && Object.keys(store.records).length >= MAX_RECORDS) {
      throw new Error(
        ...
        `unused consent (ferrox capability trust revoke) before recording a new one.`,
      );
    }
    store.records[key] = {
    ...
    writeConsentStore(ferroxHome, store);
  } finally {
 ...
 */
function revokeProjectConsent(args: { ferroxHome?: string; projectRoot: string; id: string }): void {
  const { ferroxHome, projectRoot, id } = args;
  ...
  if (lock === null) {
    throw new Error(
      `revokeProjectConsent: could not acquire the consent-store lock at ${consentLockPath(ferroxHome)} ` +
