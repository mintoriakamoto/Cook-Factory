...
// managed-hook dialect — the event names Ferrox writes into a DECLARATIVE host's
...
// hosts that load Ferrox as a plugin. This is a SEPARATE vocabulary + descriptor
...
// Ferrox binds this subset). Hoisted to a named const — rather than duplicated
...
const OPENCODE_EXTENSION_EVENTS = Object.freeze([
  ...
  // #2087 — additional documented plugin events Ferrox binds (opencode.ai/docs/plugins):
...
const EXTENSION_EVENT_SURFACES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  ...
  // by construction. See .kilo/plugins/ferrox-core.js (copied verbatim from
  // .opencode/plugins/ferrox-core.js).
  ...
  // covered the one event pi/ferrox.cjs happened to bind at the time, not the
