 ...
 * CLI --ws > FERROX_WORKSTREAM env > stored active workstream pointer.
 *
...
function getSessionScopedWorkstreamFile(cwd: string, fixedSessionKey?: string | null): SessionScopedWorkstreamFile | null {
  ...
  const dirPath = path.join(os.tmpdir(), 'ferrox-workstream-sessions', projectId);
  return {
    sessionKey,
    dirPath,
...
function resolveActiveWorkstream(
  ...
  } else if (env && typeof env['FERROX_WORKSTREAM'] === 'string' && env['FERROX_WORKSTREAM'].trim()) {
    ws = env['FERROX_WORKSTREAM'].trim();
    source = 'env';
  } else {
...
function applyResolvedWorkstreamEnv(
  ...
  env['FERROX_WORKSTREAM'] = resolution.ws;
}
