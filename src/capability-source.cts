 ...
 * Each adapter: fetch → verify integrity/SHA → check engines.ferrox → return a STAGED,
 * VALIDATED bundle.
 *
 * SECURITY CONTRACT:
 ...
 *   - engines.ferrox pre-checked before staging.
...
interface ResolveOptions {
  /** Running Ferrox version. Defaults to package.json version, fail-closed to '0.0.0'. */
  hostVersion?: string;
  /** Override the Ferrox home directory (where .ferrox/capabilities/ lives). */
  ferroxHome?: string;
  ...
  promote?: boolean;
  /**
   * When true, SKIP the resolver's `engines.ferrox` hard-throw during staging. The caller becomes
...
function realHttpsGet(url: string): Promise<HttpResponse> {
  return new Promise((resolve, reject) => {
    const req = _httpsGetImpl(
      url,
      { headers: { 'User-Agent': 'ferrox-core-capability-source/1.0' } },
      (res) => {
...
/**
 * Resolve the running Ferrox version; fail-closed to '0.0.0'.
 *
 * Prefer the authoritative `ferrox-core/VERSION` the installer writes for EVERY runtime
 * (libDir = ferrox-core/bin/lib/, so `../../VERSION` = ferrox-core/VERSION), so installed
...
function stageValidated(opts: {
  ...
  const { sourceDir, id, ferroxHome, hostVersion, source, integrity } = opts;
  const promote = opts.promote !== false;

  // Safety: validate id before using it in a path.
  assertSafeId(id);

  const capabilitiesRoot = path.join(ferroxHome, '.ferrox', 'capabilities');
  ...
  try {
    ...
    // engines.ferrox pre-check — reject before staging finalizes (unless the caller owns the gate).
    const engines = cap['engines'];
    if (!opts.skipEnginesGate && engines && typeof engines === 'object' && !Array.isArray(engines)) {
      const ferroxRange = (engines as Record<string, unknown>)['ferrox'];
      if (typeof ferroxRange === 'string' && ferroxRange) {
        if (!semverMod.semverSatisfies(hostVersion, ferroxRange)) {
          throw new Error(
            `Capability requires engines.ferrox "${ferroxRange}" but running Ferrox is ${hostVersion}`
          );
        }
      }
    }
...
function resolveLocal(
  ...
  return stageValidated({ sourceDir: absPath, id, ferroxHome, hostVersion, source: parsed.raw, integrity: null, promote: opts.promote, skipEnginesGate: opts.skipEnginesGate });
}
...
function resolveGit(
  ...
  const cloneDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-cap-git-'));
  try {
    ...
    return stageValidated({ sourceDir: cloneDir, id, ferroxHome, hostVersion, source: parsed.raw, integrity: null, promote: opts.promote, skipEnginesGate: opts.skipEnginesGate });
  } finally {
...
function resolveNpm(
  ...
  const tmpPackDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-cap-npm-pack-'));
  const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-cap-npm-ext-'));
  ...
  try {
    ...
    return stageValidated({ sourceDir, id, ferroxHome, hostVersion, source: parsed.raw, integrity: computedIntegrity, promote: opts.promote, skipEnginesGate: opts.skipEnginesGate });
  } finally {
...
async function resolveTarball(
  ...
  const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-cap-tar-'));
...
/**
 * Resolve a capability spec, validate it, and stage it into the Ferrox capabilities dir.
 *
 * @param spec  - Source spec string. Kind auto-detected via parseSpec.
 * @param opts  - Optional overrides for hostVersion, ferroxHome, integrity, exec/http seams.
...
async function resolveCapabilitySource(spec: string, opts: ResolveOptions = {}): Promise<ResolveResult> {
  ...
  const ferroxHome = opts.ferroxHome ?? process.env['FERROX_HOME'] ?? os.homedir();

  switch (parsed.kind) {
    case 'local':
      return resolveLocal(parsed, opts, ferroxHome, hostVersion);
    case 'git':
      return resolveGit(parsed, opts, ferroxHome, hostVersion);
    case 'npm':
      return resolveNpm(parsed, opts, ferroxHome, hostVersion);
    case 'tarball':
      return resolveTarball(parsed, opts, ferroxHome, hostVersion);
    case 'registry':
      throw new Error(
