 ...
 * that Ferrox writes into runtime config or prints for copy/paste. This module
...
const MANAGED_HOOK_COMMAND_BASENAMES_BY_SURFACE: Record<string, Set<string>> = {
  ...
  'codex-hooks-json': new Set([
    ...
    'ferrox-check-update.cmd',
    // #772: context-monitor is now registered for Codex SubagentStart/Stop/PostToolUse.
    'ferrox-context-monitor.js',
    // #772: Windows .cmd shim for ferrox-context-monitor — same #3426 pattern.
    'ferrox-context-monitor.cmd',
    // REACH-CODEX-01 / FF-B24: merge-gate guard registered on Codex PreToolUse.
    // Must be managed so reconcile dedups it on reinstall (else it accumulates)
    // and uninstall removes it.
    'ferrox-merge-gate-guard.js',
    'ferrox-merge-gate-guard.cmd',
  ]),
};

const LEGACY_MANAGED_HOOK_ALIASES_BY_SURFACE: Record<string, Set<string>> = {
  'codex-toml': new Set([
    'ferrox-update-check.js',
  ]),
  'codex-hooks-json': new Set([
    'ferrox-update-check.js',
  ]),
...
export function isManagedHookCommand(commandText: unknown, opts: { surface?: string; includeLegacyAliases?: boolean; configDir?: string; args?: unknown[] } = {}): boolean {
  ...
  // prevents false-positives for non-Ferrox entries that happen to share a path segment.
 ...
 * `"$CLAUDE_PROJECT_DIR"/.claude/hooks/ferrox-x.js`). Such a token is ALREADY a
...
/**
 * Result shape for {@link dispatchFerroxCommand}. Modeled on the existing
 ...
 */
export interface DispatchFerroxCommandResult {
  ok: boolean;
  stdout: string;
  stderr: string;
  code: number | null;
...
/**
 * Resolve the absolute path to ferrox-tools.cjs relative to THIS module.
 *
 * This file compiles to ferrox-core/bin/lib/shell-command-projection.cjs — a
 * sibling of ferrox-core/bin/ferrox-tools.cjs — so the relative walk-up is stable
 * regardless of install location (global/local/dev-repo layouts all ship
 * ferrox-core/bin/ as a unit).
 */
export function resolveFerroxToolsPath(): string {
  return path.resolve(__dirname, '..', 'ferrox-tools.cjs');
}

/**
 * Subprocess-shim dispatch to ferrox-tools.cjs (ADR-1239 #2102 Stage 2).
 *
 ...
 * covers the FULL family/subcommand surface is the ferrox-tools.cjs CLI itself.
 * This mirrors the SUBPROCESS-REUSE precedent already established for the
 * OpenCode/Kilo hook bridge (see .opencode/plugins/ferrox-core.js header:
 ...
 * flags accepted by every ferrox-tools.cjs family/subcommand, so passing them
 * unconditionally is safe for the full command surface.
 *
 * `family` maps 1:1 onto ferrox-tools.cjs's first positional argv token;
 * `subcommand` (when present) onto the second — e.g.
 * `{family:'phase', subcommand:'add'}` → `ferrox-tools.cjs phase add`. An empty
 ...
 *   - ENOENT / a missing ferrox-tools.cjs (via the injectable `ferroxToolsPath`)
 ...
 */
export function dispatchFerroxCommand({
  family,
  subcommand,
  args = [],
  cwd,
  ...
  cwd?: string;
  timeout?: number;
  ferroxToolsPath?: string;
} = {}): DispatchFerroxCommandResult {
  if (typeof family !== 'string' || family.length === 0) {
    return {
      ok: false,
      stdout: '',
      stderr: 'dispatchFerroxCommand requires a non-empty string "family".',
      code: null,
      timedOut: false,
    };
  }
  ...
  const toolsPath = ferroxToolsPath || resolveFerroxToolsPath();
  const argv = [
    toolsPath,
    family,
