 ...
 *
 * When no workstreams/ directory exists, Ferrox operates in "flat mode" with
...
function cmdWorkstreamCreate(cwd: string, name: string | null | undefined, options: WorkstreamCreateOptions, raw: boolean): void {
  ...
  if (!fs.existsSync(baseDir)) {
    error(`.planning/ directory not found — run ${formatFerroxSlash('new-project', resolveRuntime(cwd)) as string} first`);
  }
