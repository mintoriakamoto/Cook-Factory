...
interface CmdGenerateClaudeMdOptions {
  output?: string;
  auto?: boolean;
  // #1098: overwrite an existing instruction file that has no Ferrox section
...
function buildClaudeMdFallbacks(runtime: unknown): Record<string, string> {
  return {
    project: `Project not yet initialized. Run ${String(formatFerroxSlash('new-project', runtime))} to set up.`,
...
function buildClaudeMdProfilePlaceholder(runtime: unknown): string {
  return [
    ...
    '## Developer Profile',
    '',
    `> Profile not yet configured. Run \`${String(formatFerroxSlash('profile-user', runtime))}\` to generate your developer profile.`,
...
function extractSectionContent(fileContent: string, sectionName: string): string | null {
  const startMarker = `<!-- Ferrox:${sectionName}-start`;
  const endMarker = `<!-- Ferrox:${sectionName}-end -->`;
...
function buildSection(sectionName: string, sourceFile: string, content: string): string {
  return [
    `<!-- Ferrox:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- Ferrox:${sectionName}-end -->`,
  ].join('\n');
}
...
function generateSkillsSection(cwd: string): SectionResult {
  ...
  for (const dir of SKILL_SEARCH_DIRS) {
    ...
    for (const entry of entries) {
      ...
      // Skip Ferrox's own installed skills — only surface project-specific skills
...
function cmdWriteProfile(cwd: string, options: CmdWriteProfileOptions, raw: boolean): void {
  ...
  if (!outputPath) {
    ...
    outputPath = path.join(getGlobalConfigDir(effectiveRuntime), 'ferrox-core', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd: string, options: CmdGenerateDevPreferencesOptions, raw: boolean): void {
  ...
  if (analysis!.data_source === 'questionnaire') {
    stackBlock = `Stack preferences not available (questionnaire-only profile). Run \`${String(formatFerroxSlash('profile-user', resolveRuntime(cwd)))} --refresh\` with session data to populate.`;
  ...
  // commands/ferrox subdirectory in favor of skills/<skill>/SKILL.md under
  // the runtime config dir. This writer was missed in the migration
  // (PR #1540 targeted Ferrox-shipped command files; dev-preferences is a
  // runtime-generated user artifact). Default now points at the skills/
  // location so /ferrox:profile-user --refresh stops re-creating the legacy
  ...
  if (!outputPath) {
    ...
    const skillDir = getGlobalSkillDir(effectiveRuntime, 'ferrox-dev-preferences');
    if (!skillDir) {
  ...
  const result = {
    ...
    command_name: formatFerroxSlash('dev-preferences', resolveRuntime(cwd)),
...
function cmdGenerateClaudeProfile(cwd: string, options: CmdGenerateClaudeProfileOptions, raw: boolean): void {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by Ferrox from ${dataSource}. Run \`${String(formatFerroxSlash('profile-user', resolveRuntime(cwd)))}\` to update.`,
    '',
  ...
  if (existingContent !== null) {
    const startMarker = '<!-- Ferrox:profile-start -->';
    const endMarker = '<!-- Ferrox:profile-end -->';
...
function cmdGenerateClaudeMd(cwd: string, options: CmdGenerateClaudeMdOptions, raw: boolean): void {
  ...
  const sectionHeadings: Record<string, string> = {
    ...
    workflow: '## Ferrox Workflow Enforcement',
  };
  ...
  // repo-root `CLAUDE.md`, so generated Ferrox content does not land next to — or
  ...
  try {
    ...
    // new-project.md bash workflow via `ferrox-tools query
    ...
    // antigravity to GEMINI.md. FERROX_RUNTIME env var takes precedence
  ...
  if (existingContent === null) {
  ...
  } else if (!/<!-- Ferrox:[a-z]+-start/.test(existingContent) && !options.force) {
    // #1098: the target instruction file already exists and contains NO Ferrox
    // section markers — it is a hand-crafted CLAUDE.md/AGENTS.md, not a
    // Ferrox-managed one. Do NOT clobber it with generated project documentation
    // (broad project detail belongs in PROJECT.md / REQUIREMENTS.md, which Ferrox
    // already owns). Leave the file untouched and report a skip; `--force`
    // overwrites intentionally.
    output({
      claude_md_path: outputPath,
      action: 'skipped',
      reason: 'existing instruction file has no Ferrox markers (hand-crafted); not overwriting. Pass --force to overwrite.',
      sections_generated: [],
      sections_fallback: [],
      sections_skipped: MANAGED_SECTIONS,
      sections_total: MANAGED_SECTIONS.length,
      profile_status: 'skipped',
      message: `Left existing ${path.basename(outputPath)} untouched (no Ferrox markers found). Broad project context lives in PROJECT.md / REQUIREMENTS.md; pass --force to overwrite this file with Ferrox-managed sections.`,
    }, raw, undefined);
    return;
  } else {
    action = 'updated';
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = fileContent.indexOf(`<!-- Ferrox:${name}-start`) !== -1;
    ...
    if (!options.auto && fileContent.indexOf('<!-- Ferrox:profile-start') === -1) {
  ...
  if (finalContent && finalContent.indexOf('<!-- Ferrox:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- Ferrox:profile-start') === -1) {
  ...
  if (profileStatus === 'placeholder_added') message += ` Run ${String(formatFerroxSlash('profile-user', resolveRuntime(cwd)))} to unlock Developer Profile.`;
