 ...
 * the correct global skills directory for every Ferrox-supported runtime.
 *
 ...
 *   hermes  — Ferrox skills nest under skills/ferrox/<skillName>/ (not the flat
 ...
 *             Ferrox write target with --config-dir or KIMI_CONFIG_DIR.
...
interface DotHomeNestedDescriptor {
  ...
  probe?: string[];
  /**
   * Optional sub-path that qualifies which probe candidate Ferrox actually owns
   * (e.g. `ferrox-core/VERSION`). The same field name and check used by the
   ...
   * wins (the dir Ferrox installed into), then a bare-existence pass, then
  ...
  switch (configHome.kind) {
    ...
    case 'dot-home-nested': {
      ...
      if (configHome.probe && configHome.probe.length > 0) {
        ...
        // candidate Ferrox actually owns (its `<candidate>/<probeExists>` exists).
        // This disambiguates an active-but-shadowing sibling dir (e.g. the
        // Antigravity-IDE `~/.gemini/antigravity` dir) from the dir Ferrox was
  ...
  return resolveConfigHomeFromDescriptor(
    {
      ...
      // Prefer the candidate Ferrox installed into (carries ferrox-core/VERSION) over
      ...
      probeExists: 'ferrox-core/VERSION',
    },
...
export interface AntigravityAmbiguity {
  ...
  ambiguous: boolean;
  /** The dir Ferrox currently resolves to (where install/update will write). */
  resolved: string;
  /** All probe candidate dirs that exist on disk (absolute paths). */
  presentDirs: string[];
  /**
   * Candidate dirs that carry the Ferrox marker (ferrox-core/VERSION). When this has
 ...
 * a user upgrading from a pre-#217 install may have had Ferrox written into the
 ...
 * /ferrox-update to surface operator guidance (set ANTIGRAVITY_CONFIG_DIR or move
 * ferrox-core/ into the intended dir). The migration framework cannot relocate an
...
export function detectAntigravityDirAmbiguity(
  ...
  const home = opts.home ?? os.homedir();
  const existsSyncFn = opts.existsSync ?? fs.existsSync;
  const marker = path.join('ferrox-core', 'VERSION');
  ...
  const ferroxMarkedDirs = candidates.filter((dir) => existsSyncFn(path.join(dir, marker)));
  return {
 ...
 *
 * KIMI_CONFIG_DIR is a Ferrox installer write-location override. It is not Kimi's
 ...
 * which is a Ferrox-installer write-location override for the unrelated generic
 * Agent-Skills root resolved by resolveKimiGlobalDir above).
 *
 * This is deliberately a SEPARATE directory from Ferrox's kimi configHome
 ...
 * search paths, which are handled separately"). #2095 Upgrade 1 writes Ferrox's
