 ...
 *   - enabled axis: mutates .ferrox-surface.json via readSurface/writeSurface
 ...
 *   - ./io.cjs                 (output, error)
 *   - ./capability-state.cjs   (resolveCapabilityRuntimeState, _resolveManifest, _resolveCommandsFerroxDir)
...
// ExitError (NOT process.exit) is how every ferrox-tools command signals a non-zero exit: runMain
...
const { resolveCapabilityRuntimeState, _resolveManifest, _resolveCommandsFerroxDir } = capabilityStateMod;
  ...
  for (const entry of desired) {
    if (entry.enabled === true) {
      ...
      if (afterCap && afterCap.enabled !== true) {
        errors.push(
          `cannot enable "${entry.id}": not in the active surface/profile (widen the profile or use /ferrox:surface enable)`,
        );
      }
    }
...
/**
 * CLI command entry point for `ferrox-tools capability set`.
 *
