...
function decideIntegrationLanding(opts?: {
  mergedHeadProven?: unknown;
  ffOnlyPossible?: unknown;
  protectedPathsClean?: unknown;
}): LandingDecision {
  const o = opts && typeof opts === 'object' ? opts : {};

  // Protected-path safety comes first — a dirtied protected path blocks regardless of proof state.
  if (o.protectedPathsClean !== true) return { action: 'block', reason: 'protected-paths-dirty' };
  if (o.mergedHeadProven !== true) return { action: 'reprove', reason: 'merged-head-not-proven' };
  if (o.ffOnlyPossible !== true) return { action: 'block', reason: 'not-fast-forward-candidate-diverged' };
  return { action: 'land', reason: 'ff-only-advance-of-proven-head' };
}

export = { decideIntegrationLanding };
