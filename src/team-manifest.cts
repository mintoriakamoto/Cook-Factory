...
const MUTATION_ONLY_CODES: readonly string[] = [
  CODES.E_ROLE_EXISTS,
...
const PARSE_CODES: readonly string[] = Object.keys(CODES).filter(
  (c) => c.startsWith('E_') && !MUTATION_ONLY_CODES.includes(c)
);
...
interface TeamRole {
  ...
  provenance: string;
  binding: { agent: string } | { inline: true };
  /** After the A10 unknown-agent degrade: the rung dispatch actually uses. */
  effective_binding: 'agent' | 'inline';
  /** Required for inline roles; optional for agent-bound roles (A14). */
  tier: string | null;
  owns: string[];
  reviews: string[];
...
interface TeamManifest {
  ...
  derived_from: { brainstorm: string; milestone: string };
...
interface ParseResult {
  ...
  errors: TeamFinding[];
  warnings: TeamFinding[];
}

interface ValidateOptions {
  /** Known tier rungs (config tier_models keys). Absent = no ladder check. */
  tiers?: string[];
  /** Known agent names. Absent = no roster check. */
  agents?: string[];
}

interface MutationOptions extends ValidateOptions {
  /**
   * Role ids referenced by stamped plan files (A8). Entries are plain role-id
   * strings or { role_id, file } records; the caller scans the plans.
   */
  planRoleRefs?: Array<string | { role_id?: unknown; file?: unknown }>;
  /** Downgrade a live-in-plan refusal to a warning and proceed. */
  force?: boolean;
}
...
function findTeamBlocks(lines: string[]): TeamBlock[] {
  const blocks: TeamBlock[] = [];
  let open: { char: string; len: number; isTeam: boolean; openIdx: number } | null = null;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (open === null) {
      ...
      if (open.isTeam) blocks.push({ openIdx: open.openIdx, closeIdx: i });
      open = null;
    }
    // else: mismatched delimiter inside an open fence is content, not a boundary
  }
  if (open !== null && open.isTeam) blocks.push({ openIdx: open.openIdx, closeIdx: -1 });
  return blocks;
}
...
function stringList(v: unknown): string[] | null {
  ...
  const out: string[] = [];
  for (const item of v) {
    if (!nonEmptyString(item)) return null;
    out.push(item);
  }
  return out;
}

function push(list: TeamFinding[], code: string, path: string, message: string): void {
 ...
 */
function segmentsCoMatch(p: string, q: string): boolean {
  const memo = new Map<number, boolean>();
  const co = (i: number, j: number): boolean => {
...
function validateRole(
  ...
  if (!nonEmptyString(raw.provenance) || !PROVENANCE_RE.test(raw.provenance)) {
    push(
      errors,
      CODES.E_BAD_PROVENANCE,
      `${at}.provenance`,
      'provenance must carry the exit-confirmed marker "(stance: <stance>, confirmed at exit)" (A1: unblessed roles are notes, never manifest rows)'
    );
  }

  // binding: exactly 1 of { agent: <name> } or { inline: true }
  let binding: TeamRole['binding'] | null = null;
  let effective: 'agent' | 'inline' = 'inline';
  if (isRecord(raw.binding)) {
    ...
    if (hasAgent && !hasInline) {
      binding = { agent: raw.binding.agent as string };
      effective = 'agent';
  ...
  if (effective === 'agent' && Array.isArray(options.agents)) {
    const agent = (binding as { agent: string }).agent;
    if (!options.agents.includes(agent)) {
      push(
        warnings,
        CODES.W_UNKNOWN_AGENT,
        `${at}.binding.agent`,
        `agent ${JSON.stringify(agent)} is not in the injected agent list; role degrades to the inline-charter rung`
      );
      effective = 'inline';
    }
  }
  ...
  for (const [key, list] of [['owns', owns], ['reviews', reviews]] as const) {
    ...
    for (let i = 0; i < list.length; i++) {
      if (UNSUPPORTED_GLOB_RE.test(list[i])) {
        push(
          ...
          CODES.E_BAD_SURFACE,
          `${at}.${key}[${i}]`,
          `unsupported glob syntax in ${JSON.stringify(list[i])}: allowed syntax is *, ?, and ** over /-separated segments (case-insensitive); {, [, and \\ are refused so the overlap check can never silently false-negative (fail closed)`
        );
      }
    }
  }
 ...
 */
function checkOwnsOverlap(roles: TeamRole[], errors: TeamFinding[]): void {
    ...
    for (let j = i + 1; j < roles.length; j++) {
      ...
      for (const ga of a.owns) {
        for (const gb of b.owns) {
          ...
          const edge =
            a.reviews.some((r) => globsOverlap(r, gb)) ||
            b.reviews.some((r) => globsOverlap(r, ga));
          if (!edge) {
            push(
              errors,
              CODES.E_OWNS_OVERLAP,
              `roles[${j}].owns`,
              `roles ${a.id || `#${i}`} and ${b.id || `#${j}`} both own overlapping surfaces (${JSON.stringify(ga)} vs ${JSON.stringify(gb)}) with no review edge covering the overlap`
            );
          }
        }
      }
 ...
 */
function validateTeamManifest(
  doc: unknown,
  options?: ValidateOptions,
  internal?: { skipHashCheck?: boolean }
): { ok: boolean; manifest: TeamManifest | null; errors: TeamFinding[]; warnings: TeamFinding[] } {
  const errors: TeamFinding[] = [];
  const warnings: TeamFinding[] = [];
  const opts = options ?? {};
  ...
  let derivedFrom: TeamManifest['derived_from'] = { brainstorm: '', milestone: '' };
  ...
  if (internal?.skipHashCheck !== true) {
    if (!HASH_RE.test(manifest.manifest_hash)) {
      ...
      if (expected !== manifest.manifest_hash) {
        push(
          errors,
          CODES.E_HASH_MISMATCH,
          'manifest_hash',
          `manifest_hash ${manifest.manifest_hash.slice(0, 12)}... does not match the roster content (expected ${expected.slice(0, 12)}...); mutate through the governed ops, never by hand`
        );
      }
    }
  }
 ...
 */
function canonicalForm(manifest: TeamManifest): Record<string, unknown> {
  return {
...
function computeHashOf(manifest: TeamManifest): string {
  return crypto.createHash('sha256').update(JSON.stringify(canonicalForm(manifest)), 'utf8').digest('hex');
}
 ...
 */
function computeTeamManifestHash(manifest?: unknown): string | null {
  if (!isRecord(manifest) || !isRecord(manifest.derived_from) || !Array.isArray(manifest.roles)) return null;
  if (!(manifest.roles as unknown[]).every((r) => isRecord(r) && isRecord(r.binding))) return null;
  return computeHashOf(manifest as unknown as TeamManifest);
}

/** PURE. Recompute and compare. Never throws: a garbage input compares false. */
function verifyTeamManifestHash(manifest?: unknown): { ok: boolean; expected: string | null; actual: string | null } {
  if (!isRecord(manifest)) return { ok: false, expected: null, actual: null };
  const checked = validateTeamManifest(manifest, undefined, { skipHashCheck: true });
  if (checked.manifest === null) return { ok: false, expected: null, actual: null };
 ...
 */
function parseTeamManifest(markdown?: unknown, options?: ValidateOptions): ParseResult {
  const errors: TeamFinding[] = [];
  const warnings: TeamFinding[] = [];
  const fail = (): ParseResult => ({ ok: false, manifest: null, errors, warnings });
  ...
  try {
    ...
    push(errors, CODES.E_YAML_PARSE, '', `YAML parse failed: ${String((e as Error).message)}`);
    return fail();
  }
...
function normalizePlanRefs(refs: MutationOptions['planRoleRefs']): Array<{ role_id: string; file: string | null }> {
  if (!Array.isArray(refs)) return [];
  const out: Array<{ role_id: string; file: string | null }> = [];
  for (const r of refs) {
...
function opInput(manifest: unknown, errors: TeamFinding[]): Record<string, unknown> | null {
...
function roleIdOf(raw: unknown): string {
 ...
 */
function addTeamRole(manifest?: unknown, role?: unknown, options?: MutationOptions): MutationResult {
  const errors: TeamFinding[] = [];
  const warnings: TeamFinding[] = [];
  const opts = options ?? {};
  const doc = opInput(manifest, errors);
  if (doc === null) return { ok: false, manifest: null, errors, warnings };

  const id = roleIdOf(role);
  if (id !== '' && (doc.roles as unknown[]).some((r) => roleIdOf(r) === id)) {
    push(errors, CODES.E_ROLE_EXISTS, 'roles', `role id ${id} already exists (swap it instead)`);
    return { ok: false, manifest: null, errors, warnings };
  }

  const candidate = { ...doc, roles: [...(doc.roles as unknown[]), role] };
 ...
 */
function removeTeamRole(manifest?: unknown, roleId?: unknown, options?: MutationOptions): MutationResult {
  const errors: TeamFinding[] = [];
  const warnings: TeamFinding[] = [];
  const opts = options ?? {};
  const doc = opInput(manifest, errors);
  ...
  const roles = doc.roles as unknown[];
 ...
 */
function swapTeamRole(manifest?: unknown, roleId?: unknown, replacement?: unknown, options?: MutationOptions): MutationResult {
  const errors: TeamFinding[] = [];
  const warnings: TeamFinding[] = [];
  const opts = options ?? {};
  const doc = opInput(manifest, errors);
 ...
 */
function serializeTeamManifest(manifest?: unknown, markdown?: unknown): string {
  const checked = validateTeamManifest(manifest, undefined, { skipHashCheck: true });
