...
import workgraph = require('./workgraph.cjs');
...
import teamManifest = require('./team-manifest.cjs');
...
const TOOLS_PATH = path.join(__dirname, '..', 'ferrox-tools.cjs');
...
type ChildRunner = (args: string[], cwd: string) => ChildResult;
...
/**
 * 1 language the scan can actually read, and the layout it reads it in.
 *
 * A profile is turned on by EVIDENCE ON DISK rather than by a flag, because a
 * flag makes the caller responsible for a fact the tree already states. The
 * marker is the language's own package manifest at the repository root, which
 * is the 1 file every toolchain in that language agrees has to be there.
 */
...
/** What the roots resolved to, kept separable from what was merely asked for. */
interface RootResolution {
  present: string[];
  absent: string[];
}

/** 1 crate discovered in the tree, named the way a `use` statement names it. */
interface CrateRecord {
  /** The directory holding the crate's `Cargo.toml`, empty at the tree root. */
  dir: string;
  /** The directory the crate's module tree is anchored at. */
  srcDir: string;
  /** The indexed file `crate::` resolves to. */
  rootFile: string;
  /** The identifier a sibling crate writes, hyphens read as underscores. */
  externName: string;
}

/** The crate index, in the 2 shapes the resolver asks it questions in. */
interface CrateIndex {
  /** Longest `srcDir` first, so the containing crate of a file is the first hit. */
  byDepth: CrateRecord[];
  /** Extern name to root file, for a cross crate `use`. */
  byExternName: Map<string, string>;
}

/** 1 Rust declaration that could name a second file. */
interface RustDeclaration {
  form: string;
  /** The module name for a `mod`, the crate name for an `extern crate`. */
  name: string;
  /** The expanded leaf paths of a `use` tree, 1 segment array per leaf. */
  paths: string[][];
  /** The `#[path = "..."]` override attached to a `mod`, else empty. */
  pathAttribute: string;
  /** The inline module names enclosing this declaration, outermost first. */
  scope: string[];
  line: number;
}
...
interface ScanCounts {
  ...
  external: number;
  out_of_root: number;
  /** Dynamic require sites the closed grammar folded to a specifier. Phase 27. */
  folded: number;
  /** Dynamic require sites the closed grammar refused. Phase 27. */
  unfolded: number;
}
...
interface ScanResult {
  ...
  counts: ScanCounts;
  unreadable: string[];
  /** Files the scan read but could not fully follow, sorted. Phase 25. */
  dynamic_unresolved: string[];
}
...
interface ScanInput {
  ...
  files: string[];
  scanRoots?: string[];
  /** Crate directory to the extern name its `Cargo.toml` declares. */
  crateNames?: Record<string, string>;
}
...
interface BuildOptions {
  ...
  indexRoots?: string[];
  scanRoots?: string[];
  planIndexRunner?: ChildRunner;
  tierRunner?: ChildRunner;
 ...
 */
function detectLanguages(cwd: string): LanguageProfile[] {
  const active: LanguageProfile[] = [];
  for (const profile of LANGUAGE_PROFILES) {
    let present = false;
    try {
      present = fs.statSync(path.join(cwd, profile.marker)).isFile();
    } catch {
      present = false;
    }
    if (present) active.push(profile);
  }
 ...
 */
function resolveDefaultRoots(cwd: string, roots: string[]): RootResolution {
  const present: string[] = [];
  const absent: string[] = [];
  for (const raw of uniqueStrings(roots.map((r) => workgraph.normalizePath(r)))) {
    let there = false;
    try {
      there = fs.statSync(path.join(cwd, ...raw.split('/'))).isDirectory();
    } catch {
      there = false;
    }
    if (there) present.push(raw);
    else absent.push(raw);
  }
  return { present, absent };
}

/** The default child runner: a real spawn of the shipped CLI. */
function spawnTools(args: string[], cwd: string): ChildResult {
  const result = spawnSync(process.execPath, [TOOLS_PATH, ...args, '--cwd', cwd], {
    encoding: 'utf8',
  });
  return {
 ...
 */
function walkSourceFiles(root: string, indexRoots?: string[], extensions?: string[]): WalkResult {
  ...
  const descend = (relative: string): void => {
    ...
    for (const entry of entries) {
      ...
      if (hasExtension(entry.name, indexed)) files.push(child);
    }
  };
 ...
 */
function scanImports(input: ScanInput): ScanResult {
  ...
  const unreadable: string[] = [];
  const dynamicUnresolved = new Set<string>();
  let external = 0;
  let outOfRoot = 0;
  let read = 0;
  ...
  for (const filePath of files) {
    if (!workgraph.isInScanRoot(filePath, scanRoots)) continue;
    let text: string;
    try {
      text = fs.readFileSync(path.join(root, ...filePath.split('/')), 'utf8');
    } catch {
    ...
    if (isRustFile(filePath)) {
      ...
      for (const miss of scanned.unresolved) unresolved.push(miss);
    ...
    for (const spec of workgraph.parseImportSources(text, filePath).results) {
      ...
      if (resolved.path === filePath) continue;
      importEdges.push({
        from: filePath,
        to: resolved.path,
        form: spec.form,
...
// ─── the Rust reader ─────────────────────────────────────────────────────────
//
// WHY THIS LIVES IN THE SHELL AND NOT THE PURE CORE. Every function below is
// pure: it reads no disk, spawns nothing and asks no clock. It belongs beside
// `parseImportSources` in `workgraph.cjs` on architecture alone. It is here
// because `src/workgraph.cts` is outside this change's write lane, and moving
// it later is a rename rather than a rewrite. Nothing here consults `fs`.
//
// THE EDGE RULE, stated plainly, because a rule nobody can state is a rule
// nobody can trust. A Rust edge is emitted for exactly 3 declaration forms, and
// ONLY when its target is a file the walk already indexed:
//
//   `mod`           a `mod NAME;` declaration, any visibility, optionally
//                   carrying `#[path = "..."]`. The edge runs from the
//                   declaring file to the file that declaration names. An
//                   INLINE `mod NAME { ... }` declares no second file and
//                   therefore emits nothing.
//   `use`           a `use` or `pub use` whose head segment is `crate`,
//                   `super`, `self` or the name of a crate discovered in this
//                   tree. The tree is expanded to its leaf paths, each leaf is
//                   anchored, and the LONGEST PREFIX of the remaining segments
//                   that resolves to an indexed module file is the target. The
//                   segments after it are item names, and dropping them is what
//                   lets this scan work without a name resolver. When no
//                   segment resolves the anchor's own file is the target, which
//                   is the right answer for `use super::Config`: the item is
//                   defined or re-exported in the parent module's file.
//   `extern-crate`  an `extern crate NAME;`, which names a crate directly.
//
// A head segment that matches no crate in this tree is EXTERNAL and is counted,
// exactly as a bare JavaScript specifier is. `std`, `core` and `alloc` are
// external by that rule without being listed, which is the point of deriving it.

interface RustStripState {
  blockDepth: number;
  inQuote: boolean;
  /** Hash count of the open raw string, or a negative value when in none. */
  rawHashes: number;
  /** Net curly brace count of the last line stripped, strings excluded. */
  braceDelta: number;
}
 ...
 */
function stripRustLine(line: string, state: RustStripState): string {
  let out = '';
  let i = 0;
  state.braceDelta = 0;
  ...
  while (i < line.length) {
    if (state.rawHashes >= 0) {
      ...
      if (at === -1) {
        if (!carried) out += line.slice(i);
        return out;
      }
      if (!carried) out += line.slice(i, at + close.length);
      state.rawHashes = -1;
      carried = false;
      i = at + close.length;
      continue;
    ...
    if (state.inQuote) {
      ...
      if (ch === '\\' && i + 1 < line.length) {
        if (!carried) out += line.charAt(i + 1);
        i += 2;
        continue;
      }
      if (ch === '"') {
 ...
 */
function expandRustUseTree(tree: string, prefix: string[]): string[][] {
  ...
  if (open === -1) {
    ...
    for (const raw of text.split(RUST_PATH_SEPARATOR)) {
      ...
      if (segment === RUST_ANCHOR_SELF && segments.length > 0) continue;
  ...
  for (const raw of members) {
    ...
    for (const leaf of expandRustUseTree(trimmed, carried)) out.push(leaf);
  }
  return out;
}
 ...
 */
function parseRustDeclarations(text: unknown): RustDeclaration[] {
  const out: RustDeclaration[] = [];
  if (typeof text !== 'string' || text === '') return out;

  const state: RustStripState = { blockDepth: 0, inQuote: false, rawHashes: -1, braceDelta: 0 };
  const lines: string[] = [];
  const deltas: number[] = [];
  ...
  let pathAttribute = '';
  let depth = 0;
  const inlineScope: { name: string; bodyDepth: number }[] = [];
  const scopeNames = (): string[] => inlineScope.map((entry) => entry.name);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!RUST_ATTRIBUTE_SHAPE.test(code)) pathAttribute = '';
  }
 ...
 */
function deriveRustCrates(files: string[], crateNames?: Record<string, string>): CrateIndex {
  const names = crateNames === null || typeof crateNames !== 'object' ? {} : crateNames;
  const indexed = new Set(files);
  const byDir = new Map<string, CrateRecord>();

  for (const filePath of files) {
    if (!isRustFile(filePath)) continue;
    const srcDir = dirOf(filePath);
    if (basenameOf(srcDir) !== RUST_SRC_DIRECTORY) continue;
    const stem = basenameOf(filePath).slice(0, -RUST_SUFFIX.length);
    if (!RUST_CRATE_ROOT_STEMS.includes(stem)) continue;
    ...
    if (held !== undefined && basenameOf(held.rootFile) !== 'main' + RUST_SUFFIX) continue;
  ...
  for (const filePath of files) {
    ...
    if (RUST_TARGET_DIRECTORIES.includes(kind)) {
      ...
      (stem === 'main' || name === RUST_MOD_FILE)
      && RUST_TARGET_DIRECTORIES.includes(basenameOf(dirOf(parent)))
    ) {
      root = filePath;
      anchor = parent;
    }
    if (root === '' || !indexed.has(root)) continue;
    const held = byDir.get(anchor);
    // `main.rs` outranks `mod.rs`: where both exist cargo compiles the first.
    if (held !== undefined && basenameOf(held.rootFile) !== RUST_MOD_FILE) continue;
    byDir.set(anchor, { dir: anchor, srcDir: anchor, rootFile: root, externName: '' });
  }

  const byDepth = Array.from(byDir.values()).sort(
    (a, b) => b.srcDir.length - a.srcDir.length || compareStrings(a.srcDir, b.srcDir),
  );
  const byExternName = new Map<string, string>();
  for (const record of byDepth) {
    if (record.externName === '' || byExternName.has(record.externName)) continue;
...
function crateOf(filePath: string, crates: CrateIndex): CrateRecord | null {
  for (const record of crates.byDepth) {
    if (filePath === record.rootFile) return record;
    if (record.srcDir === '' || filePath.startsWith(record.srcDir + '/')) return record;
  }
  return null;
}
 ...
 * is `src/foo/bar.rs`.
 */
function rustModuleDir(filePath: string, crates: CrateIndex): string {
  const dir = dirOf(filePath);
  const name = basenameOf(filePath);
  if (name === RUST_MOD_FILE) return dir;
  const record = crateOf(filePath, crates);
  if (record !== null && record.rootFile === filePath) return dir;
...
function rustModuleFile(moduleDir: string, indexed: Set<string>): string {
  const asFile = moduleDir + RUST_SUFFIX;
  if (indexed.has(asFile)) return asFile;
  const asDir = joinPath(moduleDir, RUST_MOD_FILE);
  if (indexed.has(asDir)) return asDir;
  return '';
}

/** The 1 module file a name resolves to inside a directory, or the empty string. */
function resolveRustModule(moduleDir: string, name: string, indexed: Set<string>): string {
...
function scanRustFile(input: {
  ...
  indexed: Set<string>;
  crates: CrateIndex;
}): { edges: ImportEdge[]; unresolved: UnresolvedImport[]; external: number } {
  ...
  const record = (target: string, form: string): void => {
  ...
  for (const declaration of parseRustDeclarations(input.text)) {
    ...
    for (const segments of declaration.paths) {
      ...
      if (head === RUST_ANCHOR_CRATE) {
        ...
        record(
          resolveRustPath({ anchorDir: own.srcDir, anchorFile: own.rootFile, segments: rest, indexed }),
          RUST_FORM_USE,
        );
        continue;
      }
      ...
      if (head === RUST_ANCHOR_SELF) {
        record(
          resolveRustPath({ anchorDir: moduleDir, anchorFile: filePath, segments: rest, indexed }),
          RUST_FORM_USE,
        );
        continue;
      }
...
function readPlanIndex(input: {
  ...
  const runner: ChildRunner = typeof input.runner === 'function' ? input.runner : spawnTools;
  const empty: PlanIndexResult = { ok: false, phase, plans: [], warnings: [], message: '' };

  const result = runner(['phase-plan-index', phase, '--raw'], cwd);
  if (result.status !== 0) {
    return Object.assign({}, empty, {
      message:
        `the phase-plan-index verb exited ${result.status} for phase ${phase}:\n`
  ...
  try {
    ...
    return Object.assign({}, empty, {
      message:
        `the phase-plan-index verb printed output that is not JSON for phase ${phase}.\n`
  ...
  if (typeof parsed.error === 'string' && parsed.error !== '') {
    return Object.assign({}, empty, {
      message:
        `phase ${phase} has no phase directory, so there is no graph to emit (${parsed.error}).\n`
        + 'Fix: name a phase that exists, which you can list with:\n'
        + '  node ferrox-core/bin/ferrox-tools.cjs phase list',
    });
  }

  const plans = Array.isArray(parsed.plans) ? (parsed.plans as PlanRecord[]) : [];
  return {
    ok: true,
...
function resolveTierModel(input: {
  ...
  tier: string;
  runner?: ChildRunner;
}): { model_id: string | null; ladder_miss: string } {
  const runner: ChildRunner = typeof input.runner === 'function' ? input.runner : spawnTools;
 ...
 */
function readCrateNames(cwd: string, files: string[]): Record<string, string> {
  ...
  for (const dir of dirs) {
    ...
    let text: string;
    try {
      text = fs.readFileSync(path.join(cwd, ...manifest), 'utf8');
    } catch {
      continue;
    }
    let section = '';
    for (const line of workgraph.splitLines(text)) {
      ...
      if (section !== CARGO_PACKAGE_SECTION) continue;
...
function readRosterText(cwd: string): string | null {
  try {
    return fs.readFileSync(path.join(cwd, ...ROSTER_RELATIVE.split('/')), 'utf8');
  } catch {
    return null;
  }
}
...
function matchRole(input: {
  lane: string[];
  roles: { id: string; owns?: string[]; reviews?: string[] }[];
}): { role: RoleMatch | null; warnings: string[] } {
  ...
  for (const surface of ['owns', 'reviews']) {
    for (const role of roles) {
      if (role === null || typeof role !== 'object' || typeof role.id !== 'string') continue;
      const globs = asStringArray(surface === 'owns' ? role.owns : role.reviews);
      for (const glob of globs) {
        if (glob.includes(SINGLE_CHAR_WILDCARD)) {
          warnings.push(
            `role ${role.id} declares the ${surface} surface glob '${glob}', which carries the `
...
function deriveSeamGaps(input: {
  ...
  for (const filePath of asStringArray(input.lane)) {
    if (!GOVERNANCE_SURFACES.includes(basenameOf(filePath))) continue;
    const hot = evaluateHotSeam({ filesModified: [filePath], seams: hotSeams });
    if (hot.matched.length > 0) continue;
    const shared = evaluateSharedWrite({
      actor: NON_ORCHESTRATOR_ACTOR,
      targetPath: filePath,
      sharedPaths,
    });
    if (shared.decision === 'forbidden') continue;
    gaps.push({
      path: filePath,
      node: input.node,
 ...
 */
function buildWorkgraph(options: BuildOptions): BuildResult {
  const source: BuildOptions =
    options === null || typeof options !== 'object' ? ({} as BuildOptions) : options;
  ...
  const coordination = (config.coordination || {}) as Record<string, unknown>;
  ...
  const stageTiers = (model.stage_tiers || {}) as Record<string, unknown>;
  ...
  const warnings: string[] = index.warnings.slice();
  ...
  if (walk.files.length === 0) {
    warnings.push(
      'the walk indexed 0 files, so this graph measures NOTHING about imports: the '
  ...
  const rosterText = typeof source.rosterText === 'string'
    ? source.rosterText
    : readRosterText(cwd);
  let roles: { id: string; owns?: string[]; reviews?: string[] }[] = [];
  if (rosterText === null) {
    warnings.push(
      `no roster was found at ${ROSTER_RELATIVE}, so every node carries a null role`,
    );
  } else {
    const parsed = parseTeamManifest(rosterText);
    if (parsed.manifest === null) {
      warnings.push(
        `the roster at ${ROSTER_RELATIVE} did not parse, so every node carries a null role`,
      );
    } else {
  ...
  const tierMemo = new Map<string, { model_id: string | null; ladder_miss: string }>();
  const resolveMemoized = (tier: string): { model_id: string | null; ladder_miss: string } => {
  ...
  const nodes = index.plans.map((plan) => {
    ...
    const governanceMatched: string[] = [];
    ...
    for (const filePath of lane) {
      ...
      if (shared.decision !== 'forbidden' || shared.matched === null) continue;
      if (!governanceMatched.includes(shared.matched)) governanceMatched.push(shared.matched);
      if (!governanceFiles.includes(filePath)) governanceFiles.push(filePath);
    }
...
// ─── honest gaps ─────────────────────────────────────────────────────────────
//
//   - The walk indexes 7 file extensions and nothing else. A module written in
//     any third language is invisible to the import graph, and the document
//     says so twice: `generated.languages` names what was read, and
//     `generated.indexed` is false when nothing was.
//   - THE RUST GAPS, each one a construct deliberately NOT treated as an edge:
//       * `include!`, `include_str!` and `include_bytes!` name a file and are
//         not read. A module assembled by one of them is invisible.
//       * a `mod` produced by a macro expansion is invisible, because nothing
//         here expands a macro.
//       * an inline `mod NAME { ... }` is correctly no edge, and a `mod` inside
//         one resolves through that module's directory. What is NOT handled is
//         an inline module OPENED AND CLOSED ON 1 LINE: only the first
//         declaration on a line is read, so a `mod` inside it is missed. That
//         costs a missed edge and never a false one, and no such line exists in
//         the tree this was measured against.
//       * a `use` sharing a line with a preceding statement is missed, because
//         the shapes are anchored at the start of a line exactly as the
//         JavaScript shapes are. rustfmt puts each `use` on its own line.
//       * a `use` that reaches a type THROUGH a re-export in a third crate
//         yields an edge to the crate it named, not to the crate that defines
//         the type. Following that needs a name resolver this scan is not.
//       * a member of a brace group that is an ITEM rather than a module
//         collapses to its enclosing module's file, which is the anchor edge
//         and never a wrong one, only a coarser one.
//       * `build.rs` sits beside `Cargo.toml` rather than under a scan root, so
//         a build script's own `use` lines contribute nothing.
//       * a `#[cfg]` gated `mod` whose file is not in this checkout is recorded
//         as an unresolved miss, not dropped.
//       * `type_only` is always false on a Rust edge. Rust does not mark an
//         import as type only at the import site, so claiming otherwise would
//         be an invention.
//   - The Rust index roots are the cargo conventional set. A workspace member
//     outside `src`, `crates`, `examples`, `tests` and `benches`, which is what
//     a `workspace-hack` crate is, is not indexed and its edges are absent.
//   - The scan is repository wide, not phase scoped. That is deliberate: an
//     import edge is a fact about the tree, and folding it to node pairs is the
//     classifier's job. It also means every phase pays the same walk cost.
//   - The governance gap derivation asks about a file only when its BASENAME is
//     1 of the 4 surfaces. A governance file renamed to anything else is
//     invisible to it.
//   - The role match reads the roster's globs through the coordination matcher,
//     which is not the matcher the roster validator uses. The 1 syntax where
//     they differ is warned about and recorded, and no other difference is
//     claimed to be absent.
//   - The tier is the execute stage rung for every node. This module has no
//     model of a phase whose plans dispatch at different stages.
