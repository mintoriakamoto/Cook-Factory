...
/** A raw event as the strict reader returns it. */
interface FoldEvent {
  ts?: string | number;
  kind?: string;
  run_id?: string;
...
interface FoldOptions {
  /** Optional filter. When supplied, only events bearing this run_id are folded. */
  run_id?: string;
}
...
interface AttemptRow {
  attempt_id: unknown;
  queue_entered_at: string | number | null;
  queue_acquired_at: string | number | null;
  gate_started_at: string | number | null;
  gate_ended_at: string | number | null;
  land_completed_at: string | number | null;
  queue_wait_ms: number | null;
  gate_ms: number | null;
  land_ms: number | null;
  post_land_truth: unknown;
}
...
interface RunRecord {
  ...
  run_started_at: string | number | null;
  run_closed_at: string | number | null;
  workers: WorkerRow[];
  demonstrated_width: { value: number; exact: boolean; unknown_intervals: number };
  nodes: NodeRow[];
  rounds_per_artifact: Record<string, number>;
  false_green: { landed: number; later_failed: number; unknown: number };
}
...
function _deltaMs(from: unknown, to: unknown): number | null {
  const a = _toMs(from);
  const b = _toMs(to);
...
function _kindsCarryingNode(): ReadonlySet<string> {
  const out = new Set<string>();
  const map = runlog.FLEET_EVENT_REQUIRED_FIELDS as unknown as Record<string, readonly string[]>;
  for (const kind of runlog.FLEET_EVENT_KINDS as readonly string[]) {
    if ((map[kind] ?? []).includes('node_id')) out.add(kind);
  }
  return out;
}
...
function _byString(a: unknown, b: unknown): number {
  const sa = String(a);
  const sb = String(b);
...
function _emptyRecord(runId: string | null): RunRecord {
  return {
    run_id: runId,
 ...
 */
function foldRunRecord(events: FoldEvent[], opts: FoldOptions = {}): RunRecord {
  ...
  if (filter === undefined) {
    ...
    for (const e of source) if (typeof e.run_id === 'string') seen.add(e.run_id);
    if (seen.size > 1) {
      throw _codedError(
        RUNFOLD_ERROR_CODES.E_FLEET_MULTIPLE_RUNS,
        `foldRunRecord: the log holds ${seen.size} distinct run_id values `
  ...
  if (scoped.length === 0) return _emptyRecord(filter ?? null);

  let runId: string | null = filter ?? null;
  let graphGeneration: unknown = null;
  let runStartedAt: string | number | null = null;
  let runClosedAt: string | number | null = null;
  ...
  const open = new Map<string, WorkerRow[]>();
  ...
  const NUL = '\u0000';
  const tripleKey = (e: FoldEvent): string =>
    [_idString(e.worker_id), _idString(e.node_id), _idString(e.attempt_id)].join(NUL);

  // ── nodes and attempts ─────────────────────────────────────────────────────
  const nodeIds = new Set<string>();
  const attemptsByNode = new Map<string, Map<string, AttemptRow>>();
  const attemptIdsByNode = new Map<string, Set<string>>();

  const attemptRow = (nodeId: string, attemptId: unknown): AttemptRow => {
    let byAttempt = attemptsByNode.get(nodeId);
    if (byAttempt === undefined) {
      byAttempt = new Map<string, AttemptRow>();
  ...
  const setOnce = (row: AttemptRow, field: keyof AttemptRow, value: string | number | null): void => {
    if (row[field] === null) (row as unknown as Record<string, unknown>)[field] = value;
  };

  for (const e of scoped) {
    const kind = typeof e.kind === 'string' ? e.kind : '';
    const ts: string | number | null = e.ts ?? null;
    if (runId === null && typeof e.run_id === 'string') runId = e.run_id;
    ...
    if (NODE_KINDS.has(kind) && e.node_id !== undefined) {
      ...
      if (ATTEMPT_KINDS.has(kind) && e.attempt_id !== undefined) {
        ...
        if (ids === undefined) { ids = new Set<string>(); attemptIdsByNode.set(nodeId, ids); }
    ...
    switch (kind) {
      case 'run_started':
        if (runStartedAt === null) runStartedAt = ts;
        if (graphGeneration === null) graphGeneration = e.graph_generation ?? null;
        break;
      case 'run_closed':
        runClosedAt = ts;
        break;
      case 'worker_started': {
        ...
        if (queue === undefined) open.set(key, [row]);
        else queue.push(row);
        break;
      }
      case 'worker_ended': {
      ...
      case 'post_land_truth': {
        ...
        if (row.post_land_truth === null) row.post_land_truth = e.classification ?? null;
        break;
      }
      default:
        break;
  ...
  for (const w of workers) {
    if (w.status !== 'closed') { unknownIntervals++; continue; }
    const a = _toMs(w.started_at);
    const b = _toMs(w.ended_at);
    if (a === null || b === null) { unknownIntervals++; continue; }
  ...
  const nodes: NodeRow[] = [...nodeIds].sort(_byString).map((nodeId) => {
  ...
  const roundsPerArtifact: Record<string, number> = {};
  for (const nodeId of [...nodeIds].sort(_byString)) {
    roundsPerArtifact[nodeId] = (attemptIdsByNode.get(nodeId) ?? new Set<string>()).size;
  }
  ...
  for (const node of nodes) {
    for (const attempt of node.attempts) {
      ...
      if (classification === 'false_green') falseGreen.later_failed++;
      else if (classification === 'unknown') falseGreen.unknown++;
    }
  }
...
export = { foldRunRecord, RUNFOLD_UNKNOWN, RUNFOLD_ERROR_CODES };
