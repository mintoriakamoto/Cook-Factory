...
interface MergeGateInput {
  githubIssue?: number | null;
  increment: string;
  /** The strength.merge-gate decision for this increment ('pass' | 'block' | any forward-compat verdict). */
  gateDecision: string;
}

interface GhAction {
  action: 'label' | 'unlabel' | 'close' | 'comment';
  issue: number;
  value?: string;
}

interface LifecycleResult {
  /** 'noop' when there is no linked issue, or when the gate did not pass. */
  decision: 'applied' | 'noop';
  reason?: string;
  actions: GhAction[];
}

function normIssue(n: unknown): number | null {
 ...
 */
function onIncrementStart(input: StartInput): LifecycleResult {
 ...
 */
function onMergeGate(input: MergeGateInput): LifecycleResult {
  const issue = normIssue(input.githubIssue);
  if (issue === null) {
    return { decision: 'noop', reason: 'no linked github_issue', actions: [] };
  }
...
export = { onIncrementStart, onMergeGate };
