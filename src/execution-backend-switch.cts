 ...
 */
function isExplicitDecision(precedence: unknown): boolean {
...
interface Counters {
  ...
  refusals: number;
  fallbacks: number;
  consumer_gaps: number;
  notices: number;
  probes: number;
  /** How many backends were read out of words rather than off a flag. */
  inferences: number;
  /** How many times a flag was followed over a disagreeing sentence. */
  overrides: number;
}

interface DetectLike {
  available?: unknown;
  backend?: unknown;
  reason?: unknown;
}

interface RecommendLike {
  available?: unknown;
  backend?: unknown;
  reason?: unknown;
}
...
interface Providers {
  ...
  detect?: () => DetectLike;
  recommend?: () => RecommendLike;
  consumerPresent?: () => unknown;
}
...
/**
 * The shape `detectBackendIntent` returns. It is accepted as a PLAIN VALUE
 * rather than a provider function on purpose: reading a sentence is pure, it
 * costs no probe, and a contradictory sentence has to be able to refuse before
 * anything on disk is touched. `counters.probes` stays the receipt for that
 * ordering, and it is still 0 on every refusal.
 */
...
interface ExplicitDecision {
  ...
  code: string;
  reason: string;
  /** The backend a sentence asked for when a flag disagreed and won. */
  overriddenBackend: string | null;
}
 ...
 */
function parseBackendFlags(argv: unknown): FlagParse {
  ...
  if (wantsFleet && wantsInline) {
    return {
      ...
      reason:
        FLAG_FLEET + ' and ' + FLAG_INLINE + ' were both given. That is 2 '
...
function pushNotice(result: SwitchResult, level: string, code: string, text: string): void {
...
function callProvider(providers: Providers, name: keyof Providers, counters: Counters, fallback: unknown): unknown {
...
function readDetect(value: unknown): { fleetAvailable: boolean; reason: string } {
  if (value === null || value === undefined || typeof value !== 'object') {
    return { fleetAvailable: false, reason: 'detector_unavailable' };
  }
  const d = value as DetectLike;
...
function readRecommendation(value: unknown): { backend: string | null; reason: string } {
  if (value === null || value === undefined || typeof value !== 'object') {
    return { backend: null, reason: 'recommendation_unavailable:absent' };
  }
  const r = value as RecommendLike;
...
function readIntent(value: unknown): { ok: boolean; backend: string | null; phrase: string | null; reason: string } {
  if (value === null || value === undefined || typeof value !== 'object') {
    return { ok: true, backend: null, phrase: null, reason: 'no_intent_supplied' };
  }
  const v = value as IntentLike;
 ...
 */
function readExplicitDecision(flags: FlagParse, intent: unknown): ExplicitDecision {
  const none: ExplicitDecision = {
 ...
 */
function resolveExecutionBackend(argv: unknown, providers?: Providers | null, intent?: unknown): SwitchResult {
  const p: Providers = (providers !== null && providers !== undefined && typeof providers === 'object')
    ? providers
    : {};
  ...
  const phrase: string | null = decision.phrase;

  // ── Precedence levels 1 and 2: the flag, then the sentence. ──
  let requested: string | null = decision.backend;
  if (requested !== null) {
    result.precedence = decision.precedence as string;
    if (result.precedence === PRECEDENCE.LANGUAGE) {
      ...
      pushNotice(
        result, LEVEL_INFO, SWITCH_CODES.LANGUAGE_INTENT,
        'understood "' + String(phrase) + '" as backend ' + requested + '. Say '
    ...
    if (decision.overriddenBackend !== null) {
      ...
      pushNotice(
        result, LEVEL_WARN, SWITCH_CODES.LANGUAGE_OVERRIDDEN,
        'the words "' + String(phrase) + '" asked for backend ' + decision.overriddenBackend
    ...
    if (cfgRaw === BACKEND_FLEET || cfgRaw === BACKEND_INLINE) {
      ...
      if (rec.backend !== null) {
        ...
        result.detail = rec.reason;
        pushNotice(
          result, LEVEL_INFO, 'recommendation_unavailable',
          'no backend flag and no usable config default or recommendation ('
  ...
  if (!detect.fleetAvailable) {
    if (isExplicitDecision(result.precedence)) {
      ...
      pushNotice(
        result, LEVEL_ERROR, SWITCH_CODES.FLEET_UNAVAILABLE_REFUSED,
        'REFUSING: ' + result.reason + '. Nothing was executed. ' + escape,
      );
      return result;
    }
    ...
    pushNotice(
      result, LEVEL_WARN, SWITCH_CODES.FLEET_UNAVAILABLE_FALLBACK,
      'FALLING BACK: ' + result.reason + '. Pass ' + FLAG_FLEET + ' to make this a refusal instead.',
    );
    return result;
  }
  ...
  pushNotice(
    result, LEVEL_INFO, SWITCH_CODES.FLEET_ACTIVE,
    'dispatching through ' + CONSUMER_ENTRYPOINT + ' (' + result.precedence + '). Whether this '
 ...
 */
function formatDecisionLine(result: SwitchResult): string {
