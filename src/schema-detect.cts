...
export function checkSchemaDrift(
  ...
  const message = [
    ...
    pushCommands,
    '',
    'Run the appropriate push command, or set FERROX_SKIP_SCHEMA_CHECK=true to bypass this gate.',
  ].join('\n');
  if (skipCheck) {
    return {
      driftDetected: true,
      ...
      orms: detection.orms,
      unpushedOrms,
      message: 'Schema drift detected but check was skipped (FERROX_SKIP_SCHEMA_CHECK=true).',
    };
  }
  return {
    driftDetected: true,
