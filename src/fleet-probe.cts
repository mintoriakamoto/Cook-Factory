...
import externalCliMod = require('./external-cli.cjs');
 ...
 * it.
 */
const ADAPTER_PROFILES: Readonly<Record<string, readonly string[]>> = Object.freeze({
 ...
 */
const NOT_PROBED: Readonly<Record<string, string>> = Object.freeze({
...
/** The verdict over a whole roster. */
...
function _isUsableNonce(nonce: unknown): nonce is string {
  return typeof nonce === 'string' && nonce.trim() !== '';
}

/** Is this identity one the probe carries an argv profile for. */
function isDispatchable(identity: unknown): boolean {
 ...
 */
function buildProbePrompt(nonce: unknown): string {
  if (!_isUsableNonce(nonce)) {
    throw new Error(
      'fleet probe: the nonce must be a non empty string. An empty nonce is contained by every string, '
...
function buildProbeInvocation(
  ...
  if (!isDispatchable(identity)) {
    throw new Error(
      `fleet probe: no argv profile for adapter '${String(identity)}'. The dispatchable set is `
  ...
  const profile = ADAPTER_PROFILES[identity as string];
...
function evaluateProbe(input: {
  ...
  const shaped = result as { present?: unknown; code?: unknown; stdout?: unknown };
...
function evaluateRoster(input: {
  ...
  const source = (input === null || input === undefined) ? {} : input;
  const roster = Array.isArray(source.roster) ? (source.roster as unknown[]) : [];
  const named = roster.filter((id): id is string => typeof id === 'string' && id !== '');
  ...
  const verdicts = Array.isArray(source.verdicts) ? (source.verdicts as unknown[]) : [];
  for (const raw of verdicts) {
    if (raw === null || typeof raw !== 'object') continue;
    const v = raw as { identity?: unknown; verdict?: unknown };
    if (typeof v.identity !== 'string') continue;
    byIdentity.set(v.identity, typeof v.verdict === 'string' ? v.verdict : PROBE_VERDICTS.ERROR);
  }

  const notReady: string[] = [];
  for (const identity of named) {
    if (byIdentity.get(identity) !== PROBE_VERDICTS.READY) notReady.push(identity);
  }
 ...
 * is never touched.
 */
function extractEngineProfiles(text: unknown): Record<string, string[]> {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    if (/^PROFILES\s*=\s*\{\s*$/.test(lines[i])) { start = i; break; }
  }
  if (start === -1) {
    throw new Error(
      'fleet probe: no PROFILES literal found in the engine source. The roster agreement check cannot '
  ...
  if (Object.keys(profiles).length === 0) {
    throw new Error(
      'fleet probe: the PROFILES literal yielded 0 adapter identities. An empty extraction must refuse '
