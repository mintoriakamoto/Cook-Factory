...
type Transport = 'flux' | 'cli' | 'host';

interface ModelBackendResult {
  transport: Transport;
  /** The tier that was requested (echoed for the manifest/observability). */
  tier: string;
  /** The resolved model-id alias, or null when the tier isn't in the ladder. */
  modelId: string | null;
  /** Why this transport/model was chosen — for the evidence manifest + debugging. */
  reason: string;
}

/** Look up a tier in the ladder; null for a missing/garbage ladder or unknown rung. */
function ladderModel(tierModels: unknown, tier: unknown): { modelId: string | null; miss: string } {
  if (typeof tier !== 'string' || tier === '') return { modelId: null, miss: 'non-string-tier' };
  if (!tierModels || typeof tierModels !== 'object' || Array.isArray(tierModels)) {
    return { modelId: null, miss: 'no-ladder' };
  }
  const map = tierModels as Record<string, unknown>;
  if (!Object.hasOwn(map, tier)) return { modelId: null, miss: 'unknown-tier' };
  const v = map[tier];
...
function resolveModelBackend(opts?: {
  ...
  if (fluxWanted && fluxKey) {
    ...
    transport = 'cli';
    if (fluxWanted && !fluxKey) notes.push('no-flux-key');
  } else {
    transport = 'host';
    notes.push('host-native');
    if (fluxWanted && !fluxKey) notes.push('no-flux-key');
  }
  if (miss) notes.push(miss);
