...
/**
 * Dedicated Ferrox temp directory: path.join(os.tmpdir(), 'ferrox').
 * Created on first use. Keeps Ferrox temp files isolated from the system
 * temp directory so reap scans only Ferrox files (#1975).
 */
const FERROX_TEMP_DIR = path.join(os.tmpdir(), 'ferrox');

function ensureFerroxTempDir(): void {
  platformEnsureDir(FERROX_TEMP_DIR);
}
...
/**
 * Remove stale ferrox-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param prefix - filename prefix to match (e.g., 'ferrox-')
 * @param opts
 ...
 */
function reapStaleTempFiles(prefix = 'ferrox-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false }: ReapOptions = {}): void {
  try {
    ensureFerroxTempDir();
    const now = Date.now();
    const entries = fs.readdirSync(FERROX_TEMP_DIR);
    for (const entry of entries) {
      if (!entry.startsWith(prefix)) continue;
      const fullPath = path.join(FERROX_TEMP_DIR, entry);
      try {
 ...
 * instead of plain "Error: <message>" text. Set by ferrox-tools.cjs when the
