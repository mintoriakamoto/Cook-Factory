 ...
 * `.planning/codebase/STRUCTURE.md` map produced by `ferrox-codebase-mapper`.
 *
...
import { formatFerroxSlash } from './runtime-slash.cjs';
...
function buildMessage(elements: DriftElement[], affectedPaths: string[], action: string, runtime: string | undefined): string {
  ...
  if (action === 'auto-remap') {
    ...
    const mapCmd = formatFerroxSlash('map-codebase', runtime || 'claude');
    lines.push(
