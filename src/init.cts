...
function verificationNextCommand(
  ...
  if (status === 'gaps_found') {
    return `${formatFerroxSlash('plan-phase', slashRuntime) as string} ${phaseNumber} --gaps`;
  }
  if (status === 'human_needed' || status === 'stale') {
    return `${formatFerroxSlash('verify-work', slashRuntime) as string} ${phaseNumber}`;
  }
  if (status === 'missing' || status === 'unknown') {
    return `${formatFerroxSlash('execute-phase', slashRuntime) as string} ${phaseNumber}`;
  }
  return '';
}
...
function cmdInitExecutePhase(
  ...
  const result: Record<string, unknown> = {
    executor_model: resolveModelInternal(cwd, 'ferrox-executor'),
    verifier_model: resolveModelInternal(cwd, 'ferrox-verifier'),
...
function cmdInitPlanPhase(
  ...
  const result: Record<string, unknown> = {
    researcher_model: resolveModelInternal(cwd, 'ferrox-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'ferrox-planner'),
    checker_model: resolveModelInternal(cwd, 'ferrox-plan-checker'),
...
function cmdInitNewProject(cwd: string, raw: boolean): void {
  ...
  const braveKeyFile = path.join(homedir, '.ferrox', 'brave_api_key');
  const hasBraveSearch = !!(process.env['BRAVE_API_KEY'] || fs.existsSync(braveKeyFile));

  const firecrawlKeyFile = path.join(homedir, '.ferrox', 'firecrawl_api_key');
  const hasFirecrawl = !!(process.env['FIRECRAWL_API_KEY'] || fs.existsSync(firecrawlKeyFile));

  const exaKeyFile = path.join(homedir, '.ferrox', 'exa_api_key');
  ...
  const result: Record<string, unknown> = {
    researcher_model: resolveModelInternal(cwd, 'ferrox-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'ferrox-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'ferrox-roadmapper'),
...
function cmdInitQuick(cwd: string, description: string | undefined, raw: boolean): void {
  ...
  const result: Record<string, unknown> = {
    ...
    checker_model: resolveModelInternal(cwd, 'ferrox-plan-checker'),
    verifier_model: resolveModelInternal(cwd, 'ferrox-verifier'),
    // #2072: the quick review step spawns ferrox-code-reviewer; resolve its own model
    // so model_overrides / models.verification apply (was reusing executor_model).
    reviewer_model: resolveModelInternal(cwd, 'ferrox-code-reviewer'),
...
function cmdInitMapCodebase(cwd: string, raw: boolean): void {
  ...
  const result: Record<string, unknown> = {
    mapper_model: resolveModelInternal(cwd, 'ferrox-codebase-mapper'),
...
function cmdInitManager(cwd: string, raw: boolean): void {
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error(`No ROADMAP.md found. Run ${formatFerroxSlash('new-milestone', _slashRuntime) as string} first.`);
  }
  if (!fs.existsSync(paths.state)) {
    error(`No STATE.md found. Run ${formatFerroxSlash('new-milestone', _slashRuntime) as string} first.`);
  }
  ...
  for (const phase of phases) {
    ...
    if (phase['disk_status'] === 'executed') {
      ...
      recommendedActions.push({
        ...
        command: `${formatFerroxSlash('execute-phase', _slashRuntime) as string} ${phase['number'] as string}`,
      });
    } else if (
      ...
      recommendedActions.push({
        ...
        command: `${formatFerroxSlash('plan-phase', _slashRuntime) as string} ${phase['number'] as string}`,
      });
    } else if (
      (phase['disk_status'] === 'empty' || phase['disk_status'] === 'no_directory') &&
      phase['is_next_to_discuss']
      ...
      recommendedActions.push({
        ...
        command: `${formatFerroxSlash('discuss-phase', _slashRuntime) as string} ${phase['number'] as string}`,
      });
    }
  }
  ...
  const sanitizeFlags = (rawVal: unknown): string => {
    ...
    if (!safe) {
      process.stderr.write(
        `ferrox-tools: warning: manager.flags contains invalid tokens, ignoring: ${val}\n`,
      );
      return '';
    }
    return val;
...
function cmdInitProgress(cwd: string, raw: boolean): void {
  ...
  // Mirror planningDir's resolution (FERROX_WORKSTREAM env > stored active pointer) so
  // an explicit --ws (which sets FERROX_WORKSTREAM) satisfies the check.
  const _availableWorkstreams = listAvailableWorkstreams(cwd);
  const _resolvedWorkstream = process.env['FERROX_WORKSTREAM'] || getActiveWorkstream(cwd);
  if (_availableWorkstreams.length > 0 && !_resolvedWorkstream) {
    error(
      `init.progress requires a workstream in workstream mode — no active workstream is set, so root STATE.md (likely stale) would be reported. ` +
        `Pass --ws <name> or run ${formatFerroxSlash('workstream set', _slashRuntime) as string} first. ` +
...
function cmdInitNewWorkspace(cwd: string, raw: boolean): void {
  ...
  const defaultBase = path.join(homedir, 'ferrox-workspaces');
...
interface SkillManifest {
  ...
  roots: RootSummary[];
  installation: {
    ferrox_skills_installed: boolean;
...
function buildSkillManifest(cwd: string, skillsDir: string | null = null): SkillManifest {
  ...
  const canonicalRoots: CanonicalRoot[] = skillsDir
    ...
    : [
        ...
        },
        {
          ...
          // Ferrox no longer installs here (#2088).
          root: '~/.codex/skills',
        ...
        {
          root: '.claude/ferrox-core/skills',
          path: path.join(os.homedir(), '.claude', 'ferrox-core', 'skills'),
          scope: 'import-only',
          kind: 'skills',
          deprecated: true,
        },
        {
          root: '.claude/commands/ferrox',
          path: path.join(os.homedir(), '.claude', 'commands', 'ferrox'),
  ...
  for (const rootInfo of canonicalRoots) {
    ...
    // (e.g. spec-phase nested under both ferrox-ns-workflow and ferrox-ns-manage).
    ...
    for (const entry of entries) {
      ...
      // Scope to ferrox-ns-* routers only: never vacuum up an unrelated user skill
      // that happens to have its own `skills/` subdirectory.
      if (!entry.name.startsWith('ferrox-ns-')) continue;
  ...
  const ferroxSkillsInstalled = skills.some((skill) => skill.name.startsWith('ferrox-'));

  return {
    skills,
    roots,
    installation: {
      ferrox_skills_installed: ferroxSkillsInstalled,
