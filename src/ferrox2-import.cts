/**
 * ferrox2-import — Reverse migration from Ferrox-2 (.ferrox/) to Ferrox v1 (.planning/)
 *
 * Reads a Ferrox-2 project directory structure and produces a complete
 * .planning/ artifact tree in Ferrox v1 format.
 *
 * Ferrox-2 hierarchy:  Milestone → Slice → Task
 * Ferrox v1 hierarchy: Milestone (in ROADMAP.md) → Phase → Plan
 *
 * Mapping rules:
 ...
 *
 * ADR-457 build-at-publish: the hand-written bin/lib/ferrox2-import.cjs collapsed
...
/**
 * Find the .ferrox/ directory starting from a project root.
 * Returns the absolute path or null if not found.
 */
function findFerrox2Root(startPath: string): string | null {
  if (path.basename(startPath) === '.ferrox' && fs.existsSync(startPath)) {
    return startPath;
  }
  const candidate = path.join(startPath, '.ferrox');
...
/**
 * Parse the ## Slices section from a Ferrox-2 milestone ROADMAP.md.
...
/**
 * Parse the milestone title from the first heading in a Ferrox-2 ROADMAP.md.
 * Format: # M001: Title
 */
...
/**
 * Parse a task title from a Ferrox-2 T##-PLAN.md.
 * Format: # T01: Title
 */
...
/**
 * Parse the ## Description body from a Ferrox-2 task plan.
 */
...
/**
 * Parse ## Must-Haves items from a Ferrox-2 task plan.
 */
...
/**
 * Read all task plan files from a Ferrox-2 tasks/ directory.
 */
...
/**
 * Parse a complete Ferrox-2 .ferrox/ directory into a structured representation.
 */
function parseFerrox2(ferroxDir: string): Ferrox2Data {
  const data: Ferrox2Data = {
    projectContent: readOptional(path.join(ferroxDir, 'PROJECT.md')),
    requirements: readOptional(path.join(ferroxDir, 'REQUIREMENTS.md')),
    milestones: [],
  };

  const milestonesBase = path.join(ferroxDir, 'milestones');
...
/**
 * Build a Ferrox v1 PLAN.md from a Ferrox-2 task.
 */
...
/**
 * Build a Ferrox v1 SUMMARY.md from a Ferrox-2 task summary.
 * Strips the Ferrox-2 frontmatter and preserves the body.
 */
function buildSummaryMd(task: TaskInfo, phasePrefix: string, planPrefix: string): string {
  const raw = task.summary || '';
  // Strip Ferrox-2 frontmatter block (--- ... ---) if present
  ...
  return [
    ...
    '---',
    '',
    body || 'Task completed (migrated from Ferrox-2).',
    '',
  ].join('\n');
}

/**
 * Build a Ferrox v1 XX-CONTEXT.md from a Ferrox-2 slice.
 */
function buildContextMd(slice: Slice, phasePrefix: string): string {
  const lines = [
    `# Phase ${phasePrefix} Context`,
    '',
    `Migrated from Ferrox-2 slice ${slice.id}: ${slice.title}`,
  ];
...
/**
 * Build the Ferrox v1 ROADMAP.md with milestone-sectioned format.
 */
...
/**
 * Build the Ferrox v1 STATE.md reflecting the current position in the project.
 */
...
/**
 * Convert parsed Ferrox-2 data into a map of relative path → file content.
 * All paths are relative to the .planning/ root.
 */
function buildPlanningArtifacts(ferrox2Data: Ferrox2Data): Map<string, string> {
  const artifacts = new Map<string, string>();

  // Passthrough files
  artifacts.set('PROJECT.md', ferrox2Data.projectContent || '# Project\n\n(Migrated from Ferrox-2)\n');
  if (ferrox2Data.requirements) {
    artifacts.set('REQUIREMENTS.md', ferrox2Data.requirements);
  }
  ...
  let phaseNum = 1;
  for (const milestone of ferrox2Data.milestones) {
  ...
  artifacts.set('ROADMAP.md', buildRoadmapMd(ferrox2Data.milestones, phaseMap));
 ...
 */
function buildPreview(ferrox2Data: Ferrox2Data, artifacts: Map<string, string>, projectDir: string): string {
  ...
  const totalSlices = ferrox2Data.milestones.reduce((s, m) => s + m.slices.length, 0);
  const doneSlices = ferrox2Data.milestones.reduce((s, m) => s + m.slices.filter(sl => sl.done).length, 0);
  const allTasks = ferrox2Data.milestones.flatMap(m => m.slices.flatMap(sl => sl.tasks));
  const doneTasks = allTasks.filter(t => t.done).length;

  lines.push('');
  lines.push(`Milestones: ${ferrox2Data.milestones.length}`);
  ...
  lines.push('  - Ferrox-2 cost/token ledger (no v1 equivalent)');
  lines.push(`  - Ferrox-2 database state (rebuilt from files on first ${formatFerroxSlash('health', resolveRuntime(projectDir)) as string})`);
...
/**
 * Entry point called from ferrox-tools.cjs.
 * Supports: --force, --dry-run, --path <dir>
 */
function cmdFromFerrox2(args: string[], cwd: string, raw: boolean): void {
  ...
  const ferroxDir = findFerrox2Root(projectDir);
  if (!ferroxDir) {
    output({ success: false, error: `No .ferrox/ directory found in ${projectDir}` }, raw, undefined);
    return;
  }

  const planningRoot = path.join(path.dirname(ferroxDir), '.planning');
  ...
  const ferrox2Data = parseFerrox2(ferroxDir);
  const artifacts = buildPlanningArtifacts(ferrox2Data);
  ...
  const preview = buildPreview(ferrox2Data, artifacts, projectDir);
  ...
  output({
    ...
    milestones: ferrox2Data.milestones.length,
    preview,
  }, raw, undefined);
}
