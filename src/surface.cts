 ...
 *
 * Manages the runtime enable/disable surface state (the `.ferrox-surface.json` marker in
 * each runtime's config dir root (e.g., ~/.claude)) independently of the install-time profile marker
 * (`.ferrox-profile`). Runtime config locations are resolved by callers.
 *
...
const SURFACE_FILE_NAME = '.ferrox-surface.json';
 ...
 *  - parsed ferrox-file-manifest.json object ({ files: { ... } }) by rebuilding
...
function normalizeSkillManifest(runtimeConfigDir: string, manifest: Map<string, string[]> | object | null | undefined): Map<string, string[]> {
  ...
  // Parsed ferrox-file-manifest.json shape: rebuild the dependency map from source.
...
function resolveSurface(runtimeConfigDir: string, manifest: Map<string, string[]> | object, clusterMap?: ClusterMap | Record<string, string[]>, registry?: { capabilityClusters?: Record<string, string[]>; profileMembership?: Record<string, { tier: string; profiles: string[] }> }): { name: string; skills: Set<string>; agents: Set<string> } {
  ...
  // Determine base profile name: from surface state or from .ferrox-profile marker
  ...
  if (baseResolved.skills === '*') {
    ...
    // ~/.ferrox/capabilities/<id>/skills/<stem>/SKILL.md — NOT in the runtime skills
...
function applySurface(runtimeConfigDir: string, layout: Layout, manifest: Map<string, string[]> | object, clusterMap?: ClusterMap | Record<string, string[]>, registry?: { capabilityClusters?: Record<string, string[]>; profileMembership?: Record<string, { tier: string; profiles: string[] }> }, opts?: ApplySurfaceOptions): { name: string; skills: Set<string>; agents: Set<string> } {
  ...
  try {
    for (const kind of layout.kinds) {
      ...
      _syncFerroxDir(staged, dest, kind, skillManifest, layout.runtime);
    }
  } finally {
...
/**
 * Prune Ferrox-managed skill directories from a skills directory.
 *
 * Removes every directory in `skillsDir` that is Ferrox-owned but NOT listed
 * in `retainedNames`. User-owned dirs (not matching the Ferrox ownership criteria)
 * are always preserved.
 *
 * Ownership criteria:
 *   - Non-empty prefix (e.g. 'ferrox-'): dir name starts with that prefix AND
 ...
 *     preserved — this prevents data loss for user-created ferrox-* directories.
 ...
 *     dirs are removed.
 *
 * This is the single point of truth for skill-dir pruning. Both _syncFerroxDir
 * (surface apply) and callers that need stand-alone pruning use this function.
 *
 * @param skillsDir        directory that contains the ferrox-STEM sub-dirs
 * @param retainedNames    set of directory names to keep (e.g. 'ferrox-help')
 * @param prefix           Ferrox dir prefix, e.g. 'ferrox-' (or '' for Hermes)
...
function pruneSkillDirs(skillsDir: string, retainedNames: Set<string>, prefix: string, manifest?: Map<string, string[]>): void {
  ...
  for (const entry of fs.readdirSync(skillsDir)) {
    ...
    if (prefix !== '') {
      ...
      // The dir must also be in the manifest to be considered Ferrox-owned.
      // A user-created ferrox-* dir that isn't in the manifest is preserved with a warning.
      if (!canonicalStems.has(entry.slice(prefix.length))) {
        process.stderr.write(
          `[ferrox] Warning: ${entry} matches Ferrox prefix '${prefix}' but is not in the manifest — preserving (user-owned or unknown)\n`
        );
        continue;
      }
      isFerroxOwned = true;
    } else if (canonicalStems) {
      // Hermes: Ferrox-owned iff the directory name appears in the canonical manifest.
      isFerroxOwned = canonicalStems.has(entry);
    } else {
      // No manifest available: be conservative, don't remove anything.
      continue;
    }

    if (!isFerroxOwned) continue;         // Hermes path only: preserve user-owned dirs not in manifest
    if (retainedNames.has(entry)) continue; // Ferrox-owned and in retain set
    try {
 ...
 * For 'agents' kind: same, but only remove files starting with 'ferrox-' prefix.
 ...
 *
 * For Hermes (empty prefix): uses manifest membership to discriminate Ferrox-owned vs
 * user-owned dirs. Ferrox-owned = stem in manifest; removal targets = in manifest AND
 * not in staged set. User-owned (not in manifest) are always preserved.
 */
function _syncFerroxDir(stagedDir: string, destDir: string, kind: ArtifactKind | string, manifest?: Map<string, string[]>, runtime?: string): void {
  ...
  const kindPrefix = (typeof kind === 'object' && kind !== null) ? kind.prefix : 'ferrox-';
  ...
  if (kindName === 'skills') {
    ...
    // Prune Ferrox-owned dirs that are no longer in the staged set.
    ...
    // _removeFerroxEntries in bin/install.js) so surface produces the SAME files as a
    // fresh install (#816). Flat command dirs (opencode/cursor/augment/kilo) take
    // the ferrox- prefix on copy; namespaced command dirs (commands/ferrox) and agents
    // keep their staged names. Copying staged names verbatim previously diverged
    // from install and orphaned the installed ferrox-*.md files, and the unscoped
    ...
    // Prune stale Ferrox-owned files not in the staged set, preserving user-owned files
    // (mirrors install's prefix-scoped _removeFerroxEntries):
    //   - agents: only ferrox-* are Ferrox-owned (copilot: ferrox-*.agent.md)
    //   - flat command dirs: only `${kindPrefix}`-prefixed are Ferrox-owned
    //   - namespaced command dirs: the whole dir is Ferrox-owned
    ...
    if (shouldPruneAgents) {
      for (const file of fs.readdirSync(destDir).filter(f => f.endsWith('.md'))) {
        if (kindName === 'agents' && !file.startsWith('ferrox-')) continue;
