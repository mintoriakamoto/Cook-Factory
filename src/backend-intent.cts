 ...
 */
const LEADS: readonly string[] = Object.freeze([
  'use', 'used', 'using',
  'run', 'runs', 'running',
 ...
 */
const CONNECTORS: readonly string[] = Object.freeze([
...
const NEGATORS: readonly string[] = Object.freeze([
 ...
 */
const FLEET_LED_OBJECTS: readonly (readonly string[])[] = Object.freeze([
...
const FLEET_STANDALONE_OBJECTS: readonly (readonly string[])[] = Object.freeze([
  Object.freeze(['ferrox', 'fleet', 'mode']),
  Object.freeze(['fleet', 'mode']),
]);

const INLINE_LED_OBJECTS: readonly (readonly string[])[] = Object.freeze([
  Object.freeze(['inline']),
  Object.freeze(['solo']),
]);

const INLINE_STANDALONE_OBJECTS: readonly (readonly string[])[] = Object.freeze([
...
interface IntentCounters {
  texts: number;
  candidates: number;
  fleet_matches: number;
  inline_matches: number;
  negations: number;
}
...
function zeroCounters(): IntentCounters {
  return { texts: 0, candidates: 0, fleet_matches: 0, inline_matches: 0, negations: 0 };
}

/** Does `pattern` sit at `tokens[at]`. */
function patternAt(tokens: readonly string[], at: number, pattern: readonly string[]): boolean {
  if (at + pattern.length > tokens.length) return false;
  for (let k = 0; k < pattern.length; k += 1) {
    if (tokens[at + k] !== pattern[k]) return false;
  }
  return true;
}
 ...
 */
function findLead(tokens: readonly string[], objectStart: number): number {
  let crossed = 0;
  let i = objectStart - 1;
  while (i >= 0 && crossed <= MAX_CONNECTORS) {
    const word = tokens[i];
    if (LEADS.indexOf(word) !== -1) return i;
    ...
    if (!numeric && CONNECTORS.indexOf(word) === -1) return -1;
    crossed += 1;
    i -= 1;
  }
  return -1;
 ...
 */
function negatorIndex(tokens: readonly string[], at: number): number {
  const from = at - NEGATION_WINDOW < 0 ? 0 : at - NEGATION_WINDOW;
  for (let i = from; i < at; i += 1) {
    if (NEGATORS.indexOf(tokens[i]) !== -1) return i;
  }
  return -1;
}
 ...
 */
function detectBackendIntent(text: unknown): IntentResult {
  ...
  const groups: { objects: readonly (readonly string[])[]; backend: string; led: boolean }[] = [
  ...
  const claimed: boolean[] = new Array(tokens.length).fill(false) as boolean[];

  for (const group of groups) {
    for (const pattern of group.objects) {
      counters.candidates += 1;
      for (let i = 0; i < tokens.length; i += 1) {
        if (!patternAt(tokens, i, pattern)) continue;
        let overlaps = false;
        for (let k = 0; k < pattern.length; k += 1) {
          if (claimed[i + k] === true) overlaps = true;
        }
        ...
        for (let k = start; k < i + pattern.length; k += 1) claimed[k] = true;
        const phrase = tokens.slice(start, i + pattern.length).join(' ');
        result.matches.push({ backend, phrase, negated });
        if (backend === BACKEND_FLEET) counters.fleet_matches += 1;
        else counters.inline_matches += 1;
      }
    }
  }
