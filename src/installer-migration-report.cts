 ...
 * runs without a TTY (typical /ferrox:update path via Claude Code or any
...
export const RESOLUTION_ENV_VAR = 'FERROX_INSTALLER_MIGRATION_RESOLVE';
...
// set — a shape regex like `^hooks/ferrox-[^/]+\.(?:js|sh|cjs|mjs)$` also
...
export const BUNDLED_FERROX_HOOK_FILES: ReadonlySet<string> = Object.freeze(new Set([
  'hooks/ferrox-check-update-worker.js',
  'hooks/ferrox-check-update.js',
  'hooks/ferrox-config-reload.js',
  'hooks/ferrox-context-monitor.js',
  'hooks/ferrox-cursor-post-tool.js',
  'hooks/ferrox-cursor-pre-tool.js',
  'hooks/ferrox-cursor-session-start.js',
  'hooks/ferrox-cursor-stop.js',
  'hooks/ferrox-cursor-subagent-start.js',
  'hooks/ferrox-cursor-subagent-stop.js',
  // Windsurf/Cascade blocking hooks — registered by writeWindsurfHooksJson (#2100).
  'hooks/ferrox-windsurf-pre-write.js',
  'hooks/ferrox-windsurf-pre-command.js',
  'hooks/ferrox-ensure-canonical-path.js',
  'hooks/ferrox-graphify-update.sh',
  'hooks/ferrox-phase-boundary.sh',
  'hooks/ferrox-prompt-guard.js',
  'hooks/ferrox-read-guard.js',
  'hooks/ferrox-read-injection-scanner.js',
  'hooks/ferrox-session-state.sh',
  'hooks/ferrox-statusline.js',
  'hooks/ferrox-update-banner.js',
  'hooks/ferrox-validate-commit.sh',
  'hooks/ferrox-workflow-guard.js',
  'hooks/ferrox-worktree-path-guard.js',
]));
...
//
// Stale SDK build artifacts live under ferrox-core/sdk/{dist,src}/
...
export function classifyPromptUserAction(action: MigrationAction): ClassifyResult | null {
  ...
  if (/^ferrox-core\/sdk\/(dist|src)\//.test(relPath)) {
    return { category: 'stale-sdk-build-artifact', choice: 'remove' };
  }
  if (/^skills\/ferrox-[^/]+\/SKILL\.md$/.test(relPath)) {
    return { category: 'user-facing-skill', choice: 'keep' };
  }
  // #3610 / #3628: bundled Ferrox hooks shipped under `hooks/`. The whitelist
  ...
  if (BUNDLED_FERROX_HOOK_FILES.has(relPath)) {
    return { category: 'bundled-ferrox-hook', choice: 'remove' };
  }
  return null;
}
...
// migration journal under ferrox-migration-journal/<runId>-backups/).
