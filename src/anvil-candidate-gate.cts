...
type AnvilAction = 'accept-candidate' | 'reject' | 'fallback-normal';

interface AnvilCandidateDecision {
  action: AnvilAction;
  reason: string;
}
...
function decideAnvilCandidate(opts?: {
  parsed?: unknown;
  candidatePresent?: unknown;
}): AnvilCandidateDecision {
  ...
  const parsed = o.parsed && typeof o.parsed === 'object' ? (o.parsed as Record<string, unknown>) : null;
