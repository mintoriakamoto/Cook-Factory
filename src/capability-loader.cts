 ...
 *   - global:  $FERROX_HOME/.ferrox/capabilities/<id>/capability.json  (FERROX_HOME defaults to ~)
 *   - project: <projectRoot>/.ferrox/capabilities/<id>/capability.json
 *
 ...
 *     federated config key collides with first-party (or uses a reserved `ferrox-` /
 *     `ferrox-core-` / `anthropic-` id prefix) is rejected.
 *   - Load-time re-gate (default-resilient): an overlay that fails validation or
 *     whose `engines.ferrox` does not satisfy the running Ferrox version is SKIPPED
...
export interface LoadRegistryOptions {
  ...
  cwd?: string;
  /** Override the global overlay home (defaults to FERROX_HOME env or os.homedir()). */
  ferroxHome?: string;
  /** Override the running Ferrox version used for engines.ferrox satisfaction. */
  hostVersion?: string;
  /**
...
export interface OverlaySkip {
  ...
  reason: string;
  /**
   * #1459 IC-02: a STRUCTURAL discriminant for the skip so consumers (ferrox-tools `list`) classify a
...
export interface OverlayMeta {
   ...
   * declares `commands` — `capId → <scope>/.ferrox/capabilities/<capId>`. First-party capabilities
...
const RESERVED_ID_PREFIX = /^(ferrox-|ferrox-core-|anthropic-)/;
const FERROX_HOME_DIRNAME = '.ferrox';
/**
...
/**
 * Resolve the running Ferrox version; fail-closed to '0.0.0' if it cannot be read.
 *
 * Prefer the authoritative `ferrox-core/VERSION` the installer writes for EVERY runtime
 * (libDir = ferrox-core/bin/lib/, so `../../VERSION` = ferrox-core/VERSION). This is reliable
 ...
 * else a symlinked FERROX_HOME aliasing the project root is scanned once as trusted 'global' BEFORE the
 * 'project' scan and the in-repo `.ferrox/capabilities` bundle bypasses the CB-3 consent gate via aliasing.
 ...
 * preserving the candidate's ORIGINAL scope — was not strictly fail-safe: a symlinked FERROX_HOME whose
 ...
 * under the Ferrox home, or in tests where both resolve to the same fixture).
 *
 * #1459 CB-3: when the consent-global home resolves EQUAL to (or an ancestor whose .ferrox collides with)
 ...
 * in-repo bundle bypass consent simply because FERROX_HOME pointed at the repo. On a collision the
 ...
 * total — it returns `cwd` itself when no marker exists — so a bare FERROX_HOME with no project marker
 ...
 * realpath'd path (canonicalDir), so a symlinked FERROX_HOME that physically IS the project root collides
 ...
 * symlinked-FERROX_HOME bypass: when FERROX_HOME aliases the project root, the GLOBAL candidate realpaths fine
...
function overlayRoots(cwd: string, ferroxHome?: string): Array<{ dir: string; scope: 'global' | 'project' }> {
  ...
  const add = (dir: string, scope: 'global' | 'project', canonical: { path: string; realpathFailed: boolean }, genuineProject = false): void => {
    ...
    // (realpath'd) path, so a symlinked FERROX_HOME that physically IS the project root collides here (and
    ...
    if (existing) {
      ...
      // root — so a real FERROX_HOME == projectRoot (incl. via a symlink) still requires consent, while a
  ...
  const home = ferroxHome || process.env['FERROX_HOME'] || os.homedir();
  const globalDir = path.join(home, FERROX_HOME_DIRNAME, 'capabilities');
  ...
  const projectDir = projectRoot ? path.join(projectRoot, FERROX_HOME_DIRNAME, 'capabilities') : null;
  ...
  // realpath-FAILED *global* candidate) left the symlinked-FERROX_HOME bypass open: when FERROX_HOME is a symlink
  ...
  if (projectRoot && projectCanon && hasGenuineProjectMarker(projectRoot)) {
    ...
    //   - both canonicalize EQUAL (an alias — FERROX_HOME physically IS the project root).
...
/**
 * Read the per-scope ledger co-located with an overlay root (the root is `<scope>/.ferrox/capabilities`,
 * so its ledger is `<scope>/.ferrox-capabilities.json`) and classify its ids:
...
function ledgerOverlayIds(ledger: LedgerModule, rootDir: string): {
  ...
  try {
    const ledgerPath = path.join(rootDir, '..', '..', '.ferrox-capabilities.json');
...
export function loadRegistry(options: LoadRegistryOptions = {}): Registry {
  ...
  // The user-owned consent home — SAME `ferroxHome || FERROX_HOME || homedir()` rule the CLI uses, so the
  // consent the CLI records is the consent the loader checks. The consent store NEVER lives in a repo.
  const ferroxHome = options.ferroxHome || process.env['FERROX_HOME'] || os.homedir();
  ...
  for (const root of overlayRoots(cwd, options.ferroxHome)) {
    ...
    for (const ent of entries) {
      ...
      // (loop-resolver, config-loader, surface, capability-state, ferrox-tools). ADR-1244 D2 mandates a
      ...
      if (RESERVED_ID_PREFIX.test(id)) {
        skip('id uses a reserved first-party prefix (ferrox-/ferrox-core-/anthropic-)');
        continue;
      }
      ...
      // 4. Load-time engines.ferrox re-gate.
      const range = cap.engines?.ferrox;
      if (typeof range === 'string' && range && !semver.semverSatisfies(hostVersion, range)) {
        skip('incompatible with Ferrox ' + hostVersion + ' (requires engines.ferrox "' + range + '")');
        continue;
      }
      ...
      if (root.scope === 'project') {
        ...
        if (!consented) {
          ...
          // structural `kind: 'unconsented'` so ferrox-tools `list` marks it INACTIVE by discriminant, not
      ...
      // capability-state, ferrox-tools). ADR-1244 D2: a malformed overlay is SKIPPED with a warning,
