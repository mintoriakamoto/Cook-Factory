...
const {
  ...
  readFerroxCommandNames: _readFerroxCommandNames,
} = conversionModule as {
 ...
 * since Ferrox enforces flow-style arrays for requires:.
 */
 ...
 * Scans the full content for `ferrox-<stem>` patterns that correspond to
 * real agent files. Returns all unique `ferrox-*` stems found in the body.
 *
 * The caller is responsible for filtering by which agents actually exist —
 * this function returns all syntactically valid `ferrox-*` matches.
 */
function parseCallsAgents(content: string): string[] {
  // Match word-boundary ferrox-<stem> patterns; stems are lowercase letters and hyphens.
  // We use a regex that matches `ferrox-` followed by one or more lowercase-alpha-or-hyphen chars.
  // This catches `ferrox-planner`, `ferrox-plan-checker`, etc. in prose and code.
  const matches = content.match(/\bferrox-[a-z][a-z-]*/g);
  if (!matches) return [];
  // Deduplicate
...
/**
 * Load the requires: dependency graph from a commands/ferrox directory.
 * Also derives calls_agents for each skill by scanning the body text for
 * `ferrox-*` agent name references. Agent stems are stored under the special
 * key `_calls_agents_<stem>` so they don't conflict with skill stems.
 */
const DEFAULT_COMMANDS_DIR = path.resolve(__dirname, '..', '..', '..', 'commands', 'ferrox');
...
/**
 * Stage a filtered copy of commands/ferrox for a resolved profile.
...
function stageSkillsForProfile(srcDir: string, resolvedProfile: ResolvedProfile): string {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-profile-skills-'));
  try {
 ...
 * For tiered profiles, copies only agents whose full stem (e.g. 'ferrox-planner')
...
function stageAgentsForProfile(srcAgentsDir: string, resolvedProfile: ResolvedProfile): string {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-profile-agents-'));
  try {
    if (resolvedProfile.agents instanceof Set && resolvedProfile.agents.size > 0) {
      ...
      for (const entry of entries) {
        if (!entry.isFile()) continue;
        if (!entry.name.endsWith('.md')) continue;
        // Agent stem is the full filename without extension, e.g. "ferrox-planner"
        const stem = entry.name.slice(0, -3);
 ...
 * whose final cell carries a `ferrox-<stem>` token (optionally with `--flag`
...
function transformRouterBodyToNested(converted: string): string {
  ...
  const out = lines.map((line) => {
    ...
    const m = cell.match(/ferrox-([a-z0-9-]+)((?:\s+--[a-z0-9-]+)*)/i);
    if (!m) return line;
    const stem = m[1];
...
function stageSkillsForRuntimeAsSkills(
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-profile-runtime-skills-'));
  try {
 ...
 * added here — the prefix is already embedded in agent stems, e.g. `ferrox-planner.md`).
 *
...
function stageAgentsForRuntimeWithConverter(
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-profile-runtime-agents-'));
  try {
    const entries = fs.readdirSync(srcAgentsDir, { withFileTypes: true });
    // Resolve cmdNames once per staging call (not per file) for performance.
    const cmdNames = agentCtx ? _readFerroxCommandNames() : [];
    for (const entry of entries) {
      if (!entry.isFile()) continue;
      if (!entry.name.endsWith('.md')) continue;
...
function stageCommandsForRuntimeFlat(
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-profile-runtime-commands-'));
  try {
    const entries = fs.readdirSync(srcCommandsDir, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isFile()) continue;
...
const PROFILE_MARKER_NAME = '.ferrox-profile';
...
function resolveEffectiveProfile({ requestedProfileName, targetDir }: ResolveEffectiveProfileOpts): string {
  ...
  // 2. Marker-driven (ferrox update path)
...
/**
 * Stage a filtered copy of the source commands/ferrox directory.
...
function stageSkillsForMode(srcDir: string, mode: string): string {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-minimal-skills-'));
  try {
    const entries = fs.readdirSync(srcDir, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isFile()) continue;
