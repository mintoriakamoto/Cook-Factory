 ...
 * `ferrox-tools capability state` diagnostic share the same enablement answer.
 *
 ...
 *   - ./runtime-slash.cjs      (resolveRuntime — FERROX_RUNTIME > config.runtime > 'claude' precedence)
...
/**
 * Derive the commands/ferrox path from __dirname (which resolves to
 * ferrox-core/bin/lib/ at runtime). The source tree is:
 *   <repo>/ferrox-core/bin/lib/capability-state.cjs
 *   <repo>/commands/ferrox/*.md
 * So we walk up three levels: lib/ → bin/ → ferrox-core/ → <repo>/, then
 * into commands/ferrox/.
 */
function _resolveCommandsFerroxDir(): string {
  // __dirname = ferrox-core/bin/lib/
  const repoRoot = path.resolve(__dirname, '..', '..', '..');
  return path.join(repoRoot, 'commands', 'ferrox');
}

/**
 * Build a skill dependency manifest from an INSTALLED runtime's skills directory.
 *
 * In an installed runtime (e.g. Codex at ~/.codex), ferrox skills live as
 * configDir/skills/ferrox-STEM/SKILL.md. There is no commands/ferrox source tree.
 * This function scans that installed layout and builds the same
 * Map shape that loadSkillsManifest produces from sources.
 *
 * Stem extraction: a directory named ferrox-secure-phase maps to stem secure-phase.
 * Only directories whose names start with ferrox- are included so user-created
 * skills (without the ferrox- prefix) are not accidentally pulled in.
 *
...
function _loadInstalledSkillsManifest(configDir: string): Map<string, string[]> {
  ...
  for (const entry of entries) {
    ...
    // Strip the 'ferrox-' prefix to get the skill stem
    const stem = entry.name.slice(4); // 'ferrox-'.length === 4
    if (!stem) continue;
    ...
    // no file exists. Mirror that here: a stale ferrox-<stem>/ directory with no
...
/**
 * #1858 — Build a skill dependency manifest from a FLAT commands/ferrox-<stem>.md
 * source layout (the Claude local project install shape, where the `ferrox-`
 * prefix is baked into each filename at the commands/ level and there is no
 * commands/ferrox/ subdir). Strips the `ferrox-` prefix so stems match the nested
 * loader's output (ferrox-validate-phase.md → validate-phase, same as nested
 * validate-phase.md).
 *
 ...
 * no ferrox-*.md files (so _resolveManifest can use size>0 as the "flat layout
 * present" signal and fall through to the installed-skills branch otherwise).
 */
function _loadFlatCommandsFerroxManifest(commandsParentDir: string): Map<string, string[]> {
  const manifest = new Map<string, string[]>();
  let entries: fs.Dirent[];
  try {
  ...
  for (const entry of entries) {
    ...
    // Strip 'ferrox-' prefix (4 chars) and '.md' suffix (3 chars) → stem.
 ...
 *
 * Resolution order:
 *   1. If commandsFerroxDir exists, load from the nested source layout
 *      (repo-checkout behavior: <repo>/commands/ferrox/*.md).
 *   2. #1858 — otherwise, if the flat source layout is present (ferrox-<stem>.md
 *      files in dirname(commandsFerroxDir)), load from there. This is the Claude
 *      local project install shape where commands/ferrox/ does not exist but
 *      commands/ferrox-<stem>.md files do.
 *   3. #1160 — otherwise, fall back to installed skills at
 *      configDir/skills/ferrox-[stem]/SKILL.md.
 *
 ...
 */
function _resolveManifest(commandsFerroxDir: string, configDir: string): Map<string, string[]> {
  if (fs.existsSync(commandsFerroxDir)) {
    return loadSkillsManifest(commandsFerroxDir);
  }
  // #1858: flat source layout — ferrox-<stem>.md files at dirname(commandsFerroxDir).
  // Only claim the flat branch when it actually has ferrox-*.md files; otherwise
  // fall through to the installed-skills branch (a commands/ dir with no ferrox
  // files must not shadow an installed skills/ tree).
  const flat = _loadFlatCommandsFerroxManifest(path.dirname(commandsFerroxDir));
 ...
 *     process.env.FERROX_RUNTIME → config.runtime → 'claude'
...
function resolveCapabilityRuntimeState(
  ...
  // precedence:  process.env.FERROX_RUNTIME → config.runtime → 'claude'
  ...
  if (!resolvedConfigDir) {
    try {
      ...
      // fallback (FERROX_RUNTIME → config.runtime → 'claude') so, e.g., a repo with
      ...
      if (!resolvedConfigDir) {
        ...
        // Detect the active runtime via FERROX_RUNTIME → config.runtime → 'claude'.
  ...
  // #1459 IC-04: thread the consent home (process.env.FERROX_HOME) EXPLICITLY so the overlay's global root
  ...
  const registry = loadRegistry({ includeInstalled: true, cwd, ferroxHome: process.env['FERROX_HOME'] });
  ...
  try {
    const commandsFerroxDir = _resolveCommandsFerroxDir();
    // Fix #1160: use _resolveManifest so installed-runtime layouts (where
    // commands/ferrox is absent) fall back to <configDir>/skills/ferrox-*/SKILL.md.
    const manifest = _resolveManifest(commandsFerroxDir, resolvedConfigDir);
...
function cmdCapabilityState(
  ...
  // bypasses the persisted-runtime fallback (FERROX_RUNTIME → config.runtime).
...
export = {
  ...
  cmdCapabilityState,
  // Exported for tests
  _resolveCommandsFerroxDir,
  _loadInstalledSkillsManifest,
  _loadFlatCommandsFerroxManifest,
  _resolveManifest,
  _isSafePropKey,
};
