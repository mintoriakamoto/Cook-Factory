 ...
 *
 * A Ferrox verification gate resolves a project's test command and runs it. When
 * that command is a watch/dev-mode runner (vitest defaults to WATCH in an
 * interactive TTY — which is exactly a user running `ferrox-execute-phase` in their
...
/**
 * CLI handler for `ferrox-tools query normalize-test-command <raw-cmd>`: prints the
