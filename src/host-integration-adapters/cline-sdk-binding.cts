 ...
 *     Resolves Ferrox's per-subagent `model_overrides` / `model_profile_overrides`
 ...
 * (analogous to `.opencode/plugins/ferrox-core.js`). Its `beforeTool` delegates to
 * the pure `evaluateBeforeTool` so the decision logic is testable without the
 * SDK linked.
 */
export const clineFerroxPlugin: {
  name: string;
...
/**
 * Resolve the createAgentModel call params for a cline subagent from Ferrox's model
 ...
 *
 * Precedence (matches Ferrox's model_overrides > model_profile_overrides contract):
 ...
 *          override is configured (the gateway default applies — Ferrox does NOT
