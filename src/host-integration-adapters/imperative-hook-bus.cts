 ...
 *
 * Ferrox registers for the 6 events in the portable floor + subagent lifecycle
 * (AC4a upgrade, #2089):
...
/**
 * The full set of Cursor hook events Ferrox can register for.
 ...
 *
 * Convention: `ferrox-cursor-<kebab-event>.js`. The script files are authored in
...
export const CURSOR_EVENT_SCRIPT_MAP: Readonly<Record<string, string>> = Object.freeze({
  sessionStart: 'ferrox-cursor-session-start.js',
  postToolUse: 'ferrox-cursor-post-tool.js',
  preToolUse: 'ferrox-cursor-pre-tool.js',
  stop: 'ferrox-cursor-stop.js',
  subagentStart: 'ferrox-cursor-subagent-start.js',
  subagentStop: 'ferrox-cursor-subagent-stop.js',
});

/**
 * The Ferrox-managed marker written into each hooks.json entry so the
 * reconcile pass can distinguish Ferrox-owned entries from user-owned ones.
 */
export const FERROX_HOOK_MARKER = 'ferrox-managed';
 ...
 * @returns array of script filenames (e.g. `['ferrox-cursor-session-start.js', ...]`)
 */
 ...
 * Each entry is `{ type: 'command', command: <cmd>, [FERROX_HOOK_MARKER]: true }`.
 *
