...
const { output, error, reapStaleTempFiles, ensureFerroxTempDir, FERROX_TEMP_DIR } = ioModule;
...
async function cmdExtractMessages(projectArg: string, options: { sessionId?: string; limit?: number }, raw: boolean, overridePath?: string | null): Promise<void> {
  ...
  reapStaleTempFiles('ferrox-pipeline-', { dirsOnly: true });
  ensureFerroxTempDir();
  const tmpDir = fs.mkdtempSync(path.join(FERROX_TEMP_DIR, 'ferrox-pipeline-'));
...
async function cmdProfileSample(overridePath: string | null | undefined, options: { limit?: number; maxChars?: number; maxPerProject?: number }, raw: boolean): Promise<void> {
  ...
  reapStaleTempFiles('ferrox-profile-', { dirsOnly: true });
  ensureFerroxTempDir();
  const tmpDir = fs.mkdtempSync(path.join(FERROX_TEMP_DIR, 'ferrox-profile-'));
