 ...
 *
 * Invocable as: ferrox_run check ui-plan-gate <phase>
 *
 ...
 * Invocable as: ferrox_run check ui-safety-gate <phase>
 *             or ferrox_run check ui.safety-gate <phase> (dots normalized to hyphens)
 *
 * Uses checkUiPresence from ui-safety-gate.cjs — does NOT reimplement frontend detection.
...
function computeUiSafetyGate(projectDir: string, phase: string): {
  ...
  if (block) {
    result.message = `UI files changed in this wave but no UI-SPEC.md exists for Phase ${phase}. ` +
      `Run /ferrox:ui-phase ${phase} to generate the design contract before continuing.`;
  }
  if (phaseLookupFailed) result.phaseLookupFailed = true;
  return result;
}
 ...
 *
 * Logic from ferrox-core/references/tdd.md <end_of_phase_review> and
 ...
 * Invocable as: ferrox_run check gap-analysis.plan-post <phase-dir> [phase-req-ids]
 */
 ...
 *
 * Invocation:
 *   ferrox_run check predicate --predicate '<json>' \
...
function cmdApiCoverageVerifyPre(projectDir: string, args: string[], raw: boolean): void {
  ...
  // Not a Ferrox project (no phases tree at all) → fail-open: nothing to gate.
    ...
    output(
      {
        ...
        detected: false,
        message: 'api-coverage: no .planning/phases directory; gate skipped (not a Ferrox project layout)',
      },
      raw,
      undefined,
    );
  ...
  if (!resolvedDir) {
    ...
    // above, which is a genuine non-Ferrox-project → pass.)
    output(
      {
        block: true,
        passed: false,
...
function routeCheckCommand({ args, cwd, raw }: RouteCheckCommandOptions): void {
  ...
  // directly runnable as `ferrox_run check ui.plan-gate` — the dot is normalized to
  // `ui-plan-gate` before routing. The generic gate-dispatch in §5.6 reads
  // `check.query` from the active gate hook and runs `ferrox_run check ${hook.check.query}`,
  ...
  if (subcommand === 'verify-schema-drift') {
    ...
    // Honor FERROX_SKIP_SCHEMA_CHECK=true to bypass the gate (preserves the original inline gate behavior).
    const phaseArg = typeof args[2] === 'string' ? args[2] : '';
    const skipSchemaCheck = process.env['FERROX_SKIP_SCHEMA_CHECK'] === 'true';
