 ...
 *
 * Pure function: compares the resolved ferrox-tools.cjs path to the project root.
 ...
 * from the retired @ferrox-build/sdk) shadowing the project-local Ferrox install.
 *
 ...
 *
 * @param opts.resolvedPath - The absolute path of the running ferrox-tools.cjs (__filename).
 * @param opts.projectRoot - The project root (from findProjectRoot), or null if no project.
 * @param opts.projectLocalExists - Whether a project-local ferrox-tools.cjs exists.
...
export function checkCliSkew(opts: {
  ...
  const hint = resolvedPath.includes('@ferrox-build')
    ? ' If @ferrox-build/sdk: npm uninstall -g @ferrox-build/sdk'
    : '';
  return `⚠ Ferrox: ${resolvedPath} may shadow project-local Ferrox.${hint}`;
}
