...
function planningDir(cwd: string, ws?: string | null, project?: string | null): string {
  if (project === undefined) project = process.env['FERROX_PROJECT'] ?? null;
  if (ws === undefined) ws = process.env['FERROX_WORKSTREAM'] ?? null;

  // Reject path separators and traversal components in project/workstream names
  const BAD_SEGMENT = /[/\\]|\.\./;
  if (project && BAD_SEGMENT.test(project)) {
    throw new Error(`FERROX_PROJECT contains invalid path characters: ${project}`);
  }
  if (ws && BAD_SEGMENT.test(ws)) {
    throw new Error(`FERROX_WORKSTREAM contains invalid path characters: ${ws}`);
  }
 ...
 * `NN.N-CONTEXT.md`, etc.) used by ferrox-discuss-phase output.
 *
