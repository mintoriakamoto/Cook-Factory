/**
 * Runtime name policy — alias resolution and canonicalization for Ferrox runtime
 ...
 *   (B) the bash surface: `ferrox-tools query project-instruction-file --runtime <r>`,
 *       consumed by ferrox-core/workflows/new-project.md to set $INSTRUCTION_FILE
 *
 ...
 * directory-name token Ferrox would ever generate) so a future caller that
...
/**
 * The `/ferrox-new-project` invocation syntax per runtime — the post-install
 * "next step" command string. Most runtimes use the default `/ferrox-new-project`;
 ...
 */
const DEFAULT_NEW_PROJECT_COMMAND = '/ferrox-new-project';
const RUNTIME_NEW_PROJECT_COMMANDS: Readonly<Record<string, string>> = {
  codex: '$ferrox-new-project',
  cursor: 'ferrox-new-project (mention the skill name)',
  kimi: '/skill:ferrox-new-project',
};
