 ...
 * Hermes) can drive Ferrox with NO bespoke plugin:
 *
 *   - point 1 (command): tool `ferrox_invoke_command` → `dispatchFerroxCommand`
 *     (src/shell-command-projection.cts), a bounded subprocess-shim to
 *     ferrox-tools.cjs. #2102 Stage 2: `commandRoutingHub.createHub()` called
 *     with no args here always hit `if(!_cjsRegistry) return
 *     makeUnknownCommand()` — every dispatch was UnknownCommand. No
 *     fully-populated hub factory exists anywhere in ferrox-core (every
 ...
 *     the pi extension uses (pi/ferrox.cjs), mirroring the SUBPROCESS-REUSE
 *     precedent already established for the OpenCode/Kilo hook bridge.
 *   - point 5 (state IO): tools `ferrox_read_state` / `ferrox_write_state` → the
...
const { dispatchFerroxCommand } = shellCommandProjection;

export const PROTOCOL_VERSION = '2024-11-05';
export const SERVER_NAME = 'ferrox-core';
...
const TOOLS = [
  {
    name: 'ferrox_invoke_command',
    description: 'Invoke a Ferrox command via the command-routing hub (interface point 1).',
    inputSchema: {
      type: 'object',
      properties: {
...
function callTool(name: string, args: unknown, ctx: McpContext): { content: Array<{ type: string; text: string }>; isError?: boolean } {
  ...
  try {
    if (name === 'ferrox_invoke_command') {
      const family = asString(a.family);
      const subcommand = asString(a.subcommand);
      if (!family || !subcommand) {
        return { isError: true, content: [{ type: 'text', text: 'ferrox_invoke_command requires string "family" and "subcommand".' }] };
      }
      const res = dispatchFerroxCommand({ family, subcommand, args: Array.isArray(a.args) ? (a.args as string[]) : [], cwd });
      if (!res.ok) {
    ...
    if (name === 'ferrox_read_state') {
      const p = asString(a.path);
      if (!p) return { isError: true, content: [{ type: 'text', text: 'ferrox_read_state requires string "path".' }] };
      const io = stateIo.createStateIO({ io: 'filesystem' });
      return { content: [{ type: 'text', text: io.read(p) }] };
    }
    if (name === 'ferrox_write_state') {
      const p = asString(a.path);
      const content = asString(a.content);
      if (!p || content === null) return { isError: true, content: [{ type: 'text', text: 'ferrox_write_state requires string "path" and "content".' }] };
      const io = stateIo.createStateIO({ io: 'filesystem' });
      io.write(p, content);
