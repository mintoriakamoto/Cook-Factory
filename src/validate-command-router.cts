 ...
 * Keeps ferrox-tools.cjs thin while preserving existing command semantics.
 *
 ...
 * retained when:
 * - FERROX_WORKSTREAM is active (workstream-scoped requests fall through to CJS).
...
function routeValidateCommand({ verify, args, cwd, raw, output: outputFn, error }: RouteValidateCommandOptions): void {
  routeCjsCommandFamily({
    ...
    handlers: {
      ...
      // helpers (codex expects $ferrox-* command shape).
      health: () => {
      ...
      context: () => {
        ...
        const threadCmd = String(formatFerroxSlash('thread', resolveRuntime(cwd)));
