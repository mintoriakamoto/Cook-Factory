 ...
 * ferrox-core/bin/lib/verification.cjs (gitignored).
 *
...
/** The set of status values that the ferrox-verifier agent emits. */
 ...
 * internally — the verifier (ferrox-verifier.md) never emits them. The verifier only
...
const VERIFICATION_ROUTING_TABLE: Record<string, VerificationRoute> = {
  ...
  missing: {
    ...
    next_command: '/ferrox:execute-phase',
  },
...
function readVerificationStatus(
  ...
  if (rawStatus === 'gaps_found') {
    ...
    return {
      ...
      next_command: `/ferrox:plan-phase ${phaseNumber} --gaps`,
    };
  }
  ...
  if (staleVerification) {
    ...
    return {
      status: entry.status,
      next_action: entry.next_action,
      next_command: `/ferrox:verify-work ${phaseNumber}`,
    };
  }
