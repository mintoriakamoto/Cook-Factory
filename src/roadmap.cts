...
function cmdRoadmapAnalyze(cwd: string, raw: boolean): void {
  ...
  while ((match = phasePattern.exec(content)) !== null) {
    ...
    // Phases completed before Ferrox tracking (or via external tools) may lack
...
function cmdRoadmapUpdatePlanProgress(cwd: string, phaseNum: string | null | undefined, raw: boolean): void {
  ...
  // moment the last plan summary landed — before ferrox-verifier had verified.
  ...
  withPlanningLock(cwd, () => {
    ...
    //   `**Plans**: N plans`  — bold word + outer colon (ferrox-core/templates/roadmap.md)
    ...
    if (missingPlans.length > 0) {
      ...
      //
      // Canonical template (ferrox-core/templates/roadmap.md) uses BOTH lines:
