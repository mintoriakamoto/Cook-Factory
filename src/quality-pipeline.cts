...
const REGISTRY: Record<string, Omit<Toolchain, 'known'>> = {
 ...
 */
function selectToolchain(language?: unknown): Toolchain {
 ...
 */
function decideKeep(opts?: { gateBefore?: unknown; gateAfter?: unknown }): KeepDecision {
...
function summarize(opts?: {
  formatChanged?: unknown;
  lint?: unknown;
  security?: { HIGH?: unknown; MEDIUM?: unknown; LOW?: unknown };
}): QualitySummary {
  const o = opts && typeof opts === 'object' ? opts : {};
  const issues: string[] = [];
  ...
  if (high > 0) issues.push(`security-high:${high}`);
  if (med > 0) issues.push(`security-medium:${med}`);
...
export = { selectToolchain, decideKeep, summarize };
