 ...
 *   echo "$PHASE_SECTION" | node ferrox-core/bin/lib/ui-safety-gate.cjs
 *   echo $?   → 0 if UI tokens found, 1 if not, 2 on usage error
 *
 * Exit codes mirror grep: 0 = match found, 1 = no match, 2 = usage error.
 *
 * Canonical location: ferrox-core/bin/lib/ui-safety-gate.cjs (#448)
 * This path is deployed by the Ferrox installer to $RUNTIME_DIR/ferrox-core/bin/lib/.
