'use strict';
/**
 * Audit command routers — CLI dispatchers for `ferrox-tools audit-uat` and
 * `ferrox-tools audit-open`.
 *
 * ADR-959 (phase 4d-impl-3): audit command family cutover.
 * Extracted from the hardcoded `case 'audit-uat':` and `case 'audit-open':`
 * arms in ferrox-tools.cjs.  Behaviour is preserved byte-for-behaviour from the
...
function routeAuditUat({ args, cwd, raw, error, _uat }: RouteAuditUatOptions): void {
  ...
  // trivial but the observability seam (DispatchEvent, FERROX_AUDIT=1 trace) is
