 ...
 *   3. Opt-in audit file — when FERROX_AUDIT=1 OR config.audit.enabled===true,
 *      appends every event (success + error) as one JSON line to
 *      .planning/.ferrox-trace.jsonl relative to `cwd`. Creates .planning/ if absent.
 *   4. Args redaction — args omitted by default; included when FERROX_AUDIT_ARGS=1.
 *
...
const AUDIT_FILE_NAME = '.ferrox-trace.jsonl';
...
function _isAuditEnabled(config: { audit?: { enabled?: boolean } } | undefined): boolean {
  if (process.env['FERROX_AUDIT'] === '1') return true;
