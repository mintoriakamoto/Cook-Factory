...
// Code's Agent tool accepts but which is not a Ferrox model-profile tier (#1133).
...
function warnModelPolicyUnmappable(agentType: string, policyModel: string, tier: string): void {
  ...
  process.stderr.write(
    `ferrox: warning — model_policy resolved "${policyModel}" for ${agentType}, ` +
...
function warnModelOverrideUnmappable(agentType: string, overrideValue: string): void {
  ...
  process.stderr.write(
    `ferrox: warning — model_overrides value "${safe}" for ${agentType} ` +
