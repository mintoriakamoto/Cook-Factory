 ...
 *
 * Command surface: ferrox-tools loop render-hooks <point>
 *
 ...
 */
// #2009: a capability id surfaced inside the runnable `ferrox capability remove <id>`
...
function cmdLoopRenderHooks(
  ...
  // fallback (FERROX_RUNTIME → config.runtime). Without this, a repo with persisted
  ...
  // #1459 IC-04: thread the consent home (process.env.FERROX_HOME) EXPLICITLY so a consented project cap's
  ...
  // `ferrox capability remove <id>` remediation so the operator can clear the broken
  ...
  if (overlayMeta && Array.isArray(overlayMeta.blockedGates)) {
    for (const blocked of overlayMeta.blockedGates) {
      ...
      const capLabel = idValid
        ? `"${blocked.capId}"`
        : 'with an invalid id (withheld) under .ferrox/capabilities/';
      const remediation = idValid
        ? `Run \`ferrox capability remove ${blocked.capId}\` to remove it, or fix the load error.`
        : 'Remove the offending capability directory under .ferrox/capabilities/, or fix the load error.';
  ...
  for (const w of loadFailWarnings) {
    process.stderr.write(`ferrox: warning — ${w}\n`);
  }
