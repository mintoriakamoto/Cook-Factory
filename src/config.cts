 ...
 *   2. User-level defaults from ~/.ferrox/defaults.json (if present)
 *   3. userChoices — the settings the user explicitly selected during /ferrox:new-project
 *
...
function buildNewProjectConfig(userChoices: Record<string, unknown>): Record<string, unknown> {
  ...
  const tavilyKeyFile = path.join(homedir, '.ferrox', 'tavily_api_key');
  const hasTavilySearch = !!(process.env['TAVILY_API_KEY'] || fs.existsSync(tavilyKeyFile));
  const refKeyFile = path.join(homedir, '.ferrox', 'ref_api_key');
  const hasRefSearch = !!(process.env['REF_API_KEY'] || fs.existsSync(refKeyFile));
  const perplexityKeyFile = path.join(homedir, '.ferrox', 'perplexity_api_key');
  const hasPerplexity = !!(process.env['PERPLEXITY_API_KEY'] || fs.existsSync(perplexityKeyFile));
  const jinaKeyFile = path.join(homedir, '.ferrox', 'jina_api_key');
  const hasJina = !!(process.env['JINA_API_KEY'] || fs.existsSync(jinaKeyFile));

  // Load user-level defaults from ~/.ferrox/defaults.json if available
  const globalDefaultsPath = path.join(homedir, '.ferrox', 'defaults.json');
 ...
 * configured during /ferrox:new-project). All remaining keys are filled from
 * hardcoded defaults and optional ~/.ferrox/defaults.json.
 *
 ...
 * users can run `ferrox-tools migrate-config` to apply all four legacy-key
...
function cmdMigrateConfig(cwd: string, raw: boolean): void {
  const ws = process.env['FERROX_WORKSTREAM'] || null;
