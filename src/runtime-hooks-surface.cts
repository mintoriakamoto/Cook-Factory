 ...
 *
 * Owns the lifecycle writer functions for hook surfaces managed by Ferrox on four
 * runtimes:
...
// tone) so the emitted hooks/ferrox-session.json stays golden-trackable — no
// node-runner invocation, no filesystem probing beyond what sessionStart
// already does.
const FERROX_COPILOT_PRE_TOOL_MSG =
  'Ferrox: confirm this tool use is in scope for the active phase before proceeding.';
const FERROX_COPILOT_PRE_TOOL_HOOK_BASH =
  `printf '%s' '{"additionalContext":"${FERROX_COPILOT_PRE_TOOL_MSG}"}'`;
const FERROX_COPILOT_PRE_TOOL_HOOK_PWSH =
  `'{"additionalContext":"${FERROX_COPILOT_PRE_TOOL_MSG}"}'`;

const FERROX_COPILOT_POST_TOOL_MSG =
  'Ferrox: review the tool result against the active phase before continuing.';
const FERROX_COPILOT_POST_TOOL_HOOK_BASH =
  `printf '%s' '{"additionalContext":"${FERROX_COPILOT_POST_TOOL_MSG}"}'`;
const FERROX_COPILOT_POST_TOOL_HOOK_PWSH =
  `'{"additionalContext":"${FERROX_COPILOT_POST_TOOL_MSG}"}'`;

const FERROX_COPILOT_PROMPT_SUBMIT_MSG =
  'Ferrox: check this request against .planning/STATE.md scope before acting.';
const FERROX_COPILOT_PROMPT_SUBMIT_HOOK_BASH =
  `printf '%s' '{"additionalContext":"${FERROX_COPILOT_PROMPT_SUBMIT_MSG}"}'`;
const FERROX_COPILOT_PROMPT_SUBMIT_HOOK_PWSH =
  `'{"additionalContext":"${FERROX_COPILOT_PROMPT_SUBMIT_MSG}"}'`;

const FERROX_COPILOT_SESSION_END_MSG =
  'Ferrox: update .planning/STATE.md with the session outcome before ending.';
const FERROX_COPILOT_SESSION_END_HOOK_BASH =
  `printf '%s' '{"additionalContext":"${FERROX_COPILOT_SESSION_END_MSG}"}'`;
const FERROX_COPILOT_SESSION_END_HOOK_PWSH =
  `'{"additionalContext":"${FERROX_COPILOT_SESSION_END_MSG}"}'`;
...
// The full set of Cursor hook events Ferrox manages — sourced from the adapter
...
function resolveBashRunner(opts?: BashRunnerOpts): string | null {
  ...
  const candidates: string[] = [];
  if (env.FERROX_BASH_PATH) candidates.push(env.FERROX_BASH_PATH);
...
function buildCodexHookBlock(targetDir: string, opts?: BuildCodexHookBlockOpts): string | null {
  ...
  const platform = (opts && opts.platform) || process.platform;
  const updateCheckScript = path.resolve(targetDir, 'hooks', 'ferrox-check-update.js');
  const commandValue = projectCodexHookTomlCommand({
    absoluteRunner,
    scriptPath: updateCheckScript,
    platform,
  });
  return `${eol}# Ferrox Hooks${eol}` +
...
interface ReconcileCodexOpts {
  ...
  timeout?: number | null;
  /**
   * When true, the pre-add strip removes ONLY managed hooks whose command
   * references {@link ReconcileCodexOpts.scriptName}, preserving OTHER managed
   * hooks on the same event. This lets a second managed script (e.g. the
   * merge-gate guard) register on PreToolUse without clobbering the
   * context-monitor already registered there. Default (false) = the original
   * "strip all managed, add one" behavior — unchanged for every existing caller.
   */
  preserveOtherManaged?: boolean;
  /** Script filename this reconcile targets (used by preserveOtherManaged). */
...
function reconcileCodexHooksJsonEvent(targetDir: string, eventName: string, opts: ReconcileCodexOpts = {}): ReconcileResult {
  ...
  const preserveOtherManaged = opts.preserveOtherManaged === true;
  const scopedScriptName = typeof opts.scriptName === 'string' ? opts.scriptName : null;
  ...
  for (const entry of eventEntries) {
    ...
    const keptHooks = originalHooks.filter((hook) => {
      ...
      // Default: strip ALL managed hooks (then re-add the single managedCommand).
      // preserveOtherManaged: strip ONLY the target script's managed hooks, so a
      // sibling managed hook on the same event (e.g. context-monitor while we
      // register the merge-gate guard) survives.
      const stripThis =
        preserveOtherManaged && scopedScriptName
          ? managed && typeof cmd === 'string' && cmd.includes(scopedScriptName)
          : managed;
      if (stripThis) removedLegacy = true;
      return !stripThis;
    });
...
function ensureCodexHooksJsonSessionStart(targetDir: string, opts: EnsureCodexSessionStartOpts = {}): ReconcileResult {
  ...
  const scriptPath = shellCmdProjection.posixNormalize(path.resolve(targetDir, 'hooks', 'ferrox-check-update.js'));
...
interface EnsureCodexEventOpts {
  absoluteRunner?: string | null;
  platform?: NodeJS.Platform;
  /** Which hook script to register (default 'ferrox-context-monitor.js'). */
  scriptName?: string | null;
  /** Optional Codex tool matcher (omit = unmatched / all tools). */
  matcher?: string | null;
  /** Preserve other managed hooks on the event (see ReconcileCodexOpts). */
  preserveOtherManaged?: boolean;
  /** Hook timeout in seconds (default 10). */
  timeout?: number | null;
}
...
function ensureCodexHooksJsonEvent(targetDir: string, eventName: string, opts: EnsureCodexEventOpts = {}): ReconcileResult {
  ...
  const eventScriptName = typeof opts.scriptName === 'string' && opts.scriptName ? opts.scriptName : 'ferrox-context-monitor.js';
  const scriptPath = shellCmdProjection.posixNormalize(path.resolve(targetDir, 'hooks', eventScriptName));
  ...
  return reconcileCodexHooksJsonEvent(targetDir, eventName, {
    ...
    matcher: typeof opts.matcher === 'string' ? opts.matcher : undefined,
    preserveOtherManaged: opts.preserveOtherManaged === true,
    scriptName: eventScriptName,
  });
}
...
function buildClineRulesBody(): string {
  return [
    '# Ferrox Factory — Go Build Something Awesome.',
    '',
    '- Ferrox workflows live in `ferrox-core/workflows/`. Load the relevant workflow when',
    '  the user runs a `/ferrox-*` command.',
    '- Ferrox agents live in `agents/`. Use the matching agent when spawning subagents.',
    '- Ferrox tools are at `ferrox-core/bin/ferrox-tools.cjs`. Run with `node`.',
    '- Planning artifacts live in `.planning/`. Never edit them outside a Ferrox workflow.',
    '- Do not apply Ferrox workflows unless the user explicitly asks for them.',
    '- When a Ferrox command triggers a deliverable (feature, fix, docs), offer the next',
...
'use strict';
/* Ferrox-managed Cline PreToolUse hook — ferrox-core issue #787.
  ...
  try {
    ...
    if (isWrite && paths.some(isPlanningPath)) {
      return process.stdout.write(JSON.stringify({
        cancel: true,
        errorMessage:
          'Ferrox: .planning/ artifacts are managed by Ferrox workflows. Edit them only through a /ferrox-* command, not directly.',
      }));
    }
...
function mergeFerroxAgentsMd(filePath: string, ferroxContent: string): void {
  const ferroxBlock = FERROX_AGENTS_MD_MARKER + '\n' + ferroxContent.trim() + '\n' + FERROX_AGENTS_MD_CLOSE_MARKER;
  ...
  const openIndex = existing.indexOf(FERROX_AGENTS_MD_MARKER);
  const closeIndex = existing.indexOf(FERROX_AGENTS_MD_CLOSE_MARKER);

  if (openIndex !== -1 && closeIndex !== -1) {
    const before = existing.substring(0, openIndex).trimEnd();
    const after = existing.substring(closeIndex + FERROX_AGENTS_MD_CLOSE_MARKER.length).trimStart();
    let newContent = '';
...
function writeClineArtifacts(targetDir: string, isGlobalInstall: boolean): string[] {
  ...
  fs.writeFileSync(path.join(clinerulesDir, 'ferrox.md'), buildClineRulesBody());
  written.push('.clinerules/ferrox.md');
  console.log(`  ${green}✓${reset} Wrote .clinerules/ferrox.md`);
  ...
  if (isGlobalInstall) {
    try {
      ...
      mergeFerroxAgentsMd(agentsPath, buildClineAgentsMdBody());
      console.log(`  ${green}✓${reset} Merged Ferrox instructions into ~/.agents/AGENTS.md`);
    } catch (err) {
...
function buildCursorHookEntry(scriptPath: string): Record<string, unknown> {
  return {
    ...
    [FERROX_CURSOR_HOOK_MARKER]: true,
  };
}

function isManagedCursorHookEntry(entry: unknown): boolean {
  return Boolean(entry && typeof entry === 'object' && (entry as Record<string, unknown>)[FERROX_CURSOR_HOOK_MARKER]);
}
...
// (hooks/ferrox-windsurf-pre-write.js, hooks/ferrox-windsurf-pre-command.js) speak a
...
//
// Only 2 of Ferrox's 6 Cursor-parity hook events have a Cascade counterpart with
...
// response), so the 4 advisory events Ferrox registers on Cursor (sessionStart,
...
const FERROX_WINDSURF_HOOK_MARKER = 'ferrox-managed';

/** The 2 Cascade hook events Ferrox wires with blocking (exit-code-2) guards. */
...
const WINDSURF_EVENT_SCRIPT_MAP: Readonly<Record<string, string>> = Object.freeze({
  pre_write_code: FERROX_WINDSURF_PRE_WRITE_HOOK_SCRIPT,
  pre_run_command: FERROX_WINDSURF_PRE_COMMAND_HOOK_SCRIPT,
});
 ...
 * bare `command` shell string plus the Ferrox marker.
 */
function buildWindsurfHookEntry(command: string): Record<string, unknown> {
  return {
    command,
    [FERROX_WINDSURF_HOOK_MARKER]: true,
  };
}

function isManagedWindsurfHookEntry(entry: unknown): boolean {
  return Boolean(entry && typeof entry === 'object' && (entry as Record<string, unknown>)[FERROX_WINDSURF_HOOK_MARKER]);
}
...
/**
 * Reconcile Ferrox's managed Cascade hook entries into `<targetDir>/hooks.json`,
...
/**
 * Write Ferrox-managed Cascade lifecycle hooks into `<targetDir>/hooks.json`.
 * Both managed hook scripts (ferrox-windsurf-pre-write.js,
 * ferrox-windsurf-pre-command.js) are copied from the Ferrox hooks/ source to
 ...
 * @param src       - The Ferrox install source root (for copying hook scripts)
 * @param opts      - `{ platform? }`
...
/**
 * Remove all Ferrox-managed Cascade hook entries from hooks.json. User-owned
  ...
  return {
    ...
    hooks: {
      sessionStart: [
        {
          type: 'command',
          bash: FERROX_COPILOT_SESSION_HOOK_BASH,
          powershell: FERROX_COPILOT_SESSION_HOOK_PWSH,
          timeoutSec: 10,
        },
      ],
      ...
      preToolUse: [
        {
          type: 'command',
          bash: FERROX_COPILOT_PRE_TOOL_HOOK_BASH,
          powershell: FERROX_COPILOT_PRE_TOOL_HOOK_PWSH,
          timeoutSec: 10,
        },
      ],
      postToolUse: [
        {
          type: 'command',
          bash: FERROX_COPILOT_POST_TOOL_HOOK_BASH,
          powershell: FERROX_COPILOT_POST_TOOL_HOOK_PWSH,
          timeoutSec: 10,
        },
      ],
      userPromptSubmitted: [
        {
          type: 'command',
          bash: FERROX_COPILOT_PROMPT_SUBMIT_HOOK_BASH,
          powershell: FERROX_COPILOT_PROMPT_SUBMIT_HOOK_PWSH,
          timeoutSec: 10,
        },
      ],
      sessionEnd: [
        {
          type: 'command',
          bash: FERROX_COPILOT_SESSION_END_HOOK_BASH,
          powershell: FERROX_COPILOT_SESSION_END_HOOK_PWSH,
          timeoutSec: 10,
        },
      ],
    },
...
function writeCopilotHookConfig(targetDir: string): string {
  const hooksDir = path.join(targetDir, 'hooks');
  fs.mkdirSync(hooksDir, { recursive: true });
  const hookPath = path.join(hooksDir, FERROX_COPILOT_HOOK_FILE);
...
//
// MUTATES `settings` by reference — registers all Ferrox-managed hook entries
...
function applySettingsJsonHooks(settings: any, opts: ApplySettingsJsonHooksOpts): void {
  ...
  if (hooksSurface !== 'none' && hooksSurface !== 'kimi-hooks-toml') {
    ...
    const hasFerroxUpdateHook = settings.hooks.SessionStart.some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ferrox-check-update'))
    );
    ...
    if (!hasFerroxUpdateHook && fs.existsSync(checkUpdateFile) && updateCheckCommand) {
    ...
    } else if (!hasFerroxUpdateHook && !fs.existsSync(checkUpdateFile)) {
      console.warn(`  ${yellow}⚠${reset}  Skipped update check hook — ferrox-check-update.js not found at target`);
    }
    ...
    const hasContextMonitorHook = settings.hooks[postToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ferrox-context-monitor'))
    );
    ...
    if (!hasContextMonitorHook && fs.existsSync(contextMonitorFile) && contextMonitorCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped context monitor hook — ferrox-context-monitor.js not found at target`);
    } else {
      // Migrate existing context monitor hooks: add matcher and timeout if missing
      for (const entry of settings.hooks[postToolEvent]) {
        if (entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ferrox-context-monitor'))) {
          let migrated = false;
          if (!entry.matcher) {
          ...
          for (const h of entry.hooks) {
            if (referencesHook(h as Record<string, unknown>, 'ferrox-context-monitor') && !h.timeout) {
              h.timeout = 10;
              migrated = true;
            }
          }
    ...
    const hasPromptGuardHook = settings.hooks[preToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ferrox-prompt-guard'))
    );

    const promptGuardFile = path.join(targetDir, 'hooks', 'ferrox-prompt-guard.js');
    if (!hasPromptGuardHook && fs.existsSync(promptGuardFile) && promptGuardCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped prompt guard hook — ferrox-prompt-guard.js not found at target`);
    }
    ...
    const hasReadGuardHook = settings.hooks[preToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ferrox-read-guard'))
    );

    const readGuardFile = path.join(targetDir, 'hooks', 'ferrox-read-guard.js');
    if (!hasReadGuardHook && fs.existsSync(readGuardFile) && readGuardCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped read guard hook — ferrox-read-guard.js not found at target`);
    }
    ...
    const hasReadInjectionScannerHook = settings.hooks[postToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ferrox-read-injection-scanner'))
    );

    const readInjectionScannerFile = path.join(targetDir, 'hooks', 'ferrox-read-injection-scanner.js');
    if (!hasReadInjectionScannerHook && fs.existsSync(readInjectionScannerFile) && readInjectionScannerCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped read injection scanner hook — ferrox-read-injection-scanner.js not found at target`);
    }
    ...
    // Detects file edits outside Ferrox workflow context and advises using
    // /ferrox-quick or /ferrox-fast for state-tracked changes. Also hard-blocks
    // unsafe Bash commands that violate worktree-agent isolation.
    const workflowGuardCommand = isGlobal
      ? buildHookCommand(targetDir, 'ferrox-workflow-guard.js', hookOpts)
      : localCmd('ferrox-workflow-guard.js');
    ...
    const workflowGuardHookEntry = settings.hooks[preToolEvent].find((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ferrox-workflow-guard'))
    );
    const hasWorkflowGuardHook = Boolean(workflowGuardHookEntry);

    const workflowGuardFile = path.join(targetDir, 'hooks', 'ferrox-workflow-guard.js');
    if (hasWorkflowGuardHook && workflowGuardHookEntry.matcher !== workflowGuardMatcher) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped workflow guard hook — ferrox-workflow-guard.js not found at target`);
    }
    ...
    const worktreePathGuardCommand = isGlobal
      ? buildHookCommand(targetDir, 'ferrox-worktree-path-guard.js', hookOpts)
      : localCmd('ferrox-worktree-path-guard.js');
    const hasWorktreePathGuardHook = settings.hooks[preToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ferrox-worktree-path-guard'))
    );
    const worktreePathGuardFile = path.join(targetDir, 'hooks', 'ferrox-worktree-path-guard.js');
    if (!hasWorktreePathGuardHook && fs.existsSync(worktreePathGuardFile) && worktreePathGuardCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped worktree path guard hook — ferrox-worktree-path-guard.js not found at target`);
    }

    // Configure commit validation hook (Conventional Commits enforcement, opt-in)
    const validateCommitCommand = isGlobal
      ? buildHookCommand(targetDir, 'ferrox-validate-commit.sh', hookOpts)
      : localShellCmd('ferrox-validate-commit.sh');
    const hasValidateCommitHook = settings.hooks[preToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ferrox-validate-commit'))
    );
    ...
    const validateCommitFile = path.join(targetDir, 'hooks', 'ferrox-validate-commit.sh');
    if (!hasValidateCommitHook && fs.existsSync(validateCommitFile) && validateCommitCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped commit validation hook — ferrox-validate-commit.sh not found at target`);
    ...
    const graphifyUpdateCommand = isGlobal
      ? buildHookCommand(targetDir, 'ferrox-graphify-update.sh', hookOpts)
      : localShellCmd('ferrox-graphify-update.sh');
    const hasGraphifyUpdateHook = settings.hooks[postToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ferrox-graphify-update'))
    );
    const graphifyUpdateFile = path.join(targetDir, 'hooks', 'ferrox-graphify-update.sh');
    if (!hasGraphifyUpdateHook && fs.existsSync(graphifyUpdateFile) && graphifyUpdateCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped graphify auto-update hook — ferrox-graphify-update.sh not found at target`);
    ...
    const sessionStateCommand = isGlobal
      ? buildHookCommand(targetDir, 'ferrox-session-state.sh', hookOpts)
      : localShellCmd('ferrox-session-state.sh');
    const hasSessionStateHook = settings.hooks.SessionStart.some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ferrox-session-state'))
    );
    const sessionStateFile = path.join(targetDir, 'hooks', 'ferrox-session-state.sh');
    if (!hasSessionStateHook && fs.existsSync(sessionStateFile) && sessionStateCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped session state hook — ferrox-session-state.sh not found at target`);
    ...
    const phaseBoundaryCommand = isGlobal
      ? buildHookCommand(targetDir, 'ferrox-phase-boundary.sh', hookOpts)
      : localShellCmd('ferrox-phase-boundary.sh');
    const hasPhaseBoundaryHook = settings.hooks[postToolEvent].some((entry: HookGroup) =>
      entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ferrox-phase-boundary'))
    );
    const phaseBoundaryFile = path.join(targetDir, 'hooks', 'ferrox-phase-boundary.sh');
    if (!hasPhaseBoundaryHook && fs.existsSync(phaseBoundaryFile) && phaseBoundaryCommand) {
      ...
      console.warn(`  ${yellow}⚠${reset}  Skipped phase boundary hook — ferrox-phase-boundary.sh not found at target`);
    ...
    // supports SubagentStart (#2092 Phase B, Upgrade 2). Wire ferrox-context-
    ...
    // user prompt text, not a tool invocation, so ferrox-prompt-guard (which
    ...
    // so this loop is a no-op for every runtime that doesn't list SubagentStart.
    {
      ...
      for (const event of ['SubagentStop', 'Stop', 'PreCompact', 'SubagentStart']) {
        ...
        if (!alreadyHasContextMonitor && fs.existsSync(contextMonitorFile) && contextMonitorCommand) {
          ...
          console.warn(`  ${yellow}⚠${reset}  Skipped ${event} hook — ferrox-context-monitor.js not found at target`);
        }
      }
    }
    ...
    //                  plans.  Wire ferrox-context-monitor for context headroom
    //                  awareness at prompt time.
    //   AfterAgent   — fires once per turn after the model generates its final
    //                  response.  Wire ferrox-context-monitor to track headroom
    //                  after each agent turn completes.
    //   BeforeModel  — fires before each LLM call (per-turn, not per-session).
    //                  Wire ferrox-context-monitor for per-turn context injection
    //                  — more precise than session-start-only injection.
    //
    // All three reuse ferrox-context-monitor.js — no new hook files needed.
    // The `decision:"deny"` retry capability of AfterAgent is intentionally
    // left to the hook script to implement when triggered (ferrox-context-monitor
    ...
    // ferrox hook use case at this time; deferred to a follow-on issue.
    //
    ...
    for (const extendedEvent of ['BeforeAgent', 'AfterAgent', 'BeforeModel']) {
      ...
      if (!alreadyHasContextMonitor && fs.existsSync(contextMonitorFile) && contextMonitorCommand) {
        ...
        console.warn(`  ${yellow}⚠${reset}  Skipped ${extendedEvent} hook — ferrox-context-monitor.js not found at target`);
      }
    }
    // ── end Antigravity-only extended hook events ──────────────────────────────

    // ── FileChanged hook: hot-reload ferrox config on .planning/config.json edits ─
    // Claude Code fires FileChanged when a watched file changes on disk.  Wire
    // ferrox-config-reload.js to reload the ferrox config context whenever the user
    ...
    // when the changed file is not the ferrox config.
    //
    ...
    if (extendedEvents.includes('FileChanged')) {
      ...
      const configReloadFile = path.join(targetDir, 'hooks', 'ferrox-config-reload.js');
      const alreadyHasConfigReload = settings.hooks.FileChanged.some((entry: HookGroup) =>
        entry.hooks && entry.hooks.some((h: HookEntry) => referencesHook(h as Record<string, unknown>, 'ferrox-config-reload'))
      );
      if (!alreadyHasConfigReload && fs.existsSync(configReloadFile) && configReloadCommand) {
        ...
        console.warn(`  ${yellow}⚠${reset}  Skipped FileChanged hook — ferrox-config-reload.js not found at target`);
...
// file lives OUTSIDE the runtime's Ferrox configDir: kimi's configDir is the
...
//
// Ferrox-owned [[hooks]] entries are wrapped in marker comments so a reinstall
// can find-and-replace only Ferrox's own block, leaving any user-authored
// [[hooks]] entries elsewhere in the file untouched — mirrors the marker
// approach stripStaleFerroxHookBlocks uses for Codex's config.toml, simplified
...
const KIMI_HOOKS_TOML_MARKER_BEGIN = '# Ferrox Hooks BEGIN — managed by Ferrox, do not edit between these markers';
const KIMI_HOOKS_TOML_MARKER_END = '# Ferrox Hooks END';
...
/**
 * Build the full marker-delimited Ferrox [[hooks]] block for kimi's config.toml,
 * or null when no Ferrox hook resolved to a usable command (hooks/ missing, or
 ...
 * 1:1 by Ferrox hook script (update check, session-state, phase-boundary,
 ...
 * every Claude-dialect event Ferrox currently wires (SessionStart, PreToolUse,
 ...
 * dropped rather than guessed — ferrox-context-monitor's PostToolUse entry runs
...
function buildKimiHooksTomlBlock(targetDir: string, opts: { hookOpts: BuildHookCommandOpts }): string | null {
  ...
  const specs: KimiHookEntrySpec[] = [
    ...
    { event: 'SessionStart', command: cmd('ferrox-check-update.js') },
    { event: 'SessionStart', command: cmd('ferrox-session-state.sh') },

    // PreToolUse
    { event: 'PreToolUse', command: cmd('ferrox-prompt-guard.js'), matcher: 'WriteFile|StrReplaceFile', timeout: 5 },
    { event: 'PreToolUse', command: cmd('ferrox-read-guard.js'), matcher: 'WriteFile|StrReplaceFile', timeout: 5 },
    { event: 'PreToolUse', command: cmd('ferrox-worktree-path-guard.js'), matcher: 'WriteFile|StrReplaceFile', timeout: 5 },
    { event: 'PreToolUse', command: cmd('ferrox-workflow-guard.js'), matcher: 'Shell|WriteFile|StrReplaceFile', timeout: 5 },
    { event: 'PreToolUse', command: cmd('ferrox-validate-commit.sh'), matcher: 'Shell', timeout: 5 },

    // PostToolUse
    { event: 'PostToolUse', command: cmd('ferrox-context-monitor.js'), timeout: 10 },
    { event: 'PostToolUse', command: cmd('ferrox-phase-boundary.sh'), matcher: 'WriteFile|StrReplaceFile', timeout: 5 },
    { event: 'PostToolUse', command: cmd('ferrox-read-injection-scanner.js'), matcher: 'ReadFile', timeout: 5 },
    { event: 'PostToolUse', command: cmd('ferrox-graphify-update.sh'), matcher: 'Shell', timeout: 5 },

    // Extended lifecycle events — context-headroom tracking (unmatched).
    { event: 'Stop', command: cmd('ferrox-context-monitor.js'), timeout: 10 },
    { event: 'PreCompact', command: cmd('ferrox-context-monitor.js'), timeout: 10 },
    { event: 'SubagentStart', command: cmd('ferrox-context-monitor.js'), timeout: 10 },
    { event: 'SubagentStop', command: cmd('ferrox-context-monitor.js'), timeout: 10 },
  ];
...
/**
 * Strip a previously-written Ferrox [[hooks]] block from kimi's config.toml
 ...
 * nothing but whitespace (the file was Ferrox-only), so the caller can unlink
...
/**
 * Idempotently (re)write kimi's Ferrox-owned [[hooks]] block into its native
...
/**
 * Uninstall-time counterpart to writeKimiHooksToml: strips the Ferrox block and
 * deletes the file if nothing but Ferrox's own block was ever in it.
 */
...
// hook by name.  Covers all three registration shapes used by Ferrox:
