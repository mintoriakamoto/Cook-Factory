...
// The capability-registry.cjs is generated and lives in the same ferrox-core/bin/lib/ output dir.
// Both config-loader.cjs and capability-registry.cjs land in ferrox-core/bin/lib/ at build time.
...
const CONFIG_DEFAULTS = {
  ...
  // CRITICAL-1: the whole halting.* block (per-gate caps, rescope counter,
  // human-SLA / ship-clock budgets) is a first-class resolved key. Sourced from
  // the manifest defaults so a project with no halting override still resolves
  // sane values instead of undefined (which silently disabled every cap knob).
  halting: _getConfigDefault('halting'),
  // COORD-03: mirror the halting propagation for the coordination.* block (the
  // hot-seam registry, sole-writer shared_state_paths, and migration_store).
  // Sourced from the manifest default so a project with no coordination override
  // still resolves the FF-B12 hot-seam registry instead of undefined — the exact
  // Phase-3 silent-drop trap this plan exists to avoid.
  coordination: _getConfigDefault('coordination'),
  // STRONG-02/04/05: mirror the coordination propagation for the strength.* block
  // (medium_cluster_threshold, security_age_limit_days, receipt_store,
  // coverage_store, requirements_path, security_categories). Sourced from the
  // manifest default so a project with no strength override still resolves sane
  // Phase-5 gate tuning instead of undefined — the same FF-B12 silent-drop trap.
  strength: _getConfigDefault('strength'),
  // MODEL-01..05: mirror the strength propagation for the model.* block (stage_tiers,
  // tier_order, max_escalations one-hop cap, risk_boundaries, the exactly-two
  // trident_checkpoints, rtk.enabled). Sourced from the manifest default so a project
  // with no model override still resolves the Build-Line tiering backbone instead of
  // undefined — the same Phase-3 silent-drop trap the halting/coordination/strength
  // blocks defeat; the Phase-6 route/escalate/risk-grade/trident/rtk verbs read THIS.
  model: _getConfigDefault('model'),
  // MEM-01/02: mirror the model propagation for the memory.* block (fact_store,
  // recall_limit). Sourced from the manifest default so a project with no memory
  // override still resolves the bi-temporal fact-store path + recall bound instead
  // of undefined — the same Phase-3 silent-drop trap the halting/coordination/
  // strength/model blocks defeat; the Phase-7 memory router reads THIS.
  memory: _getConfigDefault('memory'),
};
    ...
    if (!_warnedConfigKeys.has(key)) {
      ...
      try {
        process.stderr.write(
          `ferrox: warning — config key "runtime" has unknown value "${runtime}". ` +
    ...
    for (const [overrideRuntime, tierMap] of Object.entries(overrides as Record<string, unknown>)) {
      if (!(KNOWN_RUNTIMES).has(overrideRuntime)) {
        ...
        if (!_warnedConfigKeys.has(key)) {
          ...
          try {
            process.stderr.write(
              `ferrox: warning — model_profile_overrides.${overrideRuntime}.* uses ` +
      ...
      for (const tierName of Object.keys(tierMap)) {
        if (!RUNTIME_OVERRIDE_TIERS.has(tierName)) {
          ...
          if (!_warnedConfigKeys.has(key)) {
            ...
            try {
              process.stderr.write(
                `ferrox: warning — model_profile_overrides.${overrideRuntime}.${tierName} ` +
  ...
  if (policy && typeof policy === 'object' && !Array.isArray(policy)) {
    ...
    if (provider && typeof provider === 'string' &&
      ...
      if (!_warnedConfigKeys.has(pkey)) {
        ...
        try {
          process.stderr.write(
            `ferrox: warning — model_policy.provider has unknown value "${provider}". ` +
      ...
      for (const [pruntime, tierMap] of Object.entries(rtOverrides as Record<string, unknown>)) {
        if (!(KNOWN_RUNTIMES).has(pruntime)) {
          ...
          if (!_warnedConfigKeys.has(key)) {
            ...
            try {
              process.stderr.write(
                `ferrox: warning — model_policy.runtime_tiers.${pruntime}.* uses ` +
        ...
        for (const tierName of Object.keys(tierMap)) {
          if (!RUNTIME_OVERRIDE_TIERS.has(tierName)) {
            ...
            if (!_warnedConfigKeys.has(key)) {
              ...
              try {
                process.stderr.write(
                  `ferrox: warning — model_policy.runtime_tiers.${pruntime}.${tierName} ` +
...
function _federatedConfigSchema(cwd?: string): Record<string, unknown> | undefined {
  ...
  if (typeof cwd === 'string' && cwd) {
    try {
      ...
      const schema = loaderMod.loadRegistry({ includeInstalled: true, cwd, ferroxHome: process.env['FERROX_HOME'] }).configSchema;
 ...
 *   D:  catch + no .planning/ + ~/.ferrox/defaults.json readable → source:'global-defaults', degraded:false
...
function loadConfigResolved(cwd: string, options: Record<string, unknown> = {}): ConfigResolution {
  ...
  const activeWorkstream = Object.prototype.hasOwnProperty.call(options, 'workstream')
    ...
    : (process.env['FERROX_WORKSTREAM'] || null);
  ...
  try {
    ...
    if (unknownKeys.length > 0) {
      ...
      if (!_warnedUnknownConfigKeys.has(warnKey)) {
        ...
        process.stderr.write(
          `ferrox-tools: warning: unknown config key(s) in .planning/config.json: ${unknownKeys.join(', ')} — these will be ignored\n`
        );
      }
    }
    ...
    const _baseConfig: Record<string, unknown> = {
      ...
      // CRITICAL-1: propagate the halting.* block into the resolved config so the
      // cap verbs (gate.cap-check / rescope.check / human-sla.check /
      // ship-clock.check) read the operator's per-gate cap_outcome / max_passes /
      // wall_clock_seconds and the rescope / SLA / ship-clock budgets — not just
      // their hard-coded fallbacks. Deep-merge over the manifest defaults so a
      // PARTIAL user block keeps defaults for the keys it did not override, while
      // any configured value wins.
      halting: (() => {
        const userHalting = parsed['halting'];
        const defaultHalting = (defaults.halting && typeof defaults.halting === 'object')
          ? defaults.halting as Record<string, unknown>
          : {};
        if (userHalting && typeof userHalting === 'object' && !Array.isArray(userHalting)) {
          return _deepMergeConfig(defaultHalting, userHalting as Record<string, unknown>);
        }
        return defaults.halting ?? null;
      })(),
      // COORD-03: propagate the coordination.* block into the resolved config so
      // every Phase-4 coordination verb reads the operator's hot_seams /
      // shared_state_paths / migration_store — not just the manifest fallback.
      // Deep-merge over the manifest defaults so a PARTIAL user block keeps the
      // defaults for the keys it did not override (arrays are replaced when
      // provided). This is the exact seam the Phase-3 halting block flows through,
      // reused here to defeat the silent-drop trap (T-04-01).
      coordination: (() => {
        const userCoord = parsed['coordination'];
        const defaultCoord = (defaults.coordination && typeof defaults.coordination === 'object')
          ? defaults.coordination as Record<string, unknown>
          : {};
        if (userCoord && typeof userCoord === 'object' && !Array.isArray(userCoord)) {
          return _deepMergeConfig(defaultCoord, userCoord as Record<string, unknown>);
        }
        return defaults.coordination ?? null;
      })(),
      // STRONG-02/04/05: propagate the strength.* block into the resolved config so
      // the Phase-5 merge-gate / severity-route / burndown-check verbs read the
      // operator's medium_cluster_threshold / security_age_limit_days / store paths
      // / security_categories — not just the manifest fallback. Deep-merge over the
      // manifest defaults so a PARTIAL user block keeps the defaults for the keys it
      // did not override (arrays are replaced when provided). Same seam the halting
      // and coordination blocks flow through, reused to defeat the FF-B12 silent-drop
      // trap (T-05-01); _deepMergeConfig carries the prototype-pollution guards (T-05-02).
      strength: (() => {
        const userStrength = parsed['strength'];
        const defaultStrength = (defaults.strength && typeof defaults.strength === 'object')
          ? defaults.strength as Record<string, unknown>
          : {};
        if (userStrength && typeof userStrength === 'object' && !Array.isArray(userStrength)) {
          return _deepMergeConfig(defaultStrength, userStrength as Record<string, unknown>);
        }
        return defaults.strength ?? null;
      })(),
      // MODEL-01..05: propagate the model.* block into the resolved config so the
      // Phase-6 route/escalate/risk-grade/trident/rtk verbs read the operator's
      // stage_tiers / tier_order / max_escalations / risk_boundaries /
      // trident_checkpoints / rtk.enabled — not just the manifest fallback. Deep-merge
      // over the manifest defaults so a PARTIAL user block keeps the defaults for the
      // keys it did not override (arrays are replaced when provided). Same seam the
      // halting/coordination/strength blocks flow through, reused to defeat the
      // silent-drop trap (T-06-02); _deepMergeConfig carries the prototype-pollution
      // guards (T-06-01).
      model: (() => {
        const userModel = parsed['model'];
        const defaultModel = (defaults.model && typeof defaults.model === 'object')
          ? defaults.model as Record<string, unknown>
          : {};
        if (userModel && typeof userModel === 'object' && !Array.isArray(userModel)) {
          return _deepMergeConfig(defaultModel, userModel as Record<string, unknown>);
        }
        return defaults.model ?? null;
      })(),
      // MEM-01/02: propagate the memory.* block into the resolved config so the
      // Phase-7 memory router reads the operator's fact_store / recall_limit — not
      // just the manifest fallback. Deep-merge over the manifest defaults so a
      // PARTIAL user block keeps the defaults for the keys it did not override.
      // Same seam the halting/coordination/strength/model blocks flow through,
      // reused to defeat the FF-B12 silent-drop trap (T-07-01); _deepMergeConfig
      // carries the prototype-pollution guards (T-07-02).
      memory: (() => {
        const userMemory = parsed['memory'];
        const defaultMemory = (defaults.memory && typeof defaults.memory === 'object')
          ? defaults.memory as Record<string, unknown>
          : {};
        if (userMemory && typeof userMemory === 'object' && !Array.isArray(userMemory)) {
          return _deepMergeConfig(defaultMemory, userMemory as Record<string, unknown>);
        }
    ...
    // This delivers the #1366 acceptance criterion: nonexistent FERROX_WORKSTREAM yields root, degraded.
    ...
    try {
      const home = process.env['FERROX_HOME'] || os.homedir();
      const globalDefaultsPath = path.join(home, '.ferrox', 'defaults.json');
      ...
      const _globalBaseCfg: Record<string, unknown> = {
        ...
        // CRITICAL-1: carry halting through the global-defaults branch too, so a
        // ~/.ferrox/defaults.json halting block (or the manifest default) is not
        // dropped when there is no project .planning/.
        halting: (() => {
          const gHalting = globalDefaults['halting'];
          const defaultHalting = (defaults.halting && typeof defaults.halting === 'object')
            ? defaults.halting as Record<string, unknown>
            : {};
          if (gHalting && typeof gHalting === 'object' && !Array.isArray(gHalting)) {
            return _deepMergeConfig(defaultHalting, gHalting as Record<string, unknown>);
          }
          return defaults.halting ?? null;
        })(),
        // COORD-03: carry coordination through the global-defaults branch too, so
        // a ~/.ferrox/defaults.json coordination block (or the manifest default)
        // is not dropped when there is no project .planning/.
        coordination: (() => {
          const gCoord = globalDefaults['coordination'];
          const defaultCoord = (defaults.coordination && typeof defaults.coordination === 'object')
            ? defaults.coordination as Record<string, unknown>
            : {};
          if (gCoord && typeof gCoord === 'object' && !Array.isArray(gCoord)) {
            return _deepMergeConfig(defaultCoord, gCoord as Record<string, unknown>);
          }
          return defaults.coordination ?? null;
        })(),
        // STRONG-02/04/05: carry strength through the global-defaults branch too, so
        // a ~/.ferrox/defaults.json strength block (or the manifest default) is not
        // dropped when there is no project .planning/.
        strength: (() => {
          const gStrength = globalDefaults['strength'];
          const defaultStrength = (defaults.strength && typeof defaults.strength === 'object')
            ? defaults.strength as Record<string, unknown>
            : {};
          if (gStrength && typeof gStrength === 'object' && !Array.isArray(gStrength)) {
            return _deepMergeConfig(defaultStrength, gStrength as Record<string, unknown>);
          }
          return defaults.strength ?? null;
        })(),
        // MODEL-01..05: carry model through the global-defaults branch too, so a
        // ~/.ferrox/defaults.json model block (or the manifest default) is not
        // dropped when there is no project .planning/.
        model: (() => {
          const gModel = globalDefaults['model'];
          const defaultModel = (defaults.model && typeof defaults.model === 'object')
            ? defaults.model as Record<string, unknown>
            : {};
          if (gModel && typeof gModel === 'object' && !Array.isArray(gModel)) {
            return _deepMergeConfig(defaultModel, gModel as Record<string, unknown>);
          }
          return defaults.model ?? null;
        })(),
        // MEM-01/02: carry memory through the global-defaults branch too, so a
        // ~/.ferrox/defaults.json memory block (or the manifest default) is not
        // dropped when there is no project .planning/.
        memory: (() => {
          const gMemory = globalDefaults['memory'];
          const defaultMemory = (defaults.memory && typeof defaults.memory === 'object')
            ? defaults.memory as Record<string, unknown>
            : {};
          if (gMemory && typeof gMemory === 'object' && !Array.isArray(gMemory)) {
            return _deepMergeConfig(defaultMemory, gMemory as Record<string, unknown>);
          }
          return defaults.memory ?? null;
        })(),
      };
