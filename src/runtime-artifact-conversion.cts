...
const { readFerroxCommandNames, transformContentToHyphen } = commandRoster;
...
// #1383: resolve Ferrox's version WITHOUT a top-level
// `require('../../../package.json')`. That require ran at module load on every
// ferrox-tools invocation (this module sits in the ferrox-tools loader chain) and
...
//   1. Installed trees carry <root>/ferrox-core/VERSION (written by the installer);
//      this module lives at <root>/ferrox-core/bin/lib, so VERSION is two dirs up.
//   2. The source / npm-package tree has no ferrox-core/VERSION but carries a real
...
function resolveVersionFrom(libDir: string): string {
  ...
  } catch { /* not an installed tree (no ferrox-core/VERSION) */ }
  try {
 ...
 * `_syncFerroxDir`) can derive the SAME rename decision as
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
  ...
  // skillName is the `ferrox-<stem>` dir name. (ADR-1239 / #2092)
...
## C. Tool Usage
Use these Augment tools when executing Ferrox workflows:
 ...
 * (`.qwen/agents/ferrox-*.md` / `<qwenhome>/agents/ferrox-*.md`, ADR-1239 / #2092
 ...
 * agents/ferrox-nyquist-auditor.md, agents/ferrox-security-auditor.md) — so no tools
 ...
 * path, and `/ferrox:`→`/ferrox-` body rewrites, then rebuilds a minimal skill
 ...
 * @param {string} skillName - Skill directory name (e.g. ferrox-help)
...
/**
 * Canonical list of every non-Claude runtime that ferrox-core emits artifacts for.
 ...
 * Ferrox's worktree isolation uses Claude Code's isolation="worktree" spawn
...
function applyRuntimeContentRewritesForCommandsInPlace(stagedDir, runtime, pathPrefix, isGlobal = false, attribution = undefined) {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-cmd-rewrites-'));
  try {
...
/**
 * Normalize `/ferrox:<cmd>` colon refs in the agent body to `/ferrox-<cmd>` for
 ...
 * @param runtime   canonical runtime ID
 * @param cmdNames  ferrox command names from readFerroxCommandNames()
 */
...
export = {
  ...
  // surface.cts's _syncFerroxDir can derive the .agent.md rename from the SAME
