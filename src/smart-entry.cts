/**
 * Smart Entry — state-aware situation classifier for the /ferrox front door.
 *
 ...
 *
 * Surfaced as `ferrox-tools smart-entry [--json]`. The markdown `/ferrox` command +
 ...
 * docs/superpowers/specs/2026-06-27-ferrox-smart-entry-design.md and
 * docs/adr/1787-ferrox-next-smart-entry.md.
 *
 * Relationship to `/ferrox:progress --next`: this classifier is a *menu* front door,
 * not a second router. For in-project forward motion (planning → executing →
 * verify-pending) the recommended action delegates to `/ferrox:progress --next`
 ...
 * exactly what got the old flat `/ferrox-next` removed (#3054). Its distinct value is
 ...
 *
 * Design note: this is the ferrox-core analog of ferrox-pi's `showSmartEntry` branch
 * tree, redesigned for ferrox-core's `.planning/` phase loop. ferrox-pi's
...
/** Situation id — the ferrox-core analog of ferrox-pi's phase enum. */
...
export function detectSignals(cwd: string, now: () => number = Date.now): SmartEntrySignals {
  ...
  // the classifier works on real STATE.md files written by current Ferrox.
...
export function actionsFor(situation: Situation, s: SmartEntrySignals): SmartEntryAction[] {
  ...
  switch (situation) {
    ...
    case 'executing':
      ...
      // (/ferrox:progress --next → workflows/next.md). Its Route 0 resume-incomplete
...
/**
 * `ferrox-tools smart-entry` entry point. `--json` emits machine JSON; default
 ...
 * `output()` so the workflow can fall back to `/ferrox:progress`.
 */
