...
function disabledResponse(): DisabledResponse {
  return { disabled: true, message: 'graphify is not enabled. Enable with: ferrox-tools config-set graphify.enabled true' };
}
...
function graphifyQuery(cwd: string, term: string, options: { budget?: number | null } = {}): unknown {
  ...
  if (!fs.existsSync(graphPath)) {
    return { error: configured
      ? `Configured graph not found at ${graphPath}. Set graphify.graph_path or run /ferrox:graphify build.`
