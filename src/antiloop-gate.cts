...
/** A terminal outcome a closed review resolves to. */
type AntiloopTerminalOutcome = 'ship-with-backlog' | 'stop-and-rescope' | 'escalate-to-human';
 ...
 * not a stale comment.
 */
const ANTILOOP_TERMINAL_OUTCOMES: readonly AntiloopTerminalOutcome[] = Object.freeze([
  'ship-with-backlog',
  'stop-and-rescope',
  'escalate-to-human',
] as AntiloopTerminalOutcome[]);
...
/** One event as it appears in the log. Fields beyond the key are open. */
interface AntiloopEvent {
  ts?: string | number;
  kind?: unknown;
  artifact?: unknown;
  question?: unknown;
...
/** A finding as filed, whether it arrives in an event or from a caller. */
interface AntiloopFinding {
  finding_id?: unknown;
  category?: unknown;
  severity?: unknown;
  /** The command that reproduces the failure. Empty means it cannot block. */
  reproducible?: unknown;
  /** The exit code that command produced when it was run at filing time. */
  reproducible_exit?: unknown;
  [k: string]: unknown;
}

/**
 * A normalized budget. `declared` is false whenever either allowance is unusable.
 *
 * BOTH allowances are the count AT WHICH the review closes, because
 * `evaluateGateOpen` closes when a derived count has REACHED its allowance.
 * `max_rounds` of 3 therefore permits 3 opens, at derived counts 0, 1 and 2.
 * `max_lineages` of 2 permits 1 distinct reviewing lineage, and closes when a 2nd
 * appears. That asymmetry is real and is stated here rather than left to be
 * inferred: a round is consumed once per open, while a lineage is a set member
 * reused across every round it runs.
 */
interface BudgetAllowance {
  declared: boolean;
  max_rounds: number;
  max_lineages: number;
}

/** The folded state of 1 reviewed pair. */
interface AntiloopPairState {
  key: string;
  artifact: string;
  question: string;
  budget: BudgetAllowance;
  /** Index of the first budget event for this pair, or -1. */
  budget_event_index: number;
  /** Index of the first round-opened event for this pair, or -1. */
  first_round_index: number;
  /** Rounds OPENED for this pair. Derived. Never settable. */
  rounds: number;
  /** Rounds recorded closed, provenance only. */
  rounds_closed: number;
  /** Distinct normalized reviewing lineages seen on this pair. */
  lineages: Set<string>;
  findings: AntiloopFinding[];
  /** Event kinds this fold did not understand, so an unknown kind is visible. */
  ignored_kinds: string[];
}

/** The gate-open decision. */
interface GateOpenResult {
  decision: 'open-ok' | 'refuse-open' | 'close-review';
  code: string | null;
  outcome: AntiloopTerminalOutcome | null;
  key: string;
  rounds: number;
  lineages: number;
  max_rounds: number;
...
/** The blocking-eligibility decision for 1 finding. */
interface FindingBlockingResult {
  blocking: boolean;
  code: string | null;
  finding_id: string;
  reason: string;
...
/** The disposition of the findings that remain when a budget is spent. */
interface BudgetSpentDispositionResult {
  outcome: AntiloopTerminalOutcome;
  reason: string;
  remaining: number;
...
/** The result-union error shape, verbatim from `src/governance-manifest.cts:49-57`. */
interface AntiloopError {
  ok: false;
  code: string;
  message: string;
  file: string;
...
/** A parsed budget declaration read out of visible human prose. */
interface BudgetDeclaration {
  ...
  max_rounds: number;
  max_lineages: number;
  allowances: { rounds: number; question: string }[];
  cross_audit_budgeted: boolean;
}
 ...
 */
function counterKey(input: { artifact?: unknown; question?: unknown }): string {
...
function allowanceOrNull(v: unknown): number | null {
 ...
 */
function normalizeBudget(raw: unknown): BudgetAllowance {
  const src = raw !== null && typeof raw === 'object' ? (raw as Record<string, unknown>) : null;
...
function emptyPairState(key: string, artifact: string, question: string): AntiloopPairState {
  return {
    key,
    artifact,
    question,
 ...
 */
function foldAntiloopEvents(events: unknown): Map<string, AntiloopPairState> {
  const list: AntiloopEvent[] = Array.isArray(events) ? (events as AntiloopEvent[]) : [];
  const pairs = new Map<string, AntiloopPairState>();

  for (let i = 0; i < list.length; i++) {
    const ev = list[i] !== null && typeof list[i] === 'object' ? list[i] : ({} as AntiloopEvent);
    ...
    if (kind === 'round-opened') {
      if (state.first_round_index === -1) state.first_round_index = i;
      state.rounds += 1;
      const lineage = norm(ev.lineage);
      if (lineage !== '') state.lineages.add(lineage);
      continue;
    }
...
function evaluateGateOpen(input: {
  ...
  const budget =
    state === undefined ? { declared: false, max_rounds: 0, max_lineages: 0 } : state.budget;
  const budgetIsLate =
    state !== undefined
  ...
  if (rounds >= budget.max_rounds || lineages >= budget.max_lineages) {
    return {
      ...
      max_lineages: budget.max_lineages,
      message:
        `the review budget for this pair is spent at ${rounds} of ${budget.max_rounds} rounds `
  ...
  return {
    ...
    max_lineages: budget.max_lineages,
    message:
      `round ${rounds + 1} of ${budget.max_rounds} may open for this pair `
 ...
 */
function evaluateFindingBlocking(input: { finding?: AntiloopFinding }): FindingBlockingResult {
  ...
  if (command === '') {
    return {
      ...
      message:
        `finding ${id} carries no reproducible command, so it cannot block at any severity. `
  ...
  if (!observedFailure) {
    return {
      ...
      message:
        `finding ${id} carries a command that did not fail when it was filed, so it is a `
...
function evaluateBudgetSpentDisposition(input: {
...
}): BudgetSpentDispositionResult {
  const rawFindings = input ? input.findings : undefined;
  const findings: AntiloopFinding[] = Array.isArray(rawFindings)
    ? (rawFindings as AntiloopFinding[])
    : [];
  ...
  if (security.length > 0) {
    return {
      ...
      message:
        `the review budget is spent and ${security.length} remaining finding(s) carry a security `
  ...
  return {
    ...
    security_findings: [],
    message:
      `the review budget is spent; ${findings.length} remaining finding(s) become backlog rows `
...
const ROUND_ALLOWANCE =
  /(\d+)\s+(?:[a-z][a-z-]*\s+){0,3}rounds?\b(?:\s+(?:for|per|at|of)\s+([a-z][a-z0-9 -]{0,40}?))?(?=\s*(?:\band\b|[,;.]|$))/gi;
...
function err(
  ...
  code: string,
  message: string,
  extra?: { line?: number; text?: string },
): AntiloopError {
  const e: AntiloopError = { ok: false, code, message, file };
  if (extra !== undefined) {
 ...
 */
function parseBudgetDeclaration(text: unknown, filename: unknown): BudgetDeclaration | AntiloopError {
  ...
  if (sentence === null) {
    return err(
      file,
      ANTILOOP_ERROR_CODES.BUDGET_UNDECLARED,
      'no review budget is declared. Add 1 bolded sentence naming the review budget, the digit '
  ...
  const allowances: { rounds: number; question: string }[] = [];
  ...
  if (allowances.length === 0) {
    return err(
      file,
      ANTILOOP_ERROR_CODES.BUDGET_UNDECLARED,
      'the review budget sentence states no digit count of rounds, so no allowance exists to '
        + 'spend. State it as a digit.',
      { line, text: quoted },
    );
  }
  ...
  let crossAuditBudgeted: boolean;
  let maxLineages: number;
  if (lineageCount !== null) {
    ...
    return err(
      file,
      ANTILOOP_ERROR_CODES.BUDGET_UNDECLARED,
      'the review budget states rounds but says nothing about reviewing lineages, so adding a '
