  ...
  if (items.length === 0 && blockLines.length > 0) {
    ...
    if (nonEmptyLines > 0) {
      process.stderr.write(
        `[ferrox-tools] WARNING: must_haves.${blockName} block has ${nonEmptyLines} content lines but parsed 0 items. ` +
...
const FRONTMATTER_SCHEMAS: Record<string, { required: string[]; optional?: string[] }> = {
  plan: {
    required: ['phase', 'plan', 'type', 'wave', 'depends_on', 'files_modified', 'autonomous', 'must_haves'],
    // UGE-02 gate metadata — OPTIONAL machine-readable routing fields. Absent on
    // every pre-v1.8 plan doc; absence is always valid (never an error). `domain`
    // is a gate-select registry key, `gate_present` a boolean, `gate_script` a path.
    optional: ['domain', 'deliverable_kind', 'gate_present', 'gate_script'],
  },
...
/**
 * UGE-02: normalize the four OPTIONAL plan gate-metadata frontmatter fields.
 *
 * Lenient by contract (fail-toward-normal — an increment without usable gate
 * metadata is simply not gateable, never an error):
 *   - `domain` / `deliverable_kind` / `gate_script`: trimmed non-empty string,
 *     else undefined.
 *   - `gate_present`: strict boolean true/false, accepting the YAML scalar
 *     strings 'true'/'false' (extractFrontmatter yields strings); anything
 *     else → undefined. Truthy-but-not-true values do NOT count as a gate.
 * Never throws, including on null/undefined/garbage input.
 */
function extractPlanGateMetadata(fm: unknown): {
  domain: string | undefined;
  deliverable_kind: string | undefined;
  gate_present: boolean | undefined;
  gate_script: string | undefined;
} {
  const src: Record<string, unknown> =
    fm !== null && typeof fm === 'object' && !Array.isArray(fm) ? (fm as Record<string, unknown>) : {};
  const asString = (v: unknown): string | undefined =>
    typeof v === 'string' && v.trim() !== '' ? v.trim() : undefined;
  const gp = src['gate_present'];
  let gatePresent: boolean | undefined;
  if (gp === true || gp === 'true') gatePresent = true;
  else if (gp === false || gp === 'false') gatePresent = false;
  return {
    domain: asString(src['domain']),
    deliverable_kind: asString(src['deliverable_kind']),
    gate_present: gatePresent,
    gate_script: asString(src['gate_script']),
  };
}
