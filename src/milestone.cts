...
function cmdMilestoneComplete(cwd: string, version: string, options: MilestoneCompleteOptions, raw: boolean): void {
  ...
  // resolves to the workstream base when FERROX_WORKSTREAM is set and to root .planning otherwise
  ...
  if (fs.existsSync(statePath)) {
    const result = transitionCore(
      fs.readFileSync(statePath, 'utf-8'),
      {
        ...
        version,
        nextMilestoneCommand: formatFerroxSlash('new-milestone', resolveRuntime(cwd)) as string,
      },
