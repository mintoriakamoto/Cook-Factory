...
function checkAuthority(opts?: { op?: unknown; denied?: unknown }): AuthorityResult {
  const o = opts && typeof opts === 'object' ? opts : {};
  const op = o.op;
  if (typeof op !== 'string' || op === '') return { allowed: false, reason: 'non-string-op-deny' };
  const denied = Array.isArray(o.denied)
    ? (o.denied as unknown[]).filter((x): x is string => typeof x === 'string')
    : DEFAULT_DENIED;
  if (denied.includes(op)) return { allowed: false, reason: `denied:${op}` };
...
function assertInvariants(opts?: {
  candidateShaBefore?: unknown;
  candidateShaAfter?: unknown;
  protectedPathsDirty?: unknown;
}): InvariantResult {
  const o = opts && typeof opts === 'object' ? opts : {};
  ...
  if (o.protectedPathsDirty === true) violations.push('protected-path-dirtied');
  return { ok: violations.length === 0, violations };
}

export = { checkAuthority, assertInvariants };
