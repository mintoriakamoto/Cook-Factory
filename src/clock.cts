...
/**
 * Parse FERROX_NOW_MS to a valid pinned epoch millisecond value, or return null.
 *
 ...
 * Returns null when FERROX_TEST_MODE is not set.
 */
function _pinnedNowMs(): number | null {
  if (!process.env.FERROX_TEST_MODE) return null;
  const raw = process.env.FERROX_NOW_MS;
...
export const realClock: Clock = {
   ...
   *
   * When both FERROX_TEST_MODE and FERROX_NOW_MS are set (subprocess time-pin adapter,
   ...
   * Uses this.now() so the subprocess time-pin adapter (FERROX_NOW_MS) is honoured
   * exactly as today()/nowIso() are — deterministic when FERROX_NOW_MS and TZ are
   * both pinned (#2136).
   *
