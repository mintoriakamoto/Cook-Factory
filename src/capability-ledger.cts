 ...
 *
 * Manages a per-runtime install manifest (`.ferrox-capabilities.json`) that records
...
const LEDGER_FILE_NAME = '.ferrox-capabilities.json';
    ...
    if (typeof reparsed === 'object' && reparsed !== null) {
      ...
      if (typeof ver === 'string' && ver !== LEDGER_SCHEMA_VERSION) {
        throw new CorruptLedgerError(
          ...
          `(this build supports version "${LEDGER_SCHEMA_VERSION}"). Upgrade Ferrox to a build that ` +
