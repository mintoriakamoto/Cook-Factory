/**
 * runtime-slash.cts — single source of truth for emitting Ferrox slash-command
 ...
 *
 * Background: #2808 unified all Ferrox skill installs to register under the hyphen
 * form (`name: ferrox-<cmd>`). The legacy colon form `/ferrox:<cmd>` is no longer
 ...
 * `$ferrox-<cmd>` form. This module is the only place the runtime should decide
 * which shape to emit.
 *
 *   - codex:                                   $ferrox-<cmd>   (shell-var syntax)
 *   - claude, cursor, opencode, kilo, etc.:    /ferrox-<cmd>
 *
...
export function formatFerroxSlash(commandName: unknown, runtime: unknown): unknown {
  ...
  // both legacy `/ferrox:<name>` and canonical hyphen-form input (plus the bare
  // `ferrox:<name>` shorthand and codex `$ferrox-<name>` shell-var input).
  const stripped = commandName.replace(/^[/$]?ferrox[-:]/i, '');
  ...
  // Defensive: a degenerate input like `/ferrox:`, `ferrox-`, or whitespace-only
  ...
  if (style === 'shell-var') {
    // shell-var runtimes (currently: codex) use $ferrox-<cmd> syntax. The command
    ...
    return `$ferrox-${token.toLowerCase()}${tail}`;
  }
  return `/ferrox-${token}${tail}`;
}
...
export function resolveRuntime(projectDir: string | null | undefined): string {
  const envRuntime = resolveRuntimeNameFromCandidates(process.env['FERROX_RUNTIME']);
 ...
 * Equivalent to `formatFerroxSlash(name, resolveRuntime(projectDir))`.
 */
export function formatFerroxSlashFor(projectDir: string | null | undefined, commandName: unknown): unknown {
  return formatFerroxSlash(commandName, resolveRuntime(projectDir));
}
