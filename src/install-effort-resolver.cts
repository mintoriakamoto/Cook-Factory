 ...
 *
 * `ferrox-tools effort sync` (src/commands.cts `cmdEffortSync`) must mirror what the
 * installer writes — home `~/.ferrox/defaults.json` merged with the project's
 ...
 * `src/*.cts` module compiled into the shipped `ferrox-core/bin/lib/` tree — lets both
 ...
 *
 * Pure with respect to config: `readFerroxEffectiveEffortConfig` performs the config
...
const { EFFORT_SET: FERROX_EFFORT_SET } = modelResolver as { EFFORT_SET: Set<string> };
...
/**
 * #2517 — Read a single Ferrox config file (defaults.json or per-project
 ...
 */
function _readFerroxConfigFile(absPath: string, label: string): Record<string, unknown> | null {
  if (!fs.existsSync(absPath)) return null;
  let raw: string;
  try {
    raw = fs.readFileSync(absPath, 'utf-8');
  } catch (err) {
    process.stderr.write(`ferrox: warning — could not read ${label} (${absPath}): ${(err as Error).message}\n`);
    return null;
  }
  try {
    return JSON.parse(raw) as Record<string, unknown>;
  } catch (err) {
    process.stderr.write(`ferrox: warning — invalid JSON in ${label} (${absPath}): ${(err as Error).message}\n`);
    return null;
  }
}
...
let _ferroxEffortCatalogCache: EffortCatalog | null = null;
function _getFerroxEffortCatalog(): EffortCatalog {
  if (_ferroxEffortCatalogCache) return _ferroxEffortCatalogCache;
  ...
  // This module lives in ferrox-core/bin/lib/, so the shared manifest is one level
  // up in ferrox-core/bin/shared/ (bin/lib → bin → shared). (In bin/install.js this
  // path was `.., ferrox-core, bin, shared` relative to the package-root bin/.)
  ...
  try {
    ...
    throw new Error(
      `ferrox install: cannot load config-defaults.manifest.json at ${manifestPath}: ${(_err as Error).message}`,
    );
  }
  ...
  _ferroxEffortCatalogCache = {
    AGENT_DEFAULT_TIERS,
    renderEffortForRuntime,
    EFFORT_MANIFEST_TIER_DEFAULTS: tierDefaults,
    EFFORT_MANIFEST_DEFAULT: effortDefault,
  };
  return _ferroxEffortCatalogCache;
}

/**
 * #443 — Read the merged `effort` config block for install-time effort resolution.
 *
 * Probes the same config sources as readFerroxRuntimeProfileResolver (per-project
 * `.planning/config.json` wins over `~/.ferrox/defaults.json`) but extracts the
 ...
 */
function readFerroxEffectiveEffortConfig(targetDir: string | null = null): EffortConfig | null {
 ...
 *
 * @param effortCfg   Result of readFerroxEffectiveEffortConfig().
 * @param agentName   e.g. 'ferrox-planner'
...
function resolveInstallTimeEffort(effortCfg: EffortConfig | null, agentName: string): string {
  ...
  if (effortCfg) {
    ...
    if (ao && typeof ao === 'object' && !Array.isArray(ao)) {
      ...
      if (typeof v === 'string' && FERROX_EFFORT_SET.has(v)) return v;
    }
  }

  // Step 2: routing_tier_defaults keyed by the agent's catalog tier
  const { AGENT_DEFAULT_TIERS, EFFORT_MANIFEST_TIER_DEFAULTS, EFFORT_MANIFEST_DEFAULT } = _getFerroxEffortCatalog();
  ...
  if (effortCfg) {
    ...
    if (typeof d === 'string' && FERROX_EFFORT_SET.has(d)) return d;
  }
  ...
  if (typeof EFFORT_MANIFEST_DEFAULT === 'string' && FERROX_EFFORT_SET.has(EFFORT_MANIFEST_DEFAULT)) {
