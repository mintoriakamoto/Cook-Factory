 ...
 * `ferrox-core/bin/lib/semver-compare.cjs`; only types are added. The
...
// ─── Range satisfaction (ADR-1244 D2 — engines.ferrox load-time gate) ────────────
//
...
// subset capability `engines.ferrox` ranges actually use: `>= <= > < =` (exact),
