...
/** One node of the `workgraph/v1` document, reduced to the 2 fields read here. */
interface ParkNode {
  id?: unknown;
  schedule_order?: unknown;
}

/** One declared edge: `from` is the DEPENDENT, `to` is the PREREQUISITE. */
interface ParkEdge {
  from?: unknown;
  to?: unknown;
  declared?: unknown;
...
/** Everything these functions are allowed to know about the graph. */
interface ParkDocument {
  nodes?: unknown;
  edges?: unknown;
}

/** The derived view of a document that every verb below reads. */
interface ParkIndex {
  ids: ReadonlySet<string>;
  sortedIds: readonly string[];
  /** prerequisite id -> its DEPENDENTS, sorted. The downstream adjacency. */
  dependents: ReadonlyMap<string, readonly string[]>;
  scheduleOrder: ReadonlyMap<string, unknown>;
}

/** A chain and the order sequence it is compared on. */
interface Chain {
  ids: readonly string[];
 ...
 */
function _index(document: ParkDocument | null | undefined): ParkIndex {
  const source = (document === null || document === undefined) ? {} : document;

  const ids = new Set<string>();
  const scheduleOrder = new Map<string, unknown>();
  for (const raw of _asArray(source.nodes)) {
    if (raw === null || typeof raw !== 'object') continue;
    const node = raw as ParkNode;
  ...
  for (const raw of _asArray(source.edges)) {
    if (raw === null || typeof raw !== 'object') continue;
    const edge = raw as ParkEdge;
    if (edge.declared !== true) continue;
    for (const endpoint of [edge.from, edge.to]) {
      if (typeof endpoint !== 'string' || !ids.has(endpoint)) {
        throw _codedError(
          FLEET_PARK_ERROR_CODES.E_FLEET_PARK_UNKNOWN_NODE,
          `fleet-park: a declared edge names ${JSON.stringify(endpoint)}, which is not a node of this document. `
    ...
    (dependents.get(edge.to as string) as string[]).push(edge.from as string);
  }

  for (const list of dependents.values()) list.sort(_compareStrings);
...
function _requireKnown(index: ParkIndex, nodeId: unknown): string {
  if (typeof nodeId === 'string' && index.ids.has(nodeId)) return nodeId;
  throw _codedError(
    FLEET_PARK_ERROR_CODES.E_FLEET_PARK_UNKNOWN_NODE,
    `fleet-park: ${JSON.stringify(nodeId)} is not a node of this document. An empty subtree is what a LEAF `
...
/** What a walk found: the nodes reached as dependents, and a post order. */
interface WalkResult {
  /** Every node reached AS A DEPENDENT of something. Roots are absent unless re-reached. */
  reached: Set<string>;
  /** Every node the walk finished, children before parents. Reverse topological. */
  postorder: string[];
}
 ...
 */
function _walk(index: ParkIndex, roots: readonly string[]): WalkResult {
  const colour = new Map<string, number>();
  const reached = new Set<string>();
  const postorder: string[] = [];

  for (const root of roots) {
    if ((colour.get(root) ?? _WHITE) !== _WHITE) continue;
    ...
    while (stack.length > 0) {
      ...
      if (seen === _GREY) {
        ...
        throw _codedError(
          FLEET_PARK_ERROR_CODES.E_FLEET_PARK_CYCLE,
          `fleet-park: the declared edges carry a cycle, ${loop.join(' -> ')}. A longest chain is undefined over `
 ...
 */
function descendantSubtree(document: ParkDocument | null | undefined, nodeId: unknown): string[] {
 ...
 */
function blockedSubtree(document: ParkDocument | null | undefined, parkedIds: unknown): string[] {
  const index = _index(document);
  const parked: string[] = [];
  for (const raw of _asArray(parkedIds)) parked.push(_requireKnown(index, raw));
  parked.sort(_compareStrings);

  const walked = _walk(index, parked);
  for (const id of parked) walked.reached.delete(id);
  return [...walked.reached].sort(_compareStrings);
}

/** The numeric schedule order of a node, or a refusal. */
function _orderOf(index: ParkIndex, id: string): number {
  const value = index.scheduleOrder.get(id);
  if (typeof value !== 'number' || !Number.isFinite(value)) {
    throw _codedError(
      FLEET_PARK_ERROR_CODES.E_FLEET_PARK_NO_SCHEDULE_ORDER,
      `fleet-park: node ${JSON.stringify(id)} carries no numeric schedule_order, so the tie break has no total `
 ...
 */
function criticalPath(document: ParkDocument | null | undefined): string[] {
  const index = _index(document);
  if (index.sortedIds.length === 0) return [];

  const orders = new Map<string, number>();
  for (const id of index.sortedIds) orders.set(id, _orderOf(index, id));

  const walked = _walk(index, index.sortedIds);

  const best = new Map<string, Chain>();
  for (const id of walked.postorder) {
    let winner: Chain | null = null;
    for (const kid of index.dependents.get(id) ?? []) {
      const candidate = best.get(kid) as Chain;
      if (winner === null || _better(candidate, winner)) winner = candidate;
    }
    const order = orders.get(id) as number;
    best.set(id, winner === null
      ? { ids: [id], orders: [order] }
      : { ids: [id, ...winner.ids], orders: [order, ...winner.orders] });
  }

  let answer: Chain | null = null;
  for (const id of index.sortedIds) {
    const candidate = best.get(id) as Chain;
    if (answer === null || _better(candidate, answer)) answer = candidate;
  }
  return [...(answer as Chain).ids];
}
 ...
 */
function onCriticalPath(document: ParkDocument | null | undefined, nodeId: unknown): boolean {
  const index = _index(document);
  const target = _requireKnown(index, nodeId);
  return criticalPath(document).includes(target);
}

// ─── the park state, folded out of the log ───────────────────────────────────

/** One already parsed run log record, reduced to the fields the fold reads. */
interface ParkEventLike {
  kind?: unknown;
  run_id?: unknown;
  node_id?: unknown;
  scope?: unknown;
  on_critical_path?: unknown;
}

/** How the caller scopes the fold. */
interface FoldParkOptions {
  runId?: unknown;
  budget?: unknown;
}

/** Which parks were on the critical path and which were not. */
interface CriticalSplit {
  on: string[];
  off: string[];
}

/** The whole park state of 1 run, derived. */
interface ParkState {
  parked: string[];
  park_count: number;
  budget: number;
  budget_alarm_due: boolean;
  alarmed: boolean;
  on_critical_path: CriticalSplit;
}
 ...
 */
function foldParkState(
  events: readonly ParkEventLike[] | null | undefined,
  options: FoldParkOptions | null | undefined = {},
): ParkState {
  ...
  for (const raw of _asArray(events)) {
    if (raw === null || typeof raw !== 'object') continue;
    const event = raw as ParkEventLike;
    if (runId !== null && event.run_id !== runId) continue;

    if (event.kind === 'node_parked') {
      if (typeof event.node_id !== 'string') continue;
      critical.set(event.node_id, event.on_critical_path === true);
      continue;
    }
    if (event.kind === 'park_alarm' && event.scope === 'budget') alarmed = true;
  }
  ...
  for (const id of parked) {
    if (critical.get(id) === true) on.push(id);
    else off.push(id);
  }
