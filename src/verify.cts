...
function cmdValidateHealth(
  ...
  // wsBase resolves to .planning/workstreams/<ws>/ when FERROX_WORKSTREAM is set (STATE.md, ROADMAP.md, phases/)
  ...
  const slash = (name: string) => formatFerroxSlash(name, _slashRuntime) as string;
    ...
    if (!agentStatus.agents_installed) {
      if ((agentStatus.installed_agents).length === 0) {
        addIssue(
          'warning',
          'W010',
          `No Ferrox agents found in ${agentStatus.agents_dir} — Task(subagent_type="ferrox-*") will fall back to general-purpose`,
          `Run the Ferrox installer: npx ${PACKAGE_NAME}@latest`,
        );
        ...
        addIssue(
          ...
          `Re-run the Ferrox installer to complete the install: npx ${PACKAGE_NAME}@latest`,
        );
      } else if ((agentStatus.incomplete_agents).length > 0) {
        addIssue(
          'warning',
          'W010',
          `Missing ${(agentStatus.missing_agents).length} Ferrox agents: ${(agentStatus.missing_agents).join(', ')}; incomplete agent installs (missing generated file): ${(agentStatus.incomplete_agents).join(', ')} — affected workflows will fall back to general-purpose`,
          `Run the Ferrox installer: npx ${PACKAGE_NAME}@latest`,
        );
      } else {
        addIssue(
          'warning',
          'W010',
          `Missing ${(agentStatus.missing_agents).length} Ferrox agents: ${(agentStatus.missing_agents).join(', ')} — affected workflows will fall back to general-purpose`,
          `Run the Ferrox installer: npx ${PACKAGE_NAME}@latest`,
        );
      }
    }
      ...
      if (fs.existsSync(roadmapPath)) {
        ...
        for (const mm of mismatches) {
          addIssue(
            ...
            'Run `ferrox-tools roadmap upgrade --convention milestone-prefixed` to migrate (dry-run by default)',
          );
        }
      }
    }
    ...
    for (const entry of entries) {
      ...
      if (!isCanonicalPlanningFile(entry.name)) {
        addIssue(
          'warning',
          'W019',
          `Unrecognized .planning/ file: ${entry.name} — not a canonical Ferrox artifact`,
  ...
  output(
    {
      ...
      // When skipCheck is true (FERROX_SKIP_SCHEMA_CHECK=true), the gate is bypassed —
