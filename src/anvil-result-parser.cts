...
interface AnvilParseResult {
  best: number | null;
  total: number | null;
  calls: number | null;
  outTok: number | null;
  flatCostUsd: number | null;
  trueCostUsd: number | null;
  green: boolean;
}

const FAIL_CLOSED: AnvilParseResult = {
 ...
 */
function parseAnvilResult(stdout?: unknown): AnvilParseResult {
  if (typeof stdout !== 'string' || stdout === '') return { ...FAIL_CLOSED };

  // find the LAST line that starts a FINAL ledger entry with a best=N/M score
  const finals = stdout
    .split('\n')
    .filter((l) => /FINAL\s+best=\d+\s*\/\s*\d+/.test(l));
  if (finals.length === 0) return { ...FAIL_CLOSED };
  const line = finals[finals.length - 1];

  const scoreM = line.match(/best=(\d+)\s*\/\s*(\d+)/);
  if (!scoreM) return { ...FAIL_CLOSED };
  const best = Number(scoreM[1]);
  const total = Number(scoreM[2]);
  if (!Number.isFinite(best) || !Number.isFinite(total)) return { ...FAIL_CLOSED };
