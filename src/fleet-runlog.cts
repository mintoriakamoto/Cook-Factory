...
const FLEET_COMMON_FIELDS = Object.freeze(['ts', 'kind', 'run_id'] as const);
...
const FLEET_EVENT_REQUIRED_FIELDS = Object.freeze({
  run_started: Object.freeze(['graph_generation'] as const),
  run_closed: Object.freeze([] as const),
  worker_started: Object.freeze(['worker_id', 'node_id', 'attempt_id', 'lease_epoch'] as const),
  worker_ended: Object.freeze(['worker_id', 'node_id', 'attempt_id', 'outcome'] as const),
  claim_acquired: Object.freeze(['node_id', 'worker_id', 'lease_epoch'] as const),
  lease_renewed: Object.freeze(['node_id', 'worker_id', 'lease_epoch'] as const),
  lease_released: Object.freeze(['node_id', 'worker_id', 'lease_epoch'] as const),
  lease_reclaimed: Object.freeze([
    'node_id', 'worker_id', 'lease_epoch', 'prior_worker_id', 'prior_lease_epoch', 'reason',
  ] as const),
  queue_entered: Object.freeze(['node_id', 'attempt_id', 'ticket'] as const),
  queue_acquired: Object.freeze(['node_id', 'attempt_id', 'ticket', 'worker_id'] as const),
  gate_started: Object.freeze(['node_id', 'attempt_id'] as const),
  gate_ended: Object.freeze(['node_id', 'attempt_id', 'verdict'] as const),
  land_completed: Object.freeze(['node_id', 'attempt_id', 'result'] as const),
  post_land_truth: Object.freeze(['node_id', 'attempt_id', 'classification'] as const),
  node_parked: Object.freeze(['node_id', 'reason', 'attempts', 'on_critical_path'] as const),
  park_alarm: Object.freeze(['node_id', 'scope', 'route', 'parked_count', 'budget'] as const),
});
...
const _KIND_SET: ReadonlySet<string> = new Set<string>(FLEET_EVENT_KINDS);

/** One append-only fleet run record. */
interface FleetEvent {
  /** Caller-supplied timestamp (epoch ms or ISO string). Never read here. */
  ts: string | number;
  /** Event kind: 1 of FLEET_EVENT_KINDS. */
  kind: string;
  /** The run this event belongs to. Present on EVERY kind. */
  run_id: string;
  /** Each kind attaches its own fields; see FLEET_EVENT_REQUIRED_FIELDS. */
  [k: string]: unknown;
}

/** Options for append/read. `path` targets an explicit log file. */
interface FleetRunlogOptions {
  path?: string;
}
 ...
 */
function fleetRunlogPath(cwd: string): string {
  return path.join(cwd, '.planning', 'fleet-runlog.jsonl');
}

/** Every field name a record of this kind must carry, common 3 included. */
function _requiredFieldsFor(kind: string): readonly string[] {
  const extra = (FLEET_EVENT_REQUIRED_FIELDS as Record<string, readonly string[]>)[kind] ?? [];
 ...
 */
function appendFleetEvent(entry: FleetEvent, opts: FleetRunlogOptions = {}): void {
  ...
  if (typeof kind !== 'string' || !_KIND_SET.has(kind)) {
    throw _codedError(
      FLEET_RUNLOG_ERROR_CODES.E_FLEET_UNKNOWN_KIND,
      `appendFleetEvent: unknown kind ${JSON.stringify(kind)}. `
  ...
  for (const field of _requiredFieldsFor(kind)) {
    ...
    if (value === undefined) {
      throw _codedError(
        FLEET_RUNLOG_ERROR_CODES.E_FLEET_MISSING_FIELD,
        `appendFleetEvent: kind ${kind} requires field ${field}, which is absent. `
  ...
  if (bytes >= FLEET_MAX_RECORD_BYTES) {
    throw _codedError(
      FLEET_RUNLOG_ERROR_CODES.E_FLEET_RECORD_TOO_LARGE,
      `appendFleetEvent: record serializes to ${bytes} bytes, at or past the `
 ...
 * `readFileSync` result.
 */
function readFleetRunlog(opts: FleetRunlogOptions = {}): FleetEvent[] {
  const target = opts.path;
  if (typeof target !== 'string' || target === '') {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    try {
      ...
      throw _codedError(
        FLEET_RUNLOG_ERROR_CODES.E_FLEET_LOG_UNPARSEABLE,
        `readFleetRunlog: line ${i + 1} of ${target} is not valid JSON. `
