...
// Matches milestone section headings: ## v1.0, ## Roadmap v2.0, ## ✅ v1.0, ## [Ferrox] v1.0, etc.
// The optional bracket-token prefix (e.g., [Ferrox]) must be tested before the emoji group.
 ...
 * e.g. "Ferrox-01-setup" → "01", "01-setup" → "01", "02-implement" → "02", "02.1-hotfix" → "02.1"
 */
function extractPhaseNumFromDir(dirName: string): string | null {
  // Strip optional project_code prefix: "Ferrox-01-setup" → "01-setup"
 ...
 * old: "01-setup"         newId: "1-02"  projectCode: "Ferrox"  → "Ferrox-01-02-setup"
 * old: "01-setup"         newId: "1-02"  projectCode: null   → "01-02-setup"
 * old: "Ferrox-01-setup"     newId: "1-02"  projectCode: "Ferrox"  → "Ferrox-01-02-setup"
 */
...
function computeMigrationPlan(cwd: string, options: Record<string, unknown> = {}): MigrationPlan {
  ...
  for (const fileName of crossRefFiles) {
    ...
    for (const m of orderedMappings) {
      ...
      // Rewrite project_code-prefixed references: "Ferrox-01-" → "Ferrox-01-02-"
      if (projectCode) {
