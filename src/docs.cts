 ...
 * with Ferrox marker detection, doc tooling detection, monorepo awareness, and
...
const FERROX_MARKER = '<!-- generated-by: ferrox-doc-writer -->';
...
/**
 * Check whether a file begins with the Ferrox doc writer marker.
 * Reads the first 500 bytes only — avoids loading large files.
 */
function hasFerroxMarker(filePath: string): boolean {
  try {
    const buf = Buffer.alloc(500);
    const fd = fs.openSync(filePath, 'r');
    const bytesRead = fs.readSync(fd, buf, 0, 500, 0);
    fs.closeSync(fd);
    return buf.slice(0, bytesRead).toString('utf-8').includes(FERROX_MARKER);
  } catch {
    return false;
  }
}
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          results.push({ path: rel, has_ferrox_marker: hasFerroxMarker(abs) });
        }
      }
 ...
 *
 * @example
 * node ferrox-tools.cjs docs-init --raw
 */
function cmdDocsInit(cwd: string, raw: boolean): void {
  const config = loadConfig(cwd);
  const result: Record<string, unknown> = {
    doc_writer_model: resolveModelInternal(cwd, 'ferrox-doc-writer'),
