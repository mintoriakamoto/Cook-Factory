...
function cmdWorktreeRecordAgent(cwd: string, args: string[] = [], deps: RecordAgentCmdDeps = {}): RecordAgentCmdResult {
  ...
  try {
    ...
    writeErr(`[ferrox] worktree.record-agent: manifest_read_failed — ${hint}\n`);
  ...
  if (!plan.ok || plan.manifest === null) {
    writeErr(`[ferrox] worktree.record-agent: ${plan.reason} — ${plan.hint || ''}\n`);
...
function cmdWorktreeReapOrphans(cwd: string): void {
  ...
  try {
    ...
    process.stderr.write(`[ferrox] worktree.reap-orphans failed: ${err && (err as Error).message ? (err as Error).message : String(err)}\n`);
    result = [];
  }
  ...
  if (skippedCount > 0) {
    ...
    process.stderr.write(`[ferrox] worktree.reap-orphans: ${skippedCount} orphan(s) skipped (run with DEBUG=1 for details)\n`);
  }
  ...
  try {
    ...
    if (pruneResult && pruneResult.timedOut) {
      process.stderr.write(
        '[ferrox-tools] WARNING: worktree health check degraded' +
