...
export const FIELD_CLASSIFICATION: Readonly<Record<string, FieldClassification>> = Object.freeze(
  Object.assign(
    ...
    {
      // Schema
      ferrox_state_version: { source: 'free', preservation: 'derive' } as FieldClassification,
...
export type StateTransitionDeps = {
  ...
  // RETIRED by phase 14.1 D3c. `roadmapProvider` fed exactly one site, the
  // completePhase progress block, and that block now consumes
  // `progressProvider` backed by a phase-directory scan. Keeping the dep would
  // keep the STATE-to-ROADMAP-to-STATE cycle reachable.
  /**
...
export type StateTransitionIntent =
  ...
  | {
      ...
      version: string;
      /** Resolved runtime slash command for the Operator Next Steps hint (e.g. '/ferrox:new-milestone'). */
...
function completePhaseCore(
  ...
  if (completedRaw !== null) {
    ...
    if (derived) {
      ...
      if (typeof dc === 'number') newCompleted = dc;
      if (typeof dt === 'number') derivedTotalPhases = dt;
    }
 ...
 * "defining requirements" starting state. `ferrox_state_version` is preserved.
...
function milestoneSwitchCore(
  ...
  // boundary). ferrox_state_version is preserved.
  const fm: Record<string, unknown> = {
    ferrox_state_version: existingFm['ferrox_state_version'] || '1.0',
...
function reconcileByPhaseTable(
  ...
  // The canonical table shape (from ferrox-core/templates/state.md):
 ...
 * `ferrox-core/templates/state.md`). Rebuild clears any `**Field:** [placeholder]`
