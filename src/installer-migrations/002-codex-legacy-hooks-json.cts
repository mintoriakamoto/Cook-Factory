/**
 * Installer migration: remove legacy Codex hooks.json Ferrox hook registrations.
 *
...
const migration: InstallerMigration = {
  ...
  title: 'Remove legacy Codex hooks.json Ferrox hook registrations',
  description: 'Remove legacy Codex hooks.json Ferrox hook registrations after config.toml migration.',
  introducedIn: '1.50.0',
  runtimes: ['codex'],
  scopes: ['global', 'local'],
  destructive: true,
    ...
    return [
      {
        ...
        value: pruned.value,
        deleteIfEmpty: true,
        reason: 'legacy Codex hooks.json Ferrox registration retired by installer migration',
        ownershipEvidence: 'pruned command matches generated Ferrox hook command under the install hooks directory',
      },
    ];
  },
};
