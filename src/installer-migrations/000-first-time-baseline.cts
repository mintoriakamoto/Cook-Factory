...
const RUNTIME_SURFACES: Record<string, string[]> = {
  claude: ['ferrox-core', 'commands/ferrox', 'skills', 'agents', 'hooks', 'settings.json'],
  codex: ['ferrox-core', 'skills', 'agents', 'hooks', 'config.toml', 'hooks.json'],
  gemini: ['ferrox-core', 'commands/ferrox', 'hooks'],
  opencode: ['ferrox-core', 'command', 'skills', 'agents'],
  kilo: ['ferrox-core', 'command', 'skills', 'agents'],
  copilot: ['ferrox-core', 'skills', 'agents'],
  antigravity: ['ferrox-core', 'skills', 'agents'],
  cursor: ['ferrox-core', 'skills', 'agents', 'hooks', 'hooks.json'],
  windsurf: ['ferrox-core', 'skills', 'agents', 'rules'],
  augment: ['ferrox-core', 'skills', 'agents'],
  trae: ['ferrox-core', 'skills', 'agents', 'rules'],
  qwen: ['ferrox-core', 'skills', 'agents'],
  hermes: ['ferrox-core', 'skills/ferrox', 'agents'],
  cline: ['ferrox-core', 'skills', 'agents'],
  codebuddy: ['ferrox-core', 'skills', 'agents'],
};

const COMMON_SURFACES = ['ferrox-core', 'skills', 'agents', 'hooks'];
const INTERNAL_TOP_LEVEL_NAMES = new Set([
  'ferrox-file-manifest.json',
  'ferrox-install-state.json',
  'ferrox-migration-backups',
  'ferrox-migration-journal',
]);
const USER_OWNED_PATHS = new Set([
  'ferrox-core/USER-PROFILE.md',
  'commands/ferrox/dev-preferences.md',
  'skills/ferrox-dev-preferences/SKILL.md',
]);
...
function isUserOwnedBaselinePath(relPath: string): boolean {
  ...
  if (parts[0] === 'skills' && parts[1] && !parts[1].startsWith('ferrox-')) return true;
  if (parts[0] === 'agents' && parts[1] && !parts[1].startsWith('ferrox-')) return true;
  return false;
}
...
function listKnownGeneratedAgentNames(): Set<string> {
  ...
  try {
    for (const entry of fs.readdirSync(agentsDir, { withFileTypes: true })) {
      if (entry.isFile() && entry.name.startsWith('ferrox-') && entry.name.endsWith('.md')) {
        knownGeneratedAgentNames.add(entry.name.replace(/\.md$/, ''));
      }
    }
  } catch {
    // If the source agent directory is unavailable, fail closed and treat
    // Ferrox-looking agent files as user-choice artifacts.
  }
...
function isStaleFerroxLookingPath(relPath: string): boolean {
  const baseName = path.posix.basename(relPath);
  if (/^ferrox[-_]/.test(baseName)) return true;
  const parts = relPath.split('/');
  if ((parts[0] === 'skills' || parts[0] === 'agents') && parts[1] && parts[1].startsWith('ferrox-')) {
    return true;
  }
  return false;
}
...
const migration: InstallerMigration = {
  ...
  plan: ({ configDir, runtime, baselineScan, classifyArtifact }: PlanContext): BaselineAction[] => {
    ...
    for (const relPath of scanBaselineFiles(configDir, runtime)) {
      ...
      if (isStaleFerroxLookingPath(relPath)) {
        actions.push({
          type: 'prompt-user',
          relPath,
          reason: 'Ferrox-looking file is not proven manifest-managed and needs explicit user choice',
          classification: 'stale-ferrox-looking',
          originalHash: artifact.originalHash ?? null,
          currentHash,
          prompt: 'Choose whether to remove this stale-looking Ferrox artifact or keep it as user-owned.',
          choices: ['keep', 'remove'],
        });
        continue;
      }
