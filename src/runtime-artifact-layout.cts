...
/**
 * Locate the Ferrox commands/ferrox source directory.
 *
 * Resolution order:
 * 1. If runtimeConfigDir provided, check <runtimeConfigDir>/.ferrox-source marker.
...
function findInstallSourceRoot(runtimeConfigDir?: string): string {
  // Step 1: marker check
  if (runtimeConfigDir) {
    const markerPath = path.join(runtimeConfigDir, '.ferrox-source');
  ...
  let dir = __dirname;
  for (let i = 0; i < 6; i++) {
    const candidate = path.join(dir, 'commands', 'ferrox');
  ...
  throw new Error(`findInstallSourceRoot: could not locate commands/ferrox from ${__dirname}`);
}

/**
 * Locate the Ferrox agents source directory.
 *
 * Resolution order:
 * 1. If runtimeConfigDir provided, check <runtimeConfigDir>/.ferrox-source marker.
 * 2. Walk up from __dirname using path.dirname (no literal .. segments).
    ...
    if (fs.existsSync(markerPath)) {
      try {
        ...
        if (src && fs.existsSync(src)) {
          // Marker points to commands/ferrox; agents/ is a sibling of commands/
 ...
 * agent stem — e.g. `ferrox-planner.md`).
 *
 ...
 * the second `layout.kinds` consumer — `applySurface` / `/ferrox:surface` /
 ...
 * is exercised only by `/ferrox:surface` and synthetic-descriptor seam tests.
 *
 ...
 * @param configDir     runtime config dir (for .ferrox-source marker resolution)
 */
...
function kimiAgentsKind(destSubpath: string, prefix: string, configDir: string): ArtifactKind {
  return {
    ...
    stage: (resolved) => {
      ...
      const rootAgent = `---\nname: ferrox\ndescription: Run Ferrox workflows in Kimi CLI.\ntools: Agent\n---\n\n# Ferrox for Kimi CLI\n\nCoordinate installed /skill:ferrox-* workflows and route work to generated Ferrox subagents when a workflow requires an agent handoff.\n`;
      const artifacts = buildKimiAgentArtifacts({ rootAgent, subagents });
      const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ferrox-kimi-agents-'));
      installProfiles.STAGED_DIRS.add(stageDir);
      fs.writeFileSync(path.join(stageDir, 'ferrox.yaml'), artifacts.root.yaml);
      fs.writeFileSync(path.join(stageDir, 'ferrox.md'), artifacts.root.prompt);
...
function skillsKind(
  ...
  return {
    ...
    stage: (resolved) => {
      ...
      const cmdNames = conversionExports.readFerroxCommandNames
        ? conversionExports.readFerroxCommandNames()
        : [];
 ...
 * @param prefix        filename prefix, e.g. 'ferrox-'
 * @param converterName name of converter function in Runtime Artifact Conversion exports
 * @param configDir     runtime config dir (for .ferrox-source marker resolution)
 */
...
//                   at the top level so Skill(skill="ferrox-plan-phase") succeeds)
//
