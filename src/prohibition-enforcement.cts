 ...
 * (#1259, ADR-550 Decision 5d "heavy half"; the D1 seam — a NEW deterministic ferrox-tools
 ...
 * `ferrox-core/bin/lib/prohibition-enforcement.cjs`. Do NOT hand-write the `.cjs`; it is emitted.
 *
...
export interface CheckDescriptor {
   ...
   * test exercises via the `FERROX_PROHIB_SUBJECT` env convention, expected to drive the test RED.
   ...
   * `FERROX_PROHIB_SUBJECT` being set. A deceptive content-independent check reds on the clean subject too
   ...
   * file — no `FERROX_PROHIB_SUBJECT` indirection, so the "reds because the env var is set" gap can't exist).
   */
  cleanFixture?: string;
}
...
/**
 * Spawn the negative `node --test` against a single subject (set via the `FERROX_PROHIB_SUBJECT`
...
function runNodeTestWithSubject(check: CheckDescriptor, cwd: string, subject: string, timeoutMs?: number): string {
  try {
    return execFileSync(process.execPath, buildNodeTestArgs(check), {
      ...
      windowsHide: true,
      env: { ...childEnv(), FERROX_PROHIB_SUBJECT: subject },
 ...
 *   - node-test: spawn the negative test (TAP) with `FERROX_PROHIB_SUBJECT` set to the `violationFixture`
...
function defaultProveFailFirst(check: CheckDescriptor, cwd: string, timeoutMs?: number): FailFirstProof {
  try {
    ...
    if (check.kind === 'node-test') {
      ...
      // `FERROX_PROHIB_SUBJECT` point at a missing file; an honest negative test reading that subject
      ...
      // sufficient — a deceptive negative test that reds merely BECAUSE `FERROX_PROHIB_SUBJECT` is set
      ...
      // `FERROX_PROHIB_SUBJECT` and resolves a relative subject against `cwd`, so the existence check must
      ...
      // because FERROX_PROHIB_SUBJECT is SET reds here too → not content-dependent → not proven. ABSENT →
...
/**
 * CLI surface: `ferrox_run check prohibition-enforcement <request.json>` (or `--json '<inline>'`).
