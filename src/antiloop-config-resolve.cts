...
// eslint-disable-next-line @typescript-eslint/no-require-imports
import configGuard = require('./antiloop-config-guard.cjs');

const { loadConfig } = configLoader;
const { evaluateLoopConfigCombination } = configGuard;

/** The 5 resolved values, with `undefined` as the UNRESOLVED marker. */
interface ResolvedLoopInputs {
  mode?: unknown;
  granularity?: unknown;
...
/** What each of the 3 paths returned for each key, for the pinning tests and the summary table. */
interface ObservedPaths {
  [key: string]: { flat?: unknown; nested?: unknown; raw?: unknown };
}
...
function flatConfig(root: string): Record<string, unknown> {
  const cfg = safeRead(() => loadConfig(root));
  return cfg && typeof cfg === 'object' ? cfg : {};
}

/** The nested `workflow` object off the loaded config, or an empty object. */
function nestedWorkflow(root: string): Record<string, unknown> {
 ...
 * an exception.
 */
function rawProjectConfig(root: string): Record<string, unknown> {
...
function rawWorkflow(root: string): Record<string, unknown> {
 ...
 */
function resolveLoopConfigInputs(root: string): ResolvedLoopInputs {
 ...
 */
function observeResolutionPaths(root: string): ObservedPaths {
  const flat = flatConfig(root);
  const nested = nestedWorkflow(root);
  const raw = rawWorkflow(root);
...
function evaluateProjectLoopConfig(root: string): {
  decision: string;
  matched: string[];
  unresolved: string[];
} {
 ...
 * interface.
 */
function formatLoopConfigDoctorLine(result: { decision: string; matched: string[]; unresolved: string[] }): string {
