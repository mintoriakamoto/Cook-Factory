...
function phaseDirectories(cwd: string): Map<string, string> {
  const out = new Map<string, string>();
  let phasesDir: string;
  try {
    phasesDir = path.join(planningDir(cwd), 'phases');
  } catch {
    return out;
  }
  let entries: string[];
 ...
 */
function scanPhaseEntries(cwd: string, roadmapText: string, file?: string): ScanResult {
  ...
  for (const skeleton of collected.entries as PhaseEntry[]) {
...
function readMilestoneGroups(cwd: string): { groups: unknown[]; errors: RoadmapError[] } {
  ...
  const candidates = names.map((n) => ({ name: n, text: fs.readFileSync(path.join(dir, n), 'utf8') }));
 ...
 * bullets alone.
 */
function rebuildRoadmapRegions(
  cwd: string,
  roadmapText: string,
  ...
  const blocking = located.errors.filter(
    (e: RoadmapError) => e.code !== 'E_ROADMAP_REGION_MISSING',
  );
...
function formatErrors(errors: RoadmapError[]): string {
 ...
 */
function rebuildFailureMessage(errors: RoadmapError[]): string {
