'use strict';
/**
 * Intel command router — CLI subcommand dispatcher for `ferrox-tools intel`.
 *
 * ADR-959 (phase 4d-impl-4): intel command family cutover — last first-party
 * command cutover in the initial capability rollout.
 * Extracted from the hardcoded `case 'intel':` arm in ferrox-tools.cjs.
  ...
  routeHubCommandFamily({
    ...
    handlers: {
      query: () => {
        const term = args[2];
        if (!term) {
          return makeInvalidArgs('term', 'Usage: ferrox-tools intel query <term>', ERROR_REASON.USAGE);
        }
      ...
      'patch-meta': () => {
        ...
        if (!filePath) {
          return makeInvalidArgs('file-path', 'Usage: ferrox-tools intel patch-meta <file-path>', ERROR_REASON.USAGE);
        }
      ...
      'extract-exports': () => {
        const filePath = args[2];
        if (!filePath) {
          return makeInvalidArgs('file-path', 'Usage: ferrox-tools intel extract-exports <file-path>', ERROR_REASON.USAGE);
        }
