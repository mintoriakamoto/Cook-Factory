...
/**
 * Single source of truth for user-owned artifacts inside ferrox-core/.
 *
 * These files are created/refreshed by user-facing workflows (e.g.
 * /ferrox-profile-user) and must be preserved across reinstalls. Critically, they
 * MUST be excluded from ferrox-file-manifest.json — otherwise saveLocalPatches()
 * will compare a refreshed file against a stale manifest hash and emit a
 * spurious "locally modified Ferrox file" warning (bug #2771).
 *
 ...
 *
 * Paths are relative to the ferrox-core/ directory.
 */
...
/**
 * Migrate a legacy dev-preferences.md (saved from commands/ferrox/) into the
 * runtime-aware SKILL.md location used by the writer after #2973.
 *
 * For runtimes with a nested skills layout (e.g. Hermes: skills/ferrox/<stem>/),
 * the target is <configDir>/skills/ferrox/dev-preferences/SKILL.md.
 * For runtimes with a flat skills layout (prefix='ferrox-'), the target is
 * <configDir>/skills/ferrox-dev-preferences/SKILL.md.
 *
...
function migrateLegacyDevPreferencesToSkill(targetDir: string, saved: Map<string, string>, runtime?: string, scope: string = 'global'): boolean {
  ...
  if (runtime) {
    ...
    const stemName = skillsKindEntry.prefix === '' ? 'dev-preferences' : 'ferrox-dev-preferences';
    ...
    skillDir = path.join(runtimeArtifactInstallPlan.assertDestWithinConfigHome(targetDir, 'skills'), 'ferrox-dev-preferences');
  }
 ...
 *     encodes the Ferrox namespace as its last segment (e.g. `commands/ferrox`), in
 *     which case write as `${stem}.md` (directory IS the namespace).
 *   - agents: write as-is (files already carry their own `ferrox-` prefix).
...
function _copyStaged(stagedDir: string, destDir: string, kind: any, configDir: string, runtime?: string): void {
  ...
  if (kind.kind === 'skills') {
    // Each child of stagedDir is a prefixed skill directory: ferrox-help/, etc.
  ...
  // represents the Ferrox namespace (e.g. 'commands/ferrox' → last segment 'ferrox').
  ...
  for (const entry of entries) {
    ...
    let destName: string;
    if (kind.kind === 'agents') {
      // Agent files already carry the ferrox- prefix in the source dir.
...
/**
 * Remove Ferrox-prefixed entries from destDir matching kind.prefix.
 ...
 */
function _removeFerroxEntries(destDir: string, kind: any): void {
  if (!fs.existsSync(destDir)) return;
  if (kind.kind === 'kimi-agents') {
    for (const fileName of ['ferrox.yaml', 'ferrox.md']) {
  ...
  if (kind.prefix === '') {
    // Whole-namespace removal (Hermes nested case — destSubpath is skills/ferrox)
    // The directory itself is the Ferrox namespace, so remove it entirely.
...
/**
 * After the layout-driven install loop writes new ferrox-<stem>/ dirs to
 * skills/ferrox/, remove any pre-existing bare-stem dirs (skills/ferrox/<stem>/)
 * that correspond to the newly installed ferrox-<stem> entries.
 *
 * @param nestedFerroxDir  absolute path to skills/ferrox/ category dir
 */
function _removeHermesBareStemDirs(nestedFerroxDir: string): void {
  if (!fs.existsSync(nestedFerroxDir)) return;
  const entries = fs.readdirSync(nestedFerroxDir, { withFileTypes: true });

  // Collect the set of stems that were installed as ferrox-<stem>/ this run.
  const installedStems = new Set<string>();
  for (const entry of entries) {
    if (entry.isDirectory() && entry.name.startsWith('ferrox-')) {
      installedStems.add(entry.name.slice('ferrox-'.length)); // e.g. 'quick', 'dev-preferences'
    }
  }

  // Remove any bare <stem>/ dir for which ferrox-<stem>/ was just installed.
  for (const entry of entries) {
    if (entry.isDirectory() && !entry.name.startsWith('ferrox-') && installedStems.has(entry.name)) {
      fs.rmSync(path.join(nestedFerroxDir, entry.name), { recursive: true });
    }
  }
}
...
function _runLegacyInstallMigrations(runtime: string, configDir: string, scope: string = 'global'): void {
  const legacyCommandsFerrox = path.join(configDir, 'commands', 'ferrox');

  // Claude / Qwen / Hermes: clean up legacy commands/ferrox/ and preserve dev-preferences
  // for migration. The actual migration call is deferred to after all layout cleanup so
  // that for Hermes the flat skills/ferrox-*/ removal (below) does not delete the freshly
  // created skills/ferrox-dev-preferences/ skill dir.
  let savedLegacyArtifacts: Map<string, string> | null = null;
  if (_hostBehaviors(runtime).legacyCommandsFerroxInstallMigration) {
    if (fs.existsSync(legacyCommandsFerrox)) {
      savedLegacyArtifacts = preserveUserArtifacts(legacyCommandsFerrox, ['dev-preferences.md']);
      fs.rmSync(legacyCommandsFerrox, { recursive: true });
    }
  }

  // Hermes: remove pre-#2841 flat skills/ferrox-*/ entries that lived alongside
  // the new skills/ferrox/ nested layout.
  if (runtime === 'hermes') {
    ...
    // Hermes: bare-stem skills/ferrox/<stem>/ cleanup is deferred to AFTER the
    // layout-driven install loop in installRuntimeArtifacts, where the exact set
    // of staged ferrox-<stem>/ dirs is known. Removing here (before staging) would
    // require readFerroxCommandNames() which misses skills like 'dev-preferences'
...
function _runLegacyUninstallCleanup(runtime: string, configDir: string, scope: string = 'global'): Map<string, string> | null {
  // commands/ferrox/ is a legacy location for Qwen, Hermes, and all Claude installs.
  // Prior to #1367 fix, Claude-local used commands/ferrox/<cmd>.md (colon-namespaced).
  // After #1367, Claude-local uses flat commands/ferrox-<cmd>.md. The inline uninstall
  // block (1c) handles removal of flat files; this function handles the legacy
  // commands/ferrox/ directory for all Claude scopes (global was already included,
  ...
  // before destructive wipe. Migration to skills/ferrox-dev-preferences/SKILL.md
  // is deferred and returned so the caller can apply it AFTER layout-driven
  // removal — this prevents the layout's ferrox-* prefix removal from wiping the
  // freshly created skill dir (same pattern as _runLegacyInstallMigrations).
  let savedLegacyArtifacts: Map<string, string> | null = null;
  // commands/ferrox/ is a legacy location for Qwen, Hermes, and Claude global.
  // Claude local is intentionally excluded: the inline uninstall block (1c) handles
  // commands/ferrox/ for claude local, preserving dev-preferences.md by restoring it
  ...
  const _lu = _hostBehaviors(runtime).legacyCommandsFerroxUninstall;
  const isLegacyCommandsFerrox = _lu === true || (_lu === 'global' && scope === 'global');
  if (isLegacyCommandsFerrox) {
    const legacyCommandsFerrox = path.join(configDir, 'commands', 'ferrox');
    if (fs.existsSync(legacyCommandsFerrox)) {
      savedLegacyArtifacts = preserveUserArtifacts(legacyCommandsFerrox, ['dev-preferences.md']);
      fs.rmSync(legacyCommandsFerrox, { recursive: true });
  ...
  // Hermes: pre-#2841 flat skills/ferrox-*/ entries
  if (runtime === 'hermes') {
    const flatSkillsDir = path.join(configDir, 'skills');
    if (fs.existsSync(flatSkillsDir)) {
      for (const entry of fs.readdirSync(flatSkillsDir, { withFileTypes: true })) {
    ...
    // Hermes: pre-#947 bare-stem skills/ferrox/<stem>/ entries (dirs that do NOT
    // start with 'ferrox-') — the #3664 layout used prefix='' so Ferrox-owned skills
    // had bare names (e.g. skills/ferrox/help/). These are stale on uninstall.
    const nestedFerroxDirForUninstall = path.join(configDir, 'skills', 'ferrox');
    if (fs.existsSync(nestedFerroxDirForUninstall)) {
      for (const entry of fs.readdirSync(nestedFerroxDirForUninstall, { withFileTypes: true })) {
        if (entry.isDirectory() && !entry.name.startsWith('ferrox-')) {
          fs.rmSync(path.join(nestedFerroxDirForUninstall, entry.name), { recursive: true });
        }
      }
    }
  }
...
function installRuntimeArtifacts(
  ...
  try {
    ...
    for (const item of planResult.plan.items) {
      ...
      if (kind.kind === 'skills' && fs.existsSync(dest)) {
        // Pre-prune: snapshot user-owned content before _removeFerroxEntries wipes it,
        // then restore after. This preserves user dirs across a wipe-and-replace
        // install (#2973 / #3664).
        //
        // All runtimes (incl. Hermes after #947) use prefix='ferrox-'.
        // _removeFerroxEntries removes only ferrox-* entries; non-ferrox-* user dirs are
        // untouched. Preserve the explicit user-owned Ferrox-prefixed skill
        // ferrox-dev-preferences, which Ferrox does not reinstall from source but must
        // survive the prune (#2973).
        const toPreserve = new Map<string, Map<string, Buffer>>(); // dirName -> Map<relPath, Buffer>

        {
          // Preserve explicitly user-owned Ferrox-prefixed skill dirs.
          // ferrox-dev-preferences is the sole user-customisable skill in this category.
          const USER_OWNED_SKILL_DIRS = ['ferrox-dev-preferences'];
        ...
        _removeFerroxEntries(dest, kind);
        ...
        // just prune stale ferrox-* entries and copy new ones.
        _removeFerroxEntries(dest, kind);
        _copyStaged(item.sourceDir, dest, kind, configDir, runtime);
      }
    }
  ...
  // Hermes: after the install loop has written all ferrox-<stem>/ dirs to
  // skills/ferrox/, remove any stale bare-stem dirs (skills/ferrox/<stem>/) that
  // correspond to the newly installed ferrox-<stem> entries. This is the robust
  // replacement for the readFerroxCommandNames()-based pre-install cleanup that
  ...
  // every ferrox-<stem>/ present now was written this run (or was there before
  // with the same prefix). User-owned bare dirs with no ferrox-<stem> counterpart
  // are untouched.
  if (runtime === 'hermes') {
    const nestedFerroxDirForCleanup = path.join(configDir, 'skills', 'ferrox');
    _removeHermesBareStemDirs(nestedFerroxDirForCleanup);
  }
  ...
  // is safe even when configDir has no .ferrox-source marker (artifactLayout: []).
  if (behaviors.nativePlugin) {
    const commandsFerroxDir = runtimeArtifactLayout.findInstallSourceRoot(configDir);
    const src = path.dirname(path.dirname(commandsFerroxDir));
 ...
 * @param resolveAttribution - injection: (runtime) => attribution string | undefined
 * @returns number of ferrox-* skill directories written
 */
function installOpencodeFamilySkills(
  ...
  // Preserve user-owned Ferrox-prefixed skill dirs across the ferrox-* prune.
  // ferrox-dev-preferences is generated by the user (via generate-dev-preferences)
  // and lives at <configDir>/skills/ferrox-dev-preferences — _removeFerroxEntries
  ...
  _removeFerroxEntries(dest, skillsKindEntry);
 ...
 * (OpenCode/Kilo): commands/ferrox/**\/*.md -> command/ferrox-<...>.md, with
 ...
 * @param srcDir - source directory to walk (commands/ferrox/, recursing into subdirectories)
 * @param pathPrefix - computed config-path prefix for body rewrites
 * @param resolveAttribution - injection: (runtime) => attribution string | undefined
 * @param prefix - filename prefix accumulator (defaults to 'ferrox'; grows on recursion)
 */
function installOpencodeFamilyCommands(
  ...
  pathPrefix: string,
  resolveAttribution: ResolveAttribution = () => undefined,
  prefix: string = 'ferrox',
): void {
  if (!fs.existsSync(srcDir)) return;

  // Remove old ferrox-*.md files before copying new ones
 ...
 * @param src       - repo/package root (two levels up from the commands/ferrox source dir)
 */
...
function installOpencodeFamilyArtifacts(
  ...
  // findInstallSourceRoot resolves DIRECTLY to the commands/ferrox source dir
  // (via the .ferrox-source marker or a walk-up from __dirname) — every other
  ...
  const commandsFerroxDir = runtimeArtifactLayout.findInstallSourceRoot(configDir);
  const src = path.dirname(path.dirname(commandsFerroxDir));
  const rawCommandsDir = installProfiles.stageSkillsForProfile(commandsFerroxDir, resolvedProfile);
 ...
 * determine which Ferrox-owned entries to remove.
 *
 * @param runtime             canonical runtime ID
 * @param configDir           resolved runtime config directory
 * @param scope
...
function uninstallRuntimeArtifacts(runtime: string, configDir: string, scope: string): void {
  ...
  // removing Claude local commands/ferrox/ which is the primary install dir).
  // Returns saved user artifacts so we can migrate AFTER layout removal
  // (the layout's ferrox-* prefix pass would wipe a skill dir created here).
  ...
  for (const item of plan.items) {
    ...
    _removeFerroxEntries(item.destDir, kind);
  }

  // Hermes: after removing ferrox-* skill dirs from skills/ferrox/, also remove
  // the Ferrox-managed DESCRIPTION.md and then the category dir itself if it
  // contains no user content (#947). _removeFerroxEntries removed ferrox-* dirs
  // but left the category container and DESCRIPTION.md intact.
  if (runtime === 'hermes') {
    const nestedFerroxDir = path.join(configDir, 'skills', 'ferrox');
    if (fs.existsSync(nestedFerroxDir)) {
      // Remove Ferrox-owned DESCRIPTION.md (written by writeHermesCategoryDescription)
      fs.rmSync(path.join(nestedFerroxDir, 'DESCRIPTION.md'), { force: true });
      // Remove the category dir if empty (no user content remaining)
      const remaining = fs.readdirSync(nestedFerroxDir, { withFileTypes: true });
      if (remaining.length === 0) {
        fs.rmSync(nestedFerroxDir, { recursive: true, force: true });
      }
    }
  }
  ...
  // complete. Do NOT restore to commands/ferrox/ — the user is uninstalling.
  if (savedLegacyArtifacts) {
    migrateLegacyDevPreferencesToSkill(configDir, savedLegacyArtifacts, runtime, scope);
  }
}
