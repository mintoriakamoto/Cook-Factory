 ...
 * caller — Ferrox has no interactive-prompt layer in lib (the runtime/CLI edge owns that), so the
 ...
 *   checkEngines(manifest, host)      — engines.ferrox hard gate + compatVersions downgrade
 ...
 */
const RESERVED_NAMESPACES = ['ferrox-', 'ferrox-core-', 'anthropic-'];
...
interface Disclosure {
  ...
  hooks: HookSurface[];
  /** Command modules the capability ships (each is require()'d into the Ferrox CLI process). */
...
interface EnginesVerdict {
  ...
  compatible: boolean;
  /** The declared engines.ferrox range, or null if unconstrained. */
  range: string | null;
...
function discloseExecutableSurfaces(manifest: CapabilityManifest, stagedDir?: string): Disclosure {
  ...
  if (manifest.mcpServers && typeof manifest.mcpServers === 'object') {
    const pushServer = (name: string, config: unknown): void => {
      ...
      // never the dangerous keys. The CAP_MARKER the writer stamps on persist (`_ferroxCapability`) is the
...
// ---------------------------------------------------------------------------
// engines.ferrox hard gate + compatVersions downgrade
// ---------------------------------------------------------------------------

/**
 * Hard-gate a manifest against the running host version via engines.ferrox, consulting
...
function checkEngines(manifest: CapabilityManifest, hostVersion: string): EnginesVerdict {
  ...
  if (engines && typeof engines === 'object' && !Array.isArray(engines)) {
    const g = (engines as Record<string, unknown>)['ferrox'];
  ...
  if (compat && typeof compat === 'object' && !Array.isArray(compat)) {
    for (const [capVer, ferroxRange] of Object.entries(compat as Record<string, unknown>)) {
      if (typeof ferroxRange !== 'string' || !ferroxRange) continue;
      if (!semverMod.semverSatisfies(hostVersion, ferroxRange)) continue;
 ...
 *
 * engines.ferrox is also enforced inside resolveCapabilitySource at resolve time; re-checking here
...
function evaluateInstallTrust(args: InstallTrustArgs): InstallTrustVerdict {
  ...
  if (!engines.compatible) {
    const hint = engines.downgradeTo
      ...
      : '';
    blockReasons.push(
      `capability requires engines.ferrox "${engines.range}" but host is ${hostVersion}${hint}`,
    );
  }
...
function summarizeDisclosure(disclosure: Disclosure): string[] {
  ...
  if (disclosure.commandModules.length > 0) {
    lines.push(
      `  command modules (${disclosure.commandModules.length}): require()'d into the Ferrox CLI process`,
    );
