/**
 * Canonical Ferrox artifact registry (ADR-457 build-at-publish: the hand-written
 ...
 * are added.
 *
 * Enumerates the file names that ferrox workflows officially produce at the
 * .planning/ root level. Used by ferrox-health (W019) to flag unrecognized files
...
export const CANONICAL_PATTERNS: ReadonlyArray<RegExp> = [
  /^v\d+\.\d+(?:\.\d+)?-MILESTONE-AUDIT\.md$/i, // ferrox-complete-milestone (pre-archive)
